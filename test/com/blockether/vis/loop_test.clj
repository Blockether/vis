(ns com.blockether.vis.loop-test
  (:require
   [babashka.fs :as fs]
          ...
          (try
            (let [db-info (:db-info env)
                  ...
                  _ (rlm-db/store-iteration! db-info {:query-ref q2
                                                      ...
                                                      :thinking "" :duration-ms 0})
                  restore-var-fn (rlm-base/make-restore-var-fn (:db-info env) conv-ref)]
...
(defdescribe document-bindings-test
  (it "search-documents with :in :pages returns empty markdown string when no pages exist"
    (with-test-env* {} (fn [env]
                         (let [result (#'rlm-core/execute-code env "(search-documents \"party\" {:in :pages})")]
                           (expect (nil? (:error result)))
  ...
  (it "search-documents without :in returns empty markdown string on empty DB"
    ...
    (it "fetch-document-content returns nil for non-existent toc lookup"
      (with-test-env* {} (fn [env]
                           (let [result (#'rlm-core/execute-code env "(fetch-document-content [:toc/id (java.util.UUID/randomUUID)])")]
                             (expect (nil? (:error result)))
                             (expect (nil? (:result result)))))))
...
(defdescribe knowledge-engine-integration-test
  (it "ingest returns base document stats"
    (let [env (sut/create-env test-ingest-router {:db :temp})
          result (sut/ingest-to-env! env [(make-test-single-page-document)])]
      (sut/dispose-env! env)
  ...
  (it "search-documents returns empty markdown on empty DB via full env"
    (with-test-env* {} (fn [env]
                         (let [result (#'rlm-core/execute-code env "(search-documents \"x\" {:in :pages})")]
                           (expect (nil? (:error result)))
                           (expect (= "" (:result result))))))))
...
(defdescribe knowledge-engine-real-llm-test
  (describe "ingestion with real LLM"
    (it "ingests single-page document"
      (when (integration-tests-enabled?)
        (with-integration-env* (fn [env]
    ...
    (it "ingests legal document with multiple pages"
      (when (integration-tests-enabled?)
        (with-integration-env* (fn [env]
                                 (let [result (sut/ingest-to-env! env [(make-test-legal-document)])]
                                   (expect (vector? result))
                                   (expect (number? (get-in result [0 :pages-stored])))))))))
