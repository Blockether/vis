  ...
  (let [required
    ...
    (it "is imperative above budget and prescribes one broad verified fold"
        (let [hint (eng/over-budget-hint required 6 6)]
          (doseq [text ["ACTION REQUIRED" "210k" "144k" "Fold settled search/tool sweeps"
                        "one broad session_fold" "last completed scope"
                        "preserve decisions, edits, and verification"
                        "preserve exact physical paths" "never bare or abbreviated filenames"
...
(defdescribe
  ...
  (it "the fold card carries the same ntr[...] recovery accessors"
      ...
      (let [sf (get (compaction-verbs (atom {"session_turn" 2
                                             "engine_iter_universe" ["t1/i1"]
