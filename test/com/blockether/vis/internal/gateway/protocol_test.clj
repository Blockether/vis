...
(deftest compatibility-verdict-test
  ...
  (testing "this release serves only the protocol it speaks"
    (is (= 8 protocol/protocol-version))
    (is (= 8 protocol/min-client-protocol))
    (is (= 8 protocol/min-gateway-protocol)))
...
(deftest client-records-the-nested-health-handshake-test
  (let [handshake-atom
        ...
        body
        {"status" "ok" "protocol" {"protocol" 9 "min_client" 9 "min_gateway" 2 "version" "9.0.0"}}]

    (try (is (= body ((client-var 'note-handshake!) body)))
         (is (= {:protocol 9 :min-client 9 :min-gateway 2 :version "9.0.0" :build nil}
                @handshake-atom))
