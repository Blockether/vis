              ...
              (fn [method path & [opts]]
                ...
                (cond
                  (= path "/v1/voice/model?engine=parakeet") {:status 200
                                                              :body "{\"status\":\"ready\"}"}
                  (= path (str prefix "?engine=parakeet")) (do (is (= "RIFF" (slurp (:body opts))))
                                                               (is (:raw-body? opts))
                                                               {:status 202
                                                                :body "{\"id\":\"job-1\"}"})
...
(deftest speech-refusals-surface-the-canonical-error-message
  (with-redefs [client/request! (fn [method path & _]
                                  (is (= [:post "/v1/voice/model?engine=parakeet"] [method path]))
                                  {:status 501
                                   :body (str
                                           "{\"error\":{\"type\":\"engine-unavailable\","
    ...
    (let [failure (try (client/prepare-speech-model! :transcribe {:engine-id "parakeet"})
      ...
      (is (= ["parakeet: model missing"] (get-in (ex-data failure) [:body "error" "reasons"])))))
