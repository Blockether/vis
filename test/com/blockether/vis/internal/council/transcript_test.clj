...
(defn- wake-instruction
  "The host notification text as it was written when Council wakes still reached
   SQLite without their provenance. Frozen history: the repair must keep matching
   it even after the live template changes."
  [entry-id]
  (str "Council notification #"
       entry-id
       ". Read the attributed Council input; if missing or truncated, use council.get("
       entry-id
       ")."))

;; Regression: wakes stored before the engine kept request provenance kept reading
;; as You with the host instruction in both the TUI and the Companion. Opening the
;; store repairs them, and text a person typed keeps its own attribution.
(deftest legacy-council-turns-regain-attribution-on-open
  (let [dir
        (.toFile (java.nio.file.Files/createTempDirectory
                   "vis-council-attribution"
                   (make-array java.nio.file.attribute.FileAttribute 0)))
    ...
    (try
      (let [{:keys [sid entry]}
            ...
            (ps/db-store-session-turn! db
                                       {:parent-session-id sid
                                        :user-request (wake-instruction (:entry_id entry))})

            unknown-tid
            (ps/db-store-session-turn! db
                                       {:parent-session-id sid
                                        :user-request (wake-instruction 99999)})
            ...
            published-at
            (:created_at (first (h/raw-query db
                                             {:select [:created_at]
                                              :from :council_entry
                                              :where [:= :id (:entry_id entry)]})))]

        ;; Someone pasting the notification back minutes later stays their own request.
        (h/raw-query db
                     {:update :session_turn_soul
                      :set {:created_at (+ published-at 300000)}
                      :where [:= :id (str late-tid)]})
        (ps/db-dispose-connection! db)
        (let [reopened (ps/db-create-connection! (.getPath dir))]
          (try (let [[wake unknown late] (ps/db-list-session-turns reopened sid)
                     transcript (with-redefs [lp/db-info (constantly reopened)]
                                  (state/transcript sid))]

                 (is (= [(str wake-tid) (str unknown-tid) (str late-tid)]
                        (mapv (comp str :id) [wake unknown late])))
                 (is (= :council (:request-kind wake)))
                 (is (= (:entry_id entry) (get-in wake [:council :entry-id])))
                 (is (= :coordination (get-in wake [:council :kind])))
                 (is (= (wake-instruction (:entry_id entry)) (:user-request wake))
                     "Keep the model instruction unchanged")
                 (is (= "council" (get-in transcript [0 "request_kind"])))
                 (is (= (:content entry) (get-in transcript [0 "request"])))
                 (is (= [:user :user] (mapv :request-kind [unknown late]))
                     "Never classify user text by its prefix")
                 (is (every? nil? (map :council [unknown late]))))
               (finally (ps/db-dispose-connection! reopened)))))
      (finally (ps/db-dispose-connection! db)
               (doseq [^java.io.File f (reverse (file-seq dir))]
                 (.delete f))))))
