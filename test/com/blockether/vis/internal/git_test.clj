...
(defdescribe
  ...
  (it
    "uses extension-owned around-hook lifecycle and fails closed"
    (let
      [root
       (make-tmp-dir)

       extension-name
       "test.git-commit-gate"
      ...
      (try (init-repo! root)
           ...
           (let [head-before (git-out root "rev-parse" "HEAD")]
             ...
             (let [blocked (git/commit! root ["commit" "-m" "blocked"] nil)]
               ...
               (expect (= [] (nth @seen 2)))
               (expect (= (.getCanonicalPath root) (get-in @seen [0 :root])))
           ...
           (finally (extension/deregister-extension! extension-name) (cleanup root)))))
  (it "resolves -C before presenting the semantic repository to hooks"
      (let
        [workspace-root
         ...
         repo
         (io/file workspace-root "other")

         extension-name
         "test.git-effective-repo"
        ...
        (try (.mkdirs repo)
             ...
             (git! repo "add" "a.txt")
             (extension/register-extension! {:ext/name extension-name
             ...
             (let
               [result
                (git/run-command workspace-root ["-C" "other" "commit" "-m" "other repo"] nil)]
               (expect (= 0 (:exit result)))
               (expect (= (.getCanonicalPath repo) (:root @seen)))
               (expect (= (:candidate-tree @seen) (git-out repo "rev-parse" "HEAD^{tree}"))))
             (finally (extension/deregister-extension! extension-name) (cleanup workspace-root)))))
        ...
        (try (init-repo! root)
             ...
             (let [hook (io/file root ".git/hooks/pre-commit")]
               ...
               (let [result (git/commit! root ["commit" "-m" "hook changed tree"] nil)]
                 ...
                 (expect (= "hook changed tree" (git-out root "log" "-1" "--format=%s")))))
             (finally (cleanup root))))))
