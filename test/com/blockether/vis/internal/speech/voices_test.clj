(ns com.blockether.vis.internal.speech.voices-test
...
(defn- wav!
  ...
  (let [frames
    ...
    (let [file (java.io.File/createTempFile "vis-speech-source" ".wav")]
      (.deleteOnExit file)
...
(defdescribe
  ...
  (it "refuses a recording it cannot use, and says which one"
      (with-voices-dir
        (fn [_dir]
          (let [garbage (doto (java.io.File/createTempFile "vis-speech-garbage" ".wav")
                          (.deleteOnExit))]
            (spit garbage "this is not a recording")
            (expect (= :speech-tts/clip-missing
                       (:type (ex-data-of #(voices/import! {:path "/nowhere/at/all.wav"})))))
            (expect (= :speech-tts/clip-too-short
            ...
            (expect (contains? #{:speech/no-ffmpeg :speech/unreadable}
