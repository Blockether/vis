(ns com.blockether.vis.internal.speech.tts-test
...
(defdescribe
  ...
  (it "refuses to speak with pocket-tts while it has no clip to clone"
      ...
      (with-redefs [sherpa/ensure-native!
        ...
        (let [data (ex-data-of #(tts/synthesize! :pocket-tts {:text "hello"}))]
          (expect (= :speech-tts/no-reference-clips (:type data)))
  ...
  (it "answers an unknown voice with the ones that exist"
      (let [data (ex-data-of #(#'tts/piper-asset-for "nope"))]
        (expect (= :speech-tts/unknown-voice (:type data)))
             ...
             (it "refuses blank text before it downloads anything"
                 (with-redefs [assets/ensure!
                   ...
                   (let [data (ex-data-of #(tts/synthesize! :piper {:text "   "}))]
                     (expect (= :speech-tts/blank-text (:type data)))
                     (expect (= :piper (:family data)))))))
             ...
             (it "still refuses synthesis when an installed archive lacks its tables"
                 (with-redefs [tts/espeak-data-dir
                   ...
                   (let [data (ex-data-of #(tts/synthesize! :piper {:text "hello"}))]
                     (expect (= :speech-tts/espeak-ng-missing (:type data)))
...
(defn- temp-models-root
  ...
  (doto (io/file (System/getProperty "java.io.tmpdir")
                 (str "vis-speech-samples-" (System/nanoTime)))
    (.mkdirs)))
             ...
             (it "refuses to make a sample that would need the voice downloaded first"
                 (let [root
                   ...
                   (with-redefs [assets/models-root
                     ...
                     (expect (= :speech-tts/sample-unavailable
