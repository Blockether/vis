(ns com.blockether.vis.internal.speech.transcode-test
  ...
  (:require [com.blockether.vis.internal.speech.transcode :as transcode]
  ...
  (when-let [ffmpeg (transcode/ffmpeg-path)]
    (let [out (doto (File/createTempFile "vis-transcode-test" ".m4a") (.deleteOnExit))
          p (.start (doto (ProcessBuilder. ^java.util.List
                                           (mapv str [ffmpeg "-v" "error" "-y" "-i" wav out]))
