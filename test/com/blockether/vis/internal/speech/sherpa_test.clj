(ns com.blockether.vis.internal.speech.sherpa-test
...
(defdescribe platform-token-test
             ...
             (it "refuses a platform it has no name for instead of guessing one"
                 (expect (= :speech/unsupported-platform
...
(defdescribe
  ...
  (it "passes a working call straight through and only translates what it must"
      (with-redefs [sherpa/ensure-native! (constantly true)]
        ...
        (let [ordinary (try (sherpa/call-native
                              #(throw (ex-info "no such voice" {:type :speech-tts/unknown-voice})))
                            nil
                            (catch Throwable t t))]
...
(def ^:private metadata-resource
  "Where the native image build reads voice reachability metadata."
  "META-INF/native-image/com.blockether/vis-speech/reachability-metadata.json")
...
(defn- registered
  ...
  (let [url (io/resource metadata-resource)]
    (when-not url
      (throw (ex-info "Core voice ships no reachability metadata." {:resource metadata-resource})))
    (into {}
