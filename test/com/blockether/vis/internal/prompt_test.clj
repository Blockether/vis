    ...
    (let [text (var-get (ns-resolve 'com.blockether.vis.internal.prompt 'CORE_SYSTEM_PROMPT))]
      ...
      (doseq [required
              ["Host project default" "`vis_docs()`" "runtime > source > docs > assumption"
               ...
               "hard preconditions" "`python_execution`"
               "`await gather(...)` only for independent calls"
               ...
               "batch independent reads" "Create no unrequested"
               "without asking permission or offering optional" "Never expose or log secrets"
               "commit, push, publish" "Treat context as a budget"
      ...
      (doseq [surplus ["Keep managed REPLs across turns" "Native results are `ntr[tool_id]`"
      ...
      (let [shims [{:shim/name "attach"
        ...
        (with-redefs [extension/sandbox-shims (constantly shims)]
          (let [text (#'prompt/sandbox-shims-prompt-block nil)]
            ...
            (doseq [global ["vis_attach" "vis_attach_bytes" "vis_attachments" "vis_read_attachment"
