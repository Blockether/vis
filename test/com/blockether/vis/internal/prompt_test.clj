...
(defdescribe prompt-test
  ...
  (it "core system prompt is minimal: env/extension plumbing stays out, key directives stay in"
    ...
    (let [content (:content (first (prompt/assemble-stable-prompt-messages
      ...
      ;; Code blocks are executable code, not a narration channel.
      (expect (not (str/includes? content "Narration is ;; comments only")))
      (expect (not (str/includes? content "Blocks can mix code and ;; comments")))
      (expect (not (str/includes? content "Before each probe, name in ;; comments")))
  ...
  (it "current turn context renders dynamic telemetry and direct runtime values without static lifecycle policy"
    (let [turn-id #uuid "11111111-1111-1111-1111-111111111111"
      ...
      (let [content (prompt/build-iteration-context
        ...
        (expect (str/includes? content "conversation_id: 00000000-0000-0000-0000-000000000000"))
        (expect (str/includes? content "turn_id: #uuid \"11111111-1111-1111-1111-111111111111\""))
        (expect (str/includes? content "turn_position: 7"))
        (expect (str/includes? content "iteration_position: 3"))
        ...
        (expect (not (str/includes? content "engine_state:")))
        (expect (not (str/includes? content "engine_phase:")))
        (expect (not (str/includes? content "engine_turn_position:")))
        (expect (not (str/includes? content "engine_iteration_position:")))
        (expect (not (str/includes? content "previous_persisted_iteration_id:")))
        (expect (not (str/includes? content "previous_persisted_iteration_position:")))
        (expect (not (str/includes? content "turn_conversation_state_id:")))
        (expect (not (str/includes? content "turn_iteration_id:")))
        (expect (not (str/includes? content "turn_iteration_position:")))
        (expect (not (str/includes? content "conversation_previous_answer:")))
