...
(set-ns-context! [(around-each [f]
                               (let [dir (throwaway-store)]
                                 (with-redefs [config/config-dir (constantly dir)
                                               config/state-path (constantly (str dir
                                                                                  "/state.yml"))]
...
(defdescribe
  ...
  (it "returns one description per image, carrying the describing model"
      (let [{:keys [result calls]} (with-asks descriptions
                                              #(vd/describe-images (mixed-fleet)
                                                                   "ctx"
                                                                   [(distinct-image "a")
                                                                    (distinct-image "b")]))]
  ...
  (it "returns nil — and asks nothing — when the whole fleet is blind"
      (let [{:keys [result calls]}
            (with-asks descriptions
                       #(vd/describe-images (blind-fleet) "ctx" [(image "/tmp/a.png")]))]
        (expect (nil? result))
        (expect (empty? calls))))
...
(defdescribe
  ...
  (it "answers an empty map when nothing describable came out of the gate"
      (let [{:keys [result]} (with-asks descriptions
...
(defdescribe
  ...
  (it
    "turns an unseeable attachment into text in the outgoing user message"
    (let [att
  ...
  (it
    "leaves the message exactly as it is today when nothing can see"
    (let [att
          {:path "/tmp/shot.png" :media-type "image/png" :base64 png-b64 :size 68 :size-label "68B"}
...
(defdescribe
  ...
  (it "describes one PAYLOAD once, however many rows carry it"
      (let [{:keys [result calls]} (with-asks (fn [_]
            ...
            (with-asks (fn [opts]
                         (let [excluded (get-in opts [:routing :exclude-providers])]
                           (cond (empty? excluded) (throw (ex-info "Exceptional status code: 400"
                                 ...
                                 :else {:result {:description "rescued"}
                                        :routed/model "backup-seer"})))
            ...
            (with-asks (fn [_]
                         (let [n (swap! attempts inc)]
                           (throw (ex-info "Exceptional status code: 500"
                                           {:status 500
                                            :provider-id (keyword (str "broken-" n))}))))
  ...
  (it "never retries when the fleet has a single provider"
      (let [{:keys [result calls]}
            (with-asks (fn [_]
                         (throw (ex-info "503 from the only seer"
                                         {:status 503 :provider-id :seeing})))
...
(defdescribe
  ...
  (it "keeps working when the store cannot be read or written"
      (with-redefs [config/state-path (fn []
