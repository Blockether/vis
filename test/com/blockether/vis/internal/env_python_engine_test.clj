...
(defn- heap-used-mb
  ...
  (let [rt (Runtime/getRuntime)]
    (quot (- (.totalMemory rt) (.freeMemory rt)) 1048576)))
...
(defdescribe engine-retains-closed-contexts-test
             ...
             (it
               "an engine per context does not accumulate, unlike a shared one"
               (let [;; ARM A — one engine, five contexts built and CLOSED on it.
                     shared
                     (ep/new-engine!)

                     _
                     (run-and-close! shared)

                     ;; warm: first context pays one-off init
                     a-before
                     (heap-used-mb)
                 ...
                 (expect (< b-growth 40)
                         (str "engine-per-context grew "
                              b-growth
                              " MB over 5 closed contexts"
                              " — something is sharing an engine again (shared arm grew "
                              a-growth
                              " MB)"))
                 ;; Directional, and safe if upstream ever fixes retention (both go to ~0).
                 (expect (<= b-growth a-growth)
                         (str "engine-per-context ("
                              b-growth
                              " MB) retained MORE than a shared engine ("
                              a-growth
                              " MB), which inverts the whole reason for new-engine!")))))

(defdescribe
  session-context-disposal-test
  ...
  (it
    "create-python-context hands back an engine, and closing both is flat"
...
(defdescribe
  engine-build-during-live-eval-test
  ...
  (it
    "builds engines and contexts while another context is mid-eval, on virtual threads"
    (let [busy
          (ep/new-engine!)
