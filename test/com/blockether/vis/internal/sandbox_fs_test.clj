...
(defn- denied? [thunk]
  (try (thunk) false (catch SecurityException _ true)))
...
(defdescribe confined-graalpy-fs-test
  (it "GraalPy open()/listdir is confined to the root; stdlib still loads"
    (let [root (tmp-root)
          fs   (sfs/confined-filesystem (fn [] [root]))
          io   (-> (IOAccess/newBuilder) (.fileSystem fs) (.build))
          ctx  (-> (Context/newBuilder (into-array String ["python"]))
                 (.allowIO io) (.allowAllAccess false) (.build))
          ev   (fn [code] (.eval ctx "python" code))]
      (try
        ;; read inside the root works
        (expect (= "ROOT-DATA" (.asString (ev (str "open(" (pr-str (str root "/inside.txt")) ").read()")))))
