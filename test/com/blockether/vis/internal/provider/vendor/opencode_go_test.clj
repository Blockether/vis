...
(defdescribe shared-key-auth-test
             ...
             (it "resolves the key + endpoint for the single provider"
                 ...
                 (with-redefs-fn {#'config/current-config
                                  (constantly {:providers [{:id :opencode-go :api-key "k"}]})}
                   (fn []
...
(defdescribe
  ...
  (it "reports the live 5h / weekly / monthly windows when authenticated"
      (reload!)
      (with-redefs-fn {#'key-store/detect-key (constantly {:api-key "k" :source :env-var})
                       #'opencode-go/fetch-usage!
                       (constantly
                         {:usage
...
(defdescribe host-envelope-test
             (it "produces rows the host limits spec accepts"
                 ...
                 (with-redefs-fn
                   {#'key-store/detect-key (constantly {:api-key "k" :source :env-var})
                    #'opencode-go/fetch-usage!
                    (constantly
                      {:usage
                       {:rolling {:status "ok" :percent 37 :resetsAt "2026-08-14T12:00:00.000Z"}
...
(defdescribe
  ...
  (it "normalizes the three metered windows shortest first"
      (let [report (opencode-go/usage->dynamic-limits usage-payload)]
  ...
  (it "reports each window as a percentage tank so channels render \"% left\""
      (let [row (first (:limits (opencode-go/usage->dynamic-limits usage-payload)))]
  ...
  (it
    ...
    (let [rows (:limits (opencode-go/usage->dynamic-limits usage-payload))]
      (expect
  ...
  (it "flags an exhausted window on the row and on the report"
      (let [report
            (opencode-go/usage->dynamic-limits
  ...
  (it "drops a window whose reset instant is unparseable rather than throwing"
      (let [report (opencode-go/usage->dynamic-limits
  ...
  (it "returns an explanatory note when the payload carries no windows"
      (let [report (opencode-go/usage->dynamic-limits {:usage {}})]
...
(defdescribe
  ...
  (it
    ...
    (let [seen (atom nil)]
      (with-redefs-fn
        ...
        (fn []
          (let [parsed (opencode-go/fetch-usage! "test-key")]
  ...
  (it "throws with the HTTP status so 401 and 403 stay distinguishable"
      (with-redefs-fn {#'babashka.http-client/get (fn [_url _opts]
        ...
        (fn []
          (expect (= 403
                     (try (opencode-go/fetch-usage! "test-key")
                          nil
