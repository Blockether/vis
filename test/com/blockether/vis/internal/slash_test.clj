...
(defn- ext
  ...
  (extension/extension
    {:ext/name        name
     :ext/description (str name " test extension")
...
(def ^:private workspace-spec
  {:slash/name   "workspace"
   ...
   :slash/run-fn (fn [_ctx] {:slash/status :ok :slash/title "workspace help"})})
...
(def ^:private voice-spec
  {:slash/name   "voice"
   ...
   :slash/availability-fn (fn [ctx] (= :tui (:channel/id ctx)))
   :slash/run-fn          (fn [_] {:slash/status :ok :slash/title "voice toggled"})})
...
(defdescribe aggregation-test
  ...
  (it "slash-children filters by parent vec"
    (let [env (env-of [(ext "alpha" [workspace-spec workspace-apply voice-spec])])]
      (expect (= #{"workspace" "voice"}
                (set (map :slash/name (slash/slash-children env)))))
      (expect (= #{"apply"}
                (set (map :slash/name (slash/slash-children env ["workspace"]))))))))

(defdescribe parse-test
  (it "parses slash text into path tokens + args (raw, no registry lookup)"
    (expect (= {:path ["workspace" "apply" "--hard"]
                :args []
                :raw  "/workspace apply --hard"}
              (slash/parse "/workspace apply --hard")))
    (expect (= {:path ["voice"] :args [] :raw "/voice"}
              (slash/parse "/voice"))))
...
(defdescribe dispatch-test
  ...
  (it "unknown slash returns :reason :unknown"
    (let [env (env-of [(ext "alpha" [workspace-spec])])
          out (slash/dispatch env {:channel/id :tui} "/missing thing")]
      (expect (true? (:handled? out)))
      (expect (= :unknown (:reason out)))))

  (it "missing :slash/requires entries surface :reason :requires-failed"
    (let [env (env-of [(ext "alpha" [workspace-apply])])
          out (slash/dispatch env
  ...
  (it "availability-fn=false yields :reason :unavailable"
    (let [env (env-of [(ext "alpha" [voice-spec])])
          out (slash/dispatch env {:channel/id :telegram} "/voice")]
      (expect (true? (:handled? out)))
      (expect (= :unavailable (:reason out)))))

  (it "availability-fn=true allows dispatch"
    (let [env (env-of [(ext "alpha" [voice-spec])])
          out (slash/dispatch env {:channel/id :tui} "/voice")]
      (expect (true? (:handled? out)))
      (expect (= :ok (get-in out [:result :slash/status])))))

  (it "run-fn throw surfaces :reason :run-failed"
    (let [throwing {:slash/name "boom"
                    :slash/run-fn (fn [_] (throw (ex-info "kaboom" {})))}
          env (env-of [(ext "alpha" [throwing])])
          out (slash/dispatch env {:channel/id :tui} "/boom")]
      (expect (true? (:handled? out)))
      (expect (= :run-failed (:reason out)))
      (expect (= "kaboom" (:error out)))))

  (it "slash with no :slash/run-fn surfaces :reason :no-run-fn"
    (let [info-only {:slash/name "info" :slash/doc "info"}
          env (env-of [(ext "alpha" [info-only])])
          out (slash/dispatch env {:channel/id :tui} "/info")]
      (expect (true? (:handled? out)))
