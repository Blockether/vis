...
(defdescribe
  ...
  (it "refuses a set carrying a key the node type has not got, naming what it does set"
      (let [why (refusal #(patched (view {:id "run" :type :status :text "queued" :tone :idle})
  ...
  (it "adds a node mid-run after the one it names, and refuses a second node with the same id"
      (let [v (-> (view (log-node) (table-node :insertion))
                  (patched {:op :add-node
                            :after "log"
                            :node-spec
                            {:id "extra" :type :status :text "scanning" :tone :running}}))]
  ...
  (it
    ...
    (let [v
          (view {:id "state" :type :status :text "queued" :tone :idle}
                {:id "reading"
                 :type :group
                 :direction :row
                 :fields [(log-node) (table-node :insertion)]})
...
(defdescribe
  ...
  (it "a descending order reverses it and still keeps blanks last"
      (let [v (-> (view (table-node {:by "n" :dir :desc}))
                  (patched {:op :append
                            :node-id "t"
...
(defdescribe
  ...
  (it "budgets a table the same way, and says how many rows it left"
      (let [v
            (patched (view (table-node :insertion))
                     {:op :append
                      :node-id "t"
                      :rows [(row "a" "A" "1") (row "b" "B" "2") (row "c" "C" "3")]})
  ...
  (it "FLATTENS the rows a surface arranges, because the model reads content, not layout"
      (let [v
            (view {:id "state" :type :status :text "queued" :tone :idle}
                  {:id "reading"
                   :type :group
  ...
  (it
    ...
    (let [v
          (patched (view (table-node :newest-first))
                   {:op :append
                    :node-id "t"
                    :rows [(row "a" "A" "1") (row "b" "B" "2") (row "c" "C" "3")]})
...
(defdescribe
  ...
  (it "gives the model a window of a long table and says how many rows it left"
      (let [v
            (patched (view (assoc (table-node :insertion) :max-rows 100))
                     {:op :append
                      :node-id "t"
                      :rows (mapv #(row (str %) (str "row " %) (str %)) (range 30))})
  ...
  (it "grows the leading tone column only when a row carries a tone"
      (let [plain (patched (view (table-node :insertion))
  ...
  (it "keeps a cell that carries a pipe or a newline inside its own row"
      (let [v (patched (view (table-node :insertion))
  ...
  (it "fences a log around whatever backticks its lines carry"
      (let [v (patched (view (log-node 10))
  ...
  (it "says so plainly when a node has nothing in it yet"
      (let [md (live/->markdown (view (log-node)
                                      (table-node :insertion)
                                      {:id "s" :type :stat :label "Score" :stats []}))]
...
(defdescribe
  ...
  (it "reads its own picture back as the view that painted it, node for node"
      (let [{:keys [md again back]} (repainted (rich-view)
                                               {:view-id "v"
                                                :is-completed false
                                                :reason :interrupted
  ...
  (it "paints an empty state that names its own type, and a table keeps its columns"
      (let [{:keys [md again back]} (repainted (view
                                                 {:id "s" :type :stat :label "Score" :stats []}
  ...
  (it "says which rows a budget left behind rather than pretending it holds them"
      (let [{:keys [back]} (repainted (view (assoc (table-node :insertion)
                                              :label "Jobs"
                                              :max-rows 5000
                                              :rows (mapv (fn [i]
                                                            (row (str "r" i) (str i) "1"))
