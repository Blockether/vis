(ns com.blockether.vis.internal.language.clojure.reflection-test
...
(defdescribe
  ...
  (it "does not evaluate a bare top-level call"
      ...
      (reflection/compile-warnings
        "(swap! com.blockether.vis.internal.language.clojure.reflection-test/side-effects inc)"
        "<stdin>")
  ...
  (it
    ...
    (reset! side-effects 0)
    (reflection/compile-warnings
      "(let [n 1] (swap! com.blockether.vis.internal.language.clojure.reflection-test/side-effects + n))"
      "<stdin>")
    (expect (zero? @side-effects)))
  (it
    ...
    (let
      [fs
       (reflection/compile-warnings
         (str
           "(def x (swap! com.blockether.vis.internal.language.clojure.reflection-test/side-effects inc))\n"
           "(defn use-x [] x)")
         "<stdin>")]
      (expect (vector? fs))
      (expect (zero? @side-effects))))
  ...
  (it
    "does not run reader-eval"
    (reset! side-effects 0)
    (reflection/compile-warnings
      "(def y #=(swap! com.blockether.vis.internal.language.clojure.reflection-test/side-effects inc))"
      "<stdin>")
    (expect (zero? @side-effects)))
...
(defn- dependency-loading-code
  ...
  []
  (str
    "(defmacro load-dependency []\n"
    "  (swap! com.blockether.vis.internal.language.clojure.reflection-test/dependency-loads inc)\n"
...
(def ^:private probe-source
  ...
  (str "(defn probe [^String s]"
       "  (com.blockether.vis.internal.language.clojure.reflection-test/counted-compile)"
       "  (.nope s))"))
