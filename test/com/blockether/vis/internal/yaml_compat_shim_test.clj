...
(defdescribe yaml-load-test
  ...
  (it "safe_load_all returns every document in a multi-doc stream"
    (let [{:keys [^Context python-context]} (ep/create-python-context {})]
      (expect (= [{"x" 1} {"y" 2}]
                (ev python-context
                  "list(yaml.safe_load_all('''x: 1\n---\ny: 2'''))"))))))

(defdescribe yaml-dump-test
  ...
  (it "dump writes to a file-like stream and returns None"
    (let [{:keys [^Context python-context]} (ep/create-python-context {})]
      (expect (= "z: 9\n"
                (ev python-context
                  "(lambda s: (yaml.dump({'z': 9}, s), s.getvalue())[1])(__import__('io').StringIO())"))))))

(defdescribe yaml-error-test
  (it "surfaces a malformed document as a catchable yaml.YAMLError"
    (let [{:keys [^Context python-context]} (ep/create-python-context {})]
      (expect (= "YAMLError"
                (ev python-context
  ...
  (it "publishes the module under sys.modules so `import yaml` resolves"
    (let [{:keys [^Context python-context]} (ep/create-python-context {})]
      (expect (true? (ev python-context "__import__('sys').modules.get('yaml') is not None"))))))
