...
(defdescribe registry-contract-test
    ...
    (with-clean-state
      (fn []
        ...
        (let [spec (t/toggle-spec :test/alpha)]
          ...
          (expect (contains? (set (map :id (t/registered-toggles)))
                    :test/alpha))))))
  ...
  (it "re-registering the same id is idempotent and preserves the live value"
    (with-clean-state
      (fn []
        ...
        (t/register-toggle! {:id :test/beta :label "Beta v2" :default false
                             :description "updated"})
  ...
  (it "rejects invalid specs with :vis.toggles/invalid-spec"
    (let [thrown? (try
                    (t/register-toggle! {:label "no-id"})
                    false
                    (catch clojure.lang.ExceptionInfo e
                      (= :vis.toggles/invalid-spec (:type (ex-data e)))))]
      (expect thrown?))))

(defdescribe value-resolution-test
  ...
  (it "listener fires on value transitions and a disposer detaches it"
    (with-clean-state
      (fn []
        ...
        (let [events (atom [])
              dispose (t/add-listener! #(swap! events conj %))]
  ...
  (it "hydrate-from-config! applies persisted values; orphaned ids are skipped"
    (with-clean-state
      (fn []
        ...
        (t/hydrate-from-config! {:toggles {:test/zeta true
                                           :test/orphan-unknown true}})
