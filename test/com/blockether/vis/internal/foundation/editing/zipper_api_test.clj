             ...
             (it "`next` from the LAST node and `prev` from the root FAIL CLOSED"
                 (doseq [{:keys [lang src]} cases]
                   (let [last-node (last (all-paths lang src))]
                     (expect (= :bad-move
                                (get-in (nav lang src last-node ["next"]) [:error :reason]))
                     ...
                     (let [m (z/moves-available lang src p)]
                       (if (seq p)
                         (let
                           ...
                           (expect (= (pos? i) (get m "left")))
                           (expect (= (< (inc i) pc) (get m "right"))))
                         (do (expect (nil? (get m "index")))
                             (expect (nil? (get m "siblings"))))))))))
...
(defdescribe
  ...
  (it "a struct_patch nav that can't resolve fails with a nav error (no write)"
      (let
        ...
        (expect
          (try
            (struct-patch "path" path "at" [] "nav" [{"find" "nope_xyz"}] "op" "replace" "code" "X")
            false
