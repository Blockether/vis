...
(def ^:private utf8-cases
  [{:ext "py"
    ...
    :keep "日本語コメント café ☕"}
   {:ext "py"
    :lang "python" ; multibyte IDENTIFIER (python allows it)
    :src "def café(x):\n    return x\n"
    :needle "café"
    :code "def café(x):\n    return x + 1"
    :want "return x + 1"
    :keep "café"}
   {:ext "clj"
    :lang "clojure" ; emoji + CJK inside a string literal
    :src "(def msg \"hi 🚀 café 日本語\")\n(defn add [a b] (+ a b))\n"
    :needle "add"
    :code "(defn plus [a b] (* a b))"
    :want "plus"
    :keep "🚀 café 日本語"}
   {:ext "rs"
    :lang "rust"
