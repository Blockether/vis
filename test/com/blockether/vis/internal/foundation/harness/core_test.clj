...
(defdescribe
  ...
  (it "dedupes only from the post-fold live-wire index, not a stale durable pointer"
      (with-redefs [d/skill-by-name
                    (fn [_]
                      {:name "demo" :description "d" :body "BODY" :dir "/x" :resources []})]
        (let [digest (extension/sha256-hex "BODY")
              stale {"session_active_skills" {"demo"
  ...
  (it "a matching post-fold live activation returns a receipt; a changed digest reactivates"
      (let [body
            ...
            env
            (skill-env {"engine_live_skill_activations"
                        {"demo" {"name" "demo" "digest" digest "scope" "t1/i1"}}
                        "session_active_skills" {"demo"
                                                 {"name" "demo" "digest" digest "scope" "t1/i1"}}}
                       1
                       2)]
...
(defdescribe
  ...
  (let [exts [core/vis-extension]]
    ...
    (it "renders the loaded SKILL.md as markdown instead of a fenced code block"
        (let [render (get (extension/native-tool-renderers exts) "skill")
              body "## Setup\n\n1. Run `context.mjs`."
