...
(defdescribe session-state-strings-only-test
  ...
  (it "returns a fully string-keyed result with no keyword keys/values at any depth"
    (let [inspect @#'introspection/foundation-inspect
          data    (:result (inspect {:session-id nil :db-info nil}))]
  ...
  (it "a POPULATED session_state (turns/calls/timeline/diagnosis + string-keyed llm maps) crosses the boundary with no keyword leak"
    (let [s (vis/db-create-connection! :memory)]
      (try
        (let [cid  (h/store-session! s {:channel :tui :title "Boundary fixture"
                                        :provider :openai :model "gpt-4o"})
              turn (vis/db-store-session-turn! s {:parent-session-id cid
                                                  :user-request "run a tool"
                                                  :status :running})
              _    (h/store-iteration!
                     s {:session-turn-id turn
                        :code "(v/tool \"echo hi\")"
