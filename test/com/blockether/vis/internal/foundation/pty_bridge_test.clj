  ...
  (let [listeners (atom [])
    ...
    {:listeners listeners
     ...
     :handle    {:add-listener (fn [f]
                 ...
                 :send (fn [^bytes b] (swap! sent conj (String. b)))}}))

(defn- tmp-sock []
  (str (System/getProperty "java.io.tmpdir") "vis-bridge-test-"
    (System/nanoTime) ".sock"))

(defn- connect ^SocketChannel [path]
...
(defn- read-str
  ...
  (let [buf (ByteBuffer/allocate 1024)
        n   (.read ch buf)]
    (.flip buf)
    (let [ba (byte-array (.remaining buf))] (.get buf ba) (String. ba))))
...
(defdescribe pty-bridge-test
  ...
  (it "replays recent output, tees live output, and forwards client input"
    (let [{:keys [sent handle listeners]} (fake-pty)
          path (tmp-sock)
          {:keys [stop] sp :path} (pb/serve! {:pty handle :path path
                                              :replay-fn (fn [] (.getBytes "REPLAY\n"))})]
      (try
        (expect (.exists (File. ^String sp)))
        (with-open [ch (connect sp)]
          ;; late attacher gets the replay buffer
          (expect (= "REPLAY\n" (read-str ch)))
          ;; live master output is teed to the attached client
          (Thread/sleep 50)
          (doseq [l @listeners] (l (.getBytes "LIVE\n")))
          (expect (= "LIVE\n" (read-str ch)))
  ...
  (it "find-socket resolves an explicit socket path"
    (let [{:keys [handle]} (fake-pty)
          path (tmp-sock)
          {:keys [stop] sp :path} (pb/serve! {:pty handle :path path})]
      (try
        (expect (= sp (str (pb/find-socket {:socket sp}))))
        (finally (stop)))))

  (it "attach! returns exit code 2 for a missing socket"
    (expect (= 2 (pb/attach! {:socket (str (System/getProperty "java.io.tmpdir")
                                        "vis-nope-" (System/nanoTime) ".sock")})))))
