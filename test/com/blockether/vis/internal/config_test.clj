...
(defdescribe ->svar-provider-test
  ...
  (it "forwards svar's catalog api-key for local no-auth presets (lmstudio/ollama)"
    (expect (= "lmstudio"
              (:api-key (config/->svar-provider
                          {:id :lmstudio :base-url "http://localhost:1234/v1"
                           :models [{:name "probe"}]}))))
    (expect (= "ollama"
              (:api-key (config/->svar-provider
                          {:id :ollama :base-url "http://localhost:11434/v1"
                           :models [{:name "probe"}]})))))

  (it "prefers an explicitly configured api-key over the catalog fallback"
    (expect (= "user-key"
              (:api-key (config/->svar-provider
                          {:id :lmstudio :api-key "user-key"
                           :models [{:name "probe"}]})))))

  (it "leaves cloud presets keyless when none is configured (no catalog dummy)"
    (expect (nil? (:api-key (config/->svar-provider
                              {:id :openrouter :models [{:name "probe"}]}))))))
