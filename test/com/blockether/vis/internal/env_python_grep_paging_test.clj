(ns com.blockether.vis.internal.env-python-grep-paging-test
  "A CAPPED SEARCH PAGES ITSELF (`resources/vis-python/async_runtime.py`).
   ...
   yields CHARACTERS), plus `next_offset` / `more()` / `pages()` / `all()`. These
...
(defdescribe
  ...
  (it "more() walks to the end and carries the WHOLE options map, not just an offset"
      (let [[out calls]
            (run (str "g = grep({\"query\": \"q\", \"paths\": [\"src\"]})\n" "p2 = g.more()\n"
                      "p3 = p2.more()\n" "print(p2.next_offset, p3.next_offset, p3.more())"))]
  ...
  (it "an UNCAPPED page already is the whole answer"
      (let [[out calls]
            (run (str "g = grep({\"query\": \"done\", \"paths\": [\"src\"]})\n"
                      "print(g.next_offset, g.is_capped, g.more(), len(list(g.pages())))"))]
