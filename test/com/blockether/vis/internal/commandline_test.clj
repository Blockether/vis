...
(defdescribe render-command-grouping-test
  ...
  (it "falls back to a single SUBCOMMANDS section when children are not mixed"
    (binding [commandline/*color-enabled?* false]
      (let [out (commandline/render-command
                  {:cmd/name "only-internal" :cmd/doc "x"
                   :cmd/subcommands [{:cmd/name "a" :cmd/doc "a" :cmd/internal? true}
