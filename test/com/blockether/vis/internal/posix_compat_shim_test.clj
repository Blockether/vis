...
(defdescribe subprocess-bridge-test
  (it "routes subprocess.run through shell_run and returns a CompletedProcess"
    (let [calls (atom [])
  ...
  (it "check_output returns stdout and raises on a non-zero exit"
    (let [calls (atom [])
      ...
      (let [msg (try (.eval python-context "python" "subprocess.check_output('boom', shell=True)")
                  nil (catch Throwable t (.getMessage t)))]
        (expect (some? msg))
