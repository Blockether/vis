...
(defdescribe scan-roots-test
  (it "stacks global → ancestor → workspace root, outermost first"
    (with-tmp-root*
      (fn [^java.io.File root]
        (let [global (doto (java.io.File. root "fake-home-vis") .mkdirs)
              parent (doto (java.io.File. root "repo") .mkdirs)
              ws     (doto (java.io.File. parent "sub") .mkdirs)]
          ...
          (let [{:keys [result warnings]} (agents/scan-roots global ws)
                files (:files result)
                scoped (mapv (juxt :scope :content) files)]
            (expect (true? (:found? result)))
            (expect (empty? warnings))
  ...
  (it "per-directory AGENTS.md beats CLAUDE.md; other dirs still fall back"
    (with-tmp-root*
      (fn [^java.io.File root]
        (let [parent (doto (java.io.File. root "repo") .mkdirs)
              ws     (doto (java.io.File. parent "sub") .mkdirs)]
          ...
          (let [{:keys [result]} (agents/scan-roots nil ws)
            ...
            (expect (= [:project :claude-md]
                      ((juxt :scope :source) (peek files))))
  ...
  (it "nothing anywhere -> :found? false"
    (with-tmp-root*
      (fn [^java.io.File root]
        (let [ws (doto (java.io.File. root "empty") .mkdirs)
              {:keys [result warnings]} (agents/scan-roots nil ws)]
          (expect (false? (:found? result)))
          (expect (empty? warnings)))))))
