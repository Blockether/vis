...
(defn- fresh-kind!
  ...
  [k sink]
  (res/declare-kinds! {k {:resource/label (name k) :resource/release (fn [h _v] (swap! sink conj h))}})
  k)

(defdescribe scope-teardown-test
  (it "frees what a scope owns, leaves other scopes alone, and is idempotent"
    (let
      [freed
       (atom [])
  ...
  (it "keeps going after a release throws"
    (let
      [freed
       ...
       _
       (res/declare-kinds!
         {::boom {:resource/label "boom" :resource/release (fn [_ _] (throw (RuntimeException. "boom")))}})
...
(defdescribe undeclared-kind-test
  ...
  (it "refuses to open a kind that was never declared"
    (let [thrown (try (res/open! nil ::never-declared :x) nil (catch clojure.lang.ExceptionInfo e e))]
  ...
  (it "releases the oldest entry once a kind is at its cap"
    (let
      [freed
       ...
       _
       (res/declare-kinds!
         {::capped {:resource/label "capped"
                    :resource/release (fn [h _] (swap! freed conj h))
                    :resource/max 3}})
       ...
       hs
       (doall (for [i (range 5)] (res/open! (res/scope-of ctx) ::capped i)))]
...
(def ^:private shim-sources
  ...
  (delay
    (->> (file-seq (io/file "src/com/blockether/vis/internal/foundation"))
         (filter #(.isFile ^java.io.File %))
         (filter #(re-matches #"shim_.*\.clj" (.getName ^java.io.File %)))
         (mapv (fn [f] [(.getName ^java.io.File f) (slurp f)])))))

(defdescribe shim-declares-what-it-lends-test
  (it "every kind a shim opens is declared in that shim's :shim/resources"
    (let
      [offenders
    ...
    (let
      ...
      (expect (empty? offenders)
              (str "these shims hand-roll a handle registry instead of declaring"
                   " :shim/resources: " (str/join ", " offenders))))))
