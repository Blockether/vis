...
(defdescribe
  ...
  (it "imports a PEM into a cached store that ALSO keeps the public roots"
      (let [{:keys [pem env]}
        ...
        (let [listing (:out (keytool! ["-list" "-keystore" store "-storepass" "changeit" "-alias"
                                       "vis-custom-ca-1"]))]
  ...
  (it "exports the truststore through JAVA_TOOL_OPTIONS, which every forked JVM reads"
      (let [{:keys [pem env]}
        ...
        (expect (zero? exit))
        (expect (re-find #"export JAVA_TOOL_OPTIONS=.*-Djavax\.net\.ssl\.trustStore=" out))
