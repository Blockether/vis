(ns com.blockether.vis.loop.hooks-test
  ...
  (:require
   ...
   [com.blockether.vis.loop.runtime.conversation.environment.core :as tools]))
...
(defdescribe tool-contracts-test
  (describe "function metadata + validator contracts"
    (it "infers :doc and :arglists from function metadata"
      (let [env (sut/create-env (stub-router) {:db :temp})
            f (with-meta-fn (fn [x] x) {:doc "identity doc" :arglists '([x])})]
        (try
          (sut/register-env-fn! env 'identity-meta f {})
          (let [tools (sut/list-registered-tools env)
                has-tool? (some #(= 'identity-meta (:sym %)) tools)]
            (expect (true? (boolean has-tool?)))
            (expect (= :ok (eval-in-sandbox env "(identity-meta :ok)")))
            (expect (= "identity doc" (get-in @(:tool-registry-atom env) ['identity-meta :doc])))
            (expect (= '([x]) (get-in @(:tool-registry-atom env) ['identity-meta :arglists]))))
          (finally (sut/dispose-env! env)))))

    (it "validate-input can reject bad args before function execution"
      (let [env (sut/create-env (stub-router) {:db :temp})
            called (atom false)]
        (try
            ...
            {:doc "needs int"
             :validate-input-fn (fn [{:keys [args]}]
                               (let [[x] args]
                                 ...
                                 {:args [x]}))})
          (let [thrown? (try
                          (eval-in-sandbox env "(needs-int :bad)")
                          false
                          (catch Exception _ true))]
            (expect (true? thrown?))
            (expect (false? @called)))
    ...
    (it "validate-output can reject invalid return shape"
      (let [env (sut/create-env (stub-router) {:db :temp})]
        (try
          (sut/register-env-fn! env 'bad-out
            ...
            {:doc "bad out"
             :validate-output-fn (fn [{:keys [result]}]
                                ...
                                {:result result})})
          (let [thrown? (try
                          (eval-in-sandbox env "(bad-out :x)")
                          false
                          (catch Exception _ true))]
            (expect (true? thrown?)))
          (finally (sut/dispose-env! env)))))

    (it "validate-input can normalize args"
      (let [env (sut/create-env (stub-router) {:db :temp})]
        (try
    ...
    (it "validate-output can normalize result"
      (let [env (sut/create-env (stub-router) {:db :temp})]
        (try
