(ns com.blockether.vis.loop.knowledge.markdown-test
  "Unit tests for the RLM gap-bundle surface: markdown parser,
   ...
   and SCI document query tool bindings."
  (:require
   [lazytest.core :refer [defdescribe describe expect it throws?]]
   [sci.core :as sci]
   [com.blockether.svar.internal.llm :as llm]
   [com.blockether.vis.core :as sut]
   [com.blockether.vis.loop.knowledge.pageindex.markdown :as md]
...
(defdescribe sci-document-bindings-test
  (describe "W3 SCI bindings"
    (it "sci sandbox resolves document search/fetch bindings"
      (let [router (llm/make-router [{:id :test :api-key "test" :base-url "http://localhost"
                                      :models [{:name "stub" :input-cost 0 :output-cost 0}]}])
        ...
        (try
          (let [sci-ctx (:sci-ctx env)
            ...
            (expect (resolved? 'fetch-document-content))
            (expect (resolved? 'search-batch)))
          (finally (sut/dispose-env! env)))))))
