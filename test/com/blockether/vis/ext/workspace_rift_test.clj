...
(def ^:private delete-tree!
  @#'com.blockether.vis.ext.workspace-rift/delete-tree!)
...
(defdescribe delete-tree-test
  ...
  (it "throws with failure and remaining-path data when deletion is partial"
    (let [root (temp-dir "vis-rift-delete-fail")
          keep (io/file root "keep.txt")
          original-delete @#'com.blockether.vis.ext.workspace-rift/delete-path!]
      (try
        (spit keep "keep")
        (with-redefs [com.blockether.vis.ext.workspace-rift/delete-path!
                      (fn [path]
                        (if (= "keep.txt" (some-> path .getFileName str))
