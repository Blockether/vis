  ...
  (it
    ...
    (let [{:debt/keys [production test]}
      ...
      (expect
        (empty? missing)
        (str
          "the contract freezes internal namespaces that are gone: "
          (str/join ", " missing)
          " — delete the entry from packages/vis-contract/resources/vis-contract/clojure-host.edn"))))
  (it
    ...
    (when-let [root (repo-root)]
      (let [found (contract/internal-requires root)
        ...
        (expect
          (empty? new-coupling)
          (str
            "extensions now require internal namespaces the host contract does not"
            " allow:\n" (str/join "\n" new-coupling)
            "\nExport what the extension needs from " (contract/facade)
            ", or — only if it is host-level behavior — add the namespace to"
            " packages/vis-contract/resources/vis-contract/clojure-host.edn with a reason in review.")))))
  (it
    ...
    (when-let [root (repo-root)]
      (let [found (contract/internal-requires root)
        ...
        (expect
          (empty? stale)
          (str
            "the contract still freezes coupling nothing uses:\n" (str/join "\n" stale)
            "\nDelete these from packages/vis-contract/resources/vis-contract/clojure-host.edn — the"
