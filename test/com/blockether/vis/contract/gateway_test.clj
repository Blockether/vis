...
(defdescribe
  ...
  (it
    ...
    (expect (= 108 (count contract/route-table)))
    (expect (= 132 (count (contract/route-methods))))
    (expect (= {:none 95 :json 33 :binary 4}
               (frequencies (map :request (mapcat (comp vals :operations) contract/route-table)))))
    (expect (= {:json 115 :resource 2 :sse 5 :empty 3 :binary 4 :negotiated 1 :html 1 :markdown 1}
