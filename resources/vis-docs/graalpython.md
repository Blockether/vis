...
GraalPy is a Truffle language on the GraalVM runtime, so the interpreter shares the process with the Clojure core. Tools are exposed to the sandbox as ordinary async functions — `grep`, `struct_index`, `struct_nodes`, `struct_patch` — while `apropos` and `doc` synchronously inspect the live surface. The model can compose, filter, and summarize many results in vars, then print only the useful slice into context.
