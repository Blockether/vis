...
Vis's core is Clojure on the JVM. `clojure -T:build native` (or `bin/vis-agent native`) can compile that core into a fast GraalVM native runtime, but the native executable is never the public distribution by itself: releases always place it beside the `vis-agent` Bash wrapper as `vis-agent-native`.
