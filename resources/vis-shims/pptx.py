def __vis_install_pptx__():
    ...
    class _Run(object):
        def __init__(self, sh, pidx, ridx):
            ...
            self.font = _Font(
                lambda props: _raise(
                    *_run_font(sh._pres._h, sh._sid, pidx, ridx, props)
                )
            )
        ...
        def text(self, value):
            ...
            _raise(
                *_run_text(
                    self._sh._pres._h, self._sh._sid, self._pidx, self._ridx, self._text
                )
            )
    ...
    class _Paragraph(object):
        def __init__(self, sh, pidx):
            ...
            self._runs = []
            self.font = _Font(
                lambda props: _raise(*_para_font(sh._pres._h, sh._sid, pidx, props))
            )
        ...
        def text(self, value):
            _raise(
                *_para_text(self._sh._pres._h, self._sh._sid, self._pidx, str(value))
            )
        ...
        def level(self, value):
            self._level = value
            _raise(
                *_para_level(self._sh._pres._h, self._sh._sid, self._pidx, int(value))
            )
    ...
    class _Fill(object):
        ...
        def fore_color(self):
            if self._kind != "solid":
                ...
                raise TypeError(
                    "fill type %s has no foreground color, call .solid() or .patterned() first"
                    % cls
                )
            return self._fore
    ...
    class _Shapes(object):
        ...
        def add_textbox(self, left, top, width, height):
            ...
            sh._left, sh._top, sh._width, sh._height = (
                int(left),
                int(top),
                int(width),
                int(height),
        ...
        def placeholders(self):
            return _Placeholders(
                [(sh._ph_idx, sh) for sh in self._items if sh._ph_type is not None]
            )
    ...
    class Slide(object):
        def __init__(self, pres, index, placeholders):
            ...
            for ph in placeholders:
                sh = Shape(
                    pres, ph["shape_id"], ph_type=ph["ph_type"], name=ph.get("name")
                )
                sh._ph_idx = ph["idx"]
    ...
    class _Slides(object):
        ...
        def add_slide(self, slide_layout=None):
            li = (
                slide_layout.index
                if isinstance(slide_layout, _SlideLayout)
                else (slide_layout if isinstance(slide_layout, int) else 6)
            )
