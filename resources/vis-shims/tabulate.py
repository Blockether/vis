...
# real `tabulate` module instead of leaking into the sandbox globals.
    ...
    def _tabulate_module_body():
        ...
        def _html_row_with_attrs(celltag, unsafe, cell_values, colwidths, colaligns):
            ...
            if unsafe:
                ...
                values_with_attrs = [
                    "<{0}{1}>{2}</{0}>".format(
                        celltag, alignment.get(a, ""), htmlescape(c)
                    )
                    for c, a in zip(cell_values, colaligns)
                ]
        ...
        def _latex_line_begin_tabular(
            colwidths, colaligns, booktabs=False, longtable=False
        ):
        ...
        def _asciidoc_row(is_header, *args):
            ...
            if len(args) == 2:
                ...
                if is_header:
                    line = (
                        make_header_line(True, colwidths, colaligns) + "\n" + data_line
                    )
                    return line.rstrip()
                else:
...
"""
    ...
    """
        ...
        def _strip_ansi(s):
            ...
            >>> repr(
            ...     _strip_ansi(
            ...         "\x1b]8;;https://example.com\x1b\\This is a link\x1b]8;;\x1b\\"
            ...     )
            ... )
            "'This is a link'"
        ...
        def _align_column(
            ...
            if is_multiline:
                if not enable_widechars and not has_invisible:
                    ...
                    s_lens = [
                        [len(s) for s in re.split("[\r\n]", ms)] for ms in strings
                    ]
                    visible_widths = [
                        [maxwidth - (w - ln) for w, ln in zip(mw, ml)]
                    ...
                    padded_strings = [
                        "\n".join(
                            [padfn(w, s) for s, w in zip((ms.splitlines() or ms), mw)]
                        )
                ...
                if not enable_widechars and not has_invisible:
                    ...
                    visible_widths = [
                        maxwidth - (w - ln) for w, ln in zip(s_widths, s_lens)
                    ]
                    # wcswidth and _visible_width don't count invisible characters;
                    # padfn doesn't need to apply another correction
                    padded_strings = [
                        padfn(w, s) for s, w in zip(strings, visible_widths)
                    ]
    ...
    """
        ...
        def _align_header(
            ...
            if is_multiline:
                ...
                padded_lines = [
                    _align_header(h, alignment, width, width_fn(h))
        ...
        def _normalize_tabular_data(tabular_data, headers, showindex="default"):
            ...
            * pandas.DataFrame (usually used with headers="keys")
            ...
            if showindex_is_a_str and showindex == "default" and index is not None:
                ...
                rows = _prepend_row_index(rows, index)
            elif showindex == "never" or (
                not _bool(showindex) and not showindex_is_a_str
            ):
                pass
    ...
    """
            ...
            coltypes = [
                _column_type(col, numparse=np) for col, np in zip(cols, numparses)
            ]
            ...
            if isinstance(intfmt, str):  # old version
                ...
                if len(int_formats) < len(cols):
                    int_formats.extend(
                        (len(cols) - len(int_formats)) * [_DEFAULT_INTFMT]
                    )
            ...
            if colglobalalign is not None:  # if global alignment provided
                ...
                aligns = [
                    numalign if ct in [int, float] else stralign for ct in coltypes
                ]
            ...
            if colalign is not None:
                ...
                for idx, align in enumerate(colalign):
                    if not idx < len(aligns):
                        ...
                        aligns[idx] = align
            minwidths = (
                [width_fn(h) + min_padding for h in headers]
                if headers
                else [0] * len(cols)
            )
                ...
                if headersalign is not None:
                    ...
                    for idx, align in enumerate(headersalign):
                        ...
                        if not hidx < len(aligns_headers):
                            break
                        elif align == "same" and hidx < len(
                            aligns
                        ):  # same as column align
        ...
        def _build_simple_row(padded_cells: list[list], rowfmt: DataRow) -> str:
            ...
            if escape_map:
                ...
                escaped_cells = [
                    "".join(map(escape_char, cell)) for cell in padded_cells
                ]
            else:
        ...
        def _format_table(
            ...
            if padded_headers:
                append_row(
                    lines, padded_headers, padded_widths, headersaligns, headerrow
                )
        ...
        class _CustomTextWrap(textwrap.TextWrapper):
            ...
            def _handle_long_word(self, reversed_chunks, cur_line, cur_len, width):
                ...
                if self.break_long_words and space_left > 0:
                    ...
                    while (
                        i <= len(stripped_chunk)
                        and self._len(stripped_chunk[:i]) <= space_left
                    ):
                        i = i + 1
                    ...
                    if _ansi_codes.search(chunk) is not None:
                        for group, _, _, _ in _ansi_codes.findall(chunk):
                            ...
                            if (
                                group
                                in chunk[
                                    last_group : i + total_escape_len + escape_len - 1
                                ]
                            ):
            ...
            def _wrap_chunks(self, chunks):
                ...
                if self.max_lines is not None:
                    ...
                    if (
                        self._len(indent) + self._len(self.placeholder.lstrip())
                        > self.width
                    ):
