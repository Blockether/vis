...
API style / default models) and `get_token`; the other callable slots are
optional.
  ...
  * `get_token` — returns the credential for each request; raising is the
    "not configured" path the CLI/TUI surfaces.
  * `status`    — powers `vis providers status acme`.
