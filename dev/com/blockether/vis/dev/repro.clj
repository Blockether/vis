...
(defn- replay-target
  ...
  {:provider (:llm-actual-provider iter)
   :model (some-> (:llm-actual-model iter)
                  str)})
...
(defn loop-symptom-summary
  ...
  (let [{:keys [turns]} (replay-session session-id)]
    (mapv
      (fn [{:keys [turn-position iterations]}]
        (let
          [n (count iterations)
           tail (take-last 5 iterations)
           cache-vals (map (fnil identity 0) (map :cached-tokens tail))
           cache-spread (when (seq cache-vals) (- (reduce max cache-vals) (reduce min cache-vals)))
           ...
           rt-last (some-> (last iterations)
                           :reasoning-tokens
                           (or 0))
           reasoning-spiral? (and (>= n 5) (pos? (or med 0)) (>= (long rt-last) (* 2 (long med))))]
          ...
          {:turn-position turn-position
           ...
           :flags (cond-> #{}
                    ...
                    replay-flatlined?
                    (conj :replay-flatlined)

                    reasoning-spiral?
                    (conj :reasoning-spiral))}))
      turns)))
