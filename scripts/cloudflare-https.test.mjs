...
const account = '1'.repeat(32),
  zone = '2'.repeat(32),
  rulesetId = '3'.repeat(32),
  ruleId = '4'.repeat(32);
const hostname = 'gateway.example.com';
const env = {
  HTTPS_URL: `https://${hostname}/healthz`,
  WORKER_NAME: 'vis-docs',
...
const failure = (status, code) =>
  Response.json({ success: false, errors: [{ code, message: canary }] }, { status });
...
test('repairs only the stable owned rule, retaining its identity and unrelated rules', async () => {
  const calls = api(
    ok([domain]),
    ok(ruleset([unrelated, { ...rule, id: ruleId, enabled: false }])),
    ok(ruleset([rule, unrelated])),
  );
  await configureHTTPS(env);
  assert.deepEqual(calls[2], {
    method: 'PATCH',
    path: `${zonePath}/${rulesetId}/rules/${ruleId}`,
    body: { ...rule, position: { index: 1 } },
  });
  assert.equal(calls.length, 3);
});
...
for (const status of [400, 409])
  test(`first-deployment race (${status}) rereads the phase and adds only its own rule`, async () => {
    const calls = api(
      ok([domain]),
      failure(404, 10003),
      failure(status, 2002),
      ok(ruleset([unrelated])),
      ok(ruleset([rule, unrelated])),
    );
    await configureHTTPS(env);
    assert.deepEqual(
...
for (const [status, code] of [
  ...
  test(`phase error ${status}/${code} never creates or overwrites rules`, async () => {
    const calls = api(ok([domain]), failure(status, code));
    await assert.rejects(
      configureHTTPS(env),
      (error) => error.message.includes(`HTTP ${status}`) && !error.message.includes(canary),
    );
    assert.equal(calls.length, 2);
    assert.ok(calls.every((call) => call.method === 'GET'));
  });
...
for (const domains of [
  ...
  [{ ...domain, zone_id: '../other' }],
  [domain, domain],
])
  test(`requires one enabled production Custom Domain: ${JSON.stringify(domains)}`, async () => {
...
for (const value of [
  ...
  ruleset(null),
  { ...ruleset([]), id: '../other' },
  { ...ruleset([]), phase: 'another_phase' },
  ruleset([{ ...rule, id: '../other' }]),
  ruleset([
    { ...rule, id: ruleId },
    { ...rule, id: ruleId },
  ]),
])
  test(`rejects malformed ruleset identity: ${JSON.stringify(value)}`, async () => {
    const calls = api(ok([domain]), ok(value));
    await assert.rejects(configureHTTPS(env), /Unexpected Cloudflare HTTPS/);
    assert.equal(calls.length, 2);
  });

for (const HTTPS_URL of [
  '',
  ...
  'https://gateway.example.com\\@another.example.com',
])
  test(`rejects invalid configuration before network: ${HTTPS_URL}`, async () => {
    const calls = api();
...
for (const key of ['CLOUDFLARE_ACCOUNT_ID', 'CLOUDFLARE_API_TOKEN', 'WORKER_NAME'])
  test(`requires ${key} before network`, async () => {
    const calls = api();
...
test('network and malformed JSON errors never expose credential or response text', async () => {
  for (const response of [new Error(canary), new Response(canary, { status: 500 })]) {
    ...
    await assert.rejects(
      configureHTTPS(env),
      (error) =>
        error.message.includes('connection or response failed') && !error.message.includes(canary),
    );
    assert.equal(calls.length, 1);
    mock.restoreAll();
  }
...
test('verifies GET and HEAD redirects without following them or sending credentials', async () => {
  ...
  assert.deepEqual(
    calls,
    ['/healthz?source=a%2Fb&check=1', '/__vis_https_probe__/a%2Fb?check=1&value=x%2Fy'].flatMap(
      (path) => [
        [path, 'GET'],
        [path, 'HEAD'],
      ],
...
for (const [status, location] of [
  ...
  [308, null],
  [308, `http://${hostname}/`],
  [308, 'https://another.example.com/'],
  [308, `https://${hostname}/wrong-path`],
  [308, `https://${hostname}/?extra=1`],
])
  ...
  mock.method(
    ...
    async (_, options) =>
      new Response(
        null,
        options.method === 'GET'
          ? { status: 308, headers: { location: `https://${hostname}/` } }
          : { status: 200 },
      ),
  );
  await assert.rejects(verifyHTTPS(`https://${hostname}`), /Expected HTTP 308/);
...
test('verification errors and cross-origin paths cannot disclose data or send a request', async () => {
  ...
  await assert.rejects(
    verifyHTTPS(`https://${hostname}`, ['//another.example.com/']),
    /configured origin/,
  );
  assert.equal(fetch.mock.callCount(), 0);
  await assert.rejects(
    verifyHTTPS(`https://${hostname}`),
    (error) => error.message === 'HTTP to HTTPS verification connection failed',
  );
});
