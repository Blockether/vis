...
function publicURL(value) {
  ...
  try {
    ...
    /* Report configuration, not its contents. */
  }
  if (
    !url ||
    ...
    throw new Error(
      'Configure HTTPS_URL as a public HTTPS URL without credentials, port or fragment',
    );
  }
  return url;
}
...
function checkedRuleset(value) {
  if (
    ...
    throw new Error('Unexpected Cloudflare HTTPS ruleset response');
  return value;
}
...
export async function configureHTTPS(env) {
  ...
  if (!isId(env.CLOUDFLARE_ACCOUNT_ID) || !env.CLOUDFLARE_API_TOKEN || !env.WORKER_NAME)
    throw new Error('Configure Cloudflare credentials and WORKER_NAME');
  async function request(method, path, body) {
    ...
    try {
      ...
      throw new Error('Cloudflare HTTPS API connection or response failed');
    }
    if (!response.ok || data?.success !== true) {
      const codes = Array.isArray(data?.errors)
        ? data.errors.map((error) => error?.code).filter(Number.isInteger)
        : [];
      const error = new Error(
        `Cloudflare HTTPS API failed (HTTP ${response.status}; codes ${codes.join(',')})`,
      );
  ...
  if (
    matches.length !== 1 ||
    domain.enabled !== true ||
    domain.service !== env.WORKER_NAME ||
    domain.environment !== 'production' ||
    !isId(domain.zone_id)
  )
    throw new Error(
      'HTTPS hostname must be an enabled production Custom Domain of WORKER_NAME in this account',
    );
  const zonePath = `/zones/${domain.zone_id}/rulesets`;
  async function entrypoint() {
    try {
      return checkedRuleset(await request('GET', `${zonePath}/phases/${phase}/entrypoint`));
    } catch (error) {
  ...
  if (owned.length > 1 || (owned.length && !isId(owned[0].id)))
    throw new Error('Unexpected Cloudflare HTTPS rule identity');
  const existing = owned[0];
  if (existing && Object.keys(rule).every((key) => isDeepStrictEqual(existing[key], rule[key])))
    return;
  await request(
    existing ? 'PATCH' : 'POST',
    `${zonePath}/${ruleset.id}/rules${existing ? '/' + existing.id : ''}`,
...
export async function verifyHTTPS(value, paths) {
  ...
  for (const path of paths ?? [
    ...
    '/__vis_https_probe__/a%2Fb?check=1&value=x%2Fy',
  ]) {
    const target = new URL(path, url);
    if (target.origin !== url.origin || target.hash)
      throw new Error('HTTPS verification paths must stay on the configured origin');
    ...
    for (const method of ['GET', 'HEAD']) {
      ...
      try {
        ...
        throw new Error('HTTP to HTTPS verification connection failed');
      }
      const location = response.headers.get('location');
      await response.body?.cancel();
      if (response.status !== 308 || location !== target.href)
        throw new Error('Expected HTTP 308 to the same HTTPS host, path and query');
    }
  }
}
