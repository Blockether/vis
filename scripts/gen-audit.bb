...
**This audit runs `PLAN → EXECUTION → EFFECTS`.**

- **PLAN** — inventory every shipped dependency (version, license, size,
  owner); scan the full resolved graph for known CVEs; keep both fresh
  automatically; settle redistribution rights on the native binary.
- **EXECUTION** — `scripts/gen-audit.bb` renders the §4 inventory, the §5
  license table and the §7 footprint from the resolved `deps.edn` graph +
  upstream POMs; `.github/workflows/security-audit.yml` runs clj-watson on
  every dependency change, weekly, and on demand;
  `.github/workflows/audit-md.yml` regenerates this file and fails any PR that
  leaves it stale.
- **EFFECTS** — current counts in §2, licenses in §5, distribution rights in
  §6. Every figure is pulled from the code, not asserted.
...
**Record shape.** Design is schema-first: every §4 inventory row is one value
of this shape, emitted by the generator and rendered verbatim —

```clojure
;; malli
[:map
 [:module  :string]                       ; declaring deps.edn module
 [:coord   :symbol]                        ; group/artifact
 [:version :string]
 [:license :string]                        ; SPDX-ish id, resolved from the POM
 [:size    [:maybe :int]]                  ; direct jar size in bytes
 [:owner   [:enum :blockether :3rd-party]]]
```
...
- **Declared jar footprint (direct coords):** ~" (format "%.0f" (/ total-b 1024.0 1024.0)) " MB; concentrated in the embedded GraalPy runtime and the optional voice/ONNX stack (§7).
