...
def patched_init_state(
    ...
    initial_step = torch.tensor(
        [0], dtype=torch.long, device=self.in_proj.weight.device
    )
...
def patched_get_mask(
    self, shape: tuple[int, torch.Tensor], shift: torch.Tensor, device: torch.device
):
...
def verify_export(flow_lm_path, mimi_path, tts_model, output_dir="onnx_models"):
    ...
    if os.path.exists(conditioner_path):
        ...
        onnx_conditioner_out = ort_conditioner.run(
            None, {"token_ids": test_tokens.numpy()}
        )[0]
