...
| Q5 | 16 KB byte-truncate, fixed constant, no config knob. Marker: `[TRUNCATED — N more bytes. Read full content via (vis/main-agent-instructions).]` |
...
| Q8 | Two new files: `environment/agents.clj` + `environment/skills.clj`. Cwd-keyed cache; reload is explicit (Q11). |
...
| Q15 | Persistence via existing `iteration.metadata` JSON column + `log` table. **No new schema, no migration.** Markers cached on the in-memory ext map at register-time, written into `iteration.metadata.extensions[]` per iteration. Reload events go to `log` via Telemere. |
...
| Q19 | `:ext/doctor-checks` extension contract field. Map-wrapping shape. Diagnostic message map. Exit codes 0 / 1 / 2 by max level. |
...
;; => {:added [com.foo.ext] :removed [com.bar.ext] :reloaded [...]
;;     :errors [{:ns ... :phase :require | :register | :deregister | :side-effects | :env-reseat
;;              :reason "..." :stack-trace "..."} ...]
;;     :envs-reseated 3 :duration-ms 847 :blocked-ms 127}
```
...
| `extensions/common/vis-foundation/src/com/blockether/vis/ext/foundation/environment/core.clj` | `def cache` → `defonce`. Remove agents/skills from `compute-snapshot` (they're independent of cwd-cache) |
...
| `src/com/blockether/vis/internal/extension.clj` | Validate `:ext/doctor-checks`. Add `resolve-source-markers` helper (paths, max-mtime, sha256). Beef up `deregister-extension!` to reverse all `:ext/cli` / `:ext/channels` / `:ext/providers` / `:ext/persistance` side effects. Keep idempotency. |
...
- `extension.clj`: `:ext/doctor-checks` validation rejects bad shapes; `deregister-extension!` reverses side effects across all four registries
...
This plan was extracted from a 22-question grilling session per the `grill-me` skill. Each decision is traceable to a numbered Q in section 1; the rationale lives in the conversation history, not duplicated here.
