(ns com.blockether.vis-extension.cancellation
  "Shared cancellation primitive used by every channel that runs an
   async query (TUI today; web/telegram/CLI tomorrow).

   Lives in vis-extension because the wrapper is purely channel-side:
   the loop only honors the bare cooperative `:cancel-atom` opt of
   `query!`. Every channel implementation pairs that flag with a
   worker `Future` so a hard cancel can interrupt a blocking HTTP
   wait, and the matching `cancellation?` helper folds an
   InterruptedException-in-cause-chain into a normal cancelled
   outcome.
