(ns com.blockether.vis.channels.tui.table-render-test
  ...
  (let [pad-cell @#'render/pad-cell]
    (describe "pad-cell pads to display columns, not Java chars"
      ...
      (it "SMP emoji '📄' (2 chars / 2 cols) padded to 5 has the same visual width as ASCII"
        (let [out (pad-cell "📄" 5 :left)]
          (expect (= 7 (p/display-width out)))))
      (it "VS-16 emoji '🏷️' (3 chars / 2 cols) ALSO has the same visual width — this was THE bug"
        (let [out (pad-cell "🏷️" 5 :left)]
          (expect (= 7 (p/display-width out)))))
      (it "Regional-indicator flag '🇵🇱' (4 chars / 2 cols) ALSO matches"
        (let [out (pad-cell "🇵🇱" 5 :left)]
          (expect (= 7 (p/display-width out)))))
      (it "All four emoji classes pad to identical display width when w=5"
...
(defdescribe render-table-width-test
    ...
    (let [headers ["Ikona" "Plik" "Rozmiar" "Typ"]
      ...
      (it "each non-marker line ends with the right corner glyph for its row type"
        (let [bare (mapv strip-marker out)]
          (expect (str/ends-with? (first bare) "┓"))            ;; top
          (expect (str/ends-with? (last bare) "┛"))             ;; bottom
          (expect (every? #(str/ends-with? % "┃")
                    (remove #(re-find #"^[┏┗┣]" %) bare))))))) ;; data lines end with vertical
  ...
  (describe "Single-row table with VS-16 emoji in isolation"
    (it "Header column-width is computed from display-width, not char count"
      ;; If column-widths used (count), the Ikona column would be
      ;; allotted 5 cols (max of "Ikona" = 5 chars, "🏷️" = 3 chars).
      ;; The 🏷️ cell would render as " 🏷️   " = 6 cols.
      ;; The 📄 cell would render as " 📄    " = 7 cols.
      ;; Inconsistent → drift.
      ;;
      ;; With display-width: max("Ikona"=5, "🏷️"=2, "📄"=2) = 5.
      ;; Both emoji cells pad to 5 cols → both render as 7-col strings.
