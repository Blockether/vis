(ns com.blockether.vis-cli.channels.cli
  "vis-loop's built-in CLI commands.

   This namespace is an EXTENSION, not a dispatcher. Loading it at
   startup self-registers every built-in (`run`, `auth`, `doctor`,
   `conversations`, `extensions`) into `com.blockether.vis-extension.commandline.base`
   so the dispatcher in `com.blockether.vis-cli.commandline.main` can
   ...
   the vis-loop jar onto the classpath is enough to get every
   built-in. No caller wiring required."
  (:require [clojure.string :as str]
            [com.blockether.vis-cli.channels.cli.agent :as agent]
            [com.blockether.vis-cli.commandline.extensions :as ext-cli]
            ...
            [com.blockether.vis-loop.loop.runtime.conversation.core :as conv-core]
...
(defn- format-date [d] (format/format-date d))
...
(defn- cli-auth!
  "Look the requested provider up through the registry and dispatch
   ...
   …); vis-loop never references them by namespace."
  [_parsed residual]
  (config/init-cli!)
...
(defn- cli-extensions! [_parsed _residual]
  (config/init-cli!)
  (let [exts (ext-cli/list-extensions)]
    (if (empty? exts)
...
;; Direct `cmd/register-global!` is the right plumbing here; vis-loop
...
;; auto-mounts every entry under `[\"extensions\"]`. vis-loop thus
