...
(def bench-root
  "Working-directory root for the benchmark harness. Datasets ship
   inside the package; results/trajectories are produced on demand."
  "packages/vis-benchmark")

(def trajectories-root (str bench-root "/trajectories"))
...
(defn trajectory-edn-path
  "Returns the final EDN file path where a task's trajectory will be written.
   Layout: packages/vis-benchmark/trajectories/{bench}/{model}/{run-ts}/{task-id}.edn"
...
(defn run-vis-task!
  "High-level wrapper that runs a single task through query/query! with full
   ...
   Params map:
     ...
     :query-opts Map, optional - extra opts merged into query! call
...
(defn save-results!
  ...
  (.mkdirs (io/file (str bench-root "/results")))
  (let [ts         (.toString (Instant/now))
        ...
        filename   (str bench-root "/results/bench-" run-id ".ednl")
