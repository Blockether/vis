(ns com.blockether.vis-runtime.loop.runtime.conversation.core
  ...
  (:require [com.blockether.svar.internal.llm :as llm]
            ...
            [com.blockether.vis-sdk.core :as config]
            ...
            [com.blockether.vis-runtime.loop.runtime.conversation.environment.query.iteration.spec :as iteration-response-spec]
...
(defn db-info
  ...
  []
  (config/shared-conn! (config/resolve-db-spec)))
...
(defn by-id
  [id]
  (when-let [conv (config/db-get-conversation (db-info) id)]
    {:id            (str (:id conv))
...
(defn by-channel
  ...
  (mapv (fn [c]
    ...
    (config/db-list-conversations (db-info) channel)))

(defn for-telegram-chat!
  [chat-id]
  (let [ext (str chat-id)]
    (or (when-let [ref (config/db-find-conversation-by-external (db-info) :telegram ext)]
          (by-id (str (second ref))))
      (create! :telegram {:external-id ext}))))

(defn set-title!
  [id title]
  (config/db-update-conversation-title! (db-info) id title)
  nil)
  ...
  (when-let [env (env-for conversation-id)]
    (let [;; Activate extensions ONCE for this snapshot — reused by both
          ...
          queries       (when db-info
                          (try (config/db-list-conversation-queries db-info (:conversation-id env))
...
(defn effective-system-prompt
  ...
  (when-let [d (some-> (env-for conversation-id) :db-info)]
    (let [queries (try (config/db-list-conversation-queries d conversation-id)
                    (catch Throwable _ nil))
          last-q  (last queries)]
      (when last-q
...
(defn delete!
  ...
  (close! id)
  (let [d (db-info)]
    (try (config/delete-conversation-tree! d id)
...
(defn sweep-orphaned-running-queries!
  ...
  ([db] (config/sweep-orphaned-running-queries! db)))
