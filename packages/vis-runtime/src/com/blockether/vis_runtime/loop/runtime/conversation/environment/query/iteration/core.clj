(ns com.blockether.vis-runtime.loop.runtime.conversation.environment.query.iteration.core
  ...
  (:require
   ...
   [com.blockether.vis-sdk.core :as vis-error]
   ...
   [com.blockether.vis-runtime.loop.runtime.conversation.environment.query.runtime
  ...
  (when-let [exts (some-> (:extensions environment) deref seq)]
    (loop [current-code  code
      ...
      (when (< iterations parse-rescue-max-iterations)
        (let [fixed (vis-error/try-rescue-parse-error exts current-code current-error environment)]
          (cond
            (nil? fixed)              nil
                ...
                (try
                  (mapv (fn [position row]
                          {:iteration-id (str "i" iteration-position "." (inc position))
                           ...
                           :error        (:error row)})
                    (range)
                    (vis-error/db-list-iteration-expressions db-info (:id iteration)))
                  (catch Throwable _ []))))
      iterations)))
...
(defn load-effective-plan
  ...
  (when (and db-info query-id)
    (try
      (let [iters (vis-error/db-list-query-iterations db-info query-id)]
  ...
  (when (and db-info conversation-id)
    (try
      (let [all-queries (sort-by :created-at
                          (vis-error/db-list-conversation-queries db-info conversation-id))
