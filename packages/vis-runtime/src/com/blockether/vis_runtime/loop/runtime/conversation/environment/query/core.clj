(ns com.blockether.vis-runtime.loop.runtime.conversation.environment.query.core
  ...
  (:require
   ...
   [com.blockether.vis-runtime.loop.runtime.conversation.environment.query.runtime :as query-runtime
    :refer [*rlm-context*]]
            ...
            (if (>= consecutive-errors max-consecutive-errors)
              ...
              (let [reasoning-level (when has-reasoning?
                    ...
                    recent-thought   (when (pos? iteration)
                                       (some-> (last (config/db-list-query-iterations db-info query-id))
                                         :thinking))
                    ...
                    attempts-ledger (try
                                      (iterate/build-attempts-from-iterations
                                        db-info
                                        (config/db-list-query-iterations db-info query-id))
                                      (catch Throwable _ []))
                ...
                (if-let [iteration-error-data (::iterate/iteration-error iteration-result)]
                  ...
                  (if (and cancel-atom @cancel-atom)
                    ...
                    (let [error-feedback (str "[Iteration " (inc iteration) "]\n"
                          ...
                          err-iteration-id (config/store-iteration! (:db-info environment)
                  ...
                  (let [_ (accumulate-usage! (:api-usage iteration-result))
                        ...
                        iteration-id (config/store-iteration! (:db-info environment)
                      ...
                      (do (iterate/log-stage! :final iteration
                        ...
                        (when on-chunk
                          (on-chunk {:iteration iteration :thinking thinking
                                     ...
                                     :results (mapv #(if (:error %) (config/format-error (:error %)) (iterate/safe-pr-str (:result %))) expressions)
...
(defn run-query!
  ...
  (let [query-id (config/store-query! (:db-info env)
        ...
        _ (config/update-query! (:db-info env) query-id
    ...
    (if status
      ...
      (do
        ...
        (let [fallback-answer (:result answer answer)]
          (try
            (config/update-query! db-info query-id
