(ns com.blockether.vis-main.core
  "vis-main — single-file binary entry point.

   Owns everything the `vis` binary needs: the CLI dispatcher, the
   built-in commands (`run`, `auth`, `doctor`, `conversations`,
   `extensions list`), the one-shot agent helper, the extension CLI
   dispatcher, and the persistence-backed Telemere `:db` handler.
   Earlier this lived in five files (`logging.clj`,
   `commandline/extensions.clj`, `commandline/main.clj`,
   `channels/cli.clj`, `channels/cli/agent.clj`); collapsing to one
   namespace removes per-file `:require` ceremony, kills the only
   self-internal alias graph in the package, and makes every binary
   concern readable in one buffer.

   Read order
   ==========

   1. `:db` Telemere handler (persistence-backed log handler).
   2. Extension CLI dispatcher (parses, validates, dispatches
      `:ext/cli` entries declared by extensions).
   3. Agent helper (one-shot agent definition + `run!` invoked by
      `vis run`).
   4. Built-in CLI commands (`vis run`, `vis auth`, `vis doctor`,
      `vis conversations`, `vis extensions list`).
   5. Dispatcher entry point (`-main`, root-command tree assembly,
      logging bootstrap)."
  (:refer-clojure :exclude [agent run!])
  (:require
   [charred.api :as json]
   [clojure.string :as str]
   [com.blockether.vis-runtime.core :as loop-core]
   [com.blockether.vis-runtime.loop.runtime.conversation.core :as conv-core]
...
(defn handler:db
  "Telemere handler that persists every signal through the
   `com.blockether.vis-sdk.core/log!` facade.
...
(defn run!
  ...
  (let [_cfg      (sdk/resolve-config config)
        ...
        {conv-id :id} (sdk/create-conversation! :cli {:title title})
        ...
        messages  (if (string? prompt) [(sdk/user-message prompt)] prompt)]
    (try
      (let [result (sdk/send! conv-id messages q-opts)]
...
(defn- cli-doctor! [_parsed _residual]
  (sdk/init-cli!)
  (let [env (loop-core/create-environment (query/get-router)
    ...
    (try
      ...
      (finally
        (loop-core/dispose-environment! env)
        (shutdown-agents)))))
...
;; Direct `sdk/register-cmd!` is the right plumbing here; vis-runtime
...
(sdk/register-extension!
  (sdk/extension
    {:ext/namespace 'com.blockether.vis-main.core
...
;; facades. Add a third-party jar with its own `sdk/register-cmd!`
