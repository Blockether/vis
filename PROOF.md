...
| Task 25 legacy write-authority deletion | NEXT / REQUIRED | Migrate remaining persistence core tests off legacy prove/fulfill writer APIs, starting with the conversation-intent happy path and ref rejection tests, then delete/hard-error underlying legacy `db-prove-gate!` / `db-fulfill-intent!` write authority. |
