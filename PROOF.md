...
| Task 26 legacy write-authority deletion | NEXT / REQUIRED | Migrate remaining persistence core ref-rejection tests off legacy `db-prove-gate!` by moving canonical-ref validation to evidence bundle/attestation path, then delete/hard-error underlying legacy `db-prove-gate!` / `db-fulfill-intent!` write authority. |
