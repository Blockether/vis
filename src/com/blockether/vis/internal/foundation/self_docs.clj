...
Vis's OWN documentation (features, configuration, extending vis). Use ONLY for questions about vis itself, never for the host project."
  ...
  ([slug]
   (let [ps    (pages)
         slug' (str/trim (str slug))]
     (if-let [page (some #(when (= slug' (:slug %)) %) ps)]
       (extension/success
         {:result (cond-> {:slug    (:slug page)
                           :title   (:title page)
                           :content (:md page)}
                    (:section page) (assoc :section (:section page)))})
       (extension/failure
         {:result nil
          :error  {:message (str "Unknown vis docs slug " (pr-str slug') ".")
                   :hint    (str "Valid slugs: " (str/join ", " (map :slug ps))
                              ". Call vis_docs() to list all pages.")}})))))

(def vis-docs-symbol
  (vis/symbol #'vis-docs-tool
    {:symbol 'vis-docs :tag :observation}))
