(ns com.blockether.vis.internal.foundation.mcp.core
  "Built-in Model Context Protocol (MCP) surface. The gateway daemon owns ONE
   ...
   Every string value in `:headers` / `:env` / `:args` / `:url` / `:command` /
   `:cwd` supports `${ENV_VAR}` interpolation from the host environment.
   `:enabled false` skips the server without deleting the entry. HTTP servers
   with no static bearer transparently negotiate OAuth 2.1 on first 401 (RFC
   9728 discovery + RFC 7591 dynamic client registration + PKCE loopback).
...
(defn- deep-interpolate
  "Walk `x`, interpolating `${VAR}` in every string leaf. Preserves keyword keys."
  [x]
  (cond (map? x) (into (empty x) (map (fn [[k v]] [k (deep-interpolate v)])) x)
...
(defn- ->client-spec
  "Coerce a user spec (already `runtime-config`d) into what `mcp/connect` wants:
   env-interpolated, `:bearer-fn` synthesised for HTTP servers without a static
   `Authorization` header, `:timeout-ms` / `:listen?` forwarded."
  [server-name spec]
  (let [s (deep-interpolate spec)
        transport (transport-of s)
        headers (or (:headers s) {})
        has-static-auth? (some (fn [[k _]]
                                 (= "authorization" (str/lower-case (name k))))
                               headers)
        bearer-fn (when (and (= :http transport) (not has-static-auth?))
                    (mcp-oauth/make-bearer-fn
                      server-name (:url s) (atom nil) (:auth s)))]
    (cond-> s
      (:timeout_ms s) (assoc :timeout-ms (:timeout_ms s))
      (:listen s) (assoc :listen? true)
      bearer-fn (assoc :bearer-fn bearer-fn))))
...
(defn- configured-servers
  ...
  []
  (let [raw (get-in (or (vis/load-config-raw) {}) [\"mcp\"])
        h (hash raw)]
    (if (= h (:hash @servers-cache))
      (:value @servers-cache)
      (let [m (get raw \"servers\")
            coerced (if (map? m)
                      (into {}
                            (keep (fn [[k v]]
                                    (let [nm (str k)
                                          rt (vis/runtime-config v)]
                                      (when (enabled? rt)
                                        [nm (->client-spec nm rt)]))))
                            m)
                      {})]
...
(defonce ^:private reconciling? (atom false)) ; single-flight guard for reconcile-async!
...
(defn- ensure-connected!
  \"Idempotently connect the configured `server`, caching the conn (with the spec it
   was connected under) in the daemon-wide pool. Returns the conn or nil (unknown /
   disabled / failed).\"
  [server]
  (let [existing (get @conns server)]
    (cond
      ;; Live-and-alive: reuse.
      (and existing (mcp/alive? (:conn existing))) (:conn existing)
      ;; Cached but the process/stream died — drop it so we can respawn.
...
(defn- reconcile!
  ...
  (let [cfg (configured-servers)]
    (doseq [[server {:keys [conn spec]}] @conns]
      (when (or (not= spec (get cfg server))
                (not (mcp/alive? conn)))
        (disconnect! server)))
...
(defn- reconcile-async!
  \"Single-flight background `reconcile!` — safe to call from a turn's ctx-fn without
   blocking it on a slow stdio spawn.\"
  []
       ...
       (mapv (fn [[nm spec]]
               (let [conn (conn-of nm)]
                 (cond->
                   ...
                   conn
                   (assoc \"tools\"
                     (count (or (some-> (:tools conn) deref) [])))
    ...
    (ok :mcp/tools
        {\"server\" server
         \"tools\" (mapv (fn [t]
                       ...
                       (mcp/list-tools conn))})
    (err :mcp/tools
         (str \"MCP server '\" server \"' is not configured or is disabled (see ~/.vis/state.yml :mcp :servers).\")
...
(defn- contribute
  \"`:ext/ctx-fn` — proactively reconcile the daemon-wide MCP pool, then surface the
   CONNECTED servers (+ tool counts) so the model sees what's reachable at
   `session[\\\"env\\\"][\\\"mcp\\\"][\\\"servers\\\"]`. The pool is shared across every session.\"
  [_env]
  (reconcile-async!)
  (let [live @conns]
    (when (seq live)
      ;; `\"session_env\"` is the merge directive read STRING-KEYED by ctx_loop /
      ;; env-digest; the whole contribution is string-keyed with no keyword values.
