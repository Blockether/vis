...
(defn- advice-for-classification
  ...
  (case classification
    ...
    :regex-unsupported-escape
    "grep matches literal substrings by default — no regex escaping needed. Pass a list of terms for OR: grep({\"query\": [\"foo\", \"bar\"], \"context\": 4}), or set is_regex: True to run the query as a real regex. grep takes ONE options map — never a positional query — and answers anchored TEXT, so filter it with .splitlines() rather than by key."
