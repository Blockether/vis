...
(defn- deep-stringify
  ...
  (cond (map? x) (reduce-kv (fn [m k v]
        ...
        ;; Force any lazy leaf so no unrealized deref crosses the boundary
        ;; as a ForeignObject and breaks json.dumps.
...
(def ^:private INTROSPECTION_PROMPT
  (str
    ...
    "- Read session history only when the answer changes with what a past session says: session diagnostics, evidence from a referenced conversation, or context the conversation lacks. A pointing word (\"that drug\", \"the file we fixed\") is a reference, not a reason: when the answer holds either way, answer from your own knowledge. A session or turn start, and `/reload`, keep the conversation intact: continue from it.\n"
    "- Raw wire history: `~/.vis/gateway/events/<id>.ndjson`; open the one file by id.\n"
    "- For current-session evidence, use `await read_session()`. Reuse the result unless newer evidence is needed. `usage` summarizes per-turn/iteration/tool/provider routing; tool rows overlap, so read each on its own.\n"
