...
(defn- stash-count [^File dir]
  (try
    (if-let [out (git-line dir ["stash" "list"])]
...
(defn- ahead-behind
  ...
  [^File dir]
  (try
    (when-let [out (git-line dir ["rev-list" "--left-right" "--count"
                                  "@{upstream}...HEAD"])]
...
(defn snapshot
  ...
  ([^File start-file {:keys [status?]
                      :or   {status? true}}]
   (when-let [^File worktree-root (vis-git/repo-work-tree start-file)]
     (try
       (let [git-dir     (git-line worktree-root ["rev-parse" "--absolute-git-dir"])
             ...
             base        (cond-> {:root         (.getPath worktree-root)
                                  :git-dir      (or git-dir
                                                  (.getPath (io/file worktree-root ".git")))
