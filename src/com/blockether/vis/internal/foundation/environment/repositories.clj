...
(def ^:private skip-directories
  #{".git" ".hg" ".svn" "node_modules" "target" "dist" "build"
    ...
    ".verification" ".verification-baseline"
    "book" "_site" "public" "site" "_book"})

(defn- file-of ^File [root]
  (cond
    (instance? File root) root
    (instance? Path root) (.toFile ^Path root)
    :else                 (io/file (str root))))

(defn- repo-root? [^Path dir]
  (Files/exists (.resolve dir ".git") (make-array java.nio.file.LinkOption 0)))

(defn- add-root! [^LinkedHashSet roots ^Path dir ^long max-repos]
  (when (< (.size roots) max-repos)
    (.add roots dir)))
...
(defn- select-repo-summary [repo-map]
  (select-keys repo-map
    [:root :git-dir :branch :detached? :detached-sha :worktree? :submodules?
     :clean? :dirty? :changes? :status-unavailable?
     :modified :added :changed :removed :missing :untracked :conflicting
     :stash-count :upstream :ahead :behind :stale?]))

(defn- discover-roots
  [^File root-file {:keys [max-files max-repos deadline-ms]
                    :or   {max-files   default-max-files
                           max-repos   default-max-repos
                           deadline-ms default-deadline-ms}}]
  (let [^Path start (.toPath root-file)
        roots       (LinkedHashSet.)
        ...
        visitor     (proxy [SimpleFileVisitor] []
                      (preVisitDirectory [^Path dir ^BasicFileAttributes _attrs]
                        (cond
                          ...
                          (do
                            (when (repo-root? dir)
                              (add-root! roots dir (long max-repos)))
                            FileVisitResult/CONTINUE)

                          :else
                          (let [name (str (.getFileName dir))]
                            (cond
                              (contains? skip-directories name)
                      ...
                      (visitFile [^Path _file ^BasicFileAttributes _attrs]
                        (let [n (inc (aget visited 0))]
                          ...
                          (if (stop?)
                            (do (aset truncated 0 true) FileVisitResult/TERMINATE)
                            FileVisitResult/CONTINUE)))
                      (visitFileFailed [^Path _file ^java.io.IOException _exception]
                        FileVisitResult/CONTINUE))]
    (try
...
(defn snapshot
  ...
  ([root opts]
   (let [root-file (file-of root)
         root-path (canonical-path root-file)
         [roots truncated?] (discover-roots root-file opts)
         status-timeout-ms  (long (or (:status-timeout-ms opts)
                                    default-status-timeout-ms))
         repos (->> roots
                 (mapv (fn [^Path repo-path]
                         (let [repo-file (.toFile repo-path)
                               summary   (try
                                           (git/snapshot repo-file
                                             {:status-timeout-ms status-timeout-ms})
                                           (catch Throwable _ nil))]
                           (cond-> {:path (rel-path (.toPath root-file) repo-path)
                                    :root (canonical-path repo-file)}
                             summary (merge (select-repo-summary summary)))))))]
     {:root         root-path
      :count        (count repos)
      :repositories repos
      :truncated?   truncated?})))
