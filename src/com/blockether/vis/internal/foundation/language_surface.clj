    ...
    {:type "object"
     ...
     {"language" {:type "string" :description "Language pack; OMIT to infer from the workspace."}
      "namespaces" {:type "array"
                    :items {:type "string"}
                    :description "Test namespaces/modules to run (e.g. [\"my.app.core-test\"]). OMIT (or pass []) to run every *_test namespace in the workspace."}
      "paths" {:type "array"
               :items {:type "string"}
               :description "Dirs/files to discover *_test namespaces under. OMIT (or pass []) to default to the workspace root; an explicit non-empty path that yields no tests is an error."}
      "only" {:type "array"
              :items {:type "string"}
