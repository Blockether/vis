...
(defn socket-path
  ...
  (let [dir  (bridge-dir)
        iid  (sanitize id)
        name (str (sanitize session) "__" iid ".sock")
        ...
        room (- 100 (count (str dir)) 1)]
    (.resolve dir (if (<= (count name) room)
                    name
...
(defn find-socket
  ...
  (cond
    ...
    (Paths/get ^String socket (make-array String 0))

    (not (str/blank? id))
    (let [dir    (bridge-dir)
          suffix (str "__" (sanitize id) ".sock")]
      (when (Files/isDirectory dir (make-array LinkOption 0))
        (with-open [s (Files/list dir)]
          (->> (.collect s (Collectors/toList))
            (filter (fn [^Path p] (str/ends-with? (str (.getFileName p)) suffix)))
            (sort-by (fn [^Path p] (.toMillis (Files/getLastModifiedTime p (make-array LinkOption 0))))
              (comp - compare))
            first))))
        ...
        (with-open [s (Files/list dir)]
          (doseq [^Path p (.collect s (Collectors/toList))]
            (when (str/ends-with? (str (.getFileName p)) ".sock")
              (let [live? (try
                            (with-open [ch (SocketChannel/open
...
(defn serve!
  ...
  (let [p         (if (instance? Path path) path (Paths/get ^String (str path) (make-array String 0)))
        parent    (.getParent p)
        _         (when parent (Files/createDirectories parent (make-array FileAttribute 0)))
        _         (Files/deleteIfExists p)
        add-listener (:add-listener pty)
        send      (:send pty)
        server    (doto (ServerSocketChannel/open StandardProtocolFamily/UNIX)
                    (.bind (UnixDomainSocketAddress/of p)))
        clients   (atom #{})
        running   (atom true)
        drop!     (fn [^SocketChannel ch unsub]
                    (when unsub (try (unsub) (catch Throwable _ nil)))
                    (swap! clients disj ch)
                    (try (.close ch) (catch Throwable _ nil)))
        on-client (fn [^SocketChannel ch]
                    (swap! clients conj ch)
                    (when replay-fn
                      (try (let [ba (replay-fn)]
                             (when (and ba (pos? (alength ^bytes ba)))
                               (write-all! ch ba)))
                        (catch Throwable _ nil)))
                    (let [unsub (add-listener
                                  (fn [^bytes ba]
                                    (try (write-all! ch ba)
                                      (catch Throwable _ (drop! ch nil)))))]
                      (doto (Thread.
                              ^Runnable
                              (fn []
                                (try
                    ...
                    (try
                      (loop []
                        (when @running
                          ...
                          (recur)))
                      (catch Throwable _ nil)))]
    (doto (Thread. ^Runnable accept "vis-pty-bridge-accept")
      (.setDaemon true)
      (.start))
    {:path (str p)
     :stop (fn []
             (reset! running false)
             (doseq [^SocketChannel ch @clients] (try (.close ch) (catch Throwable _ nil)))
             (reset! clients #{})
...
(defn- stty
  ...
  [& args]
  (try
    (let [pb   (doto (ProcessBuilder. ^java.util.List (into-array String (cons "stty" args)))
  ...
  (let [p (find-socket opts)]
    (if (or (nil? p) (not (Files/exists p (make-array LinkOption 0))))
      ...
      (let [ch    (SocketChannel/open (UnixDomainSocketAddress/of p))
            saved (stty "-g")
            restore (fn [] (when saved (stty saved)))]
        ...
        (try
          ...
          (let [^OutputStream out System/out
            ...
            (loop []
              (let [c (.read in)]
                (cond
                  (or (= c -1) (= c detach-byte)) nil
                  :else (do (write-all! ch (byte-array [(unchecked-byte c)]))
                          (recur)))))
            0)
          (finally
            (restore)
