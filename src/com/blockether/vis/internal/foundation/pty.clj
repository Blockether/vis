...
(def ^:private S ValueLayout/JAVA_SHORT)
...
(def ^:private ^SymbolLookup lookup
  ...
  (let [std  (.defaultLookup linker)
        util (try (SymbolLookup/libraryLookup "util" (Arena/global))
...
(def ^:private h-fa-chdir (delay (try (dh "posix_spawn_file_actions_addchdir_np" I [ADDR ADDR])
                                   (catch Throwable _ nil))))

(defn- invoke ^Object [h & args]
...
(defn- build-strv
  ...
  (let [strs (vec strs)
        n    (count strs)
        seg  (.allocate arena (long (* 8 (inc n))))]
    (dotimes [i n]
      (.setAtIndex seg ADDR i (.allocateFrom arena ^String (nth strs i))))
    (.setAtIndex seg ADDR n MemorySegment/NULL)
    seg))

(defn- winsize ^MemorySegment [^Arena arena rows cols]
  (let [ws (.allocate arena (long 8))]
...
(defn spawn!
  ...
  [{:keys [command dir env cols rows]
    :or   {cols 120 rows 40}}]
  (let [master
        (with-open [arena (Arena/ofConfined)]
          (let [amaster (.allocate arena (long 4))
                aslave  (.allocate arena (long 4))
                pidp    (.allocate arena (long 4))
                ...
                envp    (build-strv arena (map (fn [[k v]] (str k "=" v)) env))
                path    (.allocateFrom arena ^String (first command))]
            (when-not (zero? (int (invoke @h-openpty amaster aslave
                                    MemorySegment/NULL MemorySegment/NULL ws)))
            ...
            (let [master (int (.getAtIndex amaster I 0))
              ...
              (when dir
                (when-let [h @h-fa-chdir]
                  (try (invoke h fa (.allocateFrom arena ^String dir))
                    (catch Throwable _ nil))))
        ...
        [master-fd pid] master
        master-fd (int master-fd)
        pid       (long pid)
        pin       (PipedInputStream. (* 64 1024))
        pout      (PipedOutputStream. pin)
        listeners (atom [])
        _rthread  (doto (Thread. ^Runnable (fn [] (reader-loop! master-fd pout listeners))
        ...
        wait-fn   (fn []
                    (or @exit
                      (locking exit
                        (or @exit
                          (with-open [arena (Arena/ofConfined)]
                            (let [status (.allocate arena (long 4))]
    ...
    {:pid     pid
     ...
     :wait    wait-fn
     :alive?  (fn [] (if-let [ph (.orElse (ProcessHandle/of pid) nil)]
                       (.isAlive ph)
                       (nil? @exit)))
     :destroy (fn [force?]
     ...
     :add-listener (fn [f]
                     ...
                     (fn [] (swap! listeners (fn [ls] (vec (remove #(identical? % f) ls))))))}))
