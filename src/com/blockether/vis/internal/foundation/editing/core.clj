...
(defn- safe-path
  ^File [p]
  (try
    (workspace/resolve-file-path p)
    (catch clojure.lang.ExceptionInfo e
      ;; Keep file-tool error types and the actionable blank-path hint stable.
      (let [type (:type (ex-data e))]
        (throw
          (ex-info
            (if (= type :workspace/blank-path)
            ...
            (assoc (ex-data e)
              :type (case type
                      :workspace/blank-path
                      :ext.foundation.editing/blank-path

                      :workspace/path-denied
                      :ext.foundation.editing/path-denied
...
(defn- patch-edit-rows
  "One row per applied edit, IN THE ORDER THE CALLER LISTED THEM, each carrying the
   ...
   so the text is dropped and the addresses are kept. `diff/head-tail-cap` bounds
        ...
        (tool-success
          {:op :patch
           ...
           :metadata {:mode :patch
                      ...
                      :diff (diff/unified-diff-text original written-content)
                      :lines (diff/line-change-counts original written-content)
