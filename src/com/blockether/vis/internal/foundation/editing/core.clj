...
(defn available-editing-prompt
  ...
  (str/join "\n"
    ["Editing tools — bare Python functions: cat / find / rg / ls / outline / patch / write / struct_patch / sexpr / references + copy / move / delete / exists / is_exists. Pure helpers: anchor / anchor_exact / hunk. Canonical path only."
     ""
     "STRATEGY (λ-notation: pick the move by what you're doing; → produces, | alternatives, ¬ never; structural-FIRST for code):"
     "  λ inspect:"
     ...
     "    deep_node     → n = await sexpr(P, {\"nav\": [...]}) ; await struct_patch({\"path\": P, \"op\": \"replace\", \"at\": n[\"path\"], \"code\": S})   # an if-cond / loop-body / one arg"
     ...
     "    ¬ (cat → rebuild → write)            # the #1 way work is lost — cat TRUNCATES large files"
     ...
     ""
     "STRUCTURAL (tree-sitter, every language — the λ blocks above are the decision; the detail:)"
     "  struct_patch BY NAME : struct_patch({\"path\": P, \"op\": OP, \"target\": NAME, \"code\": S}); op = replace|insert_before|insert_after|append|add_doc|replace_doc|replace_node|rename. Re-parsed, REFUSED on a syntax break. (BY zipper PATH → sexpr, below.)"
     "  sexpr(path, {\"at\": [i, ...], \"nav\": [m, ...]}) : the tree-sitter ZIPPER cursor (full clojure.zip / rewrite-clj vocabulary, any language). at = named-child index path from the root; nav m: down|d|b · up|u|t · left|l · right|r · leftmost|first · rightmost|last · root · {\"child\": i} · DEPTH-FIRST next|n · prev|p · SEARCH {\"find\": \"text\"} · {\"find_kind\": \"if_statement\"}. Boundary / not-found FAIL CLOSED. Returns {\"kind\", \"line\", \"end_line\", \"text\", \"sexp\", \"children\": [{\"idx\",\"kind\",\"head\"}], \"can\": {down,up,left,right,next,prev,index,siblings}}."
     "    # zipper how-to (Python): find a deep node, read it, edit it — globals persist, so chain in one program:"
     "    n = await sexpr(P, {\"nav\": [{\"find\": \"compute_total\"}]}) ; print(n[\"text\"])              # jump to a named node, read its source"
     "    m = await sexpr(P, {\"at\": n[\"path\"], \"nav\": [\"down\", \"right\"]})                          # step deeper (down a child, then its next sibling)"
     "    await struct_patch({\"path\": P, \"op\": \"replace\", \"at\": m[\"path\"], \"code\": S})            # op also insert_before|insert_after|append_child|prepend_child; delete = replace \"\""
