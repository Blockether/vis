...
(defn available-editing-prompt
  ...
  (str/join "\n"
    ["Editing tools — bare Python functions: cat / find / rg / ls / outline / patch / write / struct_patch / sexpr / references + copy / move / delete / exists / is_exists. Pure helpers: anchor / anchor_exact / hunk. Canonical path only."
     ""
     "INSPECT — map before you read; cheapest first, never cat a whole file blind:"
     "  • SHAPE of a file → outline(path): every def/class/method as `kind name SIGNATURE @start..end-anchor` — names + params, NO bodies. The anchors cat/patch a def directly, so read this BEFORE cat."
     "  • FULL source of ONE symbol → from its outline anchors, cat(path, {\"range\": [start, end]}); or sexpr(path, {\"nav\": [{\"find\": \"name\"}]}) to walk to its node and read text."
     "  • ALL USAGES of a name → references(path, name) in one file, or project_references(name) across the repo — IDENTIFIER-boundary hits (not text grep), each patch-anchored. Run before any rename."
     ""
     ...
     "  Reach for the lowest-friction editor that fits; never rebuild a file from cat output then write (that is the #1 way work gets lost). Below: LOCATE (find where), then the mechanics of each tool."
     ""
     "FLOW"
     ...
     "  outline(path) → {\"skeleton\", \"language\"} : every def/class/method as `kind name SIGNATURE @start..end-anchor` — names + PARAMS + line ranges + full anchors, NO bodies. Read BEFORE cat to jump straight to a def (cat its anchor range for the body)."
