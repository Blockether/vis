...
(defn available-editing-prompt
  ...
  (str/join "\n"
    ["Editing tools — bare Python functions: cat / find / rg / ls / outline / patch / write / struct_patch / sexpr / references + copy / move / delete / exists / is_exists. Pure helpers: anchor / anchor_exact / hunk. Canonical path only."
     ...
     "    deep_node     → sexpr(P, nav=…) to the node, then struct_patch at n[\"path\"]   # an if-cond / loop-body / one arg — RARELY needed (a line anchor already disambiguates repeated text by line #; see zipper how-to below)"
     ...
     "    # zipper how-to (Python): `nav` BATCHES all moves in ONE call (not N step-calls); chain to the edit in one program:"
     "    n = await sexpr(P, {\"nav\": [{\"find\": \"compute_total\"}, \"down\", \"right\"]}) ; await struct_patch({\"path\": P, \"op\": \"replace\", \"at\": n[\"path\"], \"code\": S})   # op also insert_before|insert_after|append_child|prepend_child; delete = replace \"\""
