                    ...
                    (if (empty? matches)
                      ...
                      (let
                        ...
                        (cond
                          ...
                          (empty? in-win)
                          ;; Content moved FARTHER than the drift window. Refuse (the hash is
                          ;; only hash-width hex, so "unique in file" is weak evidence), but when
                          ;; it now sits at EXACTLY ONE line the correct anchor is literally that
                          ;; line + this same hash — hand it back so the caller recovers in ONE
                          ;; step instead of a second `cat` (mirrors `:hashline-not-found`).
                          {:error (cond->
                                    {:reason :hashline-misplaced
                                     :which which
                                     :hash hash
                                     :stated-line line
...
(defn- unicode-escape-unit
  "The 16-bit value of the `\\uXXXX` escape whose backslash sits at `i` in `s`,
   or nil when no complete escape starts there."
  [^String s ^long i]
  ;; An escape is exactly six characters wide: \ u and four hex digits.
  (when (<= (+ i 6) (.length s))
    (when (and (= \\ (.charAt s i)) (= \u0075 (.charAt s (inc i))))
  ...
  (if-not (and (string? s) (str/includes? s "\\u"))
    ...
    (let
      ...
      (loop [i 0]
        (if (>= i n)
          (.toString sb)
          (if (not= \\ (.charAt text i))
            (do (.append sb (.charAt text i)) (recur (inc i)))
            (let
              [run-end (long (loop [j i]
                               (if (and (< j n) (= \\ (.charAt text j))) (recur (inc j)) j)))]
              (if (even? (- run-end i))
                ;; Every backslash is itself escaped: this is text ABOUT an escape.
                (do (.append sb (.substring text i run-end)) (recur run-end))
                (let
                  [start (dec run-end)
