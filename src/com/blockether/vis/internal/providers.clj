...
(def oauth-provider-ids
  "Providers whose credentials come from an interactive OAuth flow and
   ...
   provider extension)."
  (into #{:openai-codex :anthropic-coding-plan}
    (keys github-copilot-account-types)))
...
(defn fetch-models
  ...
  (try
    (let [provider-id  (:id provider)
          ...
          svar-provider (config/->svar-provider probe)
          ...
          router        (svar/make-router [svar-provider]
                          (config/router-opts (config/current-config)))
...
(defn model-options
  ...
  ([provider default-models show-all?]
   (let [provider-id (:id provider)
         ...
         defaults (filterv #(config/provider-model-visible? provider-id %)
                    (or default-models []))
     ...
     {:models       visible
      :hidden-count (- (count pinned) (count visible))})))
...
(defn probe-local-reachable
  ...
  [provider]
  (let [base  (or (config/provider-base-url provider) (:base-url provider))
...
(defn provider-limits-safe
  ...
  [provider]
  (try
    (provider-limits/provider-limits (:id provider))
    (catch Throwable e
...
(defn- format-limit-window
  ...
  (when kind
    (str (name kind)
      ...
      (when resets-at-ms
        (str ", resets " (format/format-date (Date. (long resets-at-ms))))))))
...
(defn status-text
  "Multi-line human status + limits report for a configured provider.
   ...
   web status view."
  ([provider]
   (status-text provider (provider-status provider) (provider-limits-safe provider)))
  ([provider status limits]
   (let [status  (or status (initial-provider-status provider))
         ...
         title   (str (config/display-label (:id provider)) " Status")
         rows    (->> status
     ...
     (str/join "\n"
       (concat [title
                ""
                (str "Base URL: " (or (config/provider-base-url provider) "-"))
...
(defn- status-detail-rows
  ...
  [status]
  (->> status
    (remove (fn [[k _]] (contains? #{:authenticated? :error :loading?} k)))
    (sort-by (comp str key))
    (map (fn [[k v]]
           (str "- **" (status-entry-label k) ":** " (format-status-value v))))))
...
(defn status-md
  ...
  ([provider]
   (status-md provider (provider-status provider) (provider-limits-safe provider)))
  ([provider status limits]
   (let [status  (or status (initial-provider-status provider))
         limits  (or limits (initial-provider-limits provider))
         label   (config/display-label (:id provider))
         ...
         dynamic (get-in limits [:dynamic :limits])
         rpm     (get-in limits [:static :rpm])
         tpm     (get-in limits [:static :tpm])]
     (str/join "\n"
       (concat
         [(str "## " label)
          ""
          (str "**Authenticated:** " (if ok? "yes ✓" "no ✗")
            "  ·  **Base URL:** `" (or (config/provider-base-url provider) "-") "`")]
         (when-let [e (:error status)]
           ["" (str "> ⚠ " e)])
         (let [rows (status-detail-rows status)]
           (when (seq rows) (concat [""] rows)))
         ["" "### Limits" ""
          (str "_status: " (name (:status limits)) "_")]
         (if (seq dynamic)
           (concat
             [""
              "| Limit | Scope | Window | Usage | Note |"
              "|---|---|---|---|---|"]
             (map (fn [{:keys [scope kind window note] :as row}]
                    (str "| " (md-escape-cell (limits-format/generic-limit-label row))
                      " | " (md-escape-cell (or (some-> scope name)
                                              (some-> kind name) "-"))
         ...
         (when (or rpm tpm)
           ["" (str "Catalog defaults: "
                 (str/join " · " (remove nil? [(when rpm (str "RPM " rpm))
                                               (when tpm (str "TPM " tpm))]))
