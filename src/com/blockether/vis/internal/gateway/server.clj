...
(def ^:private MAX_BODY_BYTES
  "Hard cap on a JSON request body. These are control-plane payloads (session /
   project / model / root config), never bulk data, so 4 MiB sits orders of
   magnitude above anything legitimate while bounding a heap-exhaustion POST."
  (* 4 1024 1024))

(defn- bounded-input-stream
  "Wrap `in` so cumulative reads past `limit` bytes throw a `:body-too-large`
   ex-info (mapped to 413 by `wrap-errors`). The read ABORTS mid-stream instead
   of buffering the whole body, so an oversized / slow-drip POST can't exhaust
   the heap."
  ^java.io.InputStream [^java.io.InputStream in ^long limit]
  (let
    [seen
     ...
     trip!
     (fn [^long n]
       (when (and (pos? n) (> (.addAndGet seen n) limit))
         (throw (ex-info "request body too large" {:type :body-too-large :limit limit}))))]
...
(defn- body-json
  "Parse a request's JSON body into the canonical wire shape, bounded to
   [[MAX_BODY_BYTES]]. Streams the body STRAIGHT into the parser (no full-body
   slurp) through a size-limited stream: a declared-oversize Content-Length is
   rejected before a byte is read, and an actually-oversize body trips
   `:body-too-large` mid-read (→ 413 via `wrap-errors`). nil / blank / malformed
   body → nil (→ the caller's 400)."
  [request]
  (when-let [in (:body request)]
    (when-let
      [clen (some-> (get-in request [:headers "content-length"])
                    not-empty
                    parse-long)]
      (when (> clen MAX_BODY_BYTES)
        (throw (ex-info "request body too large" {:type :body-too-large :limit MAX_BODY_BYTES}))))
    (wire/parse-json-stream (bounded-input-stream in MAX_BODY_BYTES))))
    ...
    (try (handler request)
         (catch clojure.lang.ExceptionInfo e
           (if (= :body-too-large (:type (ex-data e)))
             (error-response 413
                             :payload-too-large "request body exceeds the size limit"
                             :limit_bytes (:limit (ex-data e)))
             (do (tel/log! :error
                           ["gateway: unhandled request error" (:uri request) (ex-message e)])
                 (error-response 500 :engine-error (or (ex-message e) "internal error")))))
         (catch Throwable t
