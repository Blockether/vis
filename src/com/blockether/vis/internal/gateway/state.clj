...
(def ^:private non-streaming-text-phases
  "Text generated by the model (reasoning/prose) is intentionally NOT streamed
   token-by-token over the canonical gateway. Clients get the complete thinking
   and assistant prose on the iteration boundary (`iteration.completed`) so only
   tool cards stream live."
...
(defn- coalesce-delta?
  ...
  (and (contains? non-streaming-text-phases phase)
       (not done?)
...
(defn- chunk->event
  "Translate one phased iteration chunk (progress.clj contract) into a
   ...
   are deliberately held until `iteration.completed`."
          ...
          (try (when caller-on-chunk
               ...
               (let [phase
                 ...
                 (when-not (coalesce-delta? @last-delta-ms chunk now)
                   (when (contains? non-streaming-text-phases phase)
