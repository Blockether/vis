...
(defn prompt
  ...
  (when (enabled?)
    (str
      ...
      "- When asked to find a session, search with `await list_sessions(search=...)`, check relevant history, and return matching session IDs/titles with evidence or no match; a ping or wake follows its own request.\n"
