...
(defn prompt
  ...
  (when (enabled?)
    (str
      ...
      "- Every publication requires `kind`: `complain` = broken behavior or concrete improvement (including extensions/system prompts); `coordination` = ownership/questions/dependencies; `informational` = facts/results/decisions. Classify messages, not threads. When Improve is enabled, complaints also enter its register, not an external tracker, and do not authorize action. Choose individual ping, ping='all', or none; no default broadcast.\n"
