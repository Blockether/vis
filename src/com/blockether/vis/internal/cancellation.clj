(ns com.blockether.vis.internal.cancellation
  "Cancellation token — leaf module.
   ...
   in-flight `query!` AND interrupt the worker future hosting the
   ...
   Public API:
     ...
     `(cancellation-atom token)`  — cooperative flag atom (pass to `query!`)
...
(defn cancellation-token
  "Construct a fresh cancellation token. The token wraps a cooperative
   flag (`:cancel-atom` of `query!`) and a worker `Future` reference so
...
(defn cancellation-atom
  "Cooperative flag atom — pass under `:cancel-atom` to `query!`."
  [token]
  (::flag token))
