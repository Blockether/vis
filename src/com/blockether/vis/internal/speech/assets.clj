(ns com.blockether.vis.internal.speech.assets
...
(defonce ^:private manifest*
  (delay (let [resource (io/resource manifest-resource)]
           (when-not resource
             (throw (ex-info "Speech asset manifest is missing from the classpath"
                             {:type :speech-assets/manifest-missing :resource manifest-resource})))
...
(defn entry
  ...
  (or (first (filter #(= id (:id %)) (manifest)))
      (throw (ex-info (str "No speech asset named " id " in the manifest")
                      {:type :speech-assets/unknown-asset :id id :known (mapv :id (manifest))}))))
...
(defn- fetch-archive!
  ...
  (let [archive (File/createTempFile "vis-speech-asset-" ".tar.bz2")]
   ...
   (let [staging
     ...
     (when (empty? candidates)
       (throw (ex-info (str "No source for " (:id entry) " is usable")
                       {:type :speech-assets/no-source :id (:id entry)})))
     (try
       (loop [remaining
              candidates
               ...
               (let [final (io/file dir)]
                 ...
                 (when-not (.renameTo staging final)
                   (throw (ex-info "Could not move the downloaded asset into place"
                                   {:type :speech-assets/install-failed :id (:id entry) :dir dir})))
                 dir)))
           (throw (ex-info (str "Could not install " (:id entry) " from any source")
                           {:type :speech-assets/install-failed
                            :id (:id entry)
  ...
  ([asset on-progress]
   (when (and (:is-opt-in asset) (not (installed? asset)))
     (throw (ex-info (str (:id asset) " is not installed and is not fetched automatically")
                     {:type :speech-assets/opt-in-required
                      :id (:id asset)
