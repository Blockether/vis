...
(def ^:private limit-statuses
  #{:ok :unauthenticated :unsupported :error :unknown-provider})

(def ^:private limit-scopes
  #{:account :plan :workspace :model})

(def ^:private limit-kinds
  #{:requests :tokens :usd :credits :sessions :rate})

(def ^:private limit-window-kinds
  #{:calendar :rolling :lifetime})

(def ^:private limit-window-units
  #{:minute :hour :day :week :month :year})

(def ^:private limit-precisions
  #{:exact :estimate :derived :unknown})

(def ^:private limit-sources
  #{:provider-api :derived :static :local})
...
(s/def ::report
  (s/keys :req-un [::provider-id ::status ::fetched-at-ms ::static ::dynamic]
    :opt-un [::error]))
...
(defn- static-limits
  ...
  (let [known (get svar-router/KNOWN_PROVIDERS provider-id)]
    (cond-> {}
      (some? (:rpm known)) (assoc :rpm (long (:rpm known)))
      (some? (:tpm known)) (assoc :tpm (long (:tpm known))))))

(defn- base-report
  ([provider-id status]
   (base-report provider-id status nil))
...
(defn- error-report
  ([provider-id type message]
   (error-report provider-id type message nil))
...
(defn- invalid-report
  ...
  (error-report provider-id
    ...
    {:report raw
     :explain (s/explain-data ::report raw)}))
