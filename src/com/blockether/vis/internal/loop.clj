                            ...
                            (when-not (some user-slash-iteration? iterations)
                              (let [forms (vec (mapcat :forms iterations))]
                                {:turn (long (or (:position turn) 0))
                                 ...
                                 :forms forms
                                 :iter-scopes
                                 (into #{} (keep #(iter-of-scope (:scope %))) forms)})))))
                  turns)
     ...
     (fn session-fold [scopes & [gist]]
       (if-let [[base label] (target scopes)]
         (let [turn (current-turn)
           ...
           (let [[base omitted] (exclude-protected base)
             ...
             (if (and (seq omitted) (empty? (get base "scopes")))
               ...
               (let [{:keys [note reclaimed-tokens]} (priced base)
...
(defn- ntr-recover-hint
  ...
  (let [ids (into [] (comp (filter some?) (distinct)) ids)]
    (when (seq ids)
      (let [shown (vec (take MAX_FOLD_NTR_HINTS ids))
...
(defn- iteration-image-message
  ...
  [iter-rec]
  (when-let [imgs (seq (filter image-attachment?
...
(defn- native-tools
  ...
  (let [engine-tools (cond-> [(apropos-tool) (doc-tool)]
                      ...
                      (try
                        ...
                        (catch Exception e
                          (cond
                            ...
                            (let [attempt-number (inc (long attempt))
...
(defn- run-bang-turn!
  ...
  (let [db-info
        ...
        (when enabled?
          (try
            (let [shell-fn (requiring-resolve 'com.blockether.vis.internal.foundation.shell/shell)]
  ...
  (let [depth-atom
        ...
        (fn []
          (let [ws
                ...
                live-roots
                (concat (when ws [(:root ws)])
                        (when ws (keep :trunk (workspace/filesystem-roots ws))))]
