...
(defn page-html
  ...
  (let [home? (= slug "index")]
    (str
      ...
      ;; CSS-only mobile search toggle: the checkbox precedes the box so it can
      ;; reveal it, and the magnifier is the only search control a phone header shows.
      "<input type=\"checkbox\" id=\"searchtoggle\" class=\"searchtoggle\" aria-label=\"Toggle search\">"
      "<label for=\"searchtoggle\" class=\"search-open\" title=\"Search\" aria-label=\"Search\">"
      ;; Lucide search.
      "<svg width=\"20\" height=\"20\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" "
      "stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\">"
      "<circle cx=\"11\" cy=\"11\" r=\"8\"/><path d=\"m21 21-4.3-4.3\"/></svg></label>"
