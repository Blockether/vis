...
(defn- window-row
  "One row per metered window. The dollar limits themselves ($12 / $30 / $60 at
   the time of writing) are NOT on the wire, so each window is normalized as a
   percentage tank (`:limit 100.0`) — the shape
   `limits-format/percentage-limit-row?` renders as \"63% left\"."
    ...
    (let [detected (vis/provider-key-detect BOOK plan-tag)]
      (if (nil? detected)
        ...
        (try {:provider-id PROVIDER_ID
              :status :ok
              :fetched-at-ms (util/now-ms)
              :dynamic (dynamic-limits! (:api-key detected))}
