...
(defn poll-for-token!
  ...
  ([device-code interval expires-in {:keys [enterprise-domain]}]
   (let [account-type
         ...
         deadline
         (+ (util/now-ms) (* (long (or expires-in 900)) 1000))
...
(def ^:private COPILOT_POLICY_MODELS
  ["claude-haiku-4.5" "claude-sonnet-4" "claude-sonnet-4.5" "claude-sonnet-4.6" "claude-sonnet-5"
   ...
   "gpt-5.1-codex-mini" "gpt-5.2" "gpt-5.2-codex" "gpt-5.3-codex" "gpt-5.4" "gpt-5.4-mini"
   "gpt-6-sol" "gpt-6-luna" "gpt-4.1" "gpt-4o" "gemini-2.5-pro" "gemini-3-flash-preview"
