...
(defn- credentials-result
  ...
  (let [access-token
    ...
    {:access-token access-token
     ...
     :expires-at-ms (- (+ (util/now-ms) (* (long expires-in) 1000)) (long refresh-margin-ms))}))
   ...
   (try
     (let
       ...
       (cond (= 200 status)
             ...
             (contains? #{401 403} status) (if forced-token
                                             (unauthenticated)
                                             (if-let [refreshed (try (force-refresh-token! token)
