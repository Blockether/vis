...
(def ^:private yaml-compat-shim-src
  "Pure-Python preamble that publishes a PyYAML-compatible `yaml` module backed
   ...
   by `install-yaml-compat-shim!`), looked up in `globals()` at CALL time so the
...
(defn- yaml-bridge-bindings
  "Host callables (pure-Clojure YAMLStar) the YAML-compat shim delegates to.
   Wrapped by `wrap-ifn`, so args marshal Python->Clojure and results
   Clojure->Python across the STRINGS-ONLY boundary: Python hands a YAML string
   in and gets native dict/list/str back; on dump it hands native data and gets
   a YAML string. Map keys land as strings (the boundary contract), which is the
   common case; malformed YAML surfaces as an exception the shim maps to
   `yaml.YAMLError`."
  []
...
(defn- install-yaml-compat-shim!
  "Bind the YAMLStar bridge callables into `g` and eval the YAML-compat shim into
   `ctx`, publishing a PyYAML-compatible `yaml` module. Best-effort: a failure
   here just leaves the sandbox without `import yaml`, it must never break
   context creation."
  [^Context ctx ^Value g]
  (try
    (doseq [[^String nm f] (yaml-bridge-bindings)]
      (.putMember g nm (wrap-ifn f)))
    (.eval ctx "python" ^String yaml-compat-shim-src)
    (catch Throwable _ nil)))
...
(defn- build-agent-context
  ...
  (let [stdout-baos (java.io.ByteArrayOutputStream.)
    ...
    ;; YAML-compat: publish a PyYAML-compatible `yaml` module backed by the
    ;; pure-Clojure YAMLStar loader (bridge callables bound onto `g` here).
    ;; Eval'd BEFORE the snapshot so its `__vis_yaml_*`/`yaml` names are baseline.
    (install-yaml-compat-shim! ctx g)
