...
;; NOTE: the node "contract" (what ctx flows DOWN to a sub_loop child, what folds
;; UP on return) is NOT engine functions. The model writes Python — it slices the
;; live `context` dict itself and passes the child ctx to `sub_loop(...)`, then
;; merges the return. The engine provides the `sub_loop` RUNTIME (fork-context! +
;; child loop) and the PROMPT guidance; it does NOT pre-chew the subslice. (Earlier
;; node-subslice / node-dispatch-packet / node-rollup-report / node-files were
;; removed for exactly this reason — model authors, prompt guides.)
