...
(defn- coerce-message
  ...
  [m]
  (let [level   (let [l (:level m)] (if (#{:info :warn :error} l) l :error))
        message (or (:message m) "(no message)")]
    (-> m
      (assoc :level   level
        :message message))))
...
(defn startup-hint-line
  ...
  ([] (startup-hint-line {}))
  ([environment]
   (let [msgs (try (run-checks environment)
                (catch Throwable t
                  (tel/log! {:level :error :id ::startup-hint-failed
