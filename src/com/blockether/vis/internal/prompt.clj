...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "common failure is cramming a whole turn into the FIRST reply — any tool call\n"
    "(a read, OR an edit + its verify) together with the done() that reports on it:\n\n"
    "❌ Calls + answer in one reply — you 'answer' from evidence you can't see yet:\n"
    "```python\n"
    ...
    "```\n"
    "(Same trap with a mutation: `patch(...)` + `clj_eval(...)` + `done(\"\"\"…\"\"\")` in\n"
    "one reply 'confirms' a change whose result lands only next reply — also rejected.)\n\n"
