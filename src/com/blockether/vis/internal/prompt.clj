...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- It's a real Python REPL — the full language + pure stdlib are yours\n"
    "  (comprehensions, f-strings, json, re, collections, itertools). A tool\n"
    "  returns structured data; SHAPE it in Python (filter/reshape/format) and\n"
    "  print only the compact slice you need, never the raw object.\n"
    ...
    "- Run independent calls CONCURRENTLY with `await gather(...)` — results come\n"
    "  back IN ORDER, so I/O-bound reads overlap instead of serializing:\n"
    "  `cfg, lib = await gather(cat(\"config.edn\"), cat(\"src/lib.clj\"))`. `gather` is\n"
    "  BUILT-IN (no import). One gather beats N awaits; don't gather interdependent\n"
    "  calls. `async def` helpers and `asyncio.run(...)` also work. (`doc(\"gather\")`\n"
    "  for multi-failure handling.)\n"
    "- Tools are BUILT-IN bare snake_case fns ALREADY in scope (cat / find / rg / ls /\n"
    "  patch / write / git_* / shell_run / …) — call directly, NEVER `import` or\n"
    "  namespace-qualify (`v.cat`); options are dicts with snake_case keys\n"
    "  (`rg({\"any\": [\"TODO\"], \"is_files_only\": True})`). Side effects go ONLY through\n"
    "  tools: the sandbox has no real fs / network / threads (`open()`, `requests`,\n"
    "  `socket` fail). Shell needs the shell tool (off by default, `:shell/enabled`) —\n"
    "  if off, say so rather than retrying.\n"
    "- A tool returns a STRUCTURED dict — index the REAL keys (guessing throws\n"
    "  KeyError, the #1 failure). `cat` has NO \"lines\"/\"text\" key: its content is\n"
    "  `c[\"anchors\"]` (an ordered `{\"lineno:hash\": text}` map) — reach for\n"
    "  `anchor(c, \"fragment\")` / `hunk(c, path, \"fragment\", repl)`. Any other shape:\n"
    "  `print()` it ONCE or `doc(\"name\")` (carries the full shape + mechanics), then index.\n"
    "## Working effectively\n"
    ...
    "- `patch` is ANCHOR-ONLY and anchors go STALE after ANY write — so re-`cat` a\n"
    "  file before editing it again, and each edit's anchors must come from a FRESH\n"
    "  `cat` of the current file. It's ATOMIC (one bad anchor rejects the whole\n"
    "  batch). READ enough up front to see every line you'll touch, then make ALL\n"
    "  changes in ONE patch. Whole-file/large rewrite → `write` (no anchors, never\n"
    "  fails). Full mechanics: editing-tools section below / `doc(\"patch\")`.\n"
    ...
    "    R2 reads visible → edit + prove: await patch([hunk(src,\"timeout\",\"…\")]); print(await repl_eval(\"python\",\"http.timeout_ms()\"))\n"
