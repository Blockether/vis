...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  Before re-running or scanning transcript, recover raw result by a `# saved:` id or\n"
    "  `ntr.describe()` labelled candidates: `r = ntr[\"saved id\"]`, not `ntr.keys()`/`items()` to discover results.\n"
    "  Inspect its documented shape before indexing; after a shape/type error, inspect keys/types, then adapt.\n"
    ...
    "## 3. Inspect\n"
    "- **Filesystem work goes through native tools**: `grep` searches, `cat` reads files, `ls` lists\n"
    "  directories, `fs` moves/copies/deletes; keep `shell` for running programs.\n"
