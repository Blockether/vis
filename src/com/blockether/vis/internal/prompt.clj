...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  (`rg({\"any\": [\"TODO\"], \"is_files_only\": True})`). `await` a tool to its\n"
    "  value — to see it, print the awaited result (`print(await cat(\"x\"))`), or\n"
    "  capture it (`x = await cat(\"x\")`) and print the part you need. There is no\n"
    "  automatic echo: an un-printed value is invisible to you next reply. Globals\n"
    "  persist (REPL), so holding results in variables across calls/turns is fine.\n"
