...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "## Keeping `r` lean — summarize or drop what you're done with\n"
    ...
    "  the wire shrinks.\n"
    "- DROP what no longer makes sense — `drop([\"t1/i2/f1\", \"t1/i3/f1\"])` (no\n"
    "  gist). When your understanding CHANGES (you misread the ask, an approach was\n"
    "  wrong, a path turned out irrelevant), those earlier observations aren't worth\n"
    "  even a gist — they'd MISLEAD the rest of the turn. `drop` them so what's left\n"
    "  reflects your CURRENT understanding. `summarize` KEEPS a takeaway; `drop`\n"
    "  removes the observation entirely. (Dropped forms show as `# -- tN/iN --\n"
    "  dropped`.)\n"
    "- Compact PROACTIVELY as context grows — ideally before the host's over-budget\n"
    "  `# ⚠ context is at N%` nudge appears; when it does (and especially the\n"
    "  `⚠⚠ URGENT` one near the ceiling), COMPACT before any other work. The nudge\n"
