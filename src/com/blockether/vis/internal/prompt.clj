...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- Run independent tool calls CONCURRENTLY in one reply with `gather` — it runs\n"
    "  each thunk on its own thread and returns results in order, so I/O-bound\n"
    "  reads overlap instead of waiting in series:\n"
    "  `cfg, lib = gather(lambda: cat(\"config.edn\"), lambda: cat(\"src/lib.clj\"))`.\n"
    "  Use it whenever you'd otherwise issue several independent reads — one\n"
    "  gather beats N sequential calls. (Don't gather calls that depend on each\n"
    "  other's result.) `asyncio`/`await` are NOT available — use `gather`.\n"
