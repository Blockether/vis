...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  ls(\".\")[\"entries\"][:50])) instead of dumping the raw object.\n"
...
(def weak-model-operating-rules
  ...
  (str "Five rules that override any temptation to do more:\n"
    ...
    "All user-facing prose goes in done(\"\"\"…\"\"\"). done() is a SHORT summary, "
    "not a data dump: tool outputs already show as cards, so NEVER put a raw "
    "tool result in it (no done(str(x)), no f-string of an ls/cat/rg result).\n"
