...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- `done()` must stand ALONE: NEVER in the same reply as ANY tool call — not a\n"
    "  mutation (patch / write / shell_run / …) and not even a pure read (cat / rg /\n"
    "  ls). You can't have seen their results yet, so the engine rejects a done()\n"
    "  beside any call and bounces you back. (The why and the act → read → done()\n"
    "  shape are in \"How a turn unfolds\".)\n"
