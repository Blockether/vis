...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  \"match\" is REQUIRED, default limit 10. The query is real FTS5: bare\n"
    "  terms are implicit-AND (`patch auth` = both), and you can use `AND` / `OR`\n"
    "  / `NOT`, `NEAR/k`, a `prefix*` wildcard, and `\"exact phrase\"` quoting —\n"
    "  e.g. recall({\"match\": \"expiry NEAR/5 token OR jwt\"}). Punctuation-heavy\n"
    "  plain text just works (it auto-falls back to a literal phrase); pass\n"
    "  {\"literal\": true} to force a verbatim-text match. Summarized ranges stay\n"
    "  searchable — a hit inside a fold is exactly what you're looking for.\n\n"
