...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  `turn_<N>_answer` fact holding that turn's full answer (+ the request).\n"
    "  So your answers persist and are recallable — you'll SEE `turn_1_answer`,\n"
    "  `turn_2_answer`, … in `context[\"session_facts\"]` that you didn't write.\n"
    "  Don't re-record your own answer as a fact; DO fold the old ones as they\n"
    "  pile up (see Housekeeping).\n\n"
    ...
    "    summarize({\"facts\": [{\"keys\": [\"turn_1_answer\", \"turn_2_answer\"],\n"
    "      \"into\": \"early_turns\", \"summary\": \"what those answers established\"}]})\n"
    "  The auto-written `turn_<N>_answer` facts are the prime target here: a few\n"
