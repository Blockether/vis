...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  `gather` is a BUILT-IN — call it directly; do NOT `import` it. There is no\n"
    "  `asyncio` here (`from asyncio import gather` fails: it's a reserved name).\n"
    ...
    "  `async`/`await`: write `async def` helpers freely and `await` them.\n"
    ...
    "  anchor VALUES; `mtime`/`size` are staleness guards copied by `edit(...)`; there is NO\n"
    "  \"text\"/\"lines\"/\"content\" key (`c[\"lines\"]` is the #1 KeyError) \u2014 prefer\n"
    "  `anchor(c, \"partial line fragment\")` / `edit(c, path, \"fragment\", repl)` helpers, or iterate\n"
    ...
    "  `edit(c, p, \"distinctive bit\", replacement)` for ONE line, or\n"
    "  `edit_span(c, p, \"start bit\", \"end bit\", replacement)` for a RANGE.\n"
    "  Choose the helper by edit size: `edit(...)` for exactly one line; `edit_span(...)` for an inclusive range. Do NOT print anchor\n"
...
(def weak-model-operating-rules
  ...
  (str "Nine rules that override any temptation to do more:\n"
    ...
    "in ONE program (`c = await cat(p)`, then `patch([edit(c, p, \"distinctive bit\", R)])` "
    "for one line or `patch([edit_span(c, p, \"start bit\", \"end bit\", R)])` for a range). "
    "Use `edit` for one line and `edit_span` for a range; choose before patching. "
