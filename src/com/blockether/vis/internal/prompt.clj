...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  one it covers. Your live ledger is `session[\"folds\"]`: `saved` (how much of\n"
    "  the wire is folded, e.g. \"12/19 steps (63%)\"), `folded` (each surviving\n"
    "  fold's `at` scopes — compressed `tN/*` / `tA-tB/*` / `t*`, sibling turns as\n"
    "  `tN/i1-i2,i5` — plus its gist), and `live` (the steps STILL on the wire =\n"
    "  fold those next). Anchor gists (file + line, e.g. \"http timeout @\n"
