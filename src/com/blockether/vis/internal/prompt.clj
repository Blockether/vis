...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  `# tN/iN` (turn / iteration) in its result. When a step's output has done\n"
    "  its job, `session_fold([\"tN/iN\"], \"what it established\")` replaces that\n"
    "  whole step — its call AND its output — with your one-line gist (omit the\n"
    "  gist to just drop a step). Fold several with a list, a whole turn with a\n"
    "  bare `\"tN\"`, or a range with a selector dict: `{\"through\": \"tN/iN\"}` (up\n"
    "  to that step), `{\"since\": \"tN/iN\"}` (that step onward), or `{\"from\":\n"
    "  \"tA/iA\", \"to\": \"tB/iB\"}` (a window). A wider fold SUPERSEDES a finer\n"
    "  one it covers. Your live ledger rides INSIDE the token meter —\n"
    "  `session[\"utilization\"][\"folds\"]`, ONE compact line (beside the live token\n"
    "  counts), re-shown every iteration, shaped\n"
    "  `<saved> | <fold> | … | live <scopes>`: `saved` is how much of the wire is\n"
    "  folded (e.g. \"12/19 steps (63%)\"), each `<fold>` is `<at>: <gist>` with `at`\n"
    "  the compressed scopes (`tN/*` / `tA-tB/*` / `t*` / `tN/i1-i2,i5`), and `live`\n"
    "  lists the steps STILL on the wire = fold those next. Anchor gists (file +\n"
    "  line, e.g. \"http timeout @\n"
    "  http.py:52\") and compact proactively — especially when the host warns the\n"
    "  context is filling.\n\n" "## Finishing a turn\n"
