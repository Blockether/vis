...
(defn- dag-expression-instruction
  ...
  (str
    ...
    "formatting calls, no raw tool dumps. Cite observations first. To mention "
    "resolved slot values in prose, use `answer_template` with "
    "`{{observations.<request_id>.result | transform}}`; transforms are host-owned and "
    "whitelisted user-facing summaries (`evidence_summary`, `git_status_summary`, "
    "`git_diff_summary`, `stdout_summary`).\n"
    "- If you need to write your own prose from a tool result, first request "
    "the observation without finalizing. After Vis returns the observation in "
    "the next iteration, write the literal `answer` and close with "
    ...
    "terminal advance must include a non-blank rendered answer. Final answers "
