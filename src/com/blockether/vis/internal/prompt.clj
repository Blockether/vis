...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- One broad fold causes one cache discontinuity; then continue append-only so the new prefix can cache.\n"
    "- The gist must stand alone: keep every fact the next phase needs, exact paths/symbols, decisions,\n"
