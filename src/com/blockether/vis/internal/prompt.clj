...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- Fold settled work into typed data: recorded thinking/tool results are replaced; storage and in-flight hidden reasoning remain.\n"
    "  When ready, patch first. Call `session_fold` with named fields, never Markdown: `goal`, optional `previous_goal`,\n"
    "  `confirmed`, `exploration_dead_ends`, `worth_exploring`, and `hypothesis`. Entries are `{text, references}`;\n"
    "  `references` are supporting transcript/tool locations such as `tN/iN`. Markdown is derived only for the breadcrumb.\n"
    "  Before an unavoidable fold, checkpoint exact paths; confirm reduction.\n"
