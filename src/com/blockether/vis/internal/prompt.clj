...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- `prompt_cache.token_read_percent` and `request_hit_percent` are Svar's fresh provider-cache metrics;\n"
    ...
    "- Before synthesis, distill research into a self-sufficient gist, call `fold_session` on its settled range, then continue from that gist.\n"
    "- When edit-ready and headroom permits, edit first. Every gist must stand alone: keep every fact the next phase needs, exact paths/symbols, decisions, verification, edit/test state, and dirty files; confirm reduction.\n"
