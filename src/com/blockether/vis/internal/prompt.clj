...
(def ^:private CORE_SYSTEM_PROMPT
  "Core system prompt for the hybrid tool surface: the model ACTS by calling the
   ...
   (Python program, print() to surface output) for transforms; FINISH by replying
  ...
  (str
    ...
    "  TRANSFORM / FILTER / CHAIN tool output, call `python_execution` with a Python\n"
    "  program instead. To FINISH the turn, reply with plain text and NO tool call —\n"
    ...
    "## Working effectively\n"
    "- `find(query)` first for discovery — typo-tolerant path / concept / unfamiliar-\n"
    "  module search; it returns ranked paths. Then `cat` the likely files, or\n"
    "  scoped `rg` when you know an exact symbol/error/string. `ls` for literal\n"
    "  directory contents.\n"
    "- `cat` returns the file as anchored lines under `c[\"anchors\"]` — an ordered\n"
    "  `{\"lineno:hash\": text}` map. To edit, take a `lineno:hash` STRAIGHT from cat's\n"
    "  output and pass it to `patch` as the edit's `from_anchor` (add `to_anchor`\n"
    "  for an inclusive span) with the `replace` text.\n"
    "- `patch` is ANCHOR-ONLY and anchors go STALE after ANY write — so re-`cat` a\n"
    "  file before editing it again, and each anchor must come from a FRESH `cat`.\n"
    "  It's ATOMIC (one bad anchor rejects the whole batch). READ enough up front to\n"
    "  see every line you'll touch, then make ALL changes in ONE `patch`. Whole-file\n"
    "  / large rewrite → `write` (via `python_execution`; no anchors, never fails).\n"
    ...
    "    R2 reads visible → edit + prove: await patch([{\"path\":\"http.py\",\"from_anchor\":\"52:a1b2\",\"replace\":\"…\"}]); print(await repl_eval(\"python\",\"http.timeout_ms()\"))\n"
