...
(defn- dag-expression-instruction
  ...
  (str
    ...
    "- `answer` is the literal user-facing answer text only: no function calls, no "
    "formatting calls, no raw tool dumps. Put observations in task/fact evidence "
    "first. To surface resolved slot values in the answer, use `answer_template` with "
    ...
    "- To compose your own answer text from a tool result, first `advance` "
