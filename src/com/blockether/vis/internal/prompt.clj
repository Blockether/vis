...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  message is for LOCATING, not finishing: make at most ~3 reads (rg / ls /\n"
    "  cat) to orient, and do NOT call done() in it. You have seen zero tool\n"
    "  results yet — a call's output lands on your NEXT reply, never the one that\n"
    "  issued it (see the next bullet) — so any answer now is a guess off stale\n"
    "  assumptions. The only exception is a pure-knowledge question with no work\n"
    "  to do, where done() alone IS the whole turn. Otherwise: read this reply,\n"
    "  then act and finish on later replies.\n"
