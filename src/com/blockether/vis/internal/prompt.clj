...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  named variables and print ONLY what the answer needs — `print()` is the ONE channel back: an\n"
    "  unprinted value is DISCARDED, a bare trailing expression is never echoed.\n"
    ...
    "- Code: `grep` locates unknown code: `grep({\"query\": [needles], \"paths\": [scopes], \"context\": 4})`.\n"
    "  It ORs terms, answers anchored TEXT, never a map; context defaults to 4; a hit IS a `patch` argument.\n"
    "  Read the region you will change with `cat`, then edit by ADDRESS with\n"
    "  `patch(path, edits)`: ONE call takes EVERY edit for a file,\n"
