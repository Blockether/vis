...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  Fetch chosen `ntr[id]`, not `ntr.keys()`/`items()` to discover results. Inspect its documented\n"
    "  shape before indexing; after a shape/type error, inspect keys/types, then adapt.\n"
    ...
    "- Code workflow: use `grep` to locate unknown code first — one grep ORs every needle across\n"
    "  every scope. For known supported files `struct_index` them all in ONE call, `cat` only the\n"
    "  needed bodies in ONE call (per-path `ranges`); edit with `struct_patch`, else `patch`.\n"
