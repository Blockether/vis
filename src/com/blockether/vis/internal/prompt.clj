...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  diff. After a mutation, RUN something that proves it: re-read the file, run\n"
    "  the project's tests, or eval the new logic in a live REPL if the language\n"
    "  offers one (see How you act), covering the obvious edge (nil / empty /\n"
    "  boundary). The check MAY ride in the SAME step as the mutation (it's a\n"
    "  mutation too) — what must come LATER is OBSERVING its result (it lands next\n"
    "  step) and only THEN done(). done() never shares a step with a mutation. No\n"
    "  way to verify? say so in done() — an honest \"couldn't verify, because X\"\n"
    "  beats a false \"done\".\n"
    ...
    "  the outcome is observable. The engine REJECTS such a done() and bounces you\n"
    ...
    "  mutations). Do NOT interleave them (no cat → patch → cat → patch) and do NOT\n"
    "  drip one patch per turn: gather what you need, then make ALL the changes. The\n"
    "  results land next reply — read them there, then done() on its own.\n"
...
(def weak-model-operating-rules
  ...
  (str "Nine rules that override any temptation to do more:\n"
    ...
    "results come back next reply; read them there, then done() on its own.\n"
