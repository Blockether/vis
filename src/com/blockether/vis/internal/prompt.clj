...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  edits, verification, recovery IDs, and exact physical paths—never abbreviate.\n"
    "- If a breadcrumb lacks a needed path, `grep` before any path-taking tool.\n"
    "- Never fold the live iteration. Retry a blocked fold with the last completed scope before new work.\n"
    "- After folding, use the updated utilization/fold receipt to confirm meaningful reduction; if it\n"
    "  saved nothing, choose older/larger settled scopes rather than repeating the same fold.\n\n"
