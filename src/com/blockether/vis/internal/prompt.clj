...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  (`rg({\"any\": [\"TODO\"], \"is_files_only\": True})`). Bind results to LOCALS\n"
    "  (`c = cat(\"x\")`) and `print(...)` only the distilled bit you need to see —\n"
    "  don't dump a bare `cat(\"x\")` (that puts the whole file in your context). See\n"
    "  \"Your context\" for the locals-vs-print split.\n"
    ...
    "  write them yourself.\n"
    "- Carrying things forward — two tools, and a rule: SURFACE ONLY WHAT YOU NEED.\n"
    "  • LOCALS persist across replies like a REPL: `x = cat(\"f\")`, defs, imports,\n"
    "    variables all carry forward. Use them as SCRATCH — compute, filter, hold\n"
    "    intermediate data. Locals are NOT shown to you; they just live in the\n"
    "    sandbox for your next reply's code to use.\n"
    "  • `print(x)` is how you put something in your CONTEXT — what you print is\n"
    "    what you (and the transcript) SEE on the NEXT reply. Do the heavy work in\n"
    "    locals, then print only the distilled result. This keeps your context\n"
    "    small and sharp: `c = cat(\"f\"); print(c[\"anchors\"][\"52:0e3\"])`, NOT a\n"
    "    bare `cat(\"f\")` that dumps the whole file. Tool results are structured\n"
    "    (`cat` → `{\"path\":…, \"anchors\": {\"N:hash\": \"text\", …}}`; `rg` →\n"
    "    `{\"hits\":[…]}`) — index/slice/comprehend in Python, print the takeaway.\n"
    "  • You don't OBSERVE a printed value until the reply AFTER it (print writes to\n"
    "    next reply's context). So a LATER form in the SAME reply reads an EARLIER\n"
    "    one's value via the local you bound it to, not by re-reading context.\n"
    "  Never fabricate tool output or printed lines — call the tool for real.\n\n"
...
(def weak-model-operating-rules
  ...
  (str "Nine rules that override any temptation to do more:\n"
    ...
    "7. Work in LOCALS, surface with print(): bind tool results to variables "
    "(`c = cat(\"x\")`) — they persist across replies like a REPL — then "
    "`print(...)` only the distilled bit you need to SEE next reply. Don't dump a "
    "bare `cat(\"x\")`; surface the takeaway, not the whole object.\n"
