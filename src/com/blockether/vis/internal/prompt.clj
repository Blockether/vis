...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- Use `find_files` for vague paths or exact text (it searches names AND content); `apropos`/`doc` for exact\n"
    ...
    "## 4. Edit + verify\n"
    "- Prefer structural editing: for supported code default to `struct_index`/`struct_patch`—it is\n"
    "  syntax-safe and re-parses every write, so reach for it over text edits. Use anchored `patch`\n"
    "  only for text/unsupported files or edits structural ops cannot express. Batch atomically; any\n"
    "  write or `format` stales anchors—re-read first.\n"
    ...
    "- Before every `session_fold`, read `session[\"turn\"]`. Fold SETTLED steps:\n"
    "  every prior turn AND the current turn's done iters, not the live one.\n"
    ...
    "  Recover a native result via `ntr[tool_id]`; its breadcrumb lists accessors and\n"
    "  survives restart. Otherwise use `await session_state()` →\n"
    "  `transcript/turns/iterations/blocks`.\n"
    "- A user-cited session UID: pass it — `await session_state(uid)`.\n"
    ...
    "  chain into,\" \"events fire,\" \"numbers feed\") — and swap in the literal verb; then\n"
    "  cut the single most brochure-like sentence.\n"
  ...
  (let
    ...
    (prompt-block
      ...
      (str "Auto-imported by `python_execution` (no `import`): `"
           ...
           "`."
           (when (seq shim-names)
             (str "\nPreinstalled shims (always import before use; no pip): `" (str/join "`, `"
                                                                                         shim-names)
