...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  diff. EVERY mutation (a patch / write / clj_edit / clj_eval / shell write)\n"
    "  needs a SEPARATE verification step AFTER it: run the project's tests or\n"
    "  `clj_eval` the new logic (see How you act) and cover the obvious edge\n"
    "  (nil / empty / boundary). Verification is its OWN step — never the same\n"
    "  step as the mutation, and never the same step as done(). No way to\n"
    ...
    "  back. The shape is always: (1) MUTATE in one step → (2) VERIFY the change\n"
    "  landed in a LATER step (re-read the file, run the relevant test, or\n"
    "  clj_eval the result) → (3) `done()` with NO mutation in its step. Pure\n"
    "  READS (cat / rg / ls) MAY sit beside done() and finalize — only mutations\n"
    "  are gated, because only they need their result observed first.\n"
