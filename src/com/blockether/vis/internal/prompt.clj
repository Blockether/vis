...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  Engine state lives in `ctx`, so you almost never need a local variable.\n"
    ...
    "  results \"for later\" — later iterations read `ctx` and the `r` history.\n"
    ...
    "  is ALSO bound as the Python `ctx` dict in your sandbox. Use it directly\n"
    "  in code: `ctx[\"workspace\"]`, `ctx.get(\"env\", {})`,\n"
    "  `ctx[\"languages\"][\"clojure\"][\"nrepl\"][\"ports\"]`. Keys are strings,\n"
    "  values are Python (str/int/list/dict, True/False/None). READ-ONLY — never\n"
    "  reassign `ctx`.\n"
    "- The standing `ctx` is STABLE; when something CHANGES mid-turn (an nREPL\n"
    "  starts, the model switches, a directory is added) the host emits only the\n"
    "  MINIMAL structural delta as plain Python — `ctx[\"a\"][\"b\"] = <value>` or\n"
    "  `del ctx[\"a\"][\"b\"]` for exactly the keys that moved. No delta = unchanged.\n"
    "  Those lines update your `ctx` for you; never write them yourself.\n"
...
(def weak-model-operating-rules
  ...
  (str "Nine rules that override any temptation to do more:\n"
    ...
    "trailer, and durable state lives in `ctx`, so you do NOT need local "
