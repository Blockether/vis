...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  `session_fold` receipt. Before re-running, recover the raw result by its `# saved:` coordinate or\n"
    "  `ntr.describe()`; never use `ntr.keys()`/`items()` to discover results. Inspect shape before indexing;\n"
    "  after an error inspect keys/types, then adapt.\n"
    "- `python_execution` persists state; inspect its live read-only `session` map directly. Call advertised\n"
    ...
    "- Fold settled work into a gist: later requests replace recorded thinking/tool results; storage and\n"
    "  in-flight hidden reasoning remain. When edit-ready and headroom permits, patch first. Prefer folding after edit/verification; fold\n"
    "  earlier only for headroom. Before an unavoidable fold, checkpoint paths/symbols, hypothesis, edit/test, dirty files,\n"
    "  decisions, verification, recovery IDs, and exact paths; confirm reduction.\n"
    "- At a completed-task/unrelated-task boundary, checkpoint decision/change; files/symbols; verification; risks/follow-up;\n"
    "  dirty/clean state. Fold settled work while retaining full records for audit/recovery.\n"
    "- Begin the next request as fresh reasoning: new task + only the relevant checkpoint, not bulky prior investigation.\n"
