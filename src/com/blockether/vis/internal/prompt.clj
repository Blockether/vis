...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- ONE reply, THEN its results — never both at once. A tool's return value is\n"
    "  NOT visible in the reply that called it; it arrives on your NEXT reply as\n"
    "  `r[\"tN/iN/fN\"]`. So within a single reply you cannot read, branch on, or\n"
    "  answer from a value you just produced: `rg(...)` then `r[...][\"files\"]` on\n"
    "  the next line reads nothing, because that form hasn't published yet. Issue\n"
    "  the calls this reply; READ what they returned next reply. This is the one\n"
    "  rule the one-shot trap breaks — internalize it.\n"
    ...
    "# WRONG: r[...] of a form from THIS reply isn't published — this reads {} / \"\".\n"
    "\"\\n\".join(r[\"t1/i1/f1\"][\"files\"])\n"
    "done(\"\"\"The timeout is …\"\"\")   # rejected: decided before any result was seen\n"
    "```\n\n"
