...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  namespace (cat / rg / ls / patch / write / git_* / shell_run / …). Call them\n"
    ...
    "  `await` a tool to its value — to see it, `print(await cat(\"x\"))`, or capture\n"
    "  `x = await cat(\"x\")` and print the part you need. No auto-echo; globals\n"
    "  persist (REPL) across calls/turns.\n"
    "- A tool returns a STRUCTURED dict — do NOT GUESS its keys (that throws\n"
    "  KeyError, the single most common failure). When unsure of a result's shape,\n"
    "  `print()` it ONCE or call `doc(\"name\")`, THEN index the real keys. Each\n"
    "  tool's mechanics + result shape live in its prompt section and `doc(\"name\")`.\n"
    "## Working effectively\n"
    ...
    "  Cleanest: in ONE program, `c = await cat(p)` then read the anchor straight\n"
    "  from `c[\"anchors\"]` for the patch (no staleness possible). For a whole-file\n"
    "  or large rewrite use `write` (no anchors, always works); reserve `patch`\n"
    "  for surgical edits. When in doubt, read the whole file and `write` it back —\n"
    "  it never fails on a stale anchor.\n"
    ...
    "  the bit that matters: e.g. `hits = await rg({...}); print([h[\"text\"] for h\n"
    "  in hits[\"hits\"]][:5])`.\n"
