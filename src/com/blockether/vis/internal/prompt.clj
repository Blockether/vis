...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- For background shell completion, use `shell` op `wait` (concurrent via `gather` when needed); `logs` is only\n"
    "  an immediate snapshot. Never use `time.sleep`, `asyncio.sleep`, or Python polling to wait for shell jobs.\n"
    ...
    "  read it. Reuse a live REPL across turns; query status only if it is absent or stale.\n\n"
    "## 3. Inspect\n"
    ...
    "- Code workflow: `grep` locates unknown code — ONE call ORs every needle across every scope.\n"
    "  `struct_index` every known file in ONE call; read bodies in ONE call — `struct_nodes`\n"
    "  (`anchor`→`source`+`at`) or `cat` `ranges`. Code edits: `struct_patch` by def NAME;\n"
