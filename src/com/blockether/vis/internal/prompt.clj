...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  `ntr[tool_id]`. After one shape/type error, inspect keys/types once and adapt to what you saw.\n"
    ...
    "- Fold settled search sweeps, dead ends, and superseded reads. Preserve decisions, findings,\n"
    "  edits, verification, recovery IDs, and exact physical paths verbatim.\n"
    "- If a breadcrumb lacks a needed path, `grep` before any path-taking tool.\n"
    "- Fold only settled steps, through the last completed iteration. Retry a blocked fold with\n"
    "  the last completed scope before new work.\n"
    "- After folding, use the updated utilization/fold receipt to confirm meaningful reduction; if it\n"
    "  saved nothing, choose older/larger settled scopes.\n\n"
