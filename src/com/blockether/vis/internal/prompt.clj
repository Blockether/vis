...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  FOCUSED slice and returns its result. YOU build `subctx` in Python by\n"
    "  slicing `context`: include the BIGGER PICTURE (the relevant plan + facts so\n"
    "  the child isn't blind) PLUS a `\"focus\"` = the ONE task key it owns. e.g.\n"
    "  `pic = {\"session_tasks\": context[\"session_tasks\"], \"session_facts\": {...}}`\n"
    "  then `sub_loop(\"implement oauth\", {**pic, \"focus\": \"oauth\"})`.\n"
    ...
    "  returns results in the SAME order. Each spec is a dict\n"
    "  `{\"prompt\": …, \"subctx\": {**pic, \"focus\": <key>}, \"models\": [\"…\"]}`. Use it\n"
    "  for independent siblings; merge each result back by its `task_id`. A child\n"
    "  that fails comes back as `{\"status\": \"failed\", \"error\": …}` — the others\n"
    "  still run. e.g.\n"
    "  `rs = parallel([{\"prompt\":\"impl oauth\",\"subctx\":{**pic,\"focus\":\"oauth\"}},\n"
    "  {\"prompt\":\"impl apikey\",\"subctx\":{**pic,\"focus\":\"apikey\"}}])`\n"
    "  then `for r in rs: plan_step(r[\"task_id\"], {\"status\": r[\"status\"], …})`.\n"
