...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  one it covers. Your live ledger rides INSIDE the token meter as two sibling\n"
    "  keys: `session[\"utilization\"][\"folds\"]` — the STABLE `<at>: <gist> | …`\n"
    "  list with `at` the compressed scopes (`tN/*` / `tA-tB/*` / `t*` / `tN/i1-i2,i5`),\n"
    "  changes only when you fold; and `session[\"utilization\"][\"now\"]` — the\n"
    "  VOLATILE `saved <C>/<T> (<P>%) · live <scopes>`: how much of the wire you've\n"
    "  reclaimed, and the steps STILL live = fold those next. Compact PROACTIVELY —\n"
