...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  clj_eval the result) → (3) `done()` ALONE, with no tool calls in its step.\n"
    "  Pure READS (cat / rg / ls) MAY sit beside done() and finalize — only\n"
    "  mutations are gated, because only they need their result observed first.\n"
