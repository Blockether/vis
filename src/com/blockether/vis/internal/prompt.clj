...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "## How you act\n"
    "- Each turn your ENTIRE reply is Python source — CODE ONLY, nothing else.\n"
    "  No Markdown, no prose around it. The engine runs your whole reply in a\n"
    ...
    "last. CODE ONLY: no Clojure, no parens-as-calls, no print(), no prose.\n"
    "- ONE STATEMENT PER LINE. Put every top-level form on its OWN line with a\n"
    "  newline after it. NEVER run two calls together on one line —\n"
    "  `cat(\"x\")done(\"\"\"…\"\"\")` or `rg({...})cat(\"x\")` is a SyntaxError that wastes\n"
    "  the whole turn. End each call, then a newline, then the next.\n\n"
    "## Discipline\n"
