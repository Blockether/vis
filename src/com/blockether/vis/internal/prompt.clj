...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- SHELL: to run a command / build / test, call shell_run(\"...\") — it returns\n"
    "  a dict {exit, stdout, stderr, ...} (non-zero exit is DATA, not an error).\n"
    "  Python subprocess.run(...) / os.system(...) ALSO work — they are bridged to\n"
    "  the same tool — but shell_run is cleaner (no exception dance). Both need\n"
    "  the shell tool enabled (:vis/shell-tool, off by default). Use\n"
    "  shell_bg(id, cmd) for long-running / background commands.\n"
    ...
    "- ONE bound-only extra: `context[\"session_trailer\"]` exists in the variable\n"
    ...
    "  conversation (oldest first). A single-call iteration puts the full address\n"
    ...
    "  address is `tN/iM/fK`. File reads show the `N:hash│ text` gutter — copy\n"
    "  those anchors straight into `patch {\"from_hash\": …}`. Need the\n"
    "  STRUCTURED value in code? Same reply: you already hold it (the live\n"
    "  return). Later: `context[\"session_trailer\"]` carries the full maps and\n"
    "  `recall(\"tN/iM/fK\")` windows the verbatim payload.\n\n"
    ...
    "  `session_utilization` says the window is getting tight. Old `<results>`\n"
    ...
    "  owns, plus only the FEW facts it needs (not all of `session_facts`). e.g.\n"
    "    sub_loop(\"implement oauth\",\n"
    "             {\"session_tasks\": task_subtree(\"oauth\"),\n"
    "              \"session_facts\": {\"auth_notes\": context[\"session_facts\"][\"auth_notes\"]},\n"
    "              \"focus\": \"oauth\"})\n"
    ...
    "  what it edited.\n"
    "- Use it to FAN OUT independent subtasks (disjoint `:files`) — but plan the\n"
    "  files first so children don't collide; overlapping-file work stays serial.\n"
    "- `parallel([spec, spec, …])` runs SEVERAL children AT ONCE (bounded) and\n"
    "  returns results in the SAME order. Each spec is a dict `{\"prompt\": …,\n"
    "  \"subctx\": {\"session_tasks\": task_subtree(<key>), \"focus\": <key>},\n"
    ...
    "  run. e.g.\n"
    "    rs = parallel([{\"prompt\":\"impl oauth\", \"subctx\":{\"session_tasks\":task_subtree(\"oauth\"), \"focus\":\"oauth\"}},\n"
    "                   {\"prompt\":\"impl apikey\",\"subctx\":{\"session_tasks\":task_subtree(\"apikey\"),\"focus\":\"apikey\"}}])\n"
    ...
    "  e.g. ordered build→test: `rs = sequence([{\"prompt\":\"build\",\"subctx\":{\"session_tasks\":task_subtree(\"build\"),\"focus\":\"build\"}},\n"
    "                                            {\"prompt\":\"test\", \"subctx\":{\"session_tasks\":task_subtree(\"test\"), \"focus\":\"test\"}}])`\n"
    ...
    "  \"subctx\":{\"session_tasks\":task_subtree(\"build\"),\"focus\":\"build\"}}, 3)`.\n"
