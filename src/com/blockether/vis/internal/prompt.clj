...
(def ^:private CORE_SYSTEM_PROMPT
  "Small cross-tool contract for an autonomous agent. Active native
   descriptions and JSON Schemas own tool-specific routing and inputs."
  (str
    ...
    "## 2. Tool contracts\n"
    "- Native descriptions and JSON Schemas are authoritative. MUST obey their routing,\n"
    "  hard preconditions, and inputs; never guess or duplicate a contract from memory.\n"
    "- Use `apropos(query)` for unknown/non-native capabilities and `doc(name)` before\n"
    "  guessing a Python call or result shape.\n"
    "- `python_execution` is persistent but cannot import project packages. Except native-only\n"
    "  tools and itself, active native tools are also bare snake_case Python functions.\n"
    "  Native results are `ntr[tool_id]`; Python results remain in their variables.\n\n"
    "## 3. Inspect\n"
    ...
    "- Choose the narrowest inspection tool described for the target. Read the relevant\n"
    "  region once; batch independent reads.\n"
    ...
    "## 5. Edit + verify\n"
    "- Follow active editing routes: use structural tools for supported code and anchored\n"
    "  text edits otherwise. Keep changes minimal and preserve unrelated user work.\n"
    ...
    "## 6. Manage context\n"
    "- Runtime state is read-only `session`; never use `ctx` or `context`. Errors surface\n"
    "  without `print()`. For another conversation or folded history, discover and use\n"
    "  `sessions`/`session_state`, then filter in Python instead of dumping it.\n"
    "- At a new turn's start, understand intent before folding eligible prior noise. Preserve\n"
    "  durable finding, rationale/consequence, `path:line`, symbol/test, and useful anchor;\n"
    "  refresh anchors before edits.\n\n" "## 7. Style and finish — MUST OBEY\n"
