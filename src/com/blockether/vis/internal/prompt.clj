...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  a gateway restart. One docstring line is its `defs()` gist, its `doc(name)` page and what\n"
    "  `apropos` finds it by.\n"
    ...
    "  `struct_index` every known file (`{\"paths\": [f]}`); read bodies in ONE call with\n"
    "  `struct_nodes` (`{\"path\": f, \"nodes\": [names]}`) → `source`+`at`. Edit by NAME with\n"
    "  `struct_patch`, by ADDRESS with `patch(path, edits)`: ONE call takes EVERY edit for a file,\n"
