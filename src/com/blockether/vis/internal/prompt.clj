...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- RELOAD SCOPE: a clj `:reload` / `clj_edit` changes SOURCE on disk and reloads\n"
    "  the PROJECT nREPL — NOT a running channel/app. A TUI or web GATEWAY is a\n"
    "  SEPARATE JVM that compiled its code at start and does NOT hot-reload. So a\n"
    "  renderer/UI change you reload + eval-verify in the nREPL is NOT live in those\n"
    "  surfaces until the host RESTARTS. Never claim a UI change is 'live/verified'\n"
    "  from an nREPL eval — say it applied and needs a restart to take effect.\n"
    "- READ EVERY TOOL RESULT before claiming done. A `clj_edit` that returns\n"
    "  `:status \"error\"` (e.g. `target not found`) did NOT apply — fix the target\n"
    "  and resend; do NOT report success for an edit that failed.\n"
