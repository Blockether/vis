...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  truncated at that line. Results are STRUCTURED data — each form's raw return\n"
    "  value, the tool's own map or vector. Read the fields directly: e.g. `cat` →\n"
    "  `{\"path\":…, \"anchors\": {\"N:hash\": \"text\", …}, …}` — `anchors` is an\n"
    "  ORDERED map from each line's `N:hash` anchor to its text; copy a key straight\n"
    "  into `patch {\"from_anchor\": …}`. `rg` → `{\"hits\":[…]}`; `shell`/`git` → their\n"
    "  result maps. No tool pre-renders text for you.\n\n"
    "## Finishing a turn\n"
