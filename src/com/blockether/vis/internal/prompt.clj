...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  describe it: locate → edit → VERIFY → done() in one continuous push. Do\n"
    ...
    "  NEVER open with a `##` heading, a bullet, or a prose sentence. Your whole\n"
    "  reply is handed STRAIGHT to the Python parser, so a heading or a sentence\n"
    "  is a SyntaxError, not text — e.g. an apostrophe in prose (`rift's`) opens\n"
    "  an unterminated string literal and the WHOLE turn is rejected with a\n"
    "  cryptic line error. Findings, root-cause write-ups, and any prose belong\n"
    "  in `#` comments ABOVE the code or inside done(\"\"\"…\"\"\"), never bare.\n"
    "  The engine runs your whole reply in a\n"
    ...
    "  steps to `pending`/`in_progress` and proceed; drop the rest as `cancelled`.\n"
...
(def weak-model-operating-rules
  ...
  (str "Nine rules that override any temptation to do more:\n"
    "1. Your whole reply is raw Python CODE ONLY — no prose around it. NEVER "
    "open with a `##` heading or a prose sentence: the whole reply is parsed as "
    "Python, so a heading or an apostrophe in prose (`rift's`) is a SyntaxError "
    "that wastes the turn — put findings in `#` comments or done(\"\"\"…\"\"\"). "
