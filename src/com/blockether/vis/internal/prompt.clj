...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- Keep context lean by CURATING past steps — a fold is DISTILLATION, not\n"
    "  deletion: keep the finding, drop the hunt. Each prior step is tagged\n"
    "  `# tN/iN` (turn / iteration) in its result. Once a step's output has done\n"
    "  its job, `session_fold([\"tN/iN\"], \"what it established\")` replaces that\n"
    "  whole step — its call AND its output — with your one-line gist. FOLD the\n"
    "  noise (search sweeps, raw dumps, dead ends, superseded reads); SAVE the\n"
    "  signal INTO the gist — the decision, the number, and the ANCHOR (file:line,\n"
    "  e.g. \"http timeout @ http.py:52\") so the durable fact survives the collapse.\n"
    "  Omit the gist only to drop a truly worthless step. Fold several with a list,\n"
    "  a whole turn with a bare `\"tN\"`, or a range dict: `{\"through\": \"tN/iN\"}`\n"
    "  (up to that step), `{\"since\": \"tN/iN\"}` (that step onward), or `{\"from\":\n"
    "  \"tA/iA\", \"to\": \"tB/iB\"}` (a window). A wider fold SUPERSEDES a finer\n"
    "  one it covers. Your gist lands IN PLACE — a `# ⋯ folded <scopes> · <gist>`\n"
    "  breadcrumb replaces the step where it collapsed, so the finding (with its\n"
    "  anchors) stays exactly where the hunt was. Alongside it,\n"
    "  `session[\"utilization\"][\"now\"]` shows the live budget — `saved <C>/<T>\n"
    "  (<P>%) · live <scopes>`: how much of the wire you've reclaimed, and the steps\n"
    "  STILL live (compressed `tN/*` / `tA-tB/*` / `t*` / `tN/i1-i2,i5`) = fold those\n"
    "  next. Compact PROACTIVELY —\n"
    "  before the host warns the context is filling, not after.\n\n" "## Finishing a turn\n"
