...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  after the raw read pins get compacted away.\n"
    "- Link a fact to the plan step it backs with `\"facts\": [\"calc_add\"]` on\n"
    "  update_plan/plan_step (see Planning).\n\n"
    "## Compaction — summarize\n"
    "- summarize(...) is your ONE mid-turn compaction verb: collapse stale\n"
    "  trailer ranges and settled facts/tasks into a recoverable stub so they\n"
    "  stop riding every prompt. Nothing is lost — recall(...) re-materializes\n"
    "  the full data by scope or key.\n"
    "- DO IT AS YOU GO. Once an older trailer iteration (a big git_log, a wide\n"
    "  rg, a file you already read) no longer informs the work, fold it. Shape:\n"
    "  summarize({\"trailer\": [{\"scope_start\": \"t<N>/i<M>\", \"scope_end\":\n"
    "  \"t<N>/i<K>\", \"summary\": \"what those iters established\"}]}); the same\n"
    "  verb also folds settled facts/tasks (\"facts\" / \"tasks\" keys).\n"
    ...
    "  climbs toward auto_compress_above. As it rises, proactively summarize\n"
    "  stale trailer so you stay ahead of the engine's own automatic fold\n"
    "  (a safety net under budget, not a substitute for compacting as you go).\n"
    "- COMPACT AT CLOSE: batch a final summarize(...) right before done() in\n"
    "  the same reply to fold the turn's spent trailer as you finish.\n\n"
    "## Recovery — recall\n"
    ...
    "  — use rg for files). Full-text (FTS5) match over past iterations:\n"
    "    recall({\"match\": \"patch auth\", \"scope_after\": \"t2/i1\"})\n"
    "  → [{\"scope\": …, \"preview\": …, \"rank\": …}] ranked by relevance;\n"
    "  \"match\" is REQUIRED, default limit 10. The query is real SQLite FTS5:\n"
    "  bare terms are implicit-AND (`patch auth` = both); UPPERCASE `AND` / `OR`\n"
    "  / `NOT` combine, `term*` is a prefix wildcard, `\"exact phrase\"` is a\n"
    "  verbatim run, and `NEAR(a b, k)` finds two terms within k tokens — e.g.\n"
    "  recall({\"match\": \"NEAR(expiry token, 5) OR jwt\"}).\n"
    "  Punctuation-heavy plain text just works (a query FTS5 can't parse falls\n"
    "  back to a literal phrase). Summarized ranges stay searchable — a hit\n"
    "  inside a fold is exactly what you're looking for.\n\n"
    ...
    "  `plan_step(\"step_key\", {\"status\": \"completed\", \"verified\": True})`. The\n"
    "  key is the snake_case key shown in `context[\"session_tasks\"]`. Prefer this\n"
