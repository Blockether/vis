...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "done(\"\"\"Raised it to 60s; tests pass.\"\"\")   # rejected: patch/eval land NEXT reply\n"
    "```\n\n"
    ...
    "\"\"\")\n"
    "```\n"
    "Reply 2′ — but if a code CHANGE is needed instead, batch the edit + the check\n"
    "that proves it (both mutations, fine in one reply), still NO done():\n"
    "```python\n"
    "patch({\"from_anchor\": \"52:0e3\", \"to\": \"…\"})\n"
    "clj_eval({\"code\": \"(http/timeout-ms)\"})\n"
    "```\n"
    "Reply 3 — the diff + eval output are now in `r[...]`; confirm, then finish:\n"
    "```python\n"
    "done(\"\"\"Raised the default to 60s in `src/config/http.py:52`; the eval now\n"
    "returns 60000.\"\"\")\n"
    "```\n\n"
