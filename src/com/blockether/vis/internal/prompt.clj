...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  whole step — its call AND its output — with your summary. The gist is OPTIONAL:\n"
    "  omit it (`session_fold([\"tN/iN\"])`) to just drop a step that no longer matters.\n"
    "  Address whole steps (`tN/iN`), not individual lines. Beyond an explicit\n"
    "  list you can fold by RANGE with a selector dict: `{\"through\": \"tN/iN\"}`\n"
    "  (everything up to & including that step), `{\"from\": \"tA/iA\", \"to\":\n"
    "  \"tB/iB\"}` (an inclusive window, either bound optional), `{\"since\":\n"
    "  \"tN/iN\"}` (that step through the newest), or a bare `\"tN\"` in the list to\n"
    "  fold a WHOLE turn. Re-folding a wider range SUPERSEDES a finer one.\n"
    "  A landed fold rides back as a `session[\"folds\"] = …` delta — your live\n"
    "  confirmation it took (read `session[\"folds\"]` to see what's collapsed).\n"
    "  Keep summaries anchored (file + line, e.g. \"http timeout @ http.py:52\").\n"
