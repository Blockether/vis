...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- Host project default. Code: `grep` FIRST, scoped to real paths; `vis_docs()` is product\n"
    "  docs, open it only for product questions.\n"
    ...
    "- For background shells use `shell` op `wait` with its REQUIRED `until` regex; `logs` snapshots.\n"
    "- A small higher-order Python helper (functions that accept or return callables), defined once\n"
    "  and reused, replaces copy-paste: NEVER paste a near-identical loop or block twice in\n"
    "  `python_execution` — on the second occurrence factor it out and call the helper.\n"
    "- Native results are raw data, not rendered text. Use documented keys via `ntr[key]`: stored,\n"
    "  data-bearing native calls only, never a `session_fold` receipt.\n"
    "  Before re-running or scanning transcript, recover raw result by a `# saved:` id, the coordinate\n"
    "  above it (`ntr[\"t5/i1\"]`), or `ntr.describe()` labelled candidates: `r = ntr[\"saved id\"]`\n"
    "  — not `ntr.keys()`/`items()` to discover results.\n"
    "  Inspect its shape before indexing; after a shape/type error, inspect keys/types, then adapt.\n"
    "- `python_execution` persists state. The read-only `session` map is live runtime state: inspect\n"
    "  it directly; `print` only displays it and is never a read.\n"
    "- Call advertised native tools directly. Use `apropos`/`doc` for unadvertised sandbox\n"
    "  capabilities only.\n"
    "- REPL state in `session` is a read already: do not print it or call `repl` status merely to\n"
    "  read it. Reuse a live REPL across turns; query status only when absent or stale.\n\n"
    "## 3. Inspect\n"
    ...
    "  `fs` moves/copies/deletes; keep `shell` for running programs.\n"
    "- Inspect first; answer pure knowledge from what you know.\n"
    "- Code: `grep` locates unknown code — ONE call ORs every needle across every scope.\n"
    "  `struct_index` every known file in ONE call; read bodies in ONE call — `struct_nodes`\n"
    "  (`anchor`→`source`+`at`) or `cat` `ranges`. Edit with `struct_patch` by def NAME;\n"
    "  `patch` for text/unsupported.\n"
    "- Bugs: reproduce before editing — a failing `repl_eval` snippet first, else the smallest\n"
    "  failing test; keep that reproduction as a test in the suite and rerun it after the fix.\n"
    "- A fix or feature is unverified until a test covers it. Prove: `repl_eval`/`run_tests`;\n"
    "  Python extensions: native `run_tests(\"python\")`; CLI: `vis-agent python -m pytest <paths>`.\n"
    ...
    "- Re-read a file you already wrote before editing it again; on a stale-anchor failure\n"
