...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  `ls(paths='.', depth=1, is_hidden=False, *, hidden=None, pattern=None)` takes one path or a list of paths\n"
    ...
    "  Optional `pattern` filters basenames by case-sensitive glob, e.g. `'*snapshot*'`, not regex;\n"
    "  None disables it. Applies at each depth, keeping ancestors; per-path specs override it.\n"
    ...
    "- Before reading, identify the unresolved question affecting the next step; if none, stop reading.\n"
