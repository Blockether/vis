...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- `await shell(\"npm test\")` answers a HANDLE — `sh.logs(-50)` (last n LINES)/`sh.wait(s)`/`sh.type(\"y\")`/`sh.stop()`, each carrying status.\n"
    ...
    "  `del` a helper once its callers, aliases and captured defaults confirm it is unused. After a restart, `defs(name, details=True)`\n"
