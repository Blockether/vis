...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  Spend `cat` addresses with `patch(path, edits)`, ONE call per file:\n"
    "  `[{\"from\": a, \"to\": b, \"replace\": text}]`; `to` defaults to `from`, `\"\"` deletes.\n"
    "  Exact lines only — never relocate or restate the text you replace: quote the anchor.\n"
    ...
    "- Re-read a file you already wrote before editing it again; `patch` answers a FRESH ANCHOR per\n"
    "  edit and a stale one is refused WITH the correct anchor, so neither costs a read. When a\n"
    "  locator no longer resolves, re-read that target and retry.\n"
