...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  not shell.\n" "- Write a PROGRAM, not a transcript: `project_root_path` and the names in\n"
    "  `session[\"workspace\"][\"path_globals\"]` are prebound `Path` objects; do not redefine them or guess aliases.\n"
    "  Derive paths with `/`; keep results in named variables. `print()` is the ONE channel back:\n"
