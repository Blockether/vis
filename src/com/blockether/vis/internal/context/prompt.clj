...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  not shell.\n"
    "- Write a PROGRAM, not a transcript: derive workspace paths from prebound `root` (a `Path`);\n"
    "  do not redefine it. Bind other roots ONCE off `session`; keep results in named variables and\n"
    "  print ONLY what the answer needs — `print()` is the ONE channel back: an unprinted value is DISCARDED,\n"
