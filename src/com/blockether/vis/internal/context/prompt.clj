...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "- Define once and reuse a small helper: NEVER paste a near-identical loop or block twice;\n"
    "  factor it out on the second occurrence, and CALL the one an earlier block defined — `defs()`\n"
    "  lists them, `defs(name)` reads one back to refine — a `def` outlives the block, the turn, and\n"
    "  a gateway restart. One docstring line is its `defs()` gist and its `doc(name)` page.\n"
    ...
    "- BATCH inside one block: `grep` needles, `ls` roots, `cat` reads and\n"
    "  every `patch` edit for a file — one call, never one per needle, path or edit. Each round\n"
    "  must identify, reproduce, edit, verify, or conclude; no repeated search/read without a\n"
