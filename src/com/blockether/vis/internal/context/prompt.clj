...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  Nested types | Traverse available `fn.contract` in memory; `fields` is a list of `{name, type}`. Print only matching leaves, never whole `parameters`/`returns` branches.\n"
    ...
    "  Result access: mappings `r['key']`, records `r.field`. Inspect unknown shapes via keys/types or `dir(value)`, not `__dict__`.\n"
    "  Use error-provided keys, never guessed wrappers. After a successful write, recover its saved result after print/access errors; do not repeat the write.\n"
    ...
    "  A stale-anchor refusal supplies the current anchor: confirm the intended target before retrying.\n"
    "  A parse refusal means fix the replacement syntax, not the anchors; never retry it unchanged.\n"
