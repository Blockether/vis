...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  Skip `apropos()`, `doc()` and `inspect.signature()` for known facts across turns, `/reload` and repeated calls.\n"
    ...
    "  Prior-turn context | Already in the visible conversation, fold gists included; a new turn or continuation-like wording (\"now…\", \"taking into account…\") is not a missing fact. Session history answers only a named question the conversation cannot.\n"
    ...
    "  Result shape | `doc(name)` lists return-model fields under Model schemas. Traverse available `fn.contract` in memory for nested types; `fields` is a list of `{name, type}`. Print only matching leaves, never whole `parameters`/`returns` branches.\n"
    "- Registered signatures/types own kinds, requiredness/defaults, returns and mutation tag; inspection may omit types/effects.\n"
    "  Do not copy signatures/defaults/schemas into docstrings. Omit optional arguments for defaults; never pass marker `...`.\n"
    ...
    "  Data work (YAML/JSON/TOML/CSV) is Python, not shell.\n"
    "- Prebound `Path` objects: `project_root_path` (workspace, always available)\n"
    "  and `python_name`→`cwd` in `session[\"workspace\"][\"filesystem_roots\"]`; do not redefine them or guess aliases.\n"
    "  `root` is not prebound; do not create that alias. Use `/`; keep results in variables. `print()` is the ONE channel back:\n"
    "  an unprinted value is DISCARDED; a bare trailing expression is never echoed.\n"
    "  Result access: mappings `r['key']`, records `r.field`. An extension result is a frozen record of its public fields:\n"
    "  methods never cross, only declared sequences iterate, and a wrong name raises KeyError/AttributeError listing the real fields.\n"
    "  Inspect unknown shapes via keys/types or `dir(value)`, not `__dict__`.\n"
    "  Use error-provided keys and fields, never guessed wrappers or synonyms. After a successful write, recover its saved result after print/access errors; do not repeat the write.\n"
    "- Batch independent work in ONE block: plural arguments first, `await gather(...)` for\n"
    "  independent calls. Reuse results; print needed fields or keys/types, not whole-value fallbacks. END the block, then decide in the NEXT block.\n"
    ...
    "  Saved definitions mirror the namespace after each block (best-effort): redefining replaces, `del obsolete_name` removes.\n"
    "  Check callers, aliases and captured defaults before `del`. Never delete by age. After a restart, `defs(name, details=True)`\n"
    "  lists a helper's global/captured names and whether each is present.\n"
    "- Create Python extensions only when asked; first read `doc(\"extending\")`.\n"
    "- `session` is host-owned, rebuilt before every block; writes are erased. Never store state there.\n"
    "  Reuse a live REPL — nothing lists one for you; `repl_status` does.\n\n" "## 3. Inspect\n"
    "- **Filesystem work is Python**; `shell(...)` runs programs.\n"
    ...
    "  `start` counts from the end. `Path.read_text` gives no anchors; creating/moving/deleting is plain Python.\n"
    ...
    "  Terms OR; `is_regex: True` runs a regex; answers anchored TEXT, never a map; `context`: lines per side (default 3); a hit IS a `patch` argument.\n"
    "  Use `patch(path, edits)`, ONE call per file:\n"
    "  `[{\"from\": a, \"to\": b, \"replace\": text}]`; `from`/`to` MUST be `line:hash` strings, never source text or bare numbers.\n"
    "  `to` defaults to `from`; `\"\"` deletes. Given `12:abc│ old`, a one-line edit is `{\"from\": \"12:abc\", \"replace\": \"new\"}`.\n"
    "  Exact lines only — never relocate a write. `replace` is new file text without hash gutters.\n"
    "  Copy observed anchors verbatim, checking each endpoint's line number and full three-character hash.\n"
    ...
    "- Treat code/config style as correctness: follow project rules and formatter/linter config, then consistent nearby\n"
    "  examples, not isolated inconsistencies. Preserve naming, indentation, logical grouping and blank-line separation\n"
    "  between definitions and configuration resources. Conciseness/minimal diffs must not collapse readable structure.\n"
    "  Spacing must not change whitespace-sensitive values or required document separators (e.g. YAML `---`).\n"
    ...
    "  For parse errors, fix replacement syntax, not anchors; never retry unchanged.\n"
    ...
    "- Treat context as a budget: pressure is `last_request_tokens` against `auto_compress_above`;\n"
    "  `saturation`/`headroom_tokens` price the hard limit; `hint` only arms at 75% of that operating budget.\n"
    "- `prompt_cache.token_read_percent` and `request_hit_percent` are Svar provider-cache metrics;\n"
    ...
    "  Omitting the gist discards outright; a folded step is NOT re-readable, so the gist is what survives.\n"
    ...
    "- One broad fold causes one cache discontinuity; continue append-only from its gist—never re-read settled work to refold it.\n"
  ...
  (let [shims
    ...
    (prompt-block
      ...
      (str "Auto-imported by `python_execution` (no `import`): `"
           ...
           "Import `blockether.vis.extension` for types; "
