...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  Reuse contracts from this task or recovered session context. A new turn, `/reload`, or repeated call does not\n"
    "  invalidate known contracts: skip `doc()`, `apropos()` and tool-spec discovery when signature and preconditions are known.\n"
    ...
    "- For Python tools, registered signature and resolved types are authoritative for call shape:\n"
    "  parameter kinds, required/default status, return type and mutation tag—not handwritten signatures.\n"
    "  Unknown call shape: use narrow `doc(name)`. For omitted details, traverse the callable's `.contract` dictionary\n"
    "  from `['parameters']` or `['returns']`; print only matching leaf fields, never whole schema branches or large results.\n"
    ...
    "  independent calls. Need to inspect a result? Print only needed fields or keys/types, END the block, then decide in the NEXT block.\n"
