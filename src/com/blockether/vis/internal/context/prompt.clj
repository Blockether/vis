...
(def ^:private CORE_SYSTEM_PROMPT
  ...
  (str
    ...
    "  `defs(name, details=True)` gives source SHA-256 and advisory globals/types; liveness unknown, not a complete dependency graph.\n"
    "  At phase boundaries, review your helpers: check callers, aliases and captured defaults before `del obsolete_name`. Never delete by age.\n"
    "  For proven cross-session reuse, propose to Improve with concrete uses, sanitized source/name + SHA-256, globals/preconditions\n"
    "  and verification of that source. Proposals do not authorize promotion or automatic extension creation.\n"
