...
(def ^:private introspection-doc
  ...
  {'apropos
   ...
   'defs
   "defs(name=None, *, pattern=None, limit=20, offset=0, details=False) -> str. Find and refine the functions this session defined. `defs()` shows an alphabetical index with matched/total/shown counts, block, source length and the first docstring line. `pattern` is a case-sensitive regular expression over names or those gists; `limit` is an integer from 1 to 100 (default 20), and `offset` is a nonnegative integer. Call hints are capped at 120 characters, omit annotations and show default types such as `=<str>`, never their values. `defs(name)` returns the exact source. `defs(name, details=True)` returns metadata only: source SHA-256, origin and at most 20 source-derived global or captured names with types and presence. These hints are advisory: liveness unknown, dynamic dependencies may be absent, default/decorator expressions are not analyzed, and the source digest does not identify global, default or captured state. Details requires a name; named lookups cannot combine listing controls. Missing names give a bounded search suggestion, not the whole catalogue. The full docstring remains the helper's `doc(name)` page. Imported functions and engine internals are excluded; session-local definitions remain absent from `apropos`. Helpers persist across turns and are re-created in a fresh sandbox after a gateway restart, marked `(restored)`. Read before refining under the same stable name. Delete obsolete bindings explicitly with `del name`, after checking callers; aliases and captured defaults may still hold older functions. Do not clean up by age or promote automatically. For a helper proven useful across sessions, propose an Improve entry with its source SHA-256, reusable need and verification before an authorized extension implementation (`doc(\"extending\")`)."
   'fold-session
