...
(defn get-router
  "Get or create the shared LLM router. Singleton reused across components."
  []
  (or @router-atom
...
(defn reset-router!
  "Reset the shared router (for example after config changes)."
  []
  (reset! router-atom nil))

(defn ask!
  "Structured LLM call via shared router. Convenience wrapper around llm/ask!.
