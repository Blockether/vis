(ns com.blockether.vis.loop.knowledge.pageindex
  "PageIndex: helpers, serialization, load-index, inspect, build-index, index! implementation."
  (:require
   [babashka.fs :as fs]
   [clojure.edn :as edn]
...
(defn load-index
  ...
  [index-path]
  (let [abs-path (str (fs/absolutize index-path))]
...
(defn- collect-all-nodes [pages] (mapcat :nodes pages))

(defn- has-toc-entries? [pages]
  (boolean (some #(= :toc-entry (:type %)) (collect-all-nodes pages))))

(defn- heading-level->toc-level [heading-level]
  (when heading-level
    (str "l" (subs heading-level 1))))

(defn- build-toc-from-structure [pages]
...
(defn- collect-section-descriptions [pages]
  (->> pages
    (mapcat :nodes)
    (filter #(= :section (:type %)))
    (keep :description)
...
(defn- detect-input-type [input opts]
  (cond
...
(defn- write-embedded-image-nodes! [pages output-dir]
  (let [dir-file (io/file output-dir)]
    (when-not (.exists dir-file)
...
(defn- derive-index-path [input-path]
...
(defn- read-manifest [output-path]
  (let [f (io/file output-path "manifest.edn")]
    (when (.exists f)
      (edn/read-string (slurp f)))))

(def ^:private manifest-write-lock (Object.))

(defn- write-manifest! [output-path manifest]
...
(defn- update-manifest-page! [output-path manifest-atom page-idx status-map]
  (swap! manifest-atom assoc-in [:pages page-idx] status-map)
  (write-manifest! output-path @manifest-atom))

(defn- with-index-lock! [output-path f]
  (let [dir (io/file output-path)]
    (when-not (.exists dir)
      (.mkdirs dir))
...
(defn index-impl!
  ...
  ([file-path] (index-impl! file-path {}))
  ([file-path {:keys [router output vision-model text-model parallel parallel-refine
   ...
   (let [extraction-strategy (or extraction-strategy :vision)
         ...
         abs-path (str (fs/absolutize file-path))
               ...
               (let [existing-page-map (into {} (map (fn [p] [(:index p) p])
                 ...
                 (let [final-pages (->> @page-map-atom (sort-by key) (mapv val))
                   ...
                   (let [final-manifest (cond-> (assoc @manifest-atom
                         ...
                         _ (write-manifest! output-path final-manifest)]
...
(defn ^:export index!
  "Public index! API. Thin alias over the dedicated PageIndex core namespace."
  ([file-path] (index! file-path {}))
  ([file-path opts]
   (rlm-pageindex-core/index! file-path
     (update opts :deps #(merge {:index-impl-fn index-impl!} %)))))
