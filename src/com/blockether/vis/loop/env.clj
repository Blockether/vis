(ns com.blockether.vis.loop.env
...
(defn create-env
  ...
  (let [depth-atom (atom 0)
        ;; Canonical tool registry: {sym → {:fn :doc :arglists
        ;;   :validate-input :validate-output :hooks
...
(defn register-env-fn!
  "Registers a function in the RLM environment's SCI sandbox.
   ...
   `tool-def` - Map. Tool metadata:
     - :doc - String. Description of what the function does.
     - :arglists - Optional quoted arglists (for prompt docs).
     - :validate-input - Optional fn (fn [invocation] ...) for arg validation/normalization.
     - :validate-output - Optional fn (fn [outcome] ...) for result validation/normalization.
   ...
   (register-env-fn! env 'fetch-weather
     (fn [city & [{:keys [units]}]] ...)
      ...
      :validate-input (fn [{:keys [args]}]
                        (let [[city opts] args]
                          (when-not (string? city)
                            (throw (ex-info \"city must be a string\" {:type :tool/invalid-input})))
                          {:args [city (or opts {})]}))
      :validate-output (fn [{:keys [result]}]
                         (when-not (map? result)
                           (throw (ex-info \"weather tool must return map\" {:type :tool/invalid-output})))
                         {:result result})
