(ns com.blockether.vis.rlm.tools.sci
  ...
  (:require
   ...
   [com.blockether.vis.rlm.persistence.db :as db :refer [str-truncate]]
   [com.blockether.vis.rlm.tools.conversation :as tools-conv]
   [com.blockether.vis.rlm.tools.git :as tools-git]
   [com.blockether.vis.rlm.tools.restore :as tools-restore]
   [com.blockether.vis.rlm.tools.shared :as tools-shared
...
(defn create-sci-context
  ...
  (let [restore-var-fn (when (and db-info conversation-ref)
                         (tools-restore/make-restore-var-fn db-info conversation-ref))
        base-bindings {'sub-rlm-query sub-rlm-query-fn
                       'spec spec/spec
                       'field spec/field
                       'parse-date tools-shared/parse-date 'date-before? tools-shared/date-before?
                       'date-after? tools-shared/date-after?
                       'days-between tools-shared/days-between 'date-plus-days tools-shared/date-plus-days
                       'date-minus-days tools-shared/date-minus-days 'date-format tools-shared/date-format
                       'today-str tools-shared/today-str}
        db-bindings (when db-info
                      (cond->
                        {'search-documents (tools-shared/make-search-documents-fn db-info)
                         'fetch-document-content (tools-shared/make-fetch-document-content-fn db-info)
                         ...
                         'search-batch (fn search-batch
                                         ([queries] (when db-info (tools-shared/format-docs (db/db-search-batch db-info queries))))
                                         ([queries opts] (when db-info (tools-shared/format-docs (db/db-search-batch db-info queries opts)))))
                         'get-entity (fn get-entity
                                       [entity-id] (when db-info (db/db-get-entity db-info entity-id)))
                         'list-relationships (fn list-relationships
                                               ([entity-id] (when db-info (db/db-list-relationships db-info entity-id)))
                                               ([entity-id opts] (when db-info (db/db-list-relationships db-info entity-id opts))))}
                        (and db-info conversation-ref)
                        (assoc 'conversation-history (tools-conv/make-conversation-history-fn db-info conversation-ref)
                          'conversation-code (tools-conv/make-conversation-code-fn db-info conversation-ref)
                          'conversation-results (tools-conv/make-conversation-results-fn db-info conversation-ref))
                        restore-var-fn
                        (assoc 'restore-var restore-var-fn
                          'restore-vars (tools-restore/make-restore-vars-fn restore-var-fn))))
        git-bindings (when db-info
                       (tools-git/make-git-sci-bindings db-info))
