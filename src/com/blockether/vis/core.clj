...
(defn ingest-to-env!
  "Ingests PageIndex documents into an RLM environment.
   ...
   `opts` - Optional map (reserved for future ingestion options).
...
(defn ingest-git!
  "Ingest git commits from a repository into an RLM environment.
   ...
   entities), AND persists a `:repo` entity to the DB keyed
   ...
   * Multi-repo ingestion = multiple `ingest-git!` calls with distinct
   ...
   Returns:
   Map with {:events-stored :people-stored :files-stored
...
;; =============================================================================
;; query-env! — delegated to rlm.core.query
;; =============================================================================

(def query-env!
  "Runs a query against an RLM environment using iterative code execution.
   Delegates to rlm.core.query. See there for full docstring and options."
  rlm-query/query-env!)
