...
One SDK for extensions, remote sessions and a local engine; one engine and one wire contract.
...
- Remote Python applications install the SDK only. Local applications also need a compatible
  Vis executable; the SDK owns its child process, never a user's running gateway.
- Local means an engine subprocess over stdio, not a second agent or an in-process JVM.
...
- [ ] Publish SDK packages only after explicit authorization and required checks are green.
...
- [x] Distinguish host form definitions/answers from public input Views/close receipts. Never stream
  submitted values or secret handles. Support live close receipts with or without the settled picture.
...
- Installed SDK against the fresh staged native release: **19 passed**, including public close
  receipt privacy and the SSE/polling picture distinction. Selected `test-native` cases: **5 passed**.
...
- Implementation and local SDK acceptance are complete. The following gates are not claimed green:
- Editing E2E `py-repl-compute`: the runner's relative-classpath bug is fixed and unit-tested. A real
  run converged without tool errors but the model omitted required `repl_eval`; cleanup timed out.
  A cross-model retry timed out starting its source gateway. Neither attempt is green.
- Earlier broad JVM run: 272 cases / 36 unregistered `:shell` / `:_shell-wait` failures, without a
  clean-base comparison. Selected green suites do not establish a green repository-wide suite.
