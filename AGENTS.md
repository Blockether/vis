...
- **The stylesheet is the product's own.** Copy the `:root` token block and the `@theme` type scale verbatim from `apps/vis-companion/src/index.css` into the proposal's `<style>`, and load Tailwind v4 from a CDN. The proposal must look like the app it is proposing a change to: no invented color, no invented font size, no `leading-*`, only `text-chip` … `text-display`, only the closed control vocabulary from `src/components/ui.tsx`.
...
- **Build it out of the REAL controls, copied from `src/components/ui.tsx`.** Open `ui.tsx` and reuse that control's own class string — `Button`/`NewSessionButton` (`h-8 … mouse:h-6`, square, `bg-accent text-accent-foreground`, 44px target via `after:` inset), `KebabButton` (`h-11 w-8 … mouse:h-6 mouse:w-6`), `SectionHeader` (`min-h-13 … mouse:min-h-9`, paper `bg-level-machine` vs `bg-level-project`), `HeaderTitle`/`HeaderMeta`/`HeaderTally`, `MachineSwitcher`/`MachineTab` (`h-7 px-2 font-mono text-meta`), `SessionRow`, `Modal`/`DialogFrame`/`DialogHeader`, `Menu`/`MenuItem`. Nothing is rounded, names and facts are `font-mono`, the leading edge is `pl-3 sm:pl-4` and the trailing one `pr-3 sm:pr-4` — a control invented for the mockup is a proposal about a product that does not exist. Label each block in the page with its real component name; a proposal that cannot name its controls is not a proposal.
- **The mock is INTERACTIVE — every control does the thing it claims.** A dead picture in HTML is still a picture. Each phone frame owns its screens and overlays, and one small delegated `data-act` click handler drives them: a session row PUSHES the detail screen (with a Back that returns), `NewSessionButton` opens the real `Modal` showing which machine and which canonical workspace root it would send, `KebabButton` opens the `Menu` with its actual items, a `MachineTab` scopes and says what scoping changes. Same-screen actions mutate the screen; separate screens replace it. Verify every path with `spel eval-js "…element.click()"` and check `spel errors` before attaching.
