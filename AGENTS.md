...
| Companion relay | `apps/vis-companion-relay/` is the push Worker: it holds the APNs/FCM keys, seals delivery grants (`src/seal.ts`) and keeps no device-token store, database or OAuth callback. `gateway/relay.clj` pushes through a grant, `gateway/push.clj` pushes directly with a self-hosted APNs key, and `apps/vis-companion/src/lib/relay.ts` registers the grant with its gateway. |
