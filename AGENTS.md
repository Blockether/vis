...
4. `npm run release:android -- --track beta` — the Android mirror (vite build → `cap sync android` → `android-prepare` → signed `.aab` → Play upload). `internal` (up to 100 named testers) is the default track; `beta` is Play *open testing*, the closest thing to a TestFlight public link. `--no-upload` stops at the signed bundle. It needs the stock JDK 21 exception described above (`JAVA_HOME=~/.sdkman/candidates/java/21.0.11-tem`, never GraalVM) and the Play service account in keychain `vis-play/service_account` (`node scripts/secrets.mjs play <service-account.json>`).

Shipping to BOTH stores is exactly steps 1 → 2 → 3 → 4, in that order, from `apps/vis-companion`. Before starting, confirm the credentials exist (`security find-generic-password -s vis-ios -a asc_key_id -w`, `-s vis-play -a service_account -w`); `versionCode`/build number is `git rev-list --count HEAD` and is SHARED by both platforms, so commit once and release both from that same commit.
