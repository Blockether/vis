...
- Two layers, one contract, no schema library beyond `clojure.spec.alpha`. `internal/human_input.clj` PARSES (either spelling of every key, a type name looked up in a CLOSED vocabulary, a coerced value per type); `internal/human_input/spec.clj` DECLARES the NORMALIZED form and is where the vocabulary lives — `field-types`, `text-types`, `choice-types`, `secret-types`, `group-type`/`group-type-name`, `group-directions`, `otp-defaults`, `secret-handle-prefix`/`secret-handle?`. Never keep a second copy of that table beside the parser.
- The spec is checked at four seams and nowhere else: `checked-field` per normalized field, `checked-group` per layout node, `checked-request` on the assembled request, `checked-answer` inside `settle!` — the ONE funnel every answer (submitted, cancelled, timed out, undeliverable) passes through. Once per request, never per keystroke.
- **A group is upper control flow, not a field type.** `field-types` is the vocabulary of ANSWERS and `:group` is deliberately not in it: `group-type` stands apart, `normalize-node` takes the fork ONCE on the way in (`group-node?` reads the raw type name, then `normalize-group` or `normalize-field`), and `::node` is the only place the two contracts meet — a group is `::group` (the children it arranges, their `:direction`, and no key that describes a value), a leaf is `::field`. So `normalize-field` never has to ask whether it is really layout, `field-error` refuses a group and `group-error` refuses a field, and no value path below — coercion, defaults, the answer map, an answered value's domain — carries a branch for a node that can never take one.
...
- **In a form the surface IS the cursor — no `•` gutter.** EVERY focusable row of a human-input form goes through `dialogs/draw-field-row!`: typed line, OTP boxes, option, checkbox, slider. They all start at the dialog's own inner edge, under their label, and focus is said by surface + `▎` ring + bold ink alone. `p/selection-prefix`'s `• ` marker belongs to LIST dialogs (`dialogs/draw-selectable-row!`, `dialogs/draw-checkbox-item!`) and nothing else — a marker in front of every row says the same thing about all of them, and the gutter it needs has to be reserved in every state just to keep the rows still. The status glyph is separate and has ONE source, `dialogs/choice-mark`: `●`/`○` for an EXCLUSIVE choice (`:select`), `[✓]`/`[ ]` for an INCLUSIVE one (`:multiselect`, `:checkbox`), so "choose one" can never look like "choose any".
