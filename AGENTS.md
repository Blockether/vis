...
4. **Proposal phases** — one `## Phase N — <the verb it performs>` each, ordered so every phase lands on a product that already works. Each carries exactly four things: **Rationale** (what stays broken without it); **Data** (a `clojure.spec.alpha` block when the phase changes data, in the plan before the code — no prose schemas, no second schema library, and no block at all when no data changes); **Acceptance criteria** (the files it changes, one line each, plus the test that proves it done); **Unknowns** (as questions — a phase with none says so).
