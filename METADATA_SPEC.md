...
| --- | --- | --- | --- | --- |
| `session_soul.metadata` | channel, external id | SQLite persistence session create/list/find | No | `channel`, `external_id` columns |
| `session_state.metadata` | system prompt, root provider/model | SQLite persistence session create/get/list | No | `system_prompt`, `llm_root_provider`, `llm_root_model` columns |
| `session_turn_soul.metadata` | nothing in normal path | Mostly dead | No | remove column |
| `session_turn_state.metadata` | messages, iteration count, duration, tokens, cost, provider/model | turn update/read, transcript/list summaries | No | named turn outcome columns |
| `session_turn_iteration.metadata` | LLM routing summary/trace, engine timing, extension snapshots | loop writes; TUI resume reads; diagnostics read | Mixed, mostly no | routing columns/table, `engine_timing`, `runtime_extension_snapshot` |
...
- `extensions/persistance/vis-persistance-sqlite/src/com/blockether/vis/ext/persistance_sqlite/core.clj`
  - writes session/turn/iteration metadata
  - reads metadata into public session/turn maps
  - filters `session_soul.metadata` with JSON extraction for channel/external id
...
Why it exists: bind Vis session identity to channel-specific external identity.

Where used:

```clojure
(db-get-session db id)        ;; returns :channel :external-id
(db-list-sessions db :tui)    ;; filters channel
(db-find-session-by-external) ;; filters external id
```
...
Why it exists: snapshot session-level prompt/provider/model defaults.

Where used:

```clojure
(db-get-session db id) ;; returns :system-prompt :provider :model
(row->session)         ;; title/model/provider display
```

Judgment: incorrect metadata use. Session snapshot facts are core state.
...
Target: remove column from V1. Turn soul already has identity facts: `session_state_id`, `position`, `user_request`, `created_at`.

### `session_turn_state.metadata`
...
CREATE TABLE session_turn_state (
  id                           TEXT PRIMARY KEY NOT NULL,
  session_turn_soul_id    TEXT NOT NULL REFERENCES session_turn_soul(id) ON DELETE CASCADE,
  forked_from_session_turn_state_id TEXT REFERENCES session_turn_state(id) ON DELETE SET NULL,
  version                      INTEGER NOT NULL CHECK (version >= 0),
...
### `session_turn_iteration.metadata`
...
- `:llm` is core routing/resume state -> dedicated `session_turn_iteration.llm_*` columns + `llm_routing_event` table (V1 schema).
...
CREATE TABLE session_turn_iteration (
  id                         TEXT PRIMARY KEY NOT NULL,
  session_turn_state_id TEXT NOT NULL REFERENCES session_turn_state(id) ON DELETE CASCADE,
...
CREATE TABLE runtime_extension_snapshot (
  id TEXT PRIMARY KEY,
  session_turn_iteration_id TEXT NOT NULL REFERENCES session_turn_iteration(id) ON DELETE CASCADE,
...
```text
session_soul.metadata           -> remove; use channel, external_id
session_state.metadata          -> remove; use system_prompt, llm_root_provider, llm_root_model
session_turn_soul.metadata      -> remove
session_turn_state.metadata     -> remove; use named outcome/usage/routing columns
session_turn_iteration.metadata -> remove; use routing columns/table, engine_timing, runtime_extension_snapshot
