...
- fix(tui): the wheel over a LIVE band owns every row that band paints — `band-rows` measured the pane as ONE display row while `paint!` drew as many rows as the view asked for, so the pointer scrolled the run's own log only over the four rows just above the prompt and scrolled the transcript UNDERNEATH the band everywhere else. The height is decided once now, in `band-shape`, and both the paint and the wheel read that one number: a view still takes at most half the rows between the transcript and the prompt and never fewer than a third of the screen when it has more to show — 19 rows of 40, 11 of 24 — so a live run is neither the whole terminal nor a sliver of it, and what it settles into is still one row of the transcript
