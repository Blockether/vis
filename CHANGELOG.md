...
- feat(shell): `sh.logs(lines=10)` is a LINE window — the last ten, or the NEXT ten from an offset (`sh.logs(next_offset, 10)`), so a long log is walked a screenful at a time instead of by guessing byte counts; the only line-aware read before it was the whole negative tail, and a chunk closed by the window says `is_eof` false so a reader continues rather than sleeping on a log that is already written
