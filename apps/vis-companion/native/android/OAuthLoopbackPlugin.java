...
public class OAuthLoopbackPlugin extends Plugin {
    ...
    public void authorize(PluginCall call) {
        getActivity()
                .runOnUiThread(
                        () -> {
                            if (pending != null) {
                                call.reject("Another sign-in browser is active.");
                                return;
                            }
                            try {
                                String id = call.getString("flowId");
                                Double deadline = call.getDouble("expiresAt");
                                if (id == null
                                        || id.isEmpty()
                                        || deadline == null
                                        || !Double.isFinite(deadline))
                                    throw new IllegalArgumentException();
                                OAuthLoopback started =
                                        new OAuthLoopback(
                                                call.getString("authorizationUrl"),
                                                call.getString("redirectUri"),
                                                call.getString("state"),
                                                deadline.longValue());
                                receiver = started;
                                pending = call;
                                flowId = id;
                                ...
                                if (pending == call) finish(null);
                                else call.reject("Cannot receive this sign-in callback.");
                            }
                        });
    }
    ...
    private void openBrowser() {
        ...
        Intent intent =
                new Intent(Intent.ACTION_VIEW, Uri.parse(receiver.authorization.toString()));
    ...
    public void reopen(PluginCall call) {
        getActivity()
                .runOnUiThread(
                        () -> {
                            if (receiver == null || !flowId.equals(call.getString("flowId"))) {
                            ...
                            try {
                                ...
                                call.reject("Cannot reopen sign-in.");
                            }
                        });
    }
    ...
    public void cancel(PluginCall call) {
        getActivity()
                .runOnUiThread(
                        () -> {
                            if (flowId != null && flowId.equals(call.getString("flowId")))
                                finish(null);
                            call.resolve();
                        });
    }
    ...
    private void finish(String url) {
        ...
        pending = null;
        flowId = null;
        OAuthLoopback old = receiver;
        receiver = null;
        if (old != null) old.cancel();
        if (url == null) {
            call.reject("Sign-in closed or expired.");
            return;
        }
        ...
        try {
            ...
            /* Returning manually still completes the same flow. */
        }
