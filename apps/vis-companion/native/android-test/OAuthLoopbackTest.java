...
public class OAuthLoopbackTest {
    private OAuthLoopback receiver(String redirect, long ttl) {
        String auth =
                "https://gateway.example.com/authorize?state=test-state&redirect_uri="
    ...
    private String exchange(String address, String query) throws Exception {
        try (Socket socket = new Socket(address, 53692)) {
            ...
            socket.getOutputStream().write(request(query).getBytes(StandardCharsets.UTF_8));
            return new String(socket.getInputStream().readAllBytes(), StandardCharsets.UTF_8);
        }
    }

    @Test
    public void stateDestinationAndAmbiguity() {
        ...
        try {
            ...
            for (String query :
                    new String[] {
                        ...
                        "state=test-state&code=test-code&error=",
                        "state=test-state&code=test-code&code=other",
                        "state=test-state&state=test-state&code=test-code",
                        "state=test-state&code=test-code#fragment"
                    }) assertNull(server.callback(request(query)));
            assertNull(
                    server.callback(
                            request("state=test-state&code=test-code")
                                    .replace("localhost:53692", "10.0.0.5:53692")));
            assertNull(
                    server.callback(
                            request("state=test-state&code=test-code").replace("GET ", "POST ")));
            assertEquals(
                    "http://localhost:53692/callback?state=test-state&code=a%2Bb+c",
    ...
    @Test
    public void refusesNonLoopbackAndMismatchedAuthorization() {
        for (String redirect :
                new String[] {
                    "http://10.0.0.5:53692/callback",
                    "http://localhost:80/callback",
                    "https://localhost:53692/callback",
                    "http://user@localhost:53692/callback",
                    "http://localhost:53692/callback?next=other"
                }) assertThrows(IllegalArgumentException.class, () -> receiver(redirect, 60000));
        assertThrows(
                IllegalArgumentException.class,
                () -> receiver("http://localhost:53692/callback", -1));
        assertThrows(
                IllegalArgumentException.class,
                () -> receiver("http://localhost:53692/callback", 901000));
        assertThrows(
                IllegalArgumentException.class,
                () ->
                        new OAuthLoopback(
                                "https://gateway.example.com/authorize?state=other",
    ...
    @Test
    public void freshReturnOverRealIPv4AndIPv6Listener() throws Exception {
        OAuthLoopback server = receiver("http://localhost:53692/callback", 60000);
        ...
        try {
            ...
            assertEquals(
                    "http://localhost:53692/callback?state=test-state&code=test-code",
                    returned.get(3, TimeUnit.SECONDS));
        } finally {
            server.cancel();
        }
    }
    ...
    @Test
    public void cancellationExpiryAndExclusivePortOwnership() throws Exception {
                    ...
                    try {
                        assertThrows(
                                ...
                                () ->
                                        other.start(
                                                value ->
                                                        fail("A failed bind cannot claim a flow")));
                    } finally {
                        other.cancel();
                    }
                    server.cancel();
