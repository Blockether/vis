...
export const planDistribution = ({ audiences, group, review = true } = {}) => {
  ...
  if (unknown)
    throw new Error(
      `unknown audience "${unknown}" (${[ALL_AUDIENCES, ...TESTFLIGHT_AUDIENCES].join(' | ')})`,
    );
  // A named group is a public group: there is nothing else to name.
  const isPublic = asked.length
    ? asked.includes('public') || asked.includes(ALL_AUDIENCES) || Boolean(group)
    : true;
  return {
...
export const distribute = async ({
  ...
  try {
    ...
    const everyGroup =
      (await api('GET', `/v1/betaGroups?filter[app]=${appId}&limit=200`))?.data ?? [];
