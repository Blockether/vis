...
const project = [
  ...
  config('504EC3141FED79650016851F', 'Debug', ['CODE_SIGN_IDENTITY = "iPhone Developer";']),
  config('504EC3151FED79650016851F', 'Release', ['CODE_SIGN_IDENTITY = "iPhone Developer";']),
...
const stamp = (text = project) => stampManualSigning(text, { teamId: 'JSZTFUBUBB', profileNames });
...
describe('stampManualSigning', () => {
  it('names a profile for every shipped bundle, extension included', () => {
    ...
    expect(stamped).toEqual(['com.blockether.viscompanion', 'com.blockether.viscompanion.share']);
    expect(text).toContain('PROVISIONING_PROFILE_SPECIFIER = "Vis Companion App Store";');
    expect(text).toContain('PROVISIONING_PROFILE_SPECIFIER = "Vis Companion Share App Store";');
  ...
  it('leaves automatic signing to Debug and to the project defaults', () => {
    ...
    expect(projectRelease).toContain('CODE_SIGN_IDENTITY = "iPhone Developer";');
  });
  ...
  it('skips a bundle it has no profile for rather than mis-signing it', () => {
    ...
    expect(text).toContain('PRODUCT_BUNDLE_IDENTIFIER = com.blockether.viscompanion.share;');
  });
});
...
describe('distributionIdentity', () => {
  ...
  it('returns the exact distribution fingerprint, not an ambiguous name', () => {
    expect(
      distributionIdentity(
        listing('Apple Development: Someone (ABC123)', 'Apple Distribution: Example (ABC123)'),
      ),
    ).toBe('2'.repeat(40));
  });

  it('recognises the legacy iPhone Distribution certificate', () => {
    expect(distributionIdentity(listing('iPhone Distribution: Example (ABC123)'))).toBe(
      '1'.repeat(40),
    );
  });
  ...
  it('pins the first searchable certificate when names are identical', () => {
    ...
    expect(
      distributionIdentity(
        listing('iPhone Distribution: Example (ABC123)', 'iPhone Distribution: Example (ABC123)'),
      ),
    ).toBe('1'.repeat(40));
  });
  ...
  it('ignores malformed fingerprints and development-only keychains', () => {
    ...
    expect(distributionIdentity(listing('Apple Development: Someone (ABC123)'))).toBeUndefined();
    expect(distributionIdentity('     0 valid identities found\n')).toBeUndefined();
  });
...
describe('pickProfile', () => {
  ...
  it('takes the live App Store profile that expires last', () => {
    expect(pickProfile(payload, { bundleId: 'com.blockether.viscompanion' })?.id).toBe('p-new');
  });

  it('reports nothing for a bundle id with no profile of its own', () => {
    expect(pickProfile(payload, { bundleId: 'com.blockether.viscompanion.share' })).toBeUndefined();
  });
});
...
describe('ensureProfiles', () => {
  ...
  const bundleIds = ['com.blockether.viscompanion', 'com.blockether.viscompanion.share'];
  ...
  const portal = () => {
    ...
    const fetch = async (url, init = {}) => {
      ...
      if (method === 'GET' && pathname === '/v1/profiles') {
        const linked = (searchParams.get('fields[profiles]') ?? '').split(',').includes('bundleId');
        return reply(200, {
          data: bundleIds.map((identifier, index) => ({
            ...
            ...(linked ? { relationships: { bundleId: { data: { id: `b-${index}` } } } } : {}),
          })),
