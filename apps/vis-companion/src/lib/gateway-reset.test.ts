...
const conn = (name: string) =>
  ({ id: name, name, url: `https://${name}.example.com`, token: 'test-pairing' }) as GatewayConn;
afterEach(() => {
  vi.unstubAllGlobals();
  localStorage.clear();
});
...
it('retains one account-scoped attempt across an unknown response and a new client', async () => {
  ...
  const fetch = vi.fn(async (_url: unknown, init?: RequestInit) => {
    ...
    return new Response(
      JSON.stringify(
        bodies.length === 1 ? { outcome: 'unknown' } : { outcome: 'already_redeemed' },
      ),
      { status: 200 },
    );
  });
  ...
  await expect(first.consumeProviderResetCredit('openai-codex', 'account-1')).rejects.toThrow(
    /confirm/i,
  );
  ...
  await expect(retry.consumeProviderResetCredit('openai-codex', 'account-1')).resolves.toBe(
    'already_redeemed',
  );
  ...
  expect(fetch.mock.calls[0][0]).toBe(
    'https://reset-retry.example.com/v1/providers/openai-codex/reset-credits/consume',
  );
});
