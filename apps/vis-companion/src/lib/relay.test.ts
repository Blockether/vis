...
describe('which machines need a relay', () => {
  ...
  it('never routes browser push through the native relay', () => {
    expect(relayUrlFor(status({ relay: { is_available: true, url: RELAY } }), 'web')).toBeNull();
  });
  ...
  it('falls back to the relay this BUILD was published with', () => {
    ...
    expect(relayUrlFor(status({ relay: { is_available: false, url: null } }), 'ios')).toBe(
      PUBLISHER_RELAY_URL,
    );
  ...
  it('names the http relay a machine offered, instead of blaming its keys', () => {
    ...
    expect(refusedRelayUrl({ ...plaintext, apns: { is_available: true } }, 'ios')).toBeNull();
  ...
  it('shows a relay by host, so a self-hosted one is recognisable', () => {
    ...
    expect(relayHost('https://push.example.com:8443/v1')).toBe('push.example.com:8443');
...
describe('registering this device with one gateway', () => {
  it('hands a signing machine the token and nothing else', async () => {
    const relay = minter();
    const target = gatewayOf(status({ apns: { is_available: true }, provider: 'apns' }));
    const identity = await registerForPush(device(), target.gateway, relay.mint);
  ...
  it('tells the gateway WHICH relay sealed the grant', async () => {
    ...
    const relay = minter();
    const target = gatewayOf(status({ relay: { is_available: false, url: null } }));
  ...
  it('fails instead of registering a token the machine could never use', async () => {
    ...
    await expect(registerForPush(device(), target.gateway, refuse)).rejects.toThrow('429');
...
describe('taking this device off a gateway', () => {
  it('drops it under every name it may be filed under', async () => {
    ...
    const other = gatewayOf(status({ apns: { is_available: true }, provider: 'apns' }));
  ...
  it('drops every other name when one revocation is refused', async () => {
    ...
    await expect(unregisterFromPush(TOKEN, flaky)).rejects.toThrow('machine unreachable');
