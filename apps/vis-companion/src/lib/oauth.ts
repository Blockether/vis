...
export function reserveAuthTab(): AuthTab | undefined {
  ...
  try {
    ...
    /* Same-origin blank tab; opener is advisory here. */
  }
  return {
    navigate: (url) => {
      try {
...
export function clientAuthFlow<T extends SignInFlow>(flow: T, gatewayBase: string): T {
  ...
  try {
    if (['127.0.0.1', 'localhost', '[::1]'].includes(new URL(gatewayBase).hostname)) return flow;
  } catch {
    /* Unknown is not proof of a local gateway. */
  }
  return {
    ...flow,
...
function verifiedReturn(
  ...
  try {
    ...
    if (
      `${url.protocol}//${url.host}${url.pathname}` !== redirect ||
      url.username ||
      url.password ||
      url.hash
    )
...
export function watchAuth(
  ...
  onConnection?: (message: string | null) => void,
  tab?: AuthTab,
): AuthWatch {
  ...
  const step = (): Promise<void> => {
    ...
    running = (async () => {
      try {
        if (expired()) {
          fail('Authorization timed out. Start sign-in again.');
          return;
        }
        if (pendingInput) {
          try {
            finish(await transport.complete(pendingInput));
          } catch {
            /* A lost response is resolved by polling before retrying the same input. */
          }
        }
        if (!stopped && !expired()) {
          try {
            ...
            if (!stopped) onConnection?.('Connection interrupted. Retrying authorization status…');
          }
        }
      } finally {
        running = undefined;
  ...
  const complete = async (input: string) => {
    ...
    running = (async () => {
      try {
        finish(await transport.complete(input));
      } finally {
        running = undefined;
        schedule();
      }
  ...
  const start = async () => {
    try {
      ...
      if (local && url) {
        ...
        if (!state) {
          tab?.close();
          fail('This host cannot receive the requested callback. Start sign-in again.');
          return;
        }
        removeListener = () => {
          void local.cancel({ flowId: flow.flow_id }).catch(() => {});
        };
        void local
          .authorize({
            flowId: flow.flow_id,
          ...
          .then(({ url: returned }) => {
            ...
            if (!input) {
              fail('Invalid sign-in return. Start sign-in again.');
              return;
            }
            claimed = true;
            pendingInput = input;
          ...
          .catch(() => {
            if (!stopped)
              fail('Sign-in was closed or could not receive its callback. Start sign-in again.');
          });
        schedule();
        return;
      }
      ...
      tab?.close();
      fail('Cannot open secure sign-in. Start sign-in again.');
    }
  };
  void start();
