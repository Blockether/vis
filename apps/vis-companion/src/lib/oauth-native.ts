...
export interface NativeOAuth {
  authorize(options: { flowId: string; authorizationUrl: string; redirectUri: string; state: string; expiresAt: number }): Promise<{ url: string }>;
...
export function hasNativeLoopback(flow: SignInFlow): boolean {
  ...
  try {
    ...
    return uri.protocol === 'http:' && ['localhost', '127.0.0.1', '[::1]'].includes(uri.hostname)
      && Number(uri.port) >= 1024 && !uri.username && !uri.password && !uri.search && !uri.hash
