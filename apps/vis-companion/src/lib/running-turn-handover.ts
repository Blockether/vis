...
function rowCarriesOutput(turn: TranscriptTurn): boolean {
  if (
    turn.content?.some((block) => {
      ...
      // tool output the user has already seen in the running-turn bubble.
...
/**
 * Whether a persisted transcript row is a safe replacement for a running-turn bubble.
 *
 ...
 * previous turn, and retiring the running-turn bubble against it makes an answer that was
 * already visible disappear until the real row is persisted.
 */
export function settledTranscriptCoversRunningTurn(
  ...
  const expectedId = running.id ?? "";
  const expectedRequest = running.request?.trim() ?? "";

  return turns.some((turn) => {
    if (!isSettledRow(turn)) return false;
    if (running.requireOutput && !rowCarriesOutput(turn)) return false;
    if (running.requireProse && !rowCarriesProse(turn)) return false;
    const id = rowId(turn);
    ...
    if (
      ...
      return created >= running.startedAt - 60_000;
    }
    return true;
  });
}
