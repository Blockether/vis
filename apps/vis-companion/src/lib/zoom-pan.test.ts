...
describe('zoom limits', () => {
  it('never shrinks past the frame and never magnifies past 6x', () => {
  ...
  it('recentres the moment the picture is back to 1:1', () => {
  ...
  it('keeps the offset while it is zoomed in', () => {
    ...
    expect(zoomedBy({ scale: 1.2, x: 30, y: -10 }, 0.1)).toEqual(NO_TRANSFORM);
  });
});
