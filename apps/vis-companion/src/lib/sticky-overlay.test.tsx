...
import { claimOverlayHandover, dropOverlayHandovers, offerOverlayHandover } from './sticky-overlay';
 ...
 */
const picture = <ExpandableImage src="blob:picture" alt="chart.png" className="size-8" />;
const paint = (settled: boolean) =>
  act(() =>
    root.render(
      settled ? <section key="settled">{picture}</section> : <div key="live">{picture}</div>,
    ),
  );
