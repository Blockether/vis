...
it.each(['https://gateway.example.com', 'http://10.0.0.5:7890', 'http://100.64.0.10:7890',
  'http://example.ts.net:7890', 'http://gateway.example.com'])('uses the paired transport unchanged: %s', async base => {
  ...
  expect(fetch.mock.calls.map(([url]) => url)).toEqual(['mcp/servers/work', 'providers/openai-codex']
    .flatMap(owner => ['start', 'complete', 'poll', 'cancel'].map(action => `${base}/v1/${owner}/auth/${action}`)));
...
it.each(['127.0.0.1', 'localhost', '[::1]'])('still permits unpaired loopback: %s', async host => {
  const fetch = vi.fn().mockImplementation(async () => new Response('{}')); vi.stubGlobal('fetch', fetch);
...
it.each([undefined, '', '   '])('refuses unpaired remote HTTP and HTTPS (%s)', async token => {
  ...
  for (const url of ['http://10.0.0.5:7890', 'https://gateway.example.com']) {
    ...
    await expect(client.completeProviderAuth('openai-codex', 'test-flow', 'test-code'))
...
it('does not downgrade or replay a failed HTTPS request', async () => {
  ...
  expect(fetch).toHaveBeenCalledOnce();
  expect(fetch.mock.calls[0]![0]).toBe('https://gateway.example.com/v1/mcp/servers/work/auth/start');
});
