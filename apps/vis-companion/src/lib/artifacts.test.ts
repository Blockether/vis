...
describe('attachment classification', () => {
  it('reads the media type first and the kind only as a fallback', () => {
    ...
    expect(attachmentIsDoc({ index: 0, media_type: 'application/pdf' })).toBe(true);
  ...
  it('puts a still, a clip and a recording on the same rail and nothing else', () => {
    expect(attachmentIsPlayable({ index: 0, media_type: 'image/png' })).toBe(true);
    expect(attachmentIsPlayable({ index: 0, media_type: 'video/mp4' })).toBe(true);
    expect(attachmentIsPlayable({ index: 0, media_type: 'audio/mpeg' })).toBe(true);
    expect(attachmentIsPlayable({ index: 0, media_type: 'application/pdf' })).toBe(false);
  });
  ...
  it('sorts a settled run by the media type the engine sealed it with', () => {
    ...
    expect(attachmentIsLive({ ...run, media_type: `${LIVE_ARTIFACT_MEDIA}; charset=utf-8` })).toBe(
      true,
    );
  ...
  it('names the format from the filename, then the media type', () => {
    ...
    expect(artifactMedia({ index: 0, media_type: 'application/xhtml+xml' })).toBe('XML');
...
describe('collecting what a session produced', () => {
  it('flattens turn → iteration → attachment, newest first', () => {
    ...
    expect(list.map((entry) => entry.name)).toEqual(['report.pdf', 'notes.csv', 'revenue.png']);
  ...
  it('finds the exact cut named by an attachment link', () => {
    const current = collectArtifacts(turns).find((entry) => entry.name === 'revenue.png')!;
    const old = {
      ...current,
      key: 'i0:0',
  ...
  it('counts the turn from the start of the session, not of the window', () => {
    expect(collectArtifacts(turns).map((entry) => entry.turn)).toEqual([2, 1, 1]);
    // 40 turns live before the page we hold: the newest one is turn 42.
    expect(collectArtifacts(turns, 40).map((entry) => entry.turn)).toEqual([42, 41, 41]);
  });
...
describe('collapsing an artifact into its versions', () => {
  ...
  it('keeps ONE row per name, the newest cut, with the thread behind it', () => {
    ...
    expect(collapsed.map((entry) => entry.name)).toEqual(['chart.png', 'notes.md']);
  ...
  it('never merges two different names into one artifact', () => {
    const collapsed = collapseArtifactVersions([cut('a.png', 1), cut('b.png', 1)]);
...
describe('collapseAttachmentVersions', () => {
  ...
  it('never merges two different names into one thread', () => {
    const threads = collapseAttachmentVersions([cut('a.png', 1, 0), cut('b.png', 1, 1)]);
...
describe('folding a saved revision back into the transcript', () => {
    ...
    [
      {
        ...
        iterations: [
          {
            id: 'i1',
            attachments: [{ index: 0, filename: 'notes.md', media_type: 'text/markdown' }],
          },
        ],
      },
  ...
  it('makes the gallery show the new version with its thread behind it', () => {
    const [note] = collapseArtifactVersions(collectArtifacts(withSavedAttachment(turns(), saved)));
...
describe('the session-wide artifact index', () => {
  ...
  it('reads the whole session, newest first, with the turn that made it', () => {
    ...
    expect(list.map((entry) => entry.name)).toEqual(['report.pdf', 'revenue.png']);
  ...
  it('lists an artifact from a turn the reader never scrolled back to', () => {
    ...
    expect(merged.map((entry) => entry.iterationId)).toEqual(['i3', 'i1', 'i1', 'i9']);
