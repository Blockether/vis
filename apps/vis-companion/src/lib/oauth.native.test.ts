...
vi.mock('@capacitor/core', () => ({
  Capacitor: {
    isNativePlatform: () => host.available,
    isPluginAvailable: () => host.available,
  },
  registerPlugin: () => host,
}));
...
it('receives a fixed loopback callback on the initiating native device, without rewriting the registered URI', async () => {
  ...
  const verdict = vi.fn();
  const presented = clientAuthFlow(flow, remote);
  expect(presented.callback_mode).toBe('loopback');
  const watcher = watchAuth(presented, client, verdict);
...
it('treats a gateway-side bind failure the same way and keeps a web host honest', () => {
  expect(clientAuthFlow({ ...flow, callback_mode: 'manual' }, remote).callback_mode).toBe(
    'loopback',
  );
  host.available = false;
  expect(clientAuthFlow(flow, remote).callback_mode).toBe('manual');
});
it('reopens and cancels only the same native attempt, ignoring a late return', async () => {
  ...
  const client = transport();
  const verdict = vi.fn();
  const watcher = watchAuth(clientAuthFlow(flow, remote), client, verdict);
  await vi.advanceTimersByTimeAsync(0);
  watcher.open();
  await vi.advanceTimersByTimeAsync(0);
  expect(host.reopen).toHaveBeenCalledExactlyOnceWith({ flowId: flow.flow_id });
  watcher.stop();
  complete({ url: callback });
  await vi.advanceTimersByTimeAsync(0);
...
it.each([
  callback.replace('test-state', 'wrong'),
  callback.replace('53692', '1455'),
  `${callback}&code=other`,
])('revalidates native returns before contacting the paired gateway', async (url) => {
  host.authorize.mockResolvedValue({ url });
  const client = transport();
  const verdict = vi.fn();
  const watcher = watchAuth(clientAuthFlow(flow, remote), client, verdict);
  await vi.advanceTimersByTimeAsync(0);
...
it('never binds for device/API-key flows or non-loopback redirects', () => {
  for (const other of [
    { ...flow, kind: 'device' as const },
    { ...flow, kind: 'api-key' as const },
...
it('fails closed before opening when the authorization URL substitutes a different redirect', async () => {
  ...
  url.searchParams.set('redirect_uri', 'http://10.0.0.5/callback');
  const client = transport();
  const verdict = vi.fn();
  const watcher = watchAuth(
    clientAuthFlow({ ...flow, url: url.toString() }, remote),
    client,
    verdict,
  );
  await vi.advanceTimersByTimeAsync(0);
...
it('expires a native browser independently of a stalled native callback', async () => {
  ...
  const verdict = vi.fn();
  const watcher = watchAuth(
    clientAuthFlow({ ...flow, expires_at: Date.now() + 100 }, remote),
    client,
    verdict,
  );
...
it('closes the actual provider UI and refreshes the same remote gateway after a native return', async () => {
  ...
  const client = {
    ...
    cancelProviderAuth: vi.fn().mockResolvedValue(undefined),
  };
  ...
  try {
    ...
    expect(client.completeProviderAuth).toHaveBeenCalledExactlyOnceWith(
      provider.id,
      flow.flow_id,
      callback,
    );
