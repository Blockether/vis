...
describe('best device voices', () => {
  it('keeps a short quality-ranked list in the device languages', () => {
    ...
    expect(chosen.map((voice) => voice.id)).toEqual(['premium-local', 'premium-web', 'enhanced']);
  ...
  it('keeps the stored voice reachable without exceeding the limit', () => {
    ...
    expect(chosen).toHaveLength(BEST_DEVICE_VOICE_LIMIT);
    expect(chosen.map((voice) => voice.id)).toEqual(['premium-local', 'premium-web', 'compact']);
  });
  ...
  it('offers every public Premium and Enhanced Apple voice on iOS', () => {
    const chosen = bestDeviceVoices(
      [
        ...
        {
          id: 'com.apple.voice.premium.en-GB.Serena',
          label: 'Serena (Premium)',
          language: 'en-GB',
          quality: 500,
        },
        {
          id: 'com.apple.voice.enhanced.en-IE.Moira',
          label: 'Moira (Enhanced)',
          language: 'en-IE',
          quality: 450,
        },
        {
          id: 'com.apple.voice.premium.en-US.Samantha',
          label: 'Samantha (Premium)',
          language: 'en-US',
          quality: 500,
        },
        {
        ...
        {
          id: 'com.apple.voice.premium.fr-FR.Amelie',
          label: 'Amélie (Premium)',
          language: 'fr-FR',
          quality: 500,
        },
      ],
