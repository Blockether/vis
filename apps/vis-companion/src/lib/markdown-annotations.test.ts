...
import { parseAnnotated, quoteOf, renderAnnotated, QUOTE_LIMIT } from './markdown-annotations';
...
describe('markdown annotations', () => {
  ...
  it('drops the section again when the last comment is removed', () => {
    ...
    expect(renderAnnotated('Body.', [{ quote: 'Body.', body: '  ' }])).toBe('Body.\n');
  });

  it('keeps a comment on one line', () => {
    const saved = renderAnnotated('Body.', [{ quote: 'a\nb', body: 'one\ntwo' }]);
    expect(saved).toContain('- **“a b”** — one two');
    expect(parseAnnotated(saved).comments).toEqual([{ quote: 'a b', body: 'one two' }]);
  });
