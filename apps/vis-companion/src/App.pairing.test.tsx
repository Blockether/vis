...
describe('gateway pairing deeplinks', () => {
  ...
  it.each([
    ...
    ['a normalized URL without a token', 'https://GATEWAY.example.com:443/', undefined],
    ['a known alternative', LAN, 'stale-token'],
  ])('reuses saved settings when reopening %s', async (_name, url, token) => {
    await act(async () => {
      view = renderApp({
        machines: [{ url: DOMAIN, label: 'My server', pinned: true, alts: [LAN] }],
      });
    });
    const before = saved();
  ...
  it('recognizes the original link after the saved address has changed', async () => {
    await act(async () => {
      view = renderApp({
        machines: [{ url: TAILNET, label: 'My server', pinned: true, alts: [DOMAIN] }],
      });
    });
    const before = saved();
    await act(async () => {
  ...
  it('still fails over when the domain stops answering', async () => {
    ...
    await waitFor(() => {
      expect(saved()).toEqual([expect.objectContaining({ url: TAILNET, token: 't' })]);
    });
  });
});
