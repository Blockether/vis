...
import activityResults from '../../../../packages/vis-contract/resources/vis-contract/fixtures/activity-results.json';
import activityExecution from '../../../../packages/vis-contract/resources/vis-contract/fixtures/activity-execution.json';
...
export const ACTIVITY_INTERLEAVED = projection(
  activityGroupingCases.find(
    (sample) => sample.name === 'ten interleaved reads and searches with failed and running shell calls',
  )!.projection,
);
...
/** Retained builtin results generated by the engine and shared with the TUI. */
export const ACTIVITY_RESULTS = projection(activityResults);
export const ACTIVITY_EXECUTION = projection(activityExecution);

/** Repeated result tables with unequal content widths, shared with the TUI. */
export const ACTIVITY_TABLES = projection(activityTables);
...
export const ACTIVITY_LONG_RUNNING = projection({
  ...
  rows: Array.from({ length: 7 }, (_, index) => ({
    ...
    presentation: {
      headline: index === 6 ? 'Searching' : 'Searched',
      summary: `search-${index}`,
      content: index === 6 ? [] : [{ type: 'text', text: `result-${index}` }],
    },
    resources: [],
    evidence: [],
  })),
...
export const ACTIVITY_ALL_GROUPS = projection({
  ...
  rows: [
    ['grep', 'Searched', 'ActivityPanel', '3 matches'],
    ['cat', 'Read file', 'src/components/ActivityPanel.tsx', 'Read 80 lines'],
    ['ls', 'Listed files', 'src/components', '12 files'],
    ['patch', 'Patched file', 'src/components/ActivityPanel.tsx', 'Removed the group limit'],
    ['doc', 'Read documentation', 'activity', 'Read the Activity guide'],
    ['attach', 'Attached document', 'review.md', 'Attached 1 document'],
    ['shell', 'Ran command', 'npm run build', 'Build completed'],
  ].map(([operation, headline, summary, text], sequence) => ({
...
export const LOG_TEXT = [
  ...
  '12:04:12.902 tool     shell npm test -> 285 passed',
...
export const STORY_COMPACT_EXECUTIONS: TranscriptIteration[] = [
  {
    ...
    forms: [
      ...
      {
        source: "checks = await shell('npm test')\nprint(checks)",
        activity: ACTIVITY_RUNNING,
      },
    ],
  },
