...
export const EstimatedReuse: Story = {
  play: async ({ canvas }) => {
    ...
    await expect(canvas.getByText(/Estimated share of reusable prior input/)).toBeVisible();
  },
};
...
export const HealthInteractions: Story = {
  ...SessionHealth,
  play: async ({ canvas }) => {
    await expect(canvas.getByRole('meter', { name: 'Context budget' })).toHaveAttribute(
      'value',
      '0.6901',
    );
    ...
    await expect(input.getBoundingClientRect().bottom).toBeLessThanOrEqual(window.innerHeight);
    await userEvent.click(parts);
    await userEvent.click(roots);
  },
};
...
export const EstimateDrift: Story = {
  ...
  play: async ({ canvas }) => {
    await userEvent.click(canvas.getByRole('button', { name: /Context breakdown/ }));
    await expect(canvas.getByText('+54,773 tokens (+33.8%)')).toBeVisible();
    await expect(canvas.getByText('81%')).toBeVisible();
    await expect(canvas.getByRole('meter')).toHaveAttribute('value', '0.810885');
...
export const PreparedRequest: Story = {
  ...
  play: async ({ canvas }) => {
    await userEvent.click(canvas.getByRole('button', { name: /Context breakdown/ }));
    await expect(canvas.getByText('Prepared request · not measured usage')).toBeVisible();
