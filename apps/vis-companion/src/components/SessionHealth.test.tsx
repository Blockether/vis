...
import { STORY_SESSION_HEALTH, STORY_HEALTH_USAGE, STORY_SESSION_ROW } from '../dev/story-data';
...
describe('session health in metrics', () => {
  ...
  it.each(healthParity.cases)('renders backend metrics: $name', async ({ health, expected }) => {
    render(
      <SessionStatsPanel
        session={STORY_SESSION_ROW}
        usage={{
    ...
    if (expected.percent !== null) {
      ...
      expect(screen.getByRole('meter')).toHaveAttribute('value', String(health!.budget_used_ratio));
    ...
    if (expected.projection) {
      ...
      expect(screen.getByText(`${expected.projection} · not measured usage`)).toBeInTheDocument();
      ...
      expect(screen.getByText(new RegExp(expected.scope!))).toBeInTheDocument();
      expect(screen.getByText(/including cached input/)).toBeInTheDocument();
    } else {
      expect(screen.queryByRole('button', { name: /Context breakdown/ })).not.toBeInTheDocument();
    }
    if (health?.stale) expect(screen.getByText(/Earlier measurement/)).toBeInTheDocument();
  });
  ...
  it('renders supplied metrics rather than reconstructing them in the client', async () => {
    ...
    expect(screen.getByText('7 available · 6 with guidance estimates')).toBeInTheDocument();
    await userEvent.click(screen.getByRole('button', { name: /Context breakdown/ }));
  ...
  it.each([
    [false, 3, '91%'],
    [true, 0, '≈91%'],
  ] as const)('renders the supplied cache estimate flag %s', (estimated, samples, expected) => {
    render(
      <SessionStatsPanel
        session={STORY_SESSION_ROW}
        phase="ready"
  ...
  it('opens recorded prompt parts and distinguishes filesystem access from loaded guidance', async () => {
    ...
    expect(screen.getByText(/Disk estimates do not add to context usage/)).toBeInTheDocument();
    await userEvent.click(screen.getByRole('button', { name: /Context breakdown/ }));
  ...
  it('does not turn an unrecorded guidance read into a negative claim', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /Linked filesystems/ }));
    expect(screen.getByText('Guidance estimate unavailable')).toBeInTheDocument();
    expect(screen.queryByText('Instructions not loaded')).not.toBeInTheDocument();
  });
});
