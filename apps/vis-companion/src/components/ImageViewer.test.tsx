...
describe('ImageViewer', () => {
  ...
  it("titles itself with the app's one dialog band", () => {
    ...
    // The close inherits the band's ink without a separate frame in either palette.
    const close = control('Close chart.png');
    ...
    expect(close.querySelector('span')).toBeNull();
  ...
  it('mounts the shared annotation layer, inert until Draw is pressed', () => {
    ...
    expect(document.querySelector('[aria-label="Draw on image"]')).toBeNull();
    expect(document.querySelector('canvas')?.getAttribute('data-annotation')).toBe('active');
...
it('draws a stroke that starts beside the picture and crosses onto it', () => {
  Element.prototype.setPointerCapture = vi.fn();
  act(() => control('Draw on image').click());
  const viewport = document.querySelector<HTMLDivElement>('[role="dialog"] .cursor-grab');
  const canvas = document.querySelector('canvas');
...
describe('a gallery in the viewer', () => {
  ...
  const shownSrc = () => document.querySelector('[role="dialog"] img')?.getAttribute('src');
  const press = (key: string) =>
    act(() => {
      window.dispatchEvent(new KeyboardEvent('keydown', { key }));
    });
  const stage = () => document.querySelector<HTMLElement>('[role="dialog"] .transform-gpu');
  const finger = (type: string, x: number, y = 300) => {
    const glass = document.querySelector<HTMLDivElement>('[role="dialog"] .cursor-grab');
  ...
  it('does not swipe a picture that is alone', () => {
    ...
    expect(document.querySelector('[aria-live="polite"]')?.textContent).not.toContain('of 1');
  });
});
...
describe('the grid is the gallery', () => {
  ...
  it('does not step an editable picture', () => {
    ...
    expect(document.querySelector('[aria-live="polite"]')?.textContent).not.toContain('of 2');
    act(() => {
      window.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowRight' }));
    });
    expect(document.querySelector('[role="dialog"] h2')?.textContent).toBe('edit-0.png');
  });
});
