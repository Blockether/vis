...
describe('browser-flow lifecycle in the shared Companion UI', () => {
  ...
  it('cancels an active gateway flow on unmount', async () => {
    ...
    const { result, unmount } = renderHook(() =>
      useProviderAuth(client as unknown as GatewayClient),
    );
    await act(async () => result.current.signIn(provider));
    unmount();
  ...
  it('cancels a superseded start without opening its browser', async () => {
    ...
    act(() => {
      first = result.current.signIn(provider);
    });
    await act(async () => result.current.signIn(provider));
    await act(async () => {
      response.resolve({ ...flow, flow_id: 'old-flow' });
      await first;
    });
  ...
  it('does not restart polling after a cancelled manual completion fails', async () => {
    ...
    await act(async () => result.current.signIn(provider));
    act(() =>
      result.current.setRedirectUrl('http://localhost:1455/auth/callback?code=test&state=test'),
    );
    let completion!: Promise<void>;
    act(() => {
      completion = result.current.finishPkce();
    });
    await act(async () => result.current.cancelFlow());
    await act(async () => {
      response.resolve({ status: 'error', message: 'Rejected' });
      await completion;
    });
    await act(async () => {
      await vi.advanceTimersByTimeAsync(5000);
  ...
  it.each(['url', 'verification_uri'] as const)(
    'opens and polls a remote device flow from %s',
    async (urlField) => {
      vi.useFakeTimers();
      const opened = vi.spyOn(window, 'open').mockReturnValue(null);
      const url = 'https://gateway.example.com/device';
      const started: AuthFlow = {
        flow_id: 'device-flow',
        provider_id: provider.id,
        kind: 'device',
        user_code: 'ABCD-EFGH',
