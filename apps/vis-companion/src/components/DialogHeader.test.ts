...
describe('every dialog header is the one dialog header', () => {
  it('leaves no surface painting its own title bar', () => {
    ...
    expect(definition, 'ui.tsx should define the band exactly once').toHaveLength(1);
  ...
  it('leaves no surface building its own way out', () => {
    for (const [name, source] of SURFACES) {
      ...
      expect(source, `${name} hand-builds a close button`).not.toContain(
        'border-l border-dialog-title-foreground/20 text-dialog-title-foreground/70',
      );
    }
  });

  it('gives the band the same height as the list headers on the same screen', () => {
    expect(ui).toContain('flex min-h-12 shrink-0 items-stretch bg-dialog-title');
  ...
  it('wraps a question instead of eating it', () => {
    expect(ui).toContain('line-clamp-3 font-mono text-body font-bold tracking-wide');
  });

  it('routes every close through DialogClose', () => {
    expect(ui).toContain('<DialogClose label={closeLabel ?? \'Close\'} tone="title"');
  });
});
