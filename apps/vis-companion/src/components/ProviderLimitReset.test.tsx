...
it.each<ProviderResetOutcome>(['reset', 'nothing_to_reset', 'no_credit', 'already_redeemed'])(
  'reports %s and prevents double submission while pending',
  async (outcome) => {
...
it('refuses an account switch after confirmation opened', () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Reset limits…' }));
  view.rerender(
    <ProviderLimitReset {...props} credits={{ ...credits, account_id: 'account-2' }} />,
  );
...
it('refreshes missing reset data from the row action without consuming a reset', async () => {
  ...
  const provider: RouterProvider = {
    ...
    limits: { status: 'ok', dynamic: { limits: [] } },
  };
...
it('crosses the real rows, hook and HTTP client; refreshes quotas and retries a lost response after reopening', async () => {
  ...
  const limits = () => ({
    status: 'ok',
    dynamic: {
      reset_credits: { ...credits, available_count: count },
      limits: [{ label: 'Weekly', used_percent: 100 }],
    },
  });
  vi.stubGlobal(
    'fetch',
    vi.fn(async (url, init) => {
      ...
      if (path.endsWith('/consume')) {
        ...
        return new Response(
          JSON.stringify(
            calls.filter((call) => call.body).length === 1
              ? { error: 'reset-unconfirmed' }
              : { outcome: 'already_redeemed' },
          ),
          { status: calls.filter((call) => call.body).length === 1 ? 502 : 200 },
        );
      }
      if (path === '/v1/router')
        return new Response(JSON.stringify({ providers: [{ ...provider, limits: limits() }] }));
      if (path.endsWith('/status'))
        return new Response(JSON.stringify({ status: provider.status }));
      if (path.endsWith('/limits'))
        return new Response(JSON.stringify({ report: limits() }), {
...
it.runIf(!!process.env.VIS_CODEX_RESET_E2E_URL)(
  'end to end through the real gateway and simulated Codex',
  async () => {
    const client = new GatewayClient({
      id: 'reset-e2e',
      url: process.env.VIS_CODEX_RESET_E2E_URL!,
      token: 'test',
    } as GatewayConn);
    vi.spyOn(client, 'router').mockImplementation(async () => [
      {
        ...
        is_fallback: false,
        fallback_model: null,
        status: await client.providerStatus('openai-codex'),
        limits: await client.providerLimits('openai-codex'),
      },
    ]);
...
it.each([false, true])(
  'updates mounted limits views after reset even if status fails: %s',
  async (failStatus) => {
    let used = 100;
    const provider: RouterProvider = {
      id: 'openai-codex',
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url) => {
        ...
        if (path.endsWith('/status'))
          return new Response(JSON.stringify({ status: provider.status }), {
            status: failStatus && used === 0 ? 503 : 200,
          });
    ...
    const connection = {
      id: 'reset-live',
      url: `https://reset-live-${failStatus}.example.com`,
      token: 'test',
    } as GatewayConn;
    ...
    function OtherLimits() {
      ...
      return (
        <output data-testid="other-quota">
          {providers?.[0]?.limits?.dynamic?.limits?.[0]?.used}
        </output>
      );
    }
    render(
      <>
        <ConnectedRows client={client} />
        <OtherLimits />
      </>,
    );
