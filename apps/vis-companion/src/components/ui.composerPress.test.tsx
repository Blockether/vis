...
describe('the send button under a finger', () => {
  it('sends on a tap that never becomes a click', async () => {
    ...
    const submitTurn = vi.fn(() => Promise.resolve({ turn_id: 'sent-by-tap', status: 'running' }));
    ...
    expect(
      await within(screen.getByRole('region', { name: 'Transcript' })).findByText('run the tests'),
    ).toBeInTheDocument();
