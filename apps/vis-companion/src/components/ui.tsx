...
export function Button({
  ...
  return (
    <button
      className={`min-h-11 rounded-[2px] border px-3.5 py-2 text-xs font-semibold transition-[background-color,border-color,color,transform] duration-150 active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/60 disabled:opacity-40 disabled:active:scale-100 motion-reduce:transition-none sm:min-h-9 ${styles} ${className}`}
      {...props}
    />
  );
}
