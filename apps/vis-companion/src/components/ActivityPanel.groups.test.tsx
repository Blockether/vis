...
it("labels extension groups and preserves disclosures, arguments and live failures", () => {
  ...
  expect(
    screen.getByRole("button", { name: /Check review deployment ×2/ }),
  ).toBeTruthy();
  expect(
    screen.getByText(/Closed reviews · Review service unavailable/),
  ).toBeTruthy();
  ...
  expect(
    group.querySelector("[data-activity-arguments]")
      ?.getAttribute("data-activity-arguments"),
  ).toBe("search-1");
  ...
  expect(
    screen.getByRole("button", { name: /Search reviews ×3/ })
      .getAttribute("aria-expanded"),
  ).toBe("true");
  expect(
    within(group).getByRole("button", { name: /Changes: 0 ×2/ })
      .getAttribute("aria-expanded"),
  ).toBe("true");
});
...
it("keeps an extension disclosure open when its fallback gains a headline", () => {
  ...
  missing.rows.forEach((row) => { delete row.presentation; });
  ...
  expect(
    screen.getByRole("button", { name: /Search open reviews ×4/ })
      .getAttribute("aria-expanded"),
  ).toBe("true");
