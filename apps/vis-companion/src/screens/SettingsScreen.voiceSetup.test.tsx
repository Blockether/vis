...
function mount(answer = catalogue) {
  const client = {
    ...
    importSpeechVoice: vi.fn().mockResolvedValue({ id: 'my-voice', label: 'My voice' }),
  };
  const view = render(
    <VoicesPanel client={client as unknown as GatewayClient} prefs={prefs} onChange={vi.fn()} />,
  );
  return { ...view, client };
}
...
function recorder(): WavRecording {
  return {
    stop: vi.fn().mockResolvedValue(new Blob(['recorded speech'], { type: 'audio/wav' })),
...
describe('creating a voice from a recording or a file', () => {
  ...
  it('offers recording and file import as separate actions', async () => {
    mount();
    expect(await screen.findByRole('button', { name: 'Record your voice' })).toBeEnabled();
  ...
  it('records locally and only uploads after confirmation', async () => {
    ...
    const { client } = mount();
    fireEvent.click(await screen.findByRole('button', { name: 'Record your voice' }));
    ...
    await waitFor(() => expect(client.importSpeechVoice).toHaveBeenCalledOnce());
    ...
    expect(await screen.findByText('My voice can speak on this machine now.')).toBeVisible();
  });
  ...
  it('offers a retry or import after microphone permission is denied', async () => {
    ...
    expect(await screen.findByText(/Microphone permission denied/)).toBeVisible();
    expect(screen.getByRole('button', { name: 'Record your voice' })).toBeEnabled();
    expect(screen.getByRole('button', { name: 'Import voice' })).toBeEnabled();
  });
  ...
  it('recovers when capture is interrupted', async () => {
    ...
    await screen.findByRole('button', { name: 'Stop recording' });
    act(() =>
      vi.mocked(startWavRecording).mock.calls[0][0]?.onInterrupted?.('Microphone disconnected'),
    );
    expect(take.cancel).toHaveBeenCalledOnce();
  ...
  it('keeps the file and name after a failed save so it can be retried', async () => {
    const { client } = mount();
    client.importSpeechVoice.mockRejectedValueOnce(new Error('Machine disconnected'));
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Save voice' }));
    expect(screen.getByRole('button', { name: 'Saving voice…' })).toBeDisabled();
  ...
  it('recovers without offering a silent recording for upload', async () => {
    const take = recorder();
    vi.mocked(take.stop).mockRejectedValue(new Error('Microphone captured only silence'));
    vi.mocked(startWavRecording).mockResolvedValue(take);
    const { client } = mount();
    fireEvent.click(await screen.findByRole('button', { name: 'Record your voice' }));
    fireEvent.click(await screen.findByRole('button', { name: 'Stop recording' }));
    expect(await screen.findByText(/Microphone captured only silence/)).toBeVisible();
    expect(screen.getByRole('button', { name: 'Record your voice' })).toBeEnabled();
