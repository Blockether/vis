...
function setup(names: string[] = []) {
  const submitTurn = vi.fn(
    (_sid: string, _request: string, _options: unknown) =>
      new Promise(() => {}),
  );
...
describe("file references in submitted requests", () => {
  ...
  it.each([false, true])(
    "preserves expanded paste contents when queued=%s",
    (queued) => {
      const { composer, submitTurn } = setup();
      if (queued) {
  ...
  it.each([query, ""])(
    "keeps uploaded attachments with request %s",
    async (text) => {
      const { composer, submitTurn } = setup();
  ...
  it.each([
    ...
    "inserts a visible file reference for %s selected via %s",
    async (path, method) => {
      ...
      await waitFor(() =>
        expect(composer.selectionStart).toBe(before.length + reference.length),
      );
