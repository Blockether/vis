...
it("updates the header from a goal event without another HTTP request", async () => {
  ...
  renderSessionScreen({ session: row, subscriptions: hub, client: { session: read, noteSessionGoal: note } });
  await screen.findByRole("button", { name: /^Goal: Active/ });
  const before = read.mock.calls.length;
  act(() => hub.emit({ type: "session.goal_updated", goal: { ...STORY_GOAL, revision: 4, status: "complete" } } as never));
  await waitFor(() => expect(screen.getByRole("button", { name: /^Goal: Complete/ })).toBeInTheDocument());
...
it("refreshes an idle goal from the canonical reconnect snapshot", async () => {
  ...
  await screen.findByRole("button", { name: /^Goal: Active/ });
  act(() => hub.emit({ type: "subscription.ready", is_live: false, goal: { ...STORY_GOAL, revision: 4, status: "paused" } } as never));
  await waitFor(() => expect(screen.getByRole("button", { name: /^Goal: Paused/ })).toBeInTheDocument());
  expect(note).toHaveBeenCalledOnce();
});
