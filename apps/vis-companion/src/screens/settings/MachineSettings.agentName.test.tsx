...
const setting = { id: "agent_name", label: "Agent name", type: "string" as const, value: "Ada", max_length: 80 };
beforeEach(() => {
...
async function open() {
  render(<MachineSettings gateway={gateway} speechPrefs={DEFAULT_SPEECH_PREFS}
...
it("saves explicitly to this gateway and adopts its normalized response", async () => {
  ...
  await waitFor(() => expect(screen.getByRole("textbox", { name: "Agent name" })).toHaveValue("Grace"));
...
it("cancels a draft, rejects a blank name locally and retries a failed write", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "Save" }));
  await waitFor(() => expect(screen.queryByText("Gateway could not save the name; retry.")).toBeNull());
