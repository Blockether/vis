...
/** Let every promise, timer and repaint that fits inside `ms` happen. */
...
it('keeps asking while the signed-in server is still connecting, until it lands', async () => {
  // The window the component waits between questions (MCP_SETTLE_POLL_MS). On a fake
  // clock the test spends none of it: a wrong window shows up as a missing question.
  const pollWindow = 1500;
  vi.useFakeTimers();
  try {
