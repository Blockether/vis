(ns com.blockether.vis.tui.screen-test
...
(defdescribe lanterna-input-ownership-test
             (it "keeps terminal decoding, coalescing, geometry, and tty modes out of Vis"
                 (let [source (->> ["dialogs" "input" "screen" "scroll"]
                                   (map #(str "com/blockether/vis/tui/" % ".clj"))
                                   (map io/resource)
                                   (map slurp)
                                   (str/join "\\n"))]
...
(defdescribe live-view-wheel-physics-test
             (it "steps a tall pane by the shared notch and a compact one by one row"
                 (with-redefs-fn {(ns-resolve 'com.blockether.vis.tui.screen 'live-band-pane)
                                  (fn [_db _my]
  ...
  (it "routes minimize and restore clicks through pane state only"
      (let [events (atom [])]
        ...
        (expect (= [[:live-view-minimize "view-1"] [:bump-render-version]
                   ...
                   @events))))
  (it "routes a clicked row through the gateway View action"
      (let [called
            (promise)
...
(defdescribe
  ...
  (it
    "fetches the gateway slash catalog asynchronously"
    (let [previous
