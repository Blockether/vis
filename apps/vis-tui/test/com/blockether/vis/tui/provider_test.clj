(ns com.blockether.vis.tui.provider-test
...
(defn- band-stub
  ...
  [answers log]
  {:read! (fn read! ([label] (read! label {})) ([label opts] (swap! log conj [:read label opts])
                                                (get answers label)))
   :choose! (fn [title choices]
              (swap! log conj [:choose title choices])
              (get answers title))
   :confirm!
   (fn confirm! ([question] (confirm! question nil)) ([question opts] (swap! log conj [:confirm
                                                                                       question
                                                                                       opts])
...
(defdescribe provider-dialog-namespace-test
             (it "loads the provider dialog namespace"
                 (expect (some? (find-ns 'com.blockether.vis.tui.provider)))))

(defdescribe provider-dialog-save-through-gateway-test
             (it "hands one preset id and endpoint to the daemon-owned provider route"
                 (let [saved
                       (atom nil)

                       item
                       {:id :ollama :base-url "http://localhost:11434" :models [{:name "qwen"}]}]
...
(defdescribe configured-provider-status-test
             (it "routes configured provider status through the gateway"
                 (let [config-path "/tmp/vis-state.yml"]
             ...
             (it "routes local no-auth provider status through the gateway"
                 (with-redefs [vis/gateway-provider-status (fn [provider-id]
                                                             {"is_authenticated" true
                                                              "source" "gateway"
                                                              "provider_id" (name provider-id)})]
...
(defdescribe
  ...
  (it "anthropic success closes the band silently (parity with copilot/codex)"
      (let [log (atom [])]
        (with-redefs [vis/gateway-provider-status (constantly {"is_authenticated" false})
                      vis/gateway-provider-auth-start!
                      (constantly {"flow_id" "flow-9" "url" "https://claude.ai/oauth"})
                      vis/gateway-provider-auth-complete! (constantly {"status" "ok"})
                      opener/open! (constantly true)
                      dlg/text-view-dialog! (fn [& _]
                                              (throw (ex-info "success opened a dialog" {})))]
  ...
  (it "an api-key provider succeeds silently through the gateway"
      (let [log (atom [])]
        (with-redefs [vis/gateway-provider-auth-start!
                      (constantly
                        {"flow_id" "flow-1" "kind" "api-key" "instructions" ["Paste your key."]})
...
(defdescribe provider-transient-spec-test
             (it "groups a provider's actions by what they touch, keeping their keys"
                 (let [spec (provider/provider-transient-spec
                   ...
                   (expect (= [["Set as Default..." "Set as Fallback..."]
                               ["Authenticate" "Remove Provider"]]
  ...
  ([^long cols ^long rows keystrokes]
   ...
   (with-redefs [vis/load-config
                 (constantly {:providers [{:id :acme-llm
                                           :models [{:name "acme-1"} {:name "acme-2"}
                                                    {:name "acme-3"}]}]
                 ...
                 (constantly [{:id :acme-llm
                               :models [{:name "acme-1"} {:name "acme-2"} {:name "acme-3"}]}])
...
(defdescribe settings-provider-transient-keeps-settings-visible-test
             (it "paints the provider band inside the Settings frame"
                 (let [frames
...
(defn- settings-add-provider-frames
  ...
  (let [saved (atom nil)]
    (with-redefs-fn {#'vis/provider-presets
                     ...
                     #'provider/save-provider-config! (fn [config]
                                                        (reset! saved config)
                                                        true)}
      (fn []
...
(defdescribe
  ...
  (it
    "runs preset, endpoint and model as BANDS inside the settings frame"
    (let [{:keys [frames saved]}
...
(defdescribe
  managed-provider-key-refusal-test
  ...
  (it "offers no Authenticate verb on a managed provider's row"
      (let [ids (fn [provider]
        ...
        (expect (not (contains? (ids {:id :corp :is-managed true}) :authenticate)))
        (expect (contains? (ids {:id :other}) :authenticate))))
  (it
    "says the credential is issued for you instead of reading a key"
    (let [note (atom nil)]
      (with-redefs [dlg/host-band-region (fn [_screen region]
                                           region)
                    dlg/band-questions
