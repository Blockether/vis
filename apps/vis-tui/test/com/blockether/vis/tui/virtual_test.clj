(ns com.blockether.vis.tui.virtual-test
  ...
  (:require [com.blockether.vis.tui.render :as render]
...
(def ^:private settings {:show-thinking true :show-iterations true :show-timestamps false})

;; ─── Tests ──────────────────────────────────────────────────────────────────

(defdescribe
  estimated-height-test
  (describe "estimated-height never returns negative or zero"
            (it "user msg >= 1" (expect (>= (estimated-height (user-msg "hi") bubble-w) 1)))
            (it "empty user msg >= 1" (expect (>= (estimated-height (user-msg "") bubble-w) 1)))
  ...
  (describe
    ...
    (it "folds a huge streaming result behind its execution disclosure"
        ...
        (let [huge-result
          ...
          (expect (re-find #"CODE  \+\d+ more ▸" (:text projected)))
          (expect (re-find #"CODE[^\n]*▸" (:text projected)))
...
(defdescribe
  sticky-height-cache-test
  (describe
    "once a message has been measured, layout returns its REAL height forever"
    ...
    (it "layout pins the real height of any message it measures"
        (virtual/invalidate-heights!)
        (render/invalidate-cache!)
        (let [msgs [(trace-assistant-msg 3 2 "first") (trace-assistant-msg 3 2 "second")]]
    ...
    (it "mid-scroll trace assistants render the trace instead of an empty Markdown window"
        ...
        (let [msgs
          ...
          (expect (nil? (-> trace-row
                            :projected
                            :lines-window)))
          (expect (pos? (count (-> trace-row
                                   :projected
                                   :prewrapped-lines))))))
...
(defdescribe
  ...
  (describe
    "user messages strip timestamps and render markdown"
...
(defdescribe turn-identity-test
             ...
             (describe
               ...
               (it "prefers :client-turn-id when present (live and done)"
                   ...
                   (expect (= "abc12345"
                              (turn-identity {:client-turn-id "abc12345"
...
(defn- source-form
  ...
  [marker]
  (let [src
        (slurp (io/resource "com/blockether/vis/tui/virtual.clj"))
...
(defn- compile-warnings
  ...
  (let [shadow
    ...
    (try (binding [*err*
           ...
           (alias 'vis 'com.blockether.vis.tui.client)
           (alias 'render 'com.blockether.vis.tui.render)
           (eval form))
         (str sw)
