...
(defn wire-canonical
  ...
  (cond (map? value) (into {}
                           (map (fn [[k v]]
                                  [(if (keyword? k) (str/replace (name k) "-" "_") k)
                                   (wire-canonical v)]))
                           value)
        (vector? value) (mapv wire-canonical value)
  ...
  (wire-canonical (mapv (fn [turn]
                          (-> turn
                              (assoc :turn-id (:id turn)
                                     ...
                                     :iterations (vec (db-list-session-turn-iterations :db
                                                                                       (:id turn))))
                              (dissoc :id :user-request)))
                        (db-list-session-turns :db sid))))
...
(defdescribe
  ...
  (it "render-answer projects canonical blocks"
      (expect (= "raw **markdown**"
                 ((var-get (resolve 'com.blockether.vis.tui.chat/render-answer))
...
(defdescribe
  ...
  (let [g->c @#'chat/gateway-event->chunk]
    (it
      "projects all canonical terminal event variants"
      (expect
        (= [{:phase :turn-terminal
             :turn-id "t1"
             :client-id "c1"
    ...
    (let [g->c
          ...
          (g->c (wire-canonical
                  {:type "iteration.error"
                   ...
                   :provider-error-data {:kind :gateway-unavailable
                                         :title "Provider unavailable"
                                         :explanation "WHAT HAPPENED: the provider is unavailable."
                                         :next-step "NEXT STEP: retry."
                                         :facts [["HTTP" "503"]]}}))]

      (expect (= :iteration-error (:phase chunk)))
      (expect (= {:type :svar.core/http-error
                  :message "upstream reset"
                  :status 503
                  :provider-error-info {:kind :gateway-unavailable
                                        :title "Provider unavailable"
                                        :explanation "WHAT HAPPENED: the provider is unavailable."
                                        :next-step "NEXT STEP: retry."
                                        :facts [["HTTP" "503"]]}}
