(ns com.blockether.vis.tui.code-block-test
...
;; edge when the code block carries `:wrap?`. A fence whose source already
;; carries ANSI folds SGR-aware too, so a wide colored line stops overflowing.

(def ^:private code-block->lines @#'layout/code-block->lines)

(def ^:private folded-code-cache @#'layout/folded-code-cache)
...
(defdescribe
  ...
  (it "ansi-char-folds a wide pre-colored fence value to the budget"
      ;; A fence whose source already carries ANSI (zprint evidence, colored tool
      ;; output) folds at the bubble edge, SGR-aware, so its tail stays visible
      ...
      (let [strip-ansi
            ...
            wide
            (str "\u001b[32m" (apply str (repeat 100 "ab")) "\u001b[0m")
        ...
        (expect (= (strip-ansi wide) (apply str rows)))))
...
(defdescribe tui-code-fold-cache-test
             (it "keeps CAT source rows verbatim now that nothing highlights them"
                 (let [src
                       "   9  (def x 1)\n1200  (inc x)\n   ⋯\n1300  (str x)"
             ...
             (it "reuses a completed ANSI fold at the same width"
                 (let [src
                       (str "\u001b[32m901  (def cache-proof 1)\u001b[0m\n"
                            "\u001b[32m902  (inc cache-proof)\u001b[0m")
...
(defdescribe
  ...
  (it "colours removals red and additions green"
      (let [rows (content-rows (code-block->lines
                                 [:code {:lang "diff"}
                                  "@@ -4,3 +4,3 @@\n keep\n-old value\n+new value\n tail"]
                                 80
                                 {}))]
