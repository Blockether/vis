(ns com.blockether.vis.tui.terminal-image-test
...
(defdescribe kitty-transmit-place-test
             ;; Transmit-once + placement is the flicker/scroll fix: upload the PNG ONCE (a=t),
             ;; then re-place it (a=p) every frame instead of re-transmitting + deleting all.
             (it "transmit-only is an a=t upload with an image id, no display"
             ...
             (it "a full placement references the image id and cell box, no source crop"
                 (expect (= "\u001b_Ga=p,i=5,p=17,C=1,q=2,c=10,r=6\u001b\\"
                            (timg/kitty-place {:id 5 :placement-id 17 :cols 10 :rows 6}))))
             (it "a cropped placement carries the source rectangle (same math as encode-kitty)"
                 ...
                 (expect (= "\u001b_Ga=p,i=5,p=17,C=1,q=2,c=10,r=3,x=0,y=40,w=100,h=60\u001b\\"
                            (timg/kitty-place {:id 5
                                               :placement-id 17
                                               :cols 10
                                               :rows 6
                                               :crop-top 2
             ...
             (it "delete-placement keeps data (lowercase d=i); free-image frees it (d=I)"
                 (expect (= "\u001b_Ga=d,d=i,i=5,p=17,q=2\u001b\\"
                            (timg/kitty-delete-placement 5 17)))
