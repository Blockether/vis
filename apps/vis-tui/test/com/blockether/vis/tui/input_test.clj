(ns com.blockether.vis.tui.input-test
...
(defdescribe
  ...
  (it "C-x C-g aborts the prefix instead of running a C-x verb"
      (let [state
        ...
        ;; A PLAIN second key is still the verb: C-x h toggles help.
...
(defdescribe
  ...
  (it "bare Tab, reverse Tab and indexed chords never change sessions"
      (let [state
        ...
        (expect (= unchanged (input/handle-key (special-key KeyType/Tab) state)))
        (expect (= unchanged (input/handle-key (special-key KeyType/ReverseTab) state)))
        ...
        (expect (= {:action :toggle-all-details :state state}
                   (input/handle-key (special-key KeyType/Tab) armed)))))
...
(defdescribe file-mention-expand-test
             (it "replaces inline @mentions via the file-expander helper"
                 (with-redefs [com.blockether.vis.tui.input/file-mention->prompt-block
                               (fn [path]
                                 (str "<FILE:" path ">"))]
             ...
             (it "expands local @mentions to a tool-agnostic attached-file directive"
                 ...
                 (let [file (java.io.File/createTempFile "vis-input-large" ".clj")]
                   (spit file (str/join "\n" (repeat 121 "x")))
                   (try (with-redefs [com.blockether.vis.tui.input/resolve-local-file (fn [_path]
                                                                                        file)]
