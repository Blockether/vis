...
(defn- command-submission?
  "True when `text` has the engine's command token shape: one leading slash,
   no slash inside the root token. Absolute paths remain ordinary prompts.
...
(defn init!
  "Initialize app-db from app-local preferences and the gateway toggle projection."
  []
  (let [local-settings
...
;; Synthesis belongs to the gateway; only playback touches this TUI process.
(reg-fx :speak-reply
        (fn [text]
          (try (voice-output/speak! (:active-tab-id @app-db) text)
               (catch Throwable t
