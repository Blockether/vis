...
(defn geometry
  "Use 40–56 cells on the left; overlay if chat would fall below 60 cells."
  [db cols rows]
  (when (get-in db [:project-sidebar :open?])
    (let [cols
          ...
          width
          (min cols (max 40 (min 56 (quot cols 3))))
...
(defn paint!
  ...
  [g db cols rows]
  (binding [interactions/hit-map hit-map]
    (.beginFrame hit-map)
    (when-let [{:keys [left width]} (geometry db cols rows)]
      (let [left (long left)
        ...
        (doseq [[index project] (visible-projects sidebar rows)
                :let [row (+ 2 (- (long index) (long (ffirst (visible-projects sidebar rows)))))
          ...
          (.register interactions/hit-map
                     {:kind :project-select
                      :project project
                      :bounds {:col left :row row :width (dec width) :height 1}}))
