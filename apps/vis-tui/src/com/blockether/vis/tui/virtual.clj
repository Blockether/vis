(ns com.blockether.vis.tui.virtual
  ...
  (:require [clojure.string :as str]
            ...
            [com.blockether.vis.tui.presentation :as ast])
...
(defn- message-content-fingerprint
  ...
  (locking message-fp-memo
    (or (.get ^IdentityHashMap message-fp-memo message)
        (let [fp (hash (dissoc message :text :prewrapped-lines :line-meta :turn-separator?))]
          (when (> (long (.size ^IdentityHashMap message-fp-memo)) 4096)
            (.clear ^IdentityHashMap message-fp-memo))
          (.put ^IdentityHashMap message-fp-memo message fp)
          fp))))
...
(defn- height-key
  ...
  (let [sfp
    ...
    (locking height-key-memo
      (let [cached (.get ^IdentityHashMap height-key-memo message)]
        (if (and cached
          ...
          (let [k [(message-content-fingerprint message) (long bubble-w) sfp dek]]
            (when (> (long (.size ^IdentityHashMap height-key-memo)) 4096)
              (.clear ^IdentityHashMap height-key-memo))
            (.put ^IdentityHashMap height-key-memo message k)
            k))))))
...
(defn- height-cache-get
  ...
  (locking height-cache
    (.get ^LinkedHashMap height-cache
...
(defn- height-cache-put!
  ...
  (locking height-cache
    (.put ^LinkedHashMap height-cache
          (height-key message bubble-w settings detail-expansions session-id)
          h))
  h)
...
(defn height-cache-size
  ...
  ^long []
  (locking height-cache (.size ^LinkedHashMap height-cache)))
   ...
   (let [content-w
     ...
     (cond
       ...
       (let [n-iter
             ...
             (long (reduce (fn [^long acc it]
                             (+ acc
                                (long (let [r (prose-rows-est (or (:assistant-prose it)
                                                                  (when (empty? (:forms it))
                                                                    (:content-stream it)))
                                                              fold-w)]
                                        (if (pos? r) (+ r 2) 0)))))
                    ...
                    (cond
                      ...
                      (and (vector? prev-offsets)
                           ...
                           (not= (- (long (nth est-off (inc lo))) (long (nth est-off lo)))
                                 (- (long (nth prev-offsets (inc lo)))
                                    (long (nth prev-offsets lo)))))
                      lo
                  ...
                  (loop [i lo
                    ...
                    (if (> i hi)
                      ...
                      (recur (unchecked-inc i)
                             (if (contains? projected-by-idx i) out (conj! out i)))))
                  [])
                              ...
                              (catch Throwable t
                                ...
                                (try (require 'taoensso.telemere)
                                     ((resolve 'taoensso.telemere/log!)
                                       :warn
                                       (str "vis-tui pre-warm threw: " (or (ex-message t) (str t))))
