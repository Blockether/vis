...
(defn- it->iteration-entry
  ...
  (let [iter-code
        ...
        [(cond-> {:position 0 :code iter-code}
           ...
           (some? (get it "error"))
           (assoc :error (hydrate-error (get it "error"))))]
        ...
        (fn [idx env]
          (let [src
            ...
            (cond-> (merge (vis/form<-wire env)
              ...
              (contains? env "error")
              (assoc :error (hydrate-error (get env "error")))
  ...
  (let [type
    ...
    (case type
      ...
      {:phase :iteration-error
       :iteration iteration
       :thinking thinking
       :error (hydrate-error (or (wire-error-map (event-get event :error-data)) error)
                             (event-get event :provider-error-data))}
