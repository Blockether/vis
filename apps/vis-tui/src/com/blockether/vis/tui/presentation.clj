(ns com.blockether.vis.tui.presentation
  "Transient Markdown parsing and renderer projections.

   Canonical answers are role-labelled, string-keyed content blocks from
   `com.blockether.vis.tui.content`. Parsed Markdown trees are created only
   inside renderers and are never transported or persisted.

   Renderer projections are disposable; none of their intermediate trees are canonical message data."
...
(defn- path-like-code-literal?
  "A file-path-looking code literal (`src/foo/bar.clj`, `~/x/y.edn`): no
   ...
   stay INLINE chips however long — grep/patch op-cards title every
