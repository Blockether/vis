(ns com.blockether.vis.tui.markdown-layout
  "TUI layout walker over a transient parsed Markdown tree (`com.blockether.vis.tui.presentation/->ast`).
...
(def ^:private ^java.util.Map folded-code-cache
  "Bounded access-order cache for the ANSI-fold stage. Layout used to split and
   rescan every ANSI row on each projection; keeping the width-dependent string
   segments avoids that O(lines) pass."
...
(defn- compute-folded-code-lines
  [^String content ^long budget]
  (->> (str/split-lines content)
       (mapcat #(p/ansi-fold-cols % budget))
       vec))

(defn- folded-code-lines
  "Rows of `content` char-folded to `budget`, re-opening the SGR active at each cut
...
(defn- code-block->lines
  ...
  (let [src
        ...
        ;; `ir/diff-line-kind` is the SHARED diff-line classifier, so a
        ;; diff fence colours consistently wherever it is rendered.
        kind->sgr
        ...
        diff-line
        (fn [^String line]
          ;; A diff fence is COMPACT: the unified patch line itself, coloured by
          ;; its own kind, char-folded to the bubble width. A one-sided change
          ;; also tints the row's BACKGROUND band via `:meta`.
          (let [kind (ir/diff-line-kind line)]
            (mapv (fn [seg]
                    (cond-> {:runs (if (= "" seg)
                                     []
                                     [{:text (ansi-kind seg kind) :style #{:code} :node node}])}
                      (#{:add :del} kind)
                      (assoc :meta {:diff-kind kind})))
                  (if (= "" line) [""] (p/fold-cols line budget)))))
...
(defn- heading->lines
  ...
  (let [level
        ...
        (when rule-char
          (if (= 1 level)
            (long width)
            (min (long width) (long (reduce max 0 (map line-width ls))))))
...
(defn- run->sentinel-segment
  ...
  (let [text
    ...
    (if (or code? ital? bold? link?)
      (let [sb (StringBuilder.)]
        (when link? (.append sb ^String p/INLINE_LINK_ON))
        (when bold? (.append sb ^String p/INLINE_BOLD_ON))
        (when ital? (.append sb ^String p/INLINE_ITALIC_ON))
        (when code? (.append sb ^String p/INLINE_CODE_ON))
        (.append sb text)
        (when code? (.append sb ^String p/INLINE_CODE_OFF))
        (when ital? (.append sb ^String p/INLINE_ITALIC_OFF))
        (when bold? (.append sb ^String p/INLINE_BOLD_OFF))
        (when link? (.append sb ^String p/INLINE_LINK_OFF))
        (.toString sb))
      text)))
