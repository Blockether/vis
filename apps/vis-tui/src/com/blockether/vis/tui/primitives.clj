...
(defn blit-line!
  ...
  (let [mempty?
        ...
        (.get ^java.util.Map line-cell-cache k)]

    (if cached
      (.putString g (int x) (int y) cached)
      (let [^java.util.EnumSet effective-mods
            (if mempty? no-mods mods)

            ^"[Lcom.googlecode.lanterna.TextCharacter;" arr
            (TextCharacter/fromString line fg bg effective-mods)]
        ...
        (.put ^java.util.Map line-cell-cache
              (if mempty? k [line fg bg (java.util.EnumSet/copyOf mods)])
              arr)
        ...
        (fn [^String seg]
          (when (pos? (.length seg))
            (let [^"[Lcom.googlecode.lanterna.TextCharacter;" arr
                  (if (aget code? 0)
                    (TextCharacter/fromString seg code-fg code-bg ^java.util.EnumSet no-mods)
                    (let [^java.util.EnumSet eff (java.util.EnumSet/copyOf inherited)]
                      (.addAll eff inline)
