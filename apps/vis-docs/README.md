...
and `/api/*` is its API.
JavaScript enhances filtering, navigation, searching, copying commands and repository submission.
...
The manual's header search loads `assets/search.json` — one row per page section, generated by the same
renderer with anchors that resolve in both outputs — and filters it in the browser with `assets/search.js`.
Documentation search is a static same-origin fetch: no Worker request, database query or third-party service.
