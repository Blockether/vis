...
beforeEach(async () => {
  ...
  fixture.controls.repository = null;
...
function post(path, source = {}, headers = {}) {
  ...
  return fixture.runtime.dispatchFetch('https://center.example.com' + path, {
    ...
    headers: {
      ...
      'CF-Connecting-IP': '10.0.0.' + ((serial % 250) + 1),
      ...headers,
    },
    body: JSON.stringify({
      repository_url: 'https://github.com/example/extensions',
...
test('docs and catalog share an origin, with documentation independent of D1', async () => {
  ...
  expect(await docs.text()).toContain('Extending Vis');
  ...
  const offline = await worker.fetch(
    new Request('https://center.example.com/'),
    { ASSETS: { fetch: () => new Response('docs without database') } },
    { waitUntil() {} },
  );
...
test('catalog failure still renders the documentation shell with recovery', async () => {
  const response = await worker.fetch(
    new Request('https://center.example.com/extensions/'),
    {},
    { waitUntil() {} },
  );
  expect(response.status).toBe(503);
  expect(await response.text()).toContain('Could not load the catalog');
});
test('root and monorepo previews use pinned GitHub metadata only', async () => {
  for (const subdirectory of ['', 'plugins/greeting']) {
    const response = await post('/api/preview', { subdirectory });
    const data = await response.json();
    expect(response.status, JSON.stringify(data)).toBe(200);
    expect(data.subdirectory).toBe(subdirectory);
    expect(data.revision).toBe(fixture.revision);
    expect(data.name).toBe('vis-greeter');
    expect(data.manifest_url).toContain('/blob/' + fixture.revision + '/');
  }
  expect(
    fixture.controls.requests
...
test('pending submissions and their refreshes cannot publish or replace a public listing', async () => {
  const source = { revision: fixture.revision };
  const response = await post('/api/submissions', source);
  ...
  expect(listing.name).toBe('vis-greeter');
  const added = listing.added_at;
  await post('/api/submissions', source);
  expect((await fixture.db.prepare('SELECT COUNT(*) AS n FROM extensions').first()).n).toBe(1);
  expect((await fixture.db.prepare('SELECT added_at FROM extensions').first()).added_at).toBe(
    added,
  );
  const html = await (
    await fixture.runtime.dispatchFetch(
...
test('SSR supports search, categories, sort, views and executable-free metadata', async () => {
  ...
  expect(html.match(/class="extension-card"/g)).toHaveLength(1);
  expect(html).toContain('data-name="example/local-models"');
  const malicious = { ...items[0], description: '</script><b>untrusted metadata</b>' };
  await fixture.db
    .prepare('UPDATE extensions SET metadata=? WHERE id=?')
    .bind(JSON.stringify(malicious), malicious.id)
    .run();
  await fixture.runtime.purgeCache();
  const detail = await (
    await fixture.runtime.dispatchFetch(
...
test('public catalog uses a shared cache key and HEAD returns no body', async () => {
  ...
  const item = JSON.parse(readFileSync('web/catalog.fixture.json', 'utf8'))[0];
  await fixture.db
    .prepare('INSERT INTO extensions VALUES (?, ?, ?)')
    .bind(item.id, JSON.stringify(item), item.added_at)
    .run();
  const html = await (
    await fixture.runtime.dispatchFetch('https://center.example.com/extensions/?q=github')
  ).text();
  expect(html).toContain('No repositories yet');
  const head = await fixture.runtime.dispatchFetch('https://center.example.com/extensions/', {
    method: 'HEAD',
  });
...
test('Turnstile rejects missing, reused, wrong-action and wrong-host tokens before GitHub', async () => {
  ...
  expect((await post('/api/preview')).status).toBe(403);
  fixture.controls.verification = 'ok';
  expect((await post('/api/preview', { turnstile_token: 'preview-once' })).status).toBe(200);
  fixture.controls.requests = [];
  expect((await post('/api/preview', { turnstile_token: 'preview-once' })).status).toBe(403);
...
test('writes reject cross-origin requests, unsupported input and unpinned confirmations', async () => {
  expect((await post('/api/preview', {}, { Origin: 'https://other.example.com' })).status).toBe(
    403,
  );
...
test('per-IP rate limit prevents excessive inspection requests', async () => {
  ...
  for (let n = 0; n < 11; n++)
    statuses.push(
      (await post('/api/preview', { turnstile_token: '' }, { 'CF-Connecting-IP': '10.1.0.5' }))
        .status,
    );
  expect(statuses.slice(0, 10)).toEqual(Array(10).fill(403));
  expect(statuses[10]).toBe(429);
});
test.each(['redirect', 'private', 'missing', 'rate'])(
  'GitHub %s is actionable and never creates a submission',
  async (mode) => {
    fixture.controls.github = mode;
    const response = await post('/api/preview');
    expect(response.status).toBe(mode === 'rate' ? 503 : 400);
    expect((await response.json()).error).toBeTruthy();
    expect(fixture.controls.requests.every((url) => !url.includes('other.example.com'))).toBe(true);
  },
);
test.each([
  ...
  'both review steps reject %s without saving a submission',
  async (_name, kind, value, message) => {
    if (kind === 'manifest')
      fixture.controls.manifest =
        value === '[project' ? value : fixture.controls.manifest.replace(value, '');
    else if (kind === 'append')
      fixture.controls.manifest = fixture.controls.manifest.replace(
        'category = "tools"',
        value.startsWith('category') ? value : 'category = "tools"\n' + value,
      );
    else {
...
test('SDK source and skill directories are checked again at the reviewed commit on submission', async () => {
  fixture.controls.manifest = readFileSync(
    '../../packages/vis-agent/examples/greeter/pyproject.toml',
    'utf8',
  );
  fixture.controls.contents.set('plugins/greeting/skills/greeting', [
    { name: 'SKILL.md', type: 'file' },
  ]);
  const response = await post('/api/preview');
  expect(response.status).toBe(200);
  const preview = await response.json();
  expect(preview.skills).toEqual(['skills/greeting']);
  fixture.controls.requests = [];
  expect((await post('/api/submissions', { revision: preview.revision })).status).toBe(202);
  for (const folder of ['src', 'skills/greeting'])
    expect(fixture.controls.requests).toContain(
      'https://api.github.com/repos/example/extensions/contents/plugins/greeting/' +
        folder +
        '?ref=' +
        preview.revision,
    );
  expect(fixture.controls.requests).toContain(
    'https://api.github.com/repos/example/extensions/commits/' + preview.revision,
  );
});
test('catalog display metadata accepts the SDK example, including bundled skills', () => {
  const metadata = manifestMetadata(
    readFileSync('../../packages/vis-agent/examples/greeter/pyproject.toml', 'utf8'),
  );
...
test('portable display metadata agrees with the shipped manifest and rejects unsafe paths', () => {
  ...
  expect(metadata.category).toBe('tools');
  expect(metadata.dependencies.some((d) => d.startsWith('vis-agent'))).toBe(true);
  for (const path of ['../secret', '/tmp', 'x/../y', '.env', 'x\\y'])
    expect(() => projectFolder(path)).toThrow();
  for (const url of [
    'http://github.com/a/b',
  ...
  expect(repositoryURL('https://github.com/example/repo.git/')).toBe(
...
test('live discovery includes only approved listings and shares the cached catalog snapshot', async () => {
  ...
  const missing = await fixture.runtime.dispatchFetch(
    'https://center.example.com/extensions/' + 'f'.repeat(24),
  );
...
test('every approved extension has a crawlable, server-rendered detail page', async () => {
  ...
  for (const item of items)
    await fixture.db
      ...
      .run();
  const index = await (
    await fixture.runtime.dispatchFetch('https://center.example.com/extensions/sitemap.xml')
  ).text();
  const catalog = new JSDOM(
    await (await fixture.runtime.dispatchFetch('https://center.example.com/extensions/')).text(),
  );
  try {
    for (const item of items) {
      ...
      try {
        ...
        expect(d.querySelector('#install-command').textContent).toContain(
        ...
        expect(
          JSON.parse(d.querySelector('script[type="application/ld+json"]').textContent).mainEntity
            .codeRepository,
        ).toBe(item.repository_url);
      } finally {
        detail.window.close();
      }
    }
...
test('discovery fails explicitly rather than publishing an empty catalog when D1 is unavailable', async () => {
  for (const path of ['/extensions/sitemap.xml', '/extensions/llms.txt']) {
    const response = await worker.fetch(
      new Request('https://center.example.com' + path),
      {},
      { waitUntil() {} },
    );
    expect(response.status).toBe(503);
