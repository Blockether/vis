...
export function moderationStatements(action, id) {
  if (action === 'list-comments')
    return [
      "SELECT id, extension_id, name, body, created_at FROM comments WHERE status='pending' ORDER BY id",
    ];
  if (['approve-comment', 'reject-comment'].includes(action)) {
    if (!/^[1-9][0-9]{0,15}$/.test(id || '') || !Number.isSafeInteger(Number(id)))
      throw new Error('Use the full numeric comment reference.');
    return [
      `UPDATE comments SET status='${action === 'approve-comment' ? 'approved' : 'rejected'}' WHERE id=${id}`,
    ];
  }
  if (action === 'list-sync')
    return [
      'SELECT extension_id,page,position,checked_at,error FROM release_sync ORDER BY checked_at',
    ];
  if (action === 'list')
    return [
      "SELECT id, extension_id, revision, submitted_at, json_extract(metadata, '$.repository_url') AS repository, json_extract(metadata, '$.subdirectory') AS folder, json_extract(metadata, '$.version') AS version, json_extract(metadata, '$.release_tag') AS tag FROM submissions ORDER BY submitted_at",
    ];
  if (!['approve', 'reject'].includes(action) || !/^[0-9a-f]{24}$/.test(id || ''))
    throw new Error(
      'Use list, approve ID or reject ID. ID must be the full 24-character submission reference.',
    );
...
export async function publicationStatements(metadata) {
  ...
  if (!/^[0-9a-f]{24}$/.test(metadata.id) || !/^[0-9a-f]{40}$/.test(metadata.revision))
    throw new Error('Inspect a pinned release before publication.');
  const id = await identity(metadata.id + '\n' + metadata.revision),
    quote = (value) => "'" + String(value).replaceAll("'", "''") + "'";
  const now = new Date().toISOString(),
    body = JSON.stringify({ ...metadata, checked_at: now });
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const { values, positionals } = parseArgs({
    options: { remote: { type: 'boolean' }, config: { type: 'string' } },
  ...
  if (action === 'publish') {
    ...
    if (args.length !== 4 || !/^[0-9a-f]{40}$/.test(revision))
      throw new Error(
        'Use publish REPOSITORY FOLDER RELEASE_TAG FULL_SHA after reviewing that source.',
      );
    const metadata = await inspectRepository(
      { repository_url, subdirectory, release_tag, revision },
      process.env,
    );
