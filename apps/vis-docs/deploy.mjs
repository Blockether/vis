...
function docsOrigin(hostname) {
  if (
    ...
    throw new Error('Configure DOCS_HOSTNAME as a public hostname without scheme, port or path');
...
export function deploymentConfig(base, env) {
  ...
  if (
    !/^[0-9a-f]{8}(-[0-9a-f]{4}){3}-[0-9a-f]{12}$/.test(database || '') ||
    /^0{8}-/.test(database)
  )
    throw new Error('Configure DOCS_D1_DATABASE_ID');
  if (
    !/^[A-Za-z0-9_-]{20,100}$/.test(siteKey || '') ||
    siteKey.startsWith('1x000') ||
    siteKey.startsWith('2x000') ||
    siteKey.startsWith('3x000')
  )
    throw new Error('Configure a production DOCS_TURNSTILE_SITE_KEY');
  if (!/^[1-9][0-9]{0,8}$/.test(namespace || ''))
    throw new Error('Configure DOCS_RATE_LIMIT_NAMESPACE');
  const hostname = docsOrigin(env.DOCS_HOSTNAME).hostname;
  return {
    ...base,
    workers_dev: false,
    routes: [{ pattern: hostname, custom_domain: true }],
    d1_databases: [{ ...base.d1_databases[0], database_id: database }],
    ratelimits: [{ ...base.ratelimits[0], namespace_id: namespace }],
    vars: { TURNSTILE_SITE_KEY: siteKey },
  };
}
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    try {
      for (const [path, marker] of [
        ...
        ['/python-sdk-api/blockether/vis/engine.html', 'id="Agent"'],
        ['/python-sdk-api/search.js', 'pdocSearch'],
        ['/assets/theme.css', ':root'],
        ['/assets/prism.min.js', 'Prism'],
        ['/robots.txt', 'Sitemap:'],
        ['/sitemap.xml', '<sitemapindex'],
        ['/sitemap-docs.xml', '<urlset'],
        ['/sitemap-python-sdk.xml', '<urlset'],
        ...
        if (
          ...
          (path === '/' && content.includes('id="catalog-data"'))
        )
          throw new Error('Public site is not ready');
      }
      for (const path of [
        '/favicon.ico',
        '/vis-icon.ico',
        ...
        if (!response.ok || !signature.every((byte, index) => bytes[index] === byte))
          throw new Error('Site icons are not ready');
      }
      const api = await fetch(new URL('/api/extensions', origin), {
        signal: AbortSignal.timeout(20000),
        redirect: 'error',
      });
      if (!api.ok || !Array.isArray((await api.json()).extensions))
        throw new Error('Catalog is not ready');
      await verifyHTTPS(origin, [
        '/',
        '/extensions/',
        '/python-sdk-api/blockether/vis/engine.html',
        '/assets/theme.css',
        '/api/extensions',
      ...
      if (attempt === 7)
        throw new Error(
          'Deployed docs, assets, catalog and HTTPS redirect verification failed after 8 attempts',
        );
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  if (process.argv[2] === '--verify') {
    ...
    writeFileSync(
      ...
      JSON.stringify(deploymentConfig(base, process.env), null, 2),
    );
  }
}
