...
export function mount(container,request=fetch,initial) {
  ...
  form.oninput=()=>{++submissionRevision;$('#review-submit').disabled=false;};
  form.onsubmit=event=>{event.preventDefault();submit(false);};$('#submit-confirm').onclick=()=>submit(true);
  ...
  const keys=event=>{
    ...
    if(mobile.matches&&navigation.checked) {
      if(event.key==='Escape') {event.preventDefault();closeNavigation();navigation.focus();return;}
      if(event.key==='Tab') {const links=[navigation,...container.querySelectorAll('.top a,.side a')];if(document.activeElement===(event.shiftKey?links[0]:links.at(-1))) {event.preventDefault();(event.shiftKey?links.at(-1):links[0]).focus();}}
    }
    if($('#catalog-page').hidden) return;
    const editable=['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName);
    if(event.key==='/'&&!editable) {event.preventDefault();closeNavigation();$('#search').focus();}
    if(event.key==='Escape'&&document.activeElement===$('#search')) {clearFilters();$('#search').blur();}
    if(['ArrowDown','ArrowUp'].includes(event.key)&&(!editable||document.activeElement===$('#search'))) {
      const links=[...container.querySelectorAll('.card-main')],index=links.indexOf(document.activeElement),next=links[Math.max(0,Math.min(links.length-1,index+(event.key==='ArrowDown'?1:-1)))];if(next) {event.preventDefault();next.focus();}
    }
  };
