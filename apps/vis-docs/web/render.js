...
const repositorySelector = (item) =>
  `${quote(extensionName(item))}${item.subdirectory ? ' --subdirectory ' + quote(item.subdirectory) : ''}`;
function installLines(item, scope, render = (text) => text) {
  return [
    render('vis-agent extension install', 'function'),
    render(quote(extensionName(item)), 'string'),
    ...(item.subdirectory
      ? [render('--subdirectory', 'keyword') + ' ' + render(quote(item.subdirectory), 'string')]
      : []),
    render('--version', 'keyword') + ' ' + render(quote(item.version), 'string'),
    render(scope === 'global' ? '--global' : '--project', 'keyword'),
    render('--trust', 'keyword'),
  ].join(' \\\n  ');
}
export function installCommand(item, scope = 'project') {
  return installLines(item, scope);
}
export function installCommandHTML(item, scope = 'project') {
  return installLines(
    item,
    scope,
    (text, kind) => `<span class="token ${kind}">${escapeHTML(text)}</span>`,
  );
}
export function installScopeHelp(scope = 'project') {
  return (
    (scope === 'global'
