...
import { catalogMetadata, extensionIcon, extensionName } from './discovery.js';
...
export function visibleItems(items,state) {
  ...
  return items.filter(p=>(state.category==='all'||p.category===state.category)&&[p.name,p.description,p.owner,p.repository,...p.dependencies,...p.topics].join(' ').toLowerCase().includes(term)).sort((a,b)=>(state.sort==='stars'?b.stars-a.stars:state.sort==='name'?0:Date.parse(b[state.sort==='newest'?'added_at':'updated_at'])-Date.parse(a[state.sort==='newest'?'added_at':'updated_at']))||extensionName(a).localeCompare(extensionName(b))||a.id.localeCompare(b.id));
}
...
export function cardsHTML(items,state) {
  ...
  return visible.map(item=>`<article class="extension-card" data-name="${escapeHTML(extensionName(item))}"><a class="card-main" href="/extensions/${escapeHTML(item.id)}${filterURL(state).slice('/extensions/'.length)}"><div class="card-top"><span class="tag">${escapeHTML(categories[item.category])}</span><span class="version">v${escapeHTML(item.version)}</span></div><h3>${escapeHTML(extensionName(item))}</h3><p class="card-description">${escapeHTML(item.description)}</p></a>${link(item.subdirectory||item.repository,item.subdirectory?item.source_url:item.repository_url,'repository-link')}<div class="card-meta"><span>${escapeHTML(new Intl.NumberFormat('en',{notation:'compact',maximumFractionDigits:1}).format(item.stars))} stars</span><span>Updated ${dateLabel(item.updated_at)}</span></div></article>`).join('');
}
...
export function releaseHTML(item) {
  ...
  return `<form id="version-form" class="version-picker" method="get" action="/extensions/${escapeHTML(item.id)}"><label for="release-version">Version</label><select id="release-version" name="version" aria-describedby="version-help">${releases.map(release=>`<option value="${escapeHTML(release.version)}"${release.version===item.version?' selected':''}>${escapeHTML(release.version)}${release.prerelease?' · Prerelease':release.version===item.latest_version?' · Latest approved':''}</option>`).join('')}</select><button type="submit">View release</button></form><p id="version-help" class="help">Only approved releases are listed. Each version keeps its reviewed commit.${item.latest_version&&item.version!==item.latest_version?' You are viewing a different version from the catalog default.':''}</p><details class="release-history"><summary>Approved releases (${releases.length})</summary><ol>${releases.map(release=>`<li><a data-release href="/extensions/${escapeHTML(item.id)}?version=${encodeURIComponent(release.version)}">${escapeHTML(release.version)}</a>${release.prerelease?' <span class="tag">Prerelease</span>':''}<span class="help">${dateLabel(release.release_published_at||release.approved_at)}</span>${link(release.revision.slice(0,12),release.repository_url+'/commit/'+release.revision)}${release.release_url?link('Release notes',release.release_url):''}</li>`).join('')}</ol></details>`;
}
