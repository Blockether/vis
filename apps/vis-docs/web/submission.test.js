...
const request = (path, options = {}) =>
  fixture.runtime.dispatchFetch('https://center.example.com' + path, {
    ...
    headers: {
      ...
      'CF-Connecting-IP': '10.9.0.' + address,
    },
  });
...
beforeEach(async () => {
  ...
  window.turnstile = {
    render: (_node, options) => {
      options.callback(
        (options.action === 'extension-preview'
          ? 'preview'
          : options.action === 'extension-submit'
            ? 'submit'
            : options.action) +
          '-' +
          ++serial,
...
test.each(['', 'plugins/greeting'])(
  'browser review, submission, approval and catalog agree for folder "%s"',
  async (subdirectory) => {
    await start();
    review(subdirectory);
    ...
    expect(
      (
        await fixture.runtime.dispatchFetch(
          'https://center.example.com/extensions/' + pending.extension_id,
        )
      ).status,
    ).toBe(404);
    ...
    const listing = await (
      await fixture.runtime.dispatchFetch(
        'https://center.example.com/api/extensions/' + pending.extension_id,
      )
    ).json();
    expect(listing.subdirectory).toBe(subdirectory);
    expect(listing.revision).toBe(fixture.revision);
    dispose();
    ...
    const confirmFeedback = async () => {
      await vi.waitFor(() =>
        expect($('[data-check-status]').textContent).toContain('then confirm'),
      );
      $('[data-feedback-confirm]').click();
    };
    $('[data-rating] [data-vote="1"]').click();
    ...
    commentForm.elements.name.value = 'Local reviewer';
    commentForm.elements.body.value = 'Clear setup instructions.';
    commentForm.requestSubmit();
    await confirmFeedback();
    await vi.waitFor(() =>
      expect($('[data-feedback-status]').textContent).toContain('Submitted for moderation.'),
    );
    expect($('.feedback-comment')).toBeNull();
    const comment = await fixture.db
      .prepare('SELECT id FROM comments WHERE extension_id=?')
      .bind(listing.id)
      .first();
    for (const sql of moderationStatements('approve-comment', String(comment.id)))
      await fixture.db.prepare(sql).run();
    dispose();
    ...
    $('.feedback-comment [data-vote="1"]').click();
    await confirmFeedback();
    await vi.waitFor(() =>
      expect($('.feedback-comment [data-vote="1"]').getAttribute('aria-pressed')).toBe('true'),
    );
    expect(
      (
        await fixture.db
          .prepare('SELECT COUNT(*) AS n FROM comment_votes WHERE comment_id=?')
          .bind(comment.id)
          .first()
      ).n,
    ).toBe(1);
