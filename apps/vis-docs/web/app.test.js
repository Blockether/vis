...
function setup(
  request = vi.fn(async (path) => ({
    ok: true,
    json: async () => (path === '/api/extensions' ? { extensions: fixtures } : item),
  })),
  touch = false,
) {
  ...
  window.turnstile = {
    render: vi.fn((node, options) => {
      options.callback('fixture-' + options.action);
      return 'widget';
    }),
    remove: vi.fn(),
  };
...
test.each(['grid', 'list'])(
  'the catalog has no manual layout controls or state, including with view=%s in the URL',
  async (view) => {
...
test('catalog CSS uses a two-column grid on wide screens and border-separated rows below it', () => {
  ...
  expect(
    rules
      .find((rule) => rule.selectorText === '#results')
      .style.getPropertyValue('grid-template-columns'),
  ).toBe('repeat(2, minmax(0, 1fr))');
  const compact = rules.find((rule) => rule.conditionText === '(max-width: 1200px)');
  expect(compact).toBeDefined();
  const compactStyle = (selector) =>
    [...compact.cssRules].find((rule) => rule.selectorText === selector).style;
  ...
  expect(
    ['top', 'right', 'bottom', 'left'].map((side) =>
      compactStyle('.extension-card').getPropertyValue('border-' + side + '-width'),
    ),
...
test('release controls retain pointer and touch targets without replacing native selects', () => {
  ...
  document.body.append(style);
  const rules = [...style.sheet.cssRules],
    touch = [...rules.find((rule) => rule.conditionText === '(pointer: coarse)').cssRules];
  for (const [selector, property, value] of [
    ['.version-picker select', 'height', '2.75rem'],
    ['.release-history a', 'min-height', '1.75rem'],
    ['.install-section summary', 'min-height', '1.75rem'],
  ])
    expect(
      rules.find((rule) => rule.selectorText === selector).style.getPropertyValue(property),
    ).toBe(value);
  for (const selector of ['.release-history a', '.install-section summary'])
    expect(
      touch.find((rule) => rule.selectorText === selector).style.getPropertyValue('min-height'),
    ).toBe('2.75rem');
});
test('extension detail spacing overrides prose margins without changing README typography', () => {
  document.body.innerHTML = shellHTML({
    item: { ...item, releases: [item], latest_version: item.version },
  });
  const style = document.createElement('style');
  style.textContent =
...
test('untrusted metadata remains text and unsafe links are not rendered', async () => {
  const malicious = {
    ...
    readme_url: 'javascript:alert(1)',
  };
  setup(async (path) => ({
    ok: true,
    json: async () => (path === '/api/extensions' ? { extensions: [malicious] } : malicious),
  }));
  await tick();
  $('.card-main').click();
  await tick();
...
test('loading, failure, retry and empty catalog have recovery actions', async () => {
  ...
  expect(
    [...document.querySelectorAll('#catalog-page button')].filter(
      (button) => button.textContent === 'Add a repository',
    ),
  ).toHaveLength(1);
  $('#submit-open').click();
...
test('submission previews a GitHub link and pins the reviewed revision when adding', async () => {
  ...
  expect(JSON.parse(options.body)).toEqual({
    ...
    turnstile_token: 'fixture-extension-preview',
  });
  ...
  const submission = request.mock.calls.find(
    ([path, opts]) => path === '/api/submissions' && opts?.method === 'POST',
  );
...
test('editing or closing a submission invalidates late preview results', async () => {
  ...
  setup((path) =>
    path === '/api/extensions'
      ? Promise.resolve({ ok: true, json: async () => ({ extensions: fixtures }) })
      : new Promise((r) => {
          resolve = r;
        }),
  );
  ...
  $('#submit-close').click();
  resolve({ ok: true, json: async () => item });
  await tick();
  expect($('#submit-confirm').hidden).toBe(true);
  expect($('#preview').textContent).toBe('');
  $('#submit-open').click();
  expect($('#review-submit').disabled).toBe(false);
});
...
test('the catalog uses the documentation stylesheet and three-column page shell', async () => {
  ...
  expect(readFileSync('dist/assets/theme.css')).toEqual(
...
test('repository scrolling remains scoped after visiting feedback on a detail page', async () => {
  ...
  await tick();
  $('#back-to-catalog').click();
  const feedbackBody = $('.feedback-dialog .dialog-body');
  feedbackBody.scrollTop = 77;
  $('#submit-open').click();
  const repositoryBody = $('#submit-dialog .dialog-body');
  repositoryBody.scrollTop = 200;
  $('#submit-close').click();
  expect(repositoryBody.scrollTop).toBe(0);
  expect(feedbackBody.scrollTop).toBe(77);
});
...
test('version selection changes the pinned detail, is linkable, and survives back navigation', async () => {
  const latest = {
      ...
      source_url: item.repository_url + '/tree/' + 'b'.repeat(40),
    },
    older = { ...item, version: '1.0.0' };
  const releases = [latest, older].map((release) => ({
    ...release,
    release_tag: 'v' + release.version,
    release_url: item.repository_url + '/releases/tag/v' + release.version,
  }));
  const request = vi.fn(async (path) => ({
    ...
    json: async () =>
      path === '/api/extensions'
        ? { extensions: [latest] }
        : {
            ...(path.includes('version=1.0.0') ? older : latest),
...
test('repository confirmation pins the selected release tag as well as its SHA', async () => {
  const request = vi.fn(async (path) => ({
    ...
    json: async () =>
      path === '/api/extensions'
        ? { extensions: [] }
        : { ...item, source_paths: [], release_tag: 'vis-greeter/v1.0.0' },
  }));
  setup(request);
  await tick();
  $('#submit-open').click();
  change('#repository-url', item.repository_url);
  change('#release-tag', 'vis-greeter/v1.0.0');
  $('#repository-form').dispatchEvent(
