...
export function metadataHead({title, description, path, type='WebPage', noindex=false, markdown}) {
  ...
  return `<title data-discovery>${escape(title)}</title>`+
    ...
    (markdown?`<link data-discovery rel="alternate" type="text/markdown" title="Markdown" href="${escape(markdown)}">`:'')+
    meta('og:type',type==='TechArticle'?'article':'website',true)+meta('og:site_name','Vis',true)+
    ...
    `<script data-discovery type="application/ld+json">${json({'@context':'https://schema.org','@type':type,name:title,description,url,inLanguage:'en',isPartOf:{'@type':'WebSite',name:'Vis',url:origin+'/'}})}</script>`;
}
export function catalogMetadata({item, error, detailError}={}) {
  return metadataHead({title:(item?.name||'Extension Center')+' · Vis',
    description:item?.description||'Public GitHub extensions for Vis. Tools, providers and workflows.',
...
export function sitemap(paths, index=false) {
  ...
  return `<?xml version="1.0" encoding="UTF-8"?><${index?'sitemapindex':'urlset'} xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">`+
