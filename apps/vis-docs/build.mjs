...
execFileSync('clojure', ['-M', '-e', `(require '[com.blockether.vis.internal.docs.core :as docs] '[charred.api :as json]) (docs/build-site! ${JSON.stringify(fileURLToPath(dist))} {:public? true}) (spit ${JSON.stringify(fileURLToPath(new URL('source.json', dist)))} (json/write-json-str (docs/collect)))`], {cwd: fileURLToPath(new URL('../../', import.meta.url)), stdio: 'inherit'});
...
for (const page of pages) {
  ...
  link.innerHTML=extensionIcon;link.title='Extension Center';link.setAttribute('aria-label','Extension Center');
  await writeFile(file, dom.serialize());
  dom.window.close();
  await writeFile(new URL(page.slug+'.md', dist), page.md);
}
await writeFile(new URL('sitemap.xml', dist), sitemap(['/sitemap-docs.xml','/extensions/sitemap.xml'], true));
await writeFile(new URL('sitemap-docs.xml', dist), sitemap(pages.map(page=>page.slug==='index'?'/':'/'+page.slug+'.html')));
await writeFile(new URL('robots.txt', dist), `User-agent: *\nAllow: /\nDisallow: /api/\n\nSitemap: ${origin}/sitemap.xml\n`);
await writeFile(new URL('llms.txt', dist), `# Vis\n\n> ${site.tagline}\n\n## Documentation\n\n`+
