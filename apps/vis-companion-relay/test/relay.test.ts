...
async function mint(env: Env, deps: Deps, body: Record<string, unknown> = {}): Promise<string> {
  const response = await handle(
    post('/v1/grants', { device_token: APPLE_TOKEN, platform: 'ios', ...body }),
    env,
    deps,
  );
...
function pushRequest(grant: string, body: Record<string, unknown> = {}): Request {
  return post(
    '/v1/push',
    { title: 'Vis', body: 'a session needs you', ...body },
    {
      headers: {
...
describe('minting a grant', () => {
  ...
  it('refuses a token that is not shaped like a registration token', async () => {
    ...
    for (const token of ['', 'short', `${APPLE_TOKEN}/../../3/device/victim`, 'zz'.repeat(32)]) {
      const response = await handle(
        post('/v1/grants', { device_token: token, platform: 'ios' }),
        env,
        deps,
      );
...
describe('pushing', () => {
  it('signs an ES256 provider token and posts the alert to the device path', async () => {
    ...
    const response = await handle(
      pushRequest(grant, { badge: 3, thread_id: 's-1', is_mutable: true }),
      env,
      deps,
    );
    expect(response.status).toBe(200);
  ...
  it('still opens a grant sealed with the key being rotated out', async () => {
    ...
    const rotated = makeEnv({
      RELAY_SEAL_KEY: 'the-new-key-bbbbbbbbbbbbbbbbbbbbbbbb',
      RELAY_SEAL_KEY_PREVIOUS: SEAL_KEY,
    });
  ...
  it('sends an Android grant through FCM with an RS256 token', async () => {
    const fcm = fakeFetch((url) =>
      url.includes('oauth2')
        ? new Response(JSON.stringify({ access_token: 'ya29.fake', expires_in: 3600 }), {
            status: 200,
          })
        : ok(),
    );
    ...
    expect(fcm.calls[1].url).toBe(
      'https://fcm.googleapis.com/v1/projects/vis-companion/messages:send',
    );
...
describe('what an unwelcome caller costs', () => {
  ...
  it('lets an operator lower the request cap and refuses to let one raise it', async () => {
    ...
    expect(
      (await handle(pushRequest(tightGrant, { body: 'b'.repeat(2000) }), tight, deps)).status,
    ).toBe(413);

    const greedy = makeEnv({ MAX_REQUEST_BYTES: '1048576' });
    const greedyGrant = await mint(greedy, deps);
    expect(
      (await handle(pushRequest(greedyGrant, { body: 'b'.repeat(20000) }), greedy, deps)).status,
    ).toBe(413);
  });
  ...
  it('refuses a data map too big for the provider without asking the provider', async () => {
    ...
    const response = await handle(
      pushRequest(grant, { data: { sealed: 'z'.repeat(6000) } }),
      env,
      deps,
    );
