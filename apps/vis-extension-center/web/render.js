...
const link = (text,url,className='',id='') => `<a${safeURL(url,true)?` href="${escapeHTML(safeURL(url,true))}" target="_blank" rel="noopener noreferrer"`:''} class="${className}"${id?` id="${id}"`:''}>${escapeHTML(text)}</a>`;
const quote = text => "'"+text.replaceAll("'", "'\\''")+"'";
export function installCommand(item) {
  return `vis-agent extension install ${quote(item.repository_url)}${item.subdirectory ? ' --subdirectory '+quote(item.subdirectory) : ''} --revision ${quote(item.revision)} --trust`;
}
...
export function visibleItems(items,state) {
  ...
  return items.filter(p=>(state.category==='all'||p.category===state.category)&&[p.name,p.description,p.owner,p.repository,...p.dependencies,...p.topics].join(' ').toLowerCase().includes(term)).sort((a,b)=>(state.sort==='stars'?b.stars-a.stars:state.sort==='name'?0:Date.parse(b[state.sort==='newest'?'added_at':'updated_at'])-Date.parse(a[state.sort==='newest'?'added_at':'updated_at']))||a.name.localeCompare(b.name)||a.id.localeCompare(b.id));
}
export function categoriesHTML(items,state) {
  return Object.entries(categories).map(([value,label])=>`<a data-category="${value}" href="${escapeHTML(filterURL({...state,category:value}))}"${value===state.category?' class="active" aria-current="page"':''}><span>${label}</span><span class="count">${items.filter(p=>value==='all'||p.category===value).length}</span></a>`).join('');
}
export function cardsHTML(items,state) {
  const visible=visibleItems(items,state);
  if(!visible.length) return `<div class="empty"><h2>${items.length?'No matching extensions':'No repositories yet'}</h2><p>${items.length?'Try a different search or browse all categories.':'Share a public GitHub repository for moderation.'}</p>${items.length?'<a id="clear-filters" href="/">Clear filters</a>':'<button id="empty-add" type="button">Add a repository</button>'}</div>`;
  return visible.map(item=>`<article class="extension-card" data-name="${escapeHTML(item.name)}"><a class="card-main" href="/extensions/${escapeHTML(item.id)}${filterURL(state).slice(1)}"><div class="card-top"><span class="tag">${escapeHTML(categories[item.category])}</span><span class="version">v${escapeHTML(item.version)}</span></div><h3>${escapeHTML(item.name)}</h3><p class="card-description">${escapeHTML(item.description)}</p></a>${link(item.repository,item.repository_url,'repository-link')}<div class="card-meta"><span>${escapeHTML(new Intl.NumberFormat('en',{notation:'compact',maximumFractionDigits:1}).format(item.stars))} stars</span><span>Updated ${dateLabel(item.updated_at)}</span></div></article>`).join('');
}
const fact=(label,value)=>`<dt>${label}</dt><dd>${escapeHTML(value)}</dd>`;
export function previewHTML(item) {
  return `<h3>${escapeHTML(item.name)} · v${escapeHTML(item.version)}</h3><p>${escapeHTML(item.description)}</p><dl class="facts">${fact('Repository',item.repository)+fact('Project folder',item.subdirectory||'Repository root')+fact('Category',categories[item.category])+fact('Commit',item.revision.slice(0,12))}</dl>${link('Review source on GitHub',item.source_url)}`;
}
...
export function shellHTML({items=[], item=null, search='', error='', detailError=false, siteKey='', docsURL='https://blockether.github.io/vis/'}={}) {
  const state=filters(search), detail=!!item||detailError;
  const docs=safeURL(docsURL)||'https://blockether.github.io/vis/';
  return `
    ...
    <dialog id="submit-dialog" class="content" aria-labelledby="submit-title"><div class="dialog-head"><div><p class="help" id="submit-step">1 of 2 · Repository</p><h2 id="submit-title">Add a repository</h2></div><button id="submit-close" type="button" class="quiet">Close</button></div><p>Share a public GitHub project. We read its metadata; your code stays on GitHub. Submissions are not public until approved.</p><form id="repository-form"><label for="repository-url">GitHub repository<input id="repository-url" maxlength="240" name="repository_url" type="url" placeholder="https://github.com/owner/repository" autocomplete="url" required></label><label for="project-folder">Project folder <span class="optional">Optional</span><input id="project-folder" maxlength="512" name="subdirectory" placeholder="Repository root" autocomplete="off" aria-describedby="folder-help"></label><p class="help" id="folder-help">Folder containing <code>pyproject.toml</code> and <code>extension.py</code>. Leave empty if they are at the repository root.</p><button id="review-submit" class="primary" type="submit">Review repository</button></form><div id="preview" aria-live="polite"></div><div id="turnstile" data-sitekey="${escapeHTML(siteKey)}"></div><p id="submit-status" role="status"></p><div class="dialog-actions"><button id="edit-submission" type="button" hidden>Edit repository</button><button id="submit-confirm" class="primary" type="button" hidden>Submit for review</button></div><p class="help trust-note">Listing a repository does not verify its owner or review its code.</p></dialog>`;
}
export function renderPage(data) {
  const initial=JSON.stringify(data).replace(/[<>&\u2028\u2029]/g,c=>'\\u'+c.charCodeAt(0).toString(16).padStart(4,'0'));
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover"><meta name="color-scheme" content="light"><title>${escapeHTML(data.item?.name||'Extension Center')} · Vis</title><meta name="description" content="Public GitHub extensions for Vis. Tools, providers and workflows."><link rel="preload" href="/assets/fonts/jetbrains-mono.woff2" as="font" type="font/woff2" crossorigin><link rel="stylesheet" href="/assets/theme.css"><link rel="stylesheet" href="/assets/catalog.css"></head><body><div id="app">${shellHTML(data)}</div><script id="catalog-data" type="application/json">${initial}</script><script type="module" src="/assets/app.js"></script></body></html>`;
}
