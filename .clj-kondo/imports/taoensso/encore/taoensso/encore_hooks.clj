...
(defn defalias
  ...
  (let [[alias src-raw _attrs body]
        ...
        sym
        (if src-raw (hooks/sexpr alias) (symbol (name (hooks/sexpr src))))]
             ...
             (into
               ...
               (map
                 (fn alias->defalias [alias-node]
                   (cond (hooks/token-node? alias-node)
                         (hooks/list-node [(hooks/token-node 'taoensso.encore/defalias) alias-node])
                         ...
                         (let [{:keys [src alias attrs body]} (hooks/sexpr alias-node)
                           ...
                           (hooks/list-node [(hooks/token-node 'taoensso.encore/defalias)
...
(defn defn-cached
  ...
  (let [[sym _opts binding-vec & body] (rest (:children node))]
    {:node (hooks/list-node (list (hooks/token-node 'def)
                                  sym
                                  (hooks/list-node
                                    (list* (hooks/token-node 'fn) binding-vec body))))}))
...
(defn -def-impl
  ...
  (let [[sym & args]
        ...
        [doc-string args]
        (if (and (hooks/string-node? (first args)) (next args))
          [(hooks/sexpr (first args)) (next args)]
          [nil args])

        [attr-map init-expr]
        (if (and (hooks/map-node? (first args)) (next args))
          [(hooks/sexpr (first args)) (fnext args)]
          [nil (first args)])

        attr-map
        (if doc-string (assoc attr-map :doc doc-string) attr-map)

        sym+meta
        (if attr-map (with-meta sym attr-map) sym)

        rewritten
        (hooks/list-node [(hooks/token-node core-macro-sym) sym+meta init-expr])]

    #_(println "old node:" node)
    #_(println "new node:" rewritten) {:node rewritten}))
