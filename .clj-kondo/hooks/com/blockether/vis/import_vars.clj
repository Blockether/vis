...
(defn- def-node
  ...
  [alias-node src-node]
  (api/list-node [(api/token-node 'def) (ignore-meta alias-node) src-node]))

(defn- spec->def
  "Turn one import spec node into a `(def alias src)` node, or nil if unusable."
  [spec]
  (cond (api/vector-node? spec) (let [[alias-node src-node] (:children spec)]
                                  (when (and alias-node src-node) (def-node alias-node src-node)))
        (api/token-node? spec) (let [sym (api/sexpr spec)]
                                 (when (symbol? sym)
...
(defn import-var
  ...
  (let [[_op alias-node src-node] (:children node)]
    (when (and alias-node src-node) {:node (def-node alias-node src-node)})))
