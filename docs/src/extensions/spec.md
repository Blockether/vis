...
`extension-info`, `extension-source-markers-of`,
...
| `:ext/environment-info-fn` | ✗            | -                    | `(fn [env] -> string\|seq\|map\|nil)` - live environment_info contribution rendered inside the system prompt's `<environment_info>` block. Use for cwd/repo/runtime facts; any active extension can add a sibling section. |
| `:ext/hooks`             | ✗              | `[]`                 | Vector of lifecycle hook declarations. Each entry has `:id` (keyword), `:doc` (non-blank string), `:phase` (`:session/start`\|`:turn/start`\|`:turn.iteration/start`\|`:turn.iteration/stop`\|`:turn.answer/validate`\|`:turn/stop`), `:fn`. Pre-phase hits return `nil` or canonical `{:hint string :importance :low\|:normal\|:high\|:critical}`; non-nil hits render as `<current_engine_start_nudge importance="...">` in the next iteration's prompt. `:turn.answer/validate` returns `nil` to accept or canonical `{:reject true :message ... :hint ...}` to reject. Invalid reject-looking maps are logged and ignored. Post-phase returns are ignored. See [Lifecycle Hooks (Nudges + Hard Guard Hooks)](guards.md). |
...
Aggregate rows are durable extension state, not prompt context. Extensions intentionally project small summaries through `:ext/environment-info-fn`, `:ext/prompt`, nudges, lifecycle hooks, or symbols. They must not store secrets here; use config/secret storage instead.
...
Tools that need info-aware rendering return the canonical tool-result
...
extension info onto `[:info :tool]`, `[:info :extension]`,
and `[:info :source]`. Value symbols are returned as `{sym -> value}`.
Each wrapped fn closes over the extension, symbol entry, and environment.

## Info helpers

`extension-info` returns the canonical extension-level info
map the runtime reuses everywhere:

```clojure
(sdk/extension-info ext)
