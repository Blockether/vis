...
(internally the turn-run entrypoint in `internal/loop.clj`, via
`prepare-turn-context`). They do **not** exist on the base
environment returned by `create-environment`.

| Key | Type | Description |
|-----|------|-------------|
| `:current-iteration-atom` | `atom of int` | Current iteration position in flight, 1-indexed. Bumped before every `db-store-iteration!`. Read by extensions that want to know which iteration they're in. |
