...
| `vis-foundation` | `extensions/common/vis-foundation/` | Filesystem / search helpers plus thin babashka.fs wrappers (`v/cat`, `v/ls`, `v/rg`, `v/read-all-lines`, `v/write-lines`, `v/update-file`, `v/glob`, …). Internal namespace: `com.blockether.vis.ext.foundation.editing.core`. | — | Pure `:ext/symbols` contribution. |
