...
| `vis-foundation` | `extensions/common/vis-foundation/` | Unified `v/` sandbox surface: introspection (`(v/turn)`, `(v/diagnose)`, `(v/conversation)`, `(v/find-attempts)`, `(v/var-history)`, `(v/extensions)`, `(v/extension-readme)` …) plus markdown answer builders (`(v/h1)`, `(v/p)`, `(v/table)`, `(v/file-link)`, `(v/join)` …). Internal namespaces: `com.blockether.vis.ext.foundation.introspection`, `com.blockether.vis.ext.foundation.markdown`. | — | Pure `:ext/symbols` contribution. See [vis-foundation](../extensions/common/vis-foundation.md). |
