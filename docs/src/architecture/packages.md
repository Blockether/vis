...
| `vis-foundation` | `extensions/common/vis-foundation/` | SCI sandbox introspection extension (`(vis/turn)`, `(vis/diagnose)`, `(vis/conversation)`, `(vis/find-attempts)`, `(vis/var-history)`, `(vis/extensions)`, `(vis/extension-readme)` …). Internal namespace: `com.blockether.vis.ext.foundation.introspection`. | — | Pure `:ext/symbols` contribution. See [vis-foundation](../extensions/common/vis-foundation.md). |
