...
(defdescribe clojure-lsp-plan-tools-test
  (it "returns diagnostics and dry-run edit plans without writing files"
    (let [{:keys [root file]} (temp-project)
      ...
      (expect (vector? (:op/result diag-out)))
      (expect (true? (:op/success? rename-out)))
      (expect (= 'demo.core/bar (get-in rename-out [:op/result :from])))
      (expect (= 'demo.core/baz (get-in rename-out [:op/result :to])))
      (expect (seq (get-in rename-out [:op/result :edits])))
      (expect (str/includes? (get-in rename-out [:op/result :edits 0 :new-text]) "baz"))
      (expect (true? (:op/success? clean-out)))
      (expect (seq (get-in clean-out [:op/result :edits])))
      (expect (not (str/includes? (get-in clean-out [:op/result :edits 0 :new-text]) "clojure.set")))
