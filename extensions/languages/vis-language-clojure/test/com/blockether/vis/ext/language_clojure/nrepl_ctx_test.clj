...
(defn- no-resource-mirror
  ...
  [f]
  (with-redefs [nx/ensure-resource! (fn [_ _] nil)]
    (f)))
...
(defdescribe contribute-shape-test
  (it "surfaces ONE owned REPL with id/port/status/aliases + it is the default"
    ...
    (no-resource-mirror
      (fn []
        (with-redefs [rm/session-repls (fn [_] [{:id "nrepl:/proj" :dir "/proj" :port 7001
                                                 :tool :clj :aliases [:dev :test]}])
                      nc/probe!        (fn [_] {:status :up
                                                :versions {:clojure "1.12.4"}
                                                :dialect :clj})]
          (let [b (nrepl-of (nx/contribute {:workspace/root "/proj" :session-id "s1"
...
(defdescribe default-selection-test
  (it "no default when MORE THAN ONE REPL is owned (id must be specified)"
    ...
    (no-resource-mirror
      (fn []
        (with-redefs [rm/session-repls (fn [_] [{:id "nrepl:/a" :dir "/a" :port 1 :tool :clj}
                                                {:id "nrepl:/b" :dir "/b" :port 2 :tool :clj}])
                      nc/probe!        (fn [_] {:status :up})]
          (let [b (nrepl-of (nx/contribute {:workspace/root "/a" :session-id "s1"
...
(defdescribe robustness-test
  ...
  (it "never throws — degrades to {} when session-repls blows up"
    (reset-cache!)
    (with-redefs [rm/session-repls (fn [_] (throw (ex-info "boom" {})))]
      (expect (= {} (nx/contribute {:workspace/root "/proj" :session-id "s1"
...
(defdescribe per-turn-cache-test
  (it "probes once per (turn, port-set); re-probes when the turn advances"
    ...
    (no-resource-mirror
      (fn []
        (let [calls (atom 0)]
          (with-redefs [rm/session-repls (fn [_] [{:id "nrepl:/p" :dir "/p" :port 9999 :tool :clj}])
