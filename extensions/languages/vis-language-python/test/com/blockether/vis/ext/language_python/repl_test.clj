...
(defdescribe interpreter-test
  (it "prefers a project-local .venv interpreter when present"
    (let [root (tmp-dir)
          py   (io/file root ".venv" "bin" "python")]
      (try
        (.mkdirs (.getParentFile py))
        (spit py "#!/bin/sh\n")
        (.setExecutable py true)
        (expect (= [(.getCanonicalPath py)] (interp/resolve-command (.getPath root))))
        (finally (cleanup root)))))
  (it "falls back to a system interpreter with no project env"
    (let [root (tmp-dir)]
      (try
        (let [cmd (interp/resolve-command (.getPath root))]
          (expect (= 1 (count cmd)))
