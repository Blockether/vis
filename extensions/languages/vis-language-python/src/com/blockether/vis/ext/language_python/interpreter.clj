...
(defn- windows? []
  (str/starts-with? (str (System/getProperty "os.name")) "Windows"))
...
(defn resolve-command
  ...
  (let [sys-py (if (on-path? "python3") "python3" (if (windows?) "python.exe" "python"))]
    (cond
      (and (uv-project? root) (on-path? "uv"))               ["uv" "run" "python"]
