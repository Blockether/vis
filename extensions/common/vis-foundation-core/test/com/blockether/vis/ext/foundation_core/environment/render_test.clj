(ns com.blockether.vis.ext.foundation-core.environment.render-test
  (:require
...
(defdescribe render-test
  (it "builds project ctx from runtime snapshot with no prompt labels"
    (let [out (render/project-context {:host base-host :git {:root "/tmp/x"}}
                nil nil)
          project (:project out)]
  ...
  (it "puts project guidance under ctx data"
    (let [out (render/project-context {:host base-host}
  ...
  (it "puts scan warnings under ctx data"
    (let [warnings [{:path "x" :reason "bad"}]
          out (render/project-context {:host base-host} nil warnings)]
