(ns com.blockether.vis.ext.foundation-core.environment.languages-test
  (:require
...
(defn- make-tmp-dir ^java.io.File []
  (let [path (Files/createTempDirectory "vis-env-test-"
...
(defdescribe languages-scan-test
  (it "counts files per language and picks a primary"
    (let [root (make-tmp-dir)]
      (try
        (spit-rel root "src/main.clj"           "(ns foo) (defn x [] 1)")
  ...
  (it "skips known non-source directories"
    (let [root (make-tmp-dir)]
      (try
        (spit-rel root "src/keep.clj"            "(ns keep)")
