(ns com.blockether.vis.ext.foundation-core.environment.monorepo-test
  (:require
...
(defn- make-tmp-dir ^java.io.File []
  (let [path (Files/createTempDirectory "vis-env-monorepo-"
...
(defdescribe monorepo-snapshot-test
  ...
  (it "detects submodules from .gitmodules at root"
    (let [root (make-tmp-dir)]
      (try
        (spit-rel root ".gitmodules" "[submodule \"foo\"]\n  path = foo\n  url = ./foo")
  ...
  (it "detects a workspace from multiple node manifests"
    (let [root (make-tmp-dir)]
      (try
        (spit-rel root "package.json"                "{\"name\":\"root\"}")
