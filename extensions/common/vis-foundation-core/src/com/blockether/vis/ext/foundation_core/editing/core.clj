...
(defn available-editing-prompt
  ...
  (str/join "\n"
    ["`v/` editing tools — RLM-shaped. Every tool returns a PLAIN CLOJURE MAP;"
     ...
     "  tuples — destructure with `[n t]`, no offset math. Page with"
     "  `(:next-offset prev)` until `:eof? true`. `:truncated?` only fires"
     "  when the 256KB window cap chopped the window mid-file; paginate via"
     "  :next-offset regardless. Each line's text is verbatim — no per-line"
     "  character cap. Thread `:mtime` / `:size` into a subsequent v/patch /"
     "  v/write as :expected-mtime / :expected-size to fail closed if the"
     ...
     "  occurrence by default (no global uniqueness requirement). Multi-line :search"
     "  gets a 5-pass fuzzy fallback (exact -> rstrip -> unicode -> relative-indent"
     "  -> trim) when zero exact matches land."
     ""
     "  Edit map keys (only :path/:search/:replace required):"
     "    :after  \"context\"        only match occurrences AFTER first hit of context"
     "    :before \"context\"        only match occurrences ending BEFORE first hit"
     "    :nth :first|:last|:all|N  occurrence selector (default :first; N is 1-based)"
     "    :expected-mtime MS        staleness guard; pair with (:mtime cat-result)"
     "    :expected-size  BYTES     staleness guard; pair with (:size  cat-result)"
     ""
     "  Bulk rename idiom — rename EVERY occurrence in ONE edit map:"
     ...
     "    OWN iteration. Confirm the return value, THEN call (done ...) in the"
     "    NEXT iteration. Never combine a write with (done ...) hoping both run"
     "    — only the (done) form will be observable on failure."
     "  - Defs persist across iterations. Reuse them; do not rebind identical args."
     ...
     ""
     "STRATEGIES — composing v/cat / v/rg / v/ls across iters"
     ""
     "  Each iter is a round-trip. Results land in the NEXT iter's ctx."
     "  Plan ONE step per iter; let each result shape the next form."
     ""
     "  Search across an unknown codebase:"
     "    ;; iter 1 — probe; learn shape and cost"
     "    (def probe (v/rg {:any [\"\\\\(def\\\\s+\"] :paths [\".\"] :limit 20}))"
     "    ;; iter 2 — read probe; widen with ONE call that covers everything"
     "    (def hits (v/rg {:any [\"\\\\(def\\\\s+\"] :paths [\".\"] :limit 500}))"
     "    ;; iter 3 — derive every view locally; plant ONE trimmed summary"
     "    (def summary"
     "      (let [by-file (group-by :path (:hits hits))]"
     "        {:total   (count (:hits hits))"
     "         :files   (count by-file)"
     "         :by-file (into {} (map (fn [[p hs]] [p (count hs)]) by-file))}))"
     ""
     "  Reading a file you don't know yet:"
     "    ;; iter 1 — default v/cat already gives first 2000 lines; fine for"
     "    ;; ~all source files. Use :range only when file > 2000 lines AND"
     "    ;; you know the section."
     "    (def f (v/cat \"src/foo.clj\"))"
     "    ;; iter 2 — read :lines; jump to a specific block if needed"
     "    (def section (v/cat \"src/foo.clj\" :range 800 1100))"
     ""
     "  Locating something, then opening it:"
     "    ;; iter 1 — narrow files-only scan"
     "    (def candidates (v/rg {:any [\"deftype Conn\"] :files-only? true}))"
     "    ;; iter 2 — open the one match"
     "    (def src (v/cat (first (:files candidates))))"
     ""
     "  Multi-edit in one shot (still ONE tool call, not parallel defs):"
     "    (def edits"
     "      (v/patch [{:path \"src/a.clj\" :search \"foo\" :replace \"bar\"}"
     "                {:path \"src/b.clj\" :search \"foo\" :replace \"bar\"}]))"
     ""
     "  Counts + files + sample hits from ONE scan (do NOT call v/rg three"
     "  times with different flags):"
     "    (def r (v/rg {:any [\"TODO\"] :paths [\".\"] :limit 300}))"
     "    (def view"
     "      (let [hs (:hits r)]"
     "        {:hits   hs"
     "         :files  (distinct (map :path hs))"
     "         :total  (count hs)"
     "         :counts (frequencies (map :path hs))}))"
     ""
     "  HARD RULES"
     "    - One heavy (def …) per iter (heavy = wraps v/rg / v/cat / v/ls"
     "      or has printed repr > ~2 kB)."
     "    - Use (let …) inside the fence for intermediate work; plant only"
     "      the trimmed summary with (def …). (def …) lives in the trailer"
     "      forever — every future iter pays for it."
     "    - Never two scans differing only by :counts? / :files-only? /"
     "      :context. One call; derive the views locally."
     "    - First touch of an unknown area = a small probe. Then widen."]))
