...
(defn available-editing-prompt
  ...
  (str/join "\n"
    ["`v/` editing tools — RLM-shaped. Every tool returns a PLAIN CLOJURE MAP;"
     ...
     ""
     "  Each iter is a round-trip. Trailer carries results to the next"
     "  iter. Engine captures :result for BARE and DEFED forms equally;"
     "  def only adds \"symbol survives to next iter's SCI namespace\"."
     ""
     "  PRE-EMIT CHECK   (run before writing ANY form)"
     "    ∃ prior-iter form in trailer with same (op + args)?"
     "      ⇒ reuse that result; do NOT re-issue"
     "    ∃ prior scan whose result is a superset of what you need?"
     "      ⇒ filter the prior result; do NOT re-scan with narrower args"
     ""
     "  DEF DISCIPLINE   (this is the lever)"
     "    def(X, expr) ⇔ ∃ later iter that references X by name"
     "    Will you (filter … X), (count X), (:k X) next iter? → def."
     "    Otherwise → bare form. Look at trailer next iter."
     ""
     "  SITUATION                          → RECIPE"
     "    mutation, one-shot                 → R-FIRE"
     "    read, look-once                    → R-LOOK"
     "    read, compose with next iter       → R-BIND"
     "    search probe → widen → derive      → R-PROBE-WIDEN"
     "    search → open                      → R-LOCATE-OPEN"
     "    many views from one scan           → R-ONE-SCAN"
     "    multi-file edit                    → R-BATCH-PATCH"
     "    write → done                       → R-WRITE-THEN-DONE"
     ""
     "  R-FIRE             ;; no def"
     "    (v/patch […])"
     ...
     "    (v/cat \"x.clj\")"
     "    (v/rg {:any [P] :files-only? true :limit 50})"
     "    (v/ls \"src\" :depth 2)"
     ""
     "  R-BIND             ;; def: symbol referenced by name in later iter"
     "    iN    (def hits (v/rg {:any [P] :limit 500}))"
     "    iN+1  (def s (let [g (group-by :path (:hits hits))]"
     "                   {:total (count (:hits hits)) :files (count g)}))"
     ""
     "  R-PROBE-WIDEN"
     "    i1   (v/rg {:any [P] :limit 20})                ;; bare — just look"
     "    i2   (def hits (v/rg {:any [P] :limit 500}))    ;; def — i3 references"
     "    i3   (def s (let [g (group-by :path (:hits hits))] {…}))"
     ""
     "  R-LOCATE-OPEN"
     "    i1   (v/rg {:any [P] :files-only? true})        ;; bare"
     "    i2   (v/cat <path read from trailer>)            ;; bare"
     ""
     "  R-ONE-SCAN"
     "    (def r (v/rg {:any [P] :limit 300}))             ;; def — views ref r"
     "    (def v (let [hs (:hits r)]"
     "             {:hits hs :files (distinct (map :path hs))"
     "              :total (count hs) :counts (frequencies (map :path hs))}))"
     ""
     "  R-BATCH-PATCH      ;; no def"
     "    (v/patch [{:path A :search S :replace R}"
     "              {:path B :search S :replace R}])"
     ""
     "  R-WRITE-THEN-DONE"
     "    iN     (v/patch […]) | (v/write {…})            ;; bare — :diff in trailer"
     "    iN+1   (done {:answer …})                        ;; cite the diff"
     ""
     "  INVARIANTS"
     "    heavy-defs-per-iter              ≤ 1"
     "    def(X) ⇔ X referenced in later iter"
     "    same-query-twice-across-iters    ⊥    ;; check trailer first"
     "    same-query-twice-in-one-fence    ⊥    ;; one call, derive in (let …)"
     "    let-for-intermediate             ⊤"
     "    mutation-needs-def?              ⊥"]))
