(ns com.blockether.vis.ext.foundation-core.environment.languages
...
(def ^:private generated-file-names
  ...
  #{"package-lock.json" "pnpm-lock.yaml" "yarn.lock" "Cargo.lock"
    ...
    "deno.lock" "shrinkwrap.json"})

(defn- generated? [^String filename]
  (or (contains? generated-file-names filename)
    (let [lower (.toLowerCase filename)]
      (some (fn [^String suffix] (.endsWith lower suffix))
        generated-file-suffixes))))

(defn- file-extension-of ^String [^String filename]
  (let [dot (.lastIndexOf filename ".")]
    (cond
      (= filename "Dockerfile")           "Dockerfile"
      (and (pos? dot) (< dot (dec (count filename))))
...
(defn- bucket-rollup
  ...
  [buckets]
  (let [entries     (vec buckets)
        total-files (long (reduce + 0 (map (comp :files val) entries)))
        total-bytes (long (reduce + 0 (map (comp :bytes val) entries)))
        scored      (map (fn [[lang {:keys [files bytes]}]]
                           (let [files* (long files)
                                 bytes* (long bytes)]
                             {:language  lang
                              :files     files*
                              :bytes     bytes*
                              :files-pct (if (zero? total-files) 0.0
                                           (/ (double files*) (double total-files)))
                              :bytes-pct (if (zero? total-bytes) 0.0
                                           (/ (double bytes*) (double total-bytes)))}))
                      entries)]
    (vec (sort-by (fn [m] [(- (long (:files m))) (- (long (:bytes m)))]) scored))))
...
(defn scan
  ...
  ([root {:keys [max-files deadline-ms]
   ...
   (let [^Path start    (cond
                                ...
                                (do
                                  ...
                                  (let [filename (str (.getFileName file))]
                                    (when (and (not (generated? filename))
                                      ...
                                      (when-let [lang (extension-to-language
                                                        (file-extension-of filename))]
                                        (let [bucket (or (.get buckets lang)
                                                       {:files 0 :bytes 0})
     ...
     (let [bucket-map  (into {} buckets)
           rolled      (bucket-rollup bucket-map)
           total-files (long (reduce + 0 (map :files rolled)))
           total-bytes (long (reduce + 0 (map :bytes rolled)))
           elapsed     (- (System/currentTimeMillis) start-ms)]
       {:total-files total-files
        :total-bytes total-bytes
        :truncated?  (aget truncated 0)
        :elapsed-ms  elapsed
        :primary     (some-> rolled first :language)
        :languages   rolled}))))