...
(defdescribe bridge-extension-test
  (it "configures the extension",
    (let [prompt-text ((get-in bridge/vis-extension [:ext/prompt]) {})
      ...
      (expect (str/includes? prompt-text "Use `br_next(opts?)` after `br_check()`"))
      (expect (str/includes? prompt-text "Use `br_list_evidence(opts?)`"))
      (expect (str/includes? prompt-text "`br_run_evidence(id, {\"is_dry_run\": True})`"))
      (expect (str/includes? prompt-text "\"changed_files\""))
      (expect (str/includes? prompt-text "\"timeout_seconds\""))
      ...
      (expect (str/includes? prompt-text "do not report it as completed evidence"))
      (expect (str/includes? prompt-text "instead of shelling out to `bb bridge`"))
