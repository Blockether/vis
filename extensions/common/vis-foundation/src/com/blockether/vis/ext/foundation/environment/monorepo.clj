(ns com.blockether.vis.ext.foundation.environment.monorepo
...
(defn- shape-label
  ...
  (let [ext-dir   (.exists (io/file root "extensions"))
        ...
        gitmod    (.exists (io/file root ".gitmodules"))
        any-multi (some #(>= (long %) 2) (map (comp count val) manifests-by-kind))]
    (cond
      gitmod                       "submodules"
      (and ext-dir pkg-dir)        "polylith"
      (and ext-dir
        (>= (long (count (get manifests-by-kind :clojure))) 2))
                             ...
                             (let [count* (aget visited 0)]
                               (cond
                                 ...
                                 (do
                                   ...
                                   FileVisitResult/CONTINUE))))
                           (visitFileFailed [^Path _file _exception]
                             FileVisitResult/CONTINUE))]
     (try
       (Files/walkFileTree start visitor)
       (catch Throwable _ nil))
     (let [files-by-kind (into {} buckets)
           totals        (into {} (map (fn [[k v]] [k (count v)]) files-by-kind))
           shape         (shape-label root-file files-by-kind)]
       {:shape       shape
        :totals      totals
        :files       files-by-kind
