...
(defn- cat-tool
  "Read the whole text file into `[:op/result :lines]`. `v/cat` has no display or pagination opts; the journal renders a bounded preview (first 50 + last 50 lines) and the full vector stays bound for slicing. Tool result; `:op/result` = {:path :offset :total-lines :truncated-by :lines}."
  ([path]
...
(defn- bash-tool
  "Run bounded `/usr/bin/env bash -lc` in worktree with `set -euo pipefail` prepended. Tool result envelope; shell fields live under :op/result, e.g. :op/result :stdout, :op/result :stderr, :op/result :exit. Do not read (:stdout run), (:exit run), or [:result ...]. Refuses shell-driven Clojure/EDN source edits; use z/patch for those. Emits :warnings when stderr looks like a swallowed failure."
  ([command]
...
;;
;; `result` is the unwrapped `(:op/result tool-result)`. Engine handles
...
(defn available-editing-prompt
  ...
  (str
    ...
    "`v/cat` reads the whole file into [:op/result :lines]. The journal renders a BOUNDED preview (first 50 + last 50 lines) plus a hint; bind the full result so you can slice further on a later iteration with plain Clojure: (def file (v/cat \"src/foo.clj\")) then (subvec (get-in file [:op/result :lines]) 100 200). Never use [:result :lines] - that path is nil and `subvec` will crash. "
    ...
    "`v/glob` returns cwd-relative path strings under `:op/result`. Simple patterns like `*` and `*.clj` match immediate children; recursive patterns like `**/*.clj` walk descendants. "
    "Example child listing: (->> (v/glob \"src\" \"*.clj\") :op/result sort vec). Example recursive search: (->> (v/glob \"extensions\" \"**/*.clj\") :op/result sort vec). Use `:scope :children` or `:scope :recursive` when you want to force the behavior. "
    ...
    "Tool results are envelopes and expose their payload under `:op/result`. Examples: (get-in (v/cat \"IDEAS.md\") [:op/result :lines]), "
    (when-not (config/bash-disabled?)
      "(get-in (v/bash \"pwd\") [:op/result :stdout]), ")
    "(-> (v/rg {:all [\"needle\"] :paths [\"src\" \"test\"] :include [\"*.clj\" \"*.cljc\"]}) :op/result :hits). "
