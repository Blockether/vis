...
(defn available-editing-prompt
  ...
  (str
    ...
    "`v/cat` reads the whole file into [:op/result :lines]. The journal renders a BOUNDED preview (first 50 + last 50 lines) plus a hint; bind the full result so you can slice further on a later iteration with plain Clojure: (def file (v/cat \"src/foo.clj\")) then (subvec (get-in file [:op/result :lines]) 100 200). "
