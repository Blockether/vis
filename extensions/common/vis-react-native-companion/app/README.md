...
| Voice           | Records 16-bit PCM WAV @16kHz mono (expo-audio), with a pulsing red dot, timer, and cancel/accept strip, transcribes through the gateway's local Parakeet model; drives the model download with progress if it isn't installed yet. iOS-first (Android's MediaRecorder can't emit WAV)                                                   | `POST /ui/session/:sid/voice`, `GET/POST …/voice/model` (web channel)                                                                                                                                                 |
