...
(def ^:private server-path
  "extensions/common/vis-foundation-mcp/test/resources/fake_mcp_server.py")

(defn- on-path
  "Absolute path of `exe` if it's an executable on PATH, else nil."
  [exe]
  (some (fn [d] (let [f (io/file d exe)] (when (.canExecute f) (.getPath f))))
    (str/split (or (System/getenv "PATH") "") #":")))

(defdescribe mcp-stdio-client-test
  (it "initialize + tools/list + tools/call roundtrip over stdio"
    (let [py (on-path "python3")
          f  (io/file server-path)]
      (if-not (and py (.exists f))
        (expect true)                      ; prereqs absent — skip, don't fail CI
        (let [conn (mcp/connect "fake" {:transport :stdio :command py
                                        :args [(.getPath f)]})]
          (try
