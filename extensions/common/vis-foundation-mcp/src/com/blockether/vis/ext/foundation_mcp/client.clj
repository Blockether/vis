...
(defn- rpc-error->ex [name method err]
  (ex-info (str "MCP " name " " method " failed: "
             (get err "message" "unknown error"))
    {:type :mcp/rpc-error :server name :method method
     :code (get err "code") :data (get err "data")}))
...
(defn- start-stdio!
  ...
  (let [pb   (ProcessBuilder. ^java.util.List (vec (cons command (map str (or args [])))))
        _    (when (seq env)
               (let [m (.environment pb)]
                 (doseq [[k v] env] (.put m (str (clj-name k)) (str v)))))
        _    (when (and cwd (string? cwd) (.isDirectory (io/file cwd)))
               (.directory pb (io/file cwd)))
        _    (.redirectErrorStream pb false)
        proc (.start pb)
        out  (BufferedReader. (io/reader (.getInputStream proc)))
                    ...
                    (let [msg (json-> line)]
                      (when-let [id (get msg "id")]
                        (when-let [p (.remove pending id)]
                          (deliver p msg))))
                    (catch Throwable t
                      (tel/log! {:level :debug :id ::stdio-parse
    ...
    (doto (Thread. ^Runnable read-loop (str "mcp-stdio-" name))
      (.setDaemon true)
      (.start))
    {:pid (try (.pid proc) (catch Throwable _ nil))
     :request-fn
     (fn [method params timeout-ms]
       (when @closed? (throw (ex-info "MCP server not running" {:type :mcp/closed :server name})))
       (let [id (str (swap! next-id inc))
         ...
         (let [msg (deref p timeout-ms ::timeout)]
           (.remove pending id)
           (cond
             (= msg ::timeout) (throw (ex-info (str "MCP " name " " method " timed out")
     ...
     (fn []
       ...
       (try (when (.isAlive proc)
         ...
         (catch Throwable _ nil)))
     :alive-fn (fn [] (and (not @closed?) (.isAlive proc)))}))
  ...
  (->> (str/split-lines body)
    (keep (fn [^String l]
            (when (str/starts-with? l "data:")
              (str/trim (subs l 5)))))
    (keep (fn [d] (try (json-> d) (catch Throwable _ nil))))
    vec))
...
(defn- start-http!
  ...
  (let [session (atom nil)
    ...
    {:request-fn
     (fn [method params timeout-ms]
       (let [req-id (str (System/nanoTime))
             body   (->json (cond-> {"jsonrpc" "2.0" "id" req-id "method" method}
                              (some? params) (assoc "params" params)))
             {:keys [status body]} (post! body timeout-ms)]
         (when (>= status 400)
           (throw (ex-info (str "MCP " name " HTTP " status " on " method)
                    {:type :mcp/http-error :server name :status status :body body})))
         (let [objs (if (str/includes? (str body) "data:")
                      (sse-data-objects body)
                      (try [(json-> body)] (catch Throwable _ [])))
               msg  (or (some #(when (= (get % "id") req-id) %) objs)
...
(defn- transport-of [{:keys [transport url]}]
  (or transport (if url :http :stdio)))
...
(defn connect
  ...
  [name spec]
  (let [transport (transport-of spec)
        t (case transport
            :stdio (start-stdio! name spec)
            :http  (start-http! name spec)
...
(defn list-tools
  ...
  [conn]
  (or @(:tools conn)
    (let [result ((:request-fn conn) "tools/list" {} default-timeout-ms)
          tools  (vec (get result "tools"))]
...
(defn close [conn]
  (try ((:close-fn conn)) (catch Throwable _ nil))
