...
(defdescribe
      ...
      (let
        ...
        (try (spit (str (.resolve dir "en_US-ryan-high.onnx")) "model")
             ...
             (finally (doseq [^java.io.File path (reverse (file-seq (.toFile dir)))]
                        (.delete path))))))
  (it
    "synthesizes with the default model directory when :model-dir is omitted"
    (let
      [out
  ...
  (it "redirects child TTS stdout and stderr away from the parent terminal into the Vis log"
      (let [log-file (java.io.File/createTempFile "vis-voice-log-test" ".log")]
        (try (spit log-file "before\n")
             (with-redefs
               [com.blockether.vis.ext.foundation-voice.core/tts-worker-argv
                (fn []
               ...
               (let
                 [captured (with-out-str (let [process (#'voice/start-tts-worker! log-file)]
  ...
  (it "uses the isolated worker for async TUI voice responses"
      (let [calls (atom [])]
        (with-redefs
          [com.blockether.vis.ext.foundation-voice.core/notify-progress! (fn [& args]
                                                                           (swap! calls conj
                                                                             (into [:notify] args)))
