(ns com.blockether.vis.ext.foundation-git.render
  "Op-card renderers for the whole `git/` surface — one `{:summary :body}`
   ...
   CONTRACT: a renderer receives the tool's UNWRAPPED result value AFTER it
   has round-tripped the strings-only GraalPy boundary — so keys are plain
   SNAKE_CASE STRINGS exactly as the tool built them (`\"short_sha\"`,
   `\"amend\"`, `\"up_to_date\"`) and there are no keyword/symbol values.
   Read string keys (`:strs` destructuring / `(get m \"k\")`); the renderer
   still RETURNS the keyword-keyed `{:summary :body}` IR, which is internal."
  (:require
...
(defn render-push
  ...
  [{:strs [remote branch force delete updates]}]
  {:summary (str (if delete "deleted " "pushed ") (code branch) " → " (s remote)
              (when force " (force)")
              (when (seq updates)
                (str " · " (str/join ", " (distinct (map #(s (get % "status")) updates))))))
   :body    (block (for [u updates]
                     (str "  " (s (get u "status")) "  " (s (get u "remote_name"))
                       (when-let [m (some-> (get u "message") s not-empty)] (str " — " m)))))})

(defn render-fetch
  "git_fetch → `fetched remote · STATUS · N updates` + per-ref ranges."
  [{:strs [remote status summary updates]}]
  (let [n (get summary "updated")]
    {:summary (str "fetched " (s remote) " · " (s status)
                (when (and n (pos? (long n))) (str " · " (plural n "update"))))
     :body    (block (for [u updates]
                       (str "  " (s (get u "ref"))
                         (when-let [r (some-> (get u "range") s not-empty)] (str "  " r))
                         (when-let [k (some-> (get u "kind") s not-empty)] (str " (" k ")")))))}))

(defn render-reset
  "git_reset → `reset --mode → `sha`` (or per-path reset) + head before→after."
  [{:strs [mode to short_sha head_before head_after paths]}]
  (if (seq paths)
...
(defn render-branch
  ...
  [{:strs [name from short_sha force deleted old new mode branches] :as r}]
  (cond
    (contains? r "branches")
    {:summary (str (plural (count branches) "branch") (when mode (str " (" (s mode) ")")))
     :body    (block (for [b branches]
                       (str "  " (s (get b "short"))
                         (when (get b "short_sha") (str "  " (s (get b "short_sha")))))))}
...
(defn render-checkout
  ...
  [{:strs [paths files_restored start_point branch sha short_head created detached] :as r}]
  (cond
...
(defn render-cherry-pick
  ...
  [{:strs [status picked failing_paths]}]
  {:summary (str "cherry-pick " (s status) " · " (plural (count picked) "commit")
   ...
   :body    (block (concat (for [p picked] (str "  picked " (s (get p "short_sha"))))
                     (for [fp failing_paths] (str "  conflict " (s fp)))))})

(defn render-rebase
  "git_rebase → `rebase STATUS @ sha` + conflicts / failing paths / hint."
  [{:strs [status conflicts failing_paths short_current hint]}]
...
(def ^:private status-buckets
  "Ordered [bucket-key marker] pairs for the status card body. The bucket
   keys are the STRING keys the tool result carries."
  [["added" "A"] ["modified" "M"] ["deleted" "D"] ["untracked" "?"] ["conflicted" "U"]])

(defn render-status
  "git_status → `on `branch` @ head · N changes` (or `clean`) + marked files."
  [{:strs [branch head changes]}]
...
(defn- diff-files-block
  ...
  (block
    (mapcat (fn [f]
              (cons (str "  +" (or (get f "add") 0) " -" (or (get f "del") 0) "  " (s (get f "file")))
                (some-> (get f "patch") s not-empty str/split-lines)))
      files)))
...
(defn render-diff
  ...
  [{:strs [from to stat files untracked]}]
  (let [fc (or (get stat "files") (count files))]
    {:summary (str "diff " (code from) (when to (str "…" (code to)))
                " · " (plural fc "file") " +" (or (get stat "add") 0) " -" (or (get stat "del") 0)
...
(defn render-log
  ...
  {:summary (str (plural (count commits) "commit") (when branch (str " on " (code branch))))
   :body    (block (for [c commits]
                     (str "  " (s (get c "short_sha")) "  " (or (first-line (get c "subject")) ""))))})

(defn render-show
  "git_show → `commit `sha` · subject` + author line and file numstat/patch."
  [{:strs [short_sha subject author files]}]
...
(defn- conflict-str
  "One conflict entry as a path string (handles both a bare path and the
   `{\"path\" \"state\"}` shape)."
  [c]
  (if (map? c)
    (str (s (get c "path")) (when-let [st (some-> (get c "state") s not-empty)] (str " (" st ")")))
    (s c)))

(defn render-merge-status
  "git_merge_status → in-progress card with conflict list, or a no-op note."
  [{:strs [in_progress branch conflicts]}]
  (if in_progress
...
(defn render-merge
  ...
  [{:strs [status conflicts failing_paths hint head]}]
