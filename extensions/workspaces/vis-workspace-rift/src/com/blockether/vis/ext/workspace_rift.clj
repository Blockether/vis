...
(defn- writable-dir?
  [path]
  (try (Files/isWritable (.toPath (io/file path)))
    (catch Throwable _ false)))

(defn- failure-data
  [{:keys [source-root store-root name]} t]
  (let [into-file (io/file store-root)
    ...
    {:source-root (file-path source-root)
     ...
     :store-parent (some-> parent-file .getCanonicalPath)
...
(defn- probe-key
  ...
  [(file-path source-root) (file-path store-root)])

(defn- probe!
  [{:keys [source-root store-root]}]
  (try
    (when (linked-git-worktree-source? source-root)
    ...
    (let [src (str (Files/createTempDirectory (.toPath (io/file source-root))
                     ".vis-rift-probe-" (make-array FileAttribute 0)))
          dst (str (Files/createTempDirectory (.toPath (io/file store-root))
                     ".vis-rift-probe-" (make-array FileAttribute 0)))]
      (try
        (spit (io/file src "probe") "ok")
        (rift/init {:at src})
        (if (rift/create {:from src :name "clone" :into dst})
          {:available? true}
          {:available? false :reason :probe-returned-no-workspace})
        (finally
          (try (rift/remove! {:at (str (io/file dst "clone"))}) (catch Throwable _ nil))
    ...
    (catch Throwable t
      {:available? false
       ...
       :details (failure-data {:source-root source-root
...
(defn- available?
  ...
  (let [store-root (or store-root source-root)
        opts (assoc opts :store-root store-root)]
    (try
      (if (linked-git-worktree-source? source-root)
...
(vis/register-extension!
  {:ext/name "workspace-rift"
   :ext/description "Rift copy-on-write isolated workspace backend."
   :ext/kind "workspace"
   :ext/version "0.0.10"
   :ext/owner "vis"
   :ext/workspace-backends
   [(vis/workspace-backend
