...
(defdescribe
  ...
  (it "normalizes primary and secondary Codex usage windows"
      (let
        [report (codex/usage->dynamic-limits
  ...
  (it "uses ChatGPT's explicit window duration instead of assuming primary means 5h"
      (let
        [report (codex/usage->dynamic-limits {:rate_limit
                                              {:allowed true
                                               :limit_reached false
      ...
      (let
        [report (codex/usage->dynamic-limits
                  {:rate_limit {:primary_window {:used_percent 99}}
                   :additional_rate_limits [{:limit_name "other"
  ...
  (it "keeps a visible 5h row when OpenAI temporarily omits it"
      (let
        [report (codex/usage->dynamic-limits {:rate_limit {:primary_window {:used_percent 19
