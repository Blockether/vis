...
(def ^:private spark-limit-name
  "GPT-5.3-Codex-Spark")

(def ^:private spark-model-id
  "gpt-5.3-codex-spark")

(def ^:private usage-window-specs
  [{:bucket-key :primary_window
    :id         :codex-5h
    :label      "Codex 5h quota (%)"
    :unit       :hour
    :size       5}
   {:bucket-key :secondary_window
    :id         :codex-7d
...
(defn- clamp-percent
  [value]
  (-> (double value) (max 0.0) (min 100.0)))

(defn- used->left-percent
  [used-percent]
  (when (number? used-percent)
    (clamp-percent (- 100.0 (double used-percent)))))

(defn- model-id
  [model-ref]
  (cond
    (keyword? model-ref) (name model-ref)
    (string? model-ref)  model-ref
    (map? model-ref)     (or (field model-ref :id)
                           (field model-ref :name)
                           (some-> (field model-ref :model) model-id))
    :else nil))

(defn- spark-model?
  [model-ref]
  (= spark-model-id (some-> (model-id model-ref) str/lower-case)))

(defn- normalize-rate-limit-bucket
  [value]
  (object-map value))
...
(defn- spark-rate-limit-entry
  ...
  (let [entry (object-map value)]
    (when (= spark-limit-name (some-> (field entry :limit_name) str/trim))
...
(defn- spark-rate-limit-bucket
  ...
  (let [additional (field usage :additional_rate_limits)]
    (or (when (sequential? additional)
          (some spark-rate-limit-entry additional))
...
(defn- reset-at-ms
  ...
  (or (epoch-ms (field window :reset_at))
    (when-let [seconds (field window :reset_after_seconds)]
      (when (number? seconds)
        (+ (long now-ms) (long (* 1000.0 (double seconds))))))))
  ...
  (when-let [window (object-map (field bucket bucket-key))]
    (let [used-percent (field window :used_percent)
      ...
      (cond-> {:id         id
               ...
               :source     :provider-api
               :unlimited? false
               :window     (cond-> {:kind :rolling
                                    :unit unit
                                    :size size}
                             reset-ms (assoc :resets-at-ms reset-ms))}
        (number? used-percent) (assoc :used (clamp-percent used-percent)
                                 :limit 100.0)
        (number? left-percent) (assoc :remaining left-percent)))))
...
(defn usage->dynamic-limits
  ...
  ([usage]
   (usage->dynamic-limits usage nil))
  ([usage model-ref]
   (usage->dynamic-limits usage model-ref (System/currentTimeMillis)))
  ([usage model-ref now-ms]
   (let [bucket   (select-rate-limit-bucket usage model-ref)
         rows     (if bucket
                    (into []
                      (keep #(window-row now-ms % bucket))
                      usage-window-specs)
                    [])
         limited? (or (true? (field bucket :limit_reached))
                    (false? (field bucket :allowed)))]
     (cond-> {:limits rows}
  ...
  (let [response (http/get usage-url
    ...
    (if (<= 200 status 299)
      ...
      (throw (ex-info (str "OpenAI Codex usage request failed: HTTP " status)
               {:type   :provider/openai-codex-usage-error
                :status status
                :body   body
                :url    usage-url})))))
...
(defn dynamic-limits!
  ...
  ([access-token account-id]
   (dynamic-limits! access-token account-id nil))
