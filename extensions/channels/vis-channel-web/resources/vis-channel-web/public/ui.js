...
/* provider reorder: native drag-and-drop on .pcard cards inside .pcards.
   Replaces the old up/down arrows. On dragend the new id order is POSTed to
   the container's data-reorder-url and #modal is re-rendered. Delegated on
   `document` so it survives every modal innerHTML swap. */
(function () {
  var dragEl = null;
  function cardsBox(el) { return el && el.closest ? el.closest(".pcards") : null; }
  document.addEventListener("dragstart", function (e) {
    var card = e.target.closest ? e.target.closest(".pcard[draggable]") : null;
    if (!card || !cardsBox(card)) { return; }
    dragEl = card;
  ...
  document.addEventListener("dragover", function (e) {
    if (!dragEl) { return; }
    var box = cardsBox(dragEl);
    if (!box) { return; }
    var over = e.target.closest ? e.target.closest(".pcard") : null;
    if (over && cardsBox(over) !== box) { return; }
    e.preventDefault();
  ...
  document.addEventListener("dragend", function () {
    ...
    dragEl = null;
    if (!box) { return; }
    var url = box.dataset.reorderUrl;
    if (!url) { return; }
    var ids = Array.prototype.map.call(
      box.querySelectorAll(".pcard[data-pid]"),
      function (c) { return c.dataset.pid; });
    if (typeof htmx !== "undefined" && htmx.ajax) {
      htmx.ajax("POST", url, { target: "#modal", swap: "innerHTML",
