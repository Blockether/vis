...
(def ^:private transcript-html-fn
  ...
  (delay (fn [_db sid]
           (str ((requiring-resolve 'com.blockether.vis.ext.channel-web.core/export-session-html)
                  sid)))))
...
(def ^:private default-chat-settings
  {:reasoning-level :balanced :openai-codex-verbosity :low :voice-mode :off})
...
(defn- voice-extension? [] (or (voice-input-extension?) (voice-output-extension?)))
...
(defn- extract-voice-file-id [message] (get-in message [:voice :file_id]))
...
(defn- normalize-chat-id
  [chat-id]
  (when-not (nil? chat-id) (str (if (string? chat-id) (str/trim chat-id) chat-id))))

(defn- parse-allowed-chat-ids
  [v]
  (cond (nil? v) #{}
...
(defn- chat-approved?
  ...
  (let [allowed (configured-allowed-chat-ids)]
    (or (empty? allowed) (contains? allowed (normalize-chat-id chat-id)))))

(defn- unauthorized-message
  [chat-id]
  (str "Telegram chat is not approved: "
       chat-id
       "\n\n"
       "Approve it from a trusted shell:\n"
       "vis channels telegram approve --chat-id "
       chat-id
       "\n\n"
...
(defn- read-pid-file
  []
  (try (some-> (telegram-pid-file)
               slurp
               str/trim
               parse-long)
       (catch Throwable _ nil)))

(defn- write-pid-file! [] (spit (telegram-pid-file) (str (current-pid))))
...
(defn- schedule-self-restart!
  ...
  (when (compare-and-set! restart-requested? false true)
    (future (Thread/sleep 500)
            (try (start-fresh-telegram-process!)
                 (catch Throwable t
...
(defn- wav-audio? [audio-file] (contains? #{".wav" ".wave"} (audio-extension audio-file)))

(defn- ffmpeg-audio->wav!
  [audio-file wav-file]
...
(defn- live-bubble-html
  ...
  (let [step-lines
    ...
    (cond (<= (count full) telegram-msg-limit) full
          (<= (count (str/join "\n\n" chrome)) telegram-msg-limit) (str/join "\n\n" chrome)
          :else (str/join "\n\n"
                          (cond-> ["💭 <b>Thinking…</b>"]
                            (seq steps)
                            (conj (render-step (last steps))))))))
...
(defn- bubble-state-for [chat-id] (get-in @chat-state [chat-id :live-bubble]))

(defn- update-bubble-state!
  [chat-id f & args]
...
(defn- apply-edit-result!
  ...
  (cond
    (and resp (:ok resp))
    (update-bubble-state! chat-id assoc :pending-html new-html :last-edit-ms (now-ms) :backoff-ms 0)
    (and resp
         (= 400 (:error_code resp))
  ...
  (when (bubble-state-for chat-id)
    (let [phase (:phase chunk)]
      (cond
        ...
        (= phase :reasoning) (when-let [thinking (or (:thinking chunk) (:reasoning chunk))]
        ...
        (= phase :form-start) (do (update-bubble-state! chat-id
                                                        update
                                                        :steps
                                                        (fn [steps]
...
(defn- normalize-reasoning-level
  [v]
  (normalize-choice reasoning-level-order {:low :quick :medium :balanced :high :deep} :balanced v))

(defn- normalize-codex-verbosity [v] (normalize-choice codex-verbosity-order {} :low v))

(defn- normalize-voice-mode
  [v]
  (normalize-choice voice-mode-order {:on :duplex :voice :duplex :audio :duplex} :off v))
...
(defn- persist-chat-settings!
  ...
  (try (let [chat-id
       ...
       (catch Throwable t
         (tel/log! {:level :warn
                    :id ::persist-chat-settings-failed
...
(defn- set-chat-setting! [chat-id k v] (update-chat-settings! chat-id assoc k v))

(defn- set-in-flight! [chat-id token] (swap! chat-state assoc-in [chat-id :in-flight] token) nil)

(defn- clear-in-flight!
  [chat-id token]
  (swap! chat-state update
    chat-id
    (fn [state]
      (if (= token (:in-flight state)) (dissoc state :in-flight) state)))
  nil)

(defn- in-flight-token [chat-id] (get-in @chat-state [chat-id :in-flight]))
...
(defn- turn-reasoning-default
  [settings]
  (when (reasoning-effort-configurable?) (:reasoning-level settings)))
...
(defn- model-entry-label
  [{:keys [provider-id model live-only?]}]
  (str (some-> provider-id
               name)
       "/"
       model
       (when live-only? " (live)")))
...
(defn- select-model!
  ...
  (let [base-config
    ...
    (cond (empty? entries) {:message "No models configured" :level :warn}
          ...
          :else (if-let [entry (find-model-entry entries selection)]
...
(defn- model-inline-keyboard
  [entries active]
  {"inline_keyboard" (mapv (fn [[idx entry]]
...
(defn- codex-model-active?
  ...
  (let [provider (first (:providers (or (vis/load-config) {})))]
    (or (= :openai-codex (:id provider)) (= :openai-compatible-responses (:api-style provider)))))
...
(defn- voice-mode-available?
  ...
  (case mode
    ...
    :duplex
    (and (voice-input-extension?) (voice-output-extension?))

    false))

(defn- available-voice-modes [] (filterv voice-mode-available? voice-mode-order))
...
(defn- set-voice-mode!
  ...
  (cond (not (voice-extension?))
        ...
        :else (do (set-chat-setting! chat-id :voice-mode mode) (str "Voice mode: " (name mode)))))
...
(defn- answer->markdown-string
  ...
  [answer]
  (cond (and (map? answer) (string? (:answer answer))) (:answer answer)
...
(defn- provider-id-str
  [provider-id]
  (when provider-id (if (keyword? provider-id) (name provider-id) (str provider-id))))
...
(defn- markdown-quote [s] (str "> " (str/replace (str/trim (str s)) #"\n" "\n> ")))

(defn- transcript-message [transcript] (str "*Transcription*\n" (markdown-quote transcript)))

(defn- voice-output-mode? [settings] (contains? #{:output :duplex} (:voice-mode settings)))
...
(defn- handle-user-voice!
  [token chat-id message sender]
  (when-let [file-id (extract-voice-file-id message)]
    (future (try (tg/send-chat-action! token chat-id "typing")
...
(defn- install-bot-menu!
  [token]
  (try (tg/set-my-commands! token (bot-menu-commands))
       (tel/log! {:level :info :id ::bot-menu-installed} "Telegram bot command menu installed")
       (catch Exception e
...
(defn start!
  ...
  (when (compare-and-set! running? false true)
    (let [token
      ...
      (.start t)
      (reset! poll-thread t)
      (tel/log! {:level :info :id ::started} "Telegram bot running"))))
...
(defn stop!
  ...
  (when (stop-polling!)
    (delete-pid-file!)
    (vis/close-all!)
    (tel/log! {:level :info :id ::stopped} "Telegram bot stopped")))
...
(defn -main
  ...
  (.addShutdownHook (Runtime/getRuntime)
                    (Thread. ^Runnable
                             (fn []
                               (tel/log! {:level :info :id ::shutdown} "Shutting down Telegram bot")
                               (stop!))
...
(defn- telegram-approve-command
  [{chat-id "chat-id" restart? "restart"} _residual]
  (vis/init-cli!)
...
(defn- telegram-channel-subcommands
  ...
  [{:cmd/name "approve"
    ...
    :cmd/args
    [{:name "chat-id" :kind :flag :type :string :required true :doc "Telegram chat id to approve."}
     {:name "restart"
      :kind :flag
      :type :boolean
...
(defn- telegram-only-channel? [ctx] (= :telegram (:channel/id ctx)))
  ...
  (or (:telegram/chat-id ctx)
      ...
      (when-let [db (:db-info ctx)]
        (when-let [sid (:session/id ctx)]
          (try (some-> (vis/db-get-session db sid)
                       :external-id)
               (catch Throwable _ nil))))))

(defn- argv-arg [ctx] (str/join " " (:command/argv ctx)))

(defn- ok-ir [ir] {:slash/status :ok :slash/body ir})

(defn- ok-msg
  [{:keys [message reply-markup]}]
  (cond-> {:slash/status :ok :slash/body message}
    reply-markup
    (assoc :slash/data {:reply-markup reply-markup})))
...
(def ^:private telegram-menu-slash-specs
  [{:slash/name "help"
   ...
   {:slash/name "status"
    ...
    :slash/availability-fn telegram-only-channel?
    :slash/run-fn (fn [ctx]
                    (ok-ir (command-status (ctx-chat-id ctx))))}
   {:slash/name "model"
   ...
   {:slash/name "models"
    ...
    :slash/availability-fn telegram-only-channel?
    :slash/run-fn (fn [ctx]
                    (ok-msg (command-models (argv-arg ctx))))}
   {:slash/name "reasoning"
    :slash/doc "Show or set reasoning effort"
    :slash/availability-fn telegram-only-channel?
    :slash/run-fn (fn [ctx]
                    (ok-ir (command-reasoning (ctx-chat-id ctx) (argv-arg ctx))))}
   {:slash/name "verbosity"
    :slash/doc "Show or set Codex verbosity"
    :slash/availability-fn telegram-only-channel?
    :slash/run-fn (fn [ctx]
                    (ok-ir (command-verbosity (ctx-chat-id ctx) (argv-arg ctx))))}
   ...
   {:slash/name "voice"
    ...
    :slash/availability-fn telegram-only-channel?
    :slash/run-fn (fn [ctx]
                    (ok-msg (command-voice (ctx-chat-id ctx) (argv-arg ctx))))}
   {:slash/name "cancel"
    :slash/doc "Cancel current request"
    :slash/availability-fn telegram-only-channel?
    :slash/run-fn (fn [ctx]
                    (ok-ir (command-cancel (ctx-chat-id ctx))))}
   {:slash/name "restart"
    :slash/doc "Restart the bot in a fresh JVM"
    :slash/availability-fn telegram-only-channel?
    :slash/run-fn (fn [_ctx]
                    (ok-ir (command-restart)))}
   {:slash/name "export"
    :slash/doc "Export session (Markdown + styled HTML attachment)"
    :slash/availability-fn telegram-only-channel?
    :slash/run-fn (fn [ctx]
                    (ok-ir (command-export (ctx-chat-id ctx))))}
   {:slash/name "clear"
    :slash/doc "Clear this session and start fresh"
    :slash/availability-fn telegram-only-channel?
    :slash/run-fn (fn [ctx]
                    (ok-ir (command-clear (ctx-chat-id ctx))))}])
...
(vis/register-extension!
  (vis/extension {:ext/name "channel-telegram"
