...
(defdescribe live-bubble-html-test
  ...
  (it "thinking-only state adds the blockquote"
    (let [live-bubble-html (p 'live-bubble-html)
          out (live-bubble-html {:thinking-acc "reasoning…"})]
...
(defdescribe throttle-test
  (it "flush bypasses every check"
    (let [should-edit? (p 'should-edit?)]
      (expect (true? (should-edit? {:last-edit-ms (System/currentTimeMillis)
                                    :backoff-ms 0
                                    :pending-html "x"}
                       "x" true)))))
  ...
  (it "identical text never triggers an edit"
    (let [should-edit? (p 'should-edit?)]
      (expect (false? (should-edit? {:last-edit-ms 0
                                     :backoff-ms 0
                                     :pending-html "x"}
                        "x" false)))))
  ...
  (it "within throttle window: small delta skipped"
    (let [should-edit? (p 'should-edit?)
      ...
      (expect (false? (should-edit? {:last-edit-ms now
                                     :backoff-ms 0
                                     :pending-html "abc"}
                        "abcd" false)))))
  ...
  (it "within throttle window: ≥40-char delta triggers edit"
    (let [should-edit? (p 'should-edit?)
          now          (System/currentTimeMillis)]
      (expect (true? (should-edit? {:last-edit-ms now
                                    :backoff-ms 0
                                    :pending-html "abc"}
                       (str "abc" (apply str (repeat 60 "x"))) false)))))
  ...
  (it "after throttle window: edits on any change"
    (let [should-edit? (p 'should-edit?)
          old          (- (System/currentTimeMillis) 5000)]
      (expect (true? (should-edit? {:last-edit-ms old
                                    :backoff-ms 0
                                    :pending-html "abc"}
                       "abcd" false)))))
  ...
  (it "honors backoff (locks past default throttle)"
    (let [should-edit? (p 'should-edit?)
          now          (System/currentTimeMillis)]
      (expect (false? (should-edit? {:last-edit-ms (- now 1500)
                                     :backoff-ms 5000
                                     :pending-html "abc"}
                        "abcd" false))))))
...
(defdescribe cancel-keyboard-test
  (it "produces correct callback_data and label"
    (let [cancel-keyboard (p 'cancel-keyboard)
          kb              (cancel-keyboard 12345)]
