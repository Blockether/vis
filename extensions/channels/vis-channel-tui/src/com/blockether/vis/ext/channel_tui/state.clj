    ...
    (let [visible-text (input/expand-paste-placeholders text (:pastes db))
      ...
      {:db (-> db
       ...
       :fx [[:rlm-turn workspace-id (:conversation db) agent-text token
             reasoning-level extra-body turn-features client-turn-id]]})))
...
(reg-fx :rlm-turn
  (fn [workspace-id conversation text token reasoning-level extra-body turn-features client-turn-id]
