...
(defn detect-capabilities
  ...
  ([env]
   (let [g          (fn [k] (some-> (get env k) str/lower-case))
         term-prog  (or (g "TERM_PROGRAM") "")
         term       (or (g "TERM") "")]
     (cond
       (or (get env "TMUX") (str/starts-with? term "tmux"))   {:images nil}
       (str/starts-with? term "screen")                       {:images nil}
       (or (get env "KITTY_WINDOW_ID") (= term-prog "kitty")) {:images :kitty}
       (or (= term-prog "ghostty") (str/includes? term "ghostty")
         (get env "GHOSTTY_RESOURCES_DIR"))                   {:images :kitty}
       (or (get env "WEZTERM_PANE") (= term-prog "wezterm"))  {:images :kitty}
       (or (= term-prog "warpterminal") (get env "WARP_SESSION_ID")
         (get env "WARP_TERMINAL_SESSION_UUID"))              {:images :kitty}
...
(defn set-cell-dimensions! [w h]
  (when (and (pos? (long w)) (pos? (long h)))
    (reset! cell-dims {:w (long w) :h (long h)})))
...
(defn- u32-be ^long [^bytes b ^long i]
  (+ (bit-shift-left (u8 b i) 24) (bit-shift-left (u8 b (+ i 1)) 16)
    (bit-shift-left (u8 b (+ i 2)) 8) (u8 b (+ i 3))))
(defn- u32-le ^long [^bytes b ^long i]
  (+ (u8 b i) (bit-shift-left (u8 b (+ i 1)) 8)
    (bit-shift-left (u8 b (+ i 2)) 16) (bit-shift-left (u8 b (+ i 3)) 24)))
(defn- ascii ^String [^bytes b ^long off ^long n]
  (String. b (int off) (int n) "US-ASCII"))

(defn- png-dims [^bytes b]
  (when (>= (alength b) 24)
    {:w (u32-be b 16) :h (u32-be b 20)}))

(defn- jpeg-dims [^bytes b]
  (when (>= (alength b) 4)
    (loop [off 2]
...
(defn- gif-dims [^bytes b]
  (when (>= (alength b) 10)
    {:w (u16-le b 6) :h (u16-le b 8)}))

(defn- webp-dims [^bytes b]
  (when (>= (alength b) 30)
    (let [chunk (ascii b 12 4)]
      (cond
        (= chunk "VP8 ") {:w (bit-and (u16-le b 26) 0x3fff)
                          :h (bit-and (u16-le b 28) 0x3fff)}
        (= chunk "VP8L") (let [bits (u32-le b 21)]
                           {:w (inc (bit-and bits 0x3fff))
                            :h (inc (bit-and (bit-shift-right bits 14) 0x3fff))})
        (= chunk "VP8X") {:w (inc (+ (u8 b 24) (bit-shift-left (u8 b 25) 8)
                                    (bit-shift-left (u8 b 26) 16)))
                          :h (inc (+ (u8 b 27) (bit-shift-left (u8 b 28) 8)
                                    (bit-shift-left (u8 b 29) 16)))}
        :else nil))))

(defn- bmp-dims [^bytes b]
  (when (>= (alength b) 26)
    {:w (u32-le b 18) :h (Math/abs (int (u32-le b 22)))}))
...
(defn image-dimensions
  ...
  (try
    (case mime
      "image/png"  (png-dims b)
      "image/jpeg" (jpeg-dims b)
      "image/gif"  (gif-dims b)
      "image/webp" (webp-dims b)
      "image/bmp"  (bmp-dims b)
      nil)
    (catch Throwable _ nil)))

(defn- read-head ^bytes [^File f ^long n]
...
(defn probe-dimensions
  ...
  [path mime]
  (try
    (image-dimensions (read-head (File. (str path)) 4100) mime)
    (catch Throwable _ nil)))
...
(defn cell-size
  ...
  (let [{cw :w ch :h} @cell-dims
        max-cols (max 1 (long max-cols))
        max-rows (when max-rows (max 1 (long max-rows)))
        iw       (max 1 (long (or w 1)))
        ih       (max 1 (long (or h 1)))
        w-scale  (/ (double (* max-cols cw)) iw)
        h-scale  (if max-rows (/ (double (* max-rows ch)) ih) w-scale)
        scale    (min w-scale h-scale)
        cols     (long (Math/ceil (/ (* iw scale) (double cw))))
        rows     (long (Math/ceil (/ (* ih scale) (double ch))))]
    {:cols (max 1 (min max-cols cols))
     :rows (max 1 (if max-rows (min max-rows rows) rows))}))
...
(defn encode-kitty
  ...
  (let [params (cond-> ["a=T" "f=100" "q=2" "C=1"]
                 cols (conj (str "c=" cols))
                 rows (conj (str "r=" rows)))
        head   (str/join "," params)]
    (if (<= (count data) kitty-chunk)
      (str "\u001b_G" head ";" data "\u001b\\")
      (let [n (count data)]
        (loop [off 0 first? true acc (StringBuilder.)]
          (if (>= off n)
            (.toString acc)
...
(defn encode-iterm2
  ...
  (str "\u001b]1337;File=inline=1;width=" cols ";height=auto;preserveAspectRatio=1:"
    data "\u0007"))
...
(defn read-base64
  ...
  (try
    (let [f      (File. (str path))
          ...
          size   (.length f)
          cached (get @base64-cache (str path))]
...
(defn transcode->png-base64
  ...
  (try
    (let [f     (File. (str path))
          key   [(str path) (.lastModified f) (.length f) cols rows]
          hit   (get @png-transcode-cache key)]
      (or hit
...
(defn render-sequence
  ...
  (when-let [proto (images-protocol)]
    (case proto
      ...
      :kitty  (when-let [data (kitty-png-base64 path mime box)]
                (encode-kitty data box))
      ;; iTerm2 accepts any container format as-is.
      :iterm2 (when-let [data (read-base64 path)]
...
(defn probe-paste-image
  ...
  (try
    (when-let [{:keys [path media-type filename size size-label]}
