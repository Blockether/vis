      ...
      (let
        [^java.io.File dir
         ...
         (fn [{:keys [path tag]}]
           (let
             [p
              (ellipsize (str "  \u25cf " (abbreviate-home path)) (max 1 (- list-w (count tag) 2)))
           ...
           (when manager?
             (cond target-base?
                   ...
                   (reset! notice [dir-canon-str :error
                                   (str "Already allowed by " (abbreviate-home target-covered-by))])
                   :else
                   (try (let
                          [updated (if target-already?
           ...
           (when manager?
             (cond (nil? sid) (reset! notice
                   ...
                   (let
                     ...
                     (when chosen
                       (try (let [updated (vis/gateway-remove-filesystem-root! sid (:path chosen))]
                              (reset! ws-info updated)
                              (reset! wid (get updated "id"))
                              (reset! notice [dir-canon-str :ok
                                              (str "Removed " (abbreviate-home (:path chosen)))]))
                            (catch Throwable t
                              (reset! notice [dir-canon-str :error
                                              (or (ex-message t) (str t))]))))))))
        ...
        (dotimes [i hdr-h]
          (let
            [{:keys [kind text error?]} (nth header-rows i)
          ...
          (let [idx (+ (long @scroll) i)]
            (when (< idx total)
              (let
                ...
                (let
                  [label-fg (if up? (if selected? t/dialog-title-fg t/header-active-tab-accent) fg)]
