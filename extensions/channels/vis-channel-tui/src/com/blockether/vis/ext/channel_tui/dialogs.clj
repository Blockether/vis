        ...
        (let [key (read-modal-key! screen)]
          (when key
            (condp = (.getKeyType key)
              ...
              (recur))))))))
;;; ── Plan review dialog ──────────────────────────────────────────────────────
(def ^:private review-verdict-glyph
  "Pending-verdict glyphs. ✓/⊘ mirror the task panel's done/rejected
   glyphs (the verdict PREVIEWS the status the model will write); ✎
   marks a note-carrying row. All NARROW 1-cell glyphs — same
   constraint as the footer ● (VS-16 emoji would desync the grid)."
  {:approve "✓" :reject "⊘" :comment "✎"})
(def ^:private review-status-glyph
  "Status glyph for rows WITHOUT a pending verdict — same spellings as
   the F2 panel's task glyphs so the two surfaces read identically."
  {:candidate "◇" :done "✓" :doing "◐" :cancelled "✗" :rejected "⊘"})
(defn- review-row-label
  "One list row: `<glyph> <title>[ — <note>][ · <status>]`. The status
   tag rides only on NON-candidate rows — they are read-only context
   (already accepted / resolved), not reviewable proposals."
  [{:keys [title status candidate?]} v]
  (let [glyph (or (review-verdict-glyph (:verdict v))
                (when (some? (:note v)) "✎")
                (review-status-glyph status "○"))]
    (str glyph " " title
      (when-let [n (:note v)] (str " — " n))
      (when-not candidate? (str " · " (name (or status :todo)))))))
(defn- review-entries
  "Verdict entries for `vis/plan-review-message`, in PLAN order (the
   atom is a map — iteration order would scramble the review lines). A
   note with no explicit verdict submits as COMMENT."
  [plan verdicts]
  (into []
    (keep (fn [{:keys [key]}]
            (when-let [v (get verdicts key)]
              (let [verdict (or (:verdict v) (when (:note v) :comment))]
                (when verdict {:key key :verdict verdict :note (:note v)})))))
    plan))
(defn plan-review-dialog!
  "Antigravity-style structured review of a PROPOSED plan (`:candidate`
   steps). ↑/↓ move · a approve · r reject · Enter/c comment ·
   x clear · o overall note · s send · Esc cancel.

   Pure composer: nothing here touches `:session/tasks` — verdicts
   accumulate locally and `s` RETURNS the canonical `Plan review:`
   message (via `vis/plan-review-message`) for the caller to send as
   the next user message; the MODEL re-emits the plan in response.
   Returns nil when cancelled (or when the plan has no steps)."
  [^TerminalScreen screen tasks]
  (let [plan (vec (vis/plan-reviewable tasks))
        total (count plan)
        verdicts (atom {})
        overall (atom nil)
        selected (atom 0)
        win (atom 0)
        ;; Esc on the nested input keeps the OLD note (nil result);
        ;; an empty Enter explicitly CLEARS it (empty-string result).
        edit-note! (fn [k cur]
                     (when-some [s (text-input-dialog! screen "Step comment"
                                     "Feedback for this step:" :initial (str cur))]
                       (swap! verdicts update k
                         (fn [v] (let [v (assoc v :note (when-not (str/blank? s) s))]
                                   (if (and (nil? (:note v)) (= :comment (:verdict v)))
                                     (dissoc v :verdict)
                                     v))))))]
    (when (pos? total)
      (loop []
        (let [size (or (.doResizeIfNecessary screen) (.getTerminalSize screen))
              ...
              bounds (draw-dialog-chrome! g cols rows "Plan review" content-count)
              {:keys [left inner-w]} bounds
              {:keys [content-top content-h hint-row]} (dialog-layout bounds content-count)
              ;; Last two content rows are pinned: blank spacer + Overall.
              list-h (max 1 (- content-h 2))
              start (swap! win #(visible-window-start @selected % list-h total))
              visible (min total list-h)
              overall-row (+ content-top content-h -1)]
          (swap! selected #(clamp % 0 (dec total)))
          (dotimes [i visible]
            (let [idx (+ start i)
                  step (nth plan idx)]
              (draw-list-item! g left (+ content-top i) inner-w (= idx @selected)
                (review-row-label step (get @verdicts (:key step))))))
          (p/set-colors! g t/dialog-fg t/dialog-bg)
          (p/fill-rect! g (inc left) (dec overall-row) inner-w 1)
          (p/fill-rect! g (inc left) overall-row inner-w 1)
          (p/put-str! g (+ left 2) overall-row
            (ellipsize (str "Overall: " (or @overall "—")) (max 0 (- inner-w 2))))
          (draw-hint-bar! g left hint-row inner-w
            [["↑/↓" "move"] ["a" "approve"] ["r" "reject"] ["Enter" "comment"]
             ["o" "overall"] ["s" "send"] ["Esc" "cancel"]])
          (.setCursorPosition screen (p/cursor-pos 0 0))
          (.refresh screen Screen$RefreshType/DELTA)
          (let [key (read-modal-key! screen)
                step (nth plan (clamp @selected 0 (dec total)))
                k (:key step)
                ;; A verdict only applies to a live proposal; accepted /
                ;; resolved steps are read-only context rows.
                toggle! (fn [verdict]
                          (when (:candidate? step)
                            (swap! verdicts update k
                              (fn [v] (if (= verdict (:verdict v))
                                        (not-empty (dissoc v :verdict))
                                        (assoc v :verdict verdict))))))]
            (if (nil? key)
              (recur)
              (condp = (.getKeyType key)
                KeyType/Escape nil
                KeyType/ArrowUp (do (swap! selected #(clamp (dec %) 0 (dec total))) (recur))
                KeyType/ArrowDown (do (swap! selected #(clamp (inc %) 0 (dec total))) (recur))
                KeyType/Enter (do (when (:candidate? step) (edit-note! k (:note (get @verdicts k))))
                                (recur))
                KeyType/Character
                (let [c (Character/toLowerCase ^char (.getCharacter key))]
                  (cond
                    (= c \a) (do (toggle! :approve) (recur))
                    (= c \r) (do (toggle! :reject) (recur))
                    (= c \c) (do (when (:candidate? step) (edit-note! k (:note (get @verdicts k))))
                               (recur))
                    (= c \x) (do (swap! verdicts dissoc k) (recur))
                    (= c \o) (do (when-some [s (text-input-dialog! screen "Overall note"
                                                 "Feedback on the whole plan:" :initial (str (or @overall "")))]
                                   (reset! overall (when-not (str/blank? s) s)))
                               (recur))
                    (= c \s) (if-let [msg (vis/plan-review-message (review-entries plan @verdicts) @overall)]
                               msg
                               (do (vis/notify! "Nothing to send — add a verdict or a note first"
...
(defn directory-picker-dialog!
  "Browse the filesystem like a file explorer and manage this session's context
   ...
   A breadcrumb header shows the current directory. The list shows that
   directory's subfolders (navigable) and its files (dimmed, for context); type
   any text to incrementally filter the folder/file list and Backspace to
   delete. Row 0 is always `Open here` (open the browsed directory in a new
   session tab). When `:db-info` + `:workspace-id` are supplied the picker also
   mirrors the web rail's Context roots view: it lists the session's current
   roots (workspace base + extras), offers `Add this folder as a context root`,
   and lets you remove an extra root in place. `..` ascends; every other folder
   row descends into that subdirectory. Up/Down move, Enter activates the
   highlighted row, Tab / Right opens the current directory in a tab, Left
   ascends."
  [^TerminalScreen screen start-path & {:keys [db-info workspace-id]}]
  (let [path     (atom (dir-canon (java.io.File. ^String (or start-path "."))))
    ...
    (loop []
      (let [^java.io.File dir @path
            ...
            list-top (inc content-top)
            body-h   (clamp total 1 (max 1 (dec content-h)))
            _        (swap! selected #(clamp % 0 (max 0 (dec total))))
            _        (swap! scroll #(visible-window-start @selected % body-h total))
            ascend!  (fn [] (when up?
