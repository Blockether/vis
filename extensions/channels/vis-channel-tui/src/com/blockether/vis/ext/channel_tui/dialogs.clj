      ...
      (let [size (or (.doResizeIfNecessary screen) (.getTerminalSize screen))
        ...
        (dotimes [i n]
          (let [f        (nth fields i)
            ...
            (if focused?
              (reset! cursor-screen (draw-text-input-field! g (inc left) iy inner-w val @(:cursor st)))
              (if (str/blank? val)
                (do (p/set-colors! g t/dialog-hint t/dialog-bg)
  ...
  (let [text (atom (vec initial))
    ...
    (loop []
      (let [size (or (.doResizeIfNecessary screen) (.getTerminalSize screen))
            ...
            cursor-pos (draw-text-input-field! g (inc left) input-row inner-w display @cursor)]
        (when (seq logo-lines)
  ...
  (let [path     (atom (dir-canon (java.io.File. ^String (or start-path "."))))
    ...
    (loop []
      (let [^java.io.File dir @path
            ...
            rows-n   (.getRows size)
            g        (.newTextGraphics screen)
            bounds   (draw-dialog-chrome! g cols rows-n title
            ...
            (if manager?
              (vec (concat
                     [{:kind :blank :text ""} {:kind :section :text (str "FILESYSTEM (" (count roots) ")")}]
                     (if (seq roots)
                       (concat
