...
(defonce ^:private ^AtomicLong fmt-cache-gen (AtomicLong.))

(defmacro with-cache
  "Generation-guarded double-checked read/compute/put on an access-order
   `LinkedHashMap` `m` (LRU), keyed by `k`. `gen` is the `AtomicLong` that
   `clear-cache!` bumps under `m`'s monitor. On a HIT the body is never touched
   and nothing is allocated (one locked `.get`). On a MISS `body` runs OUTSIDE
   the lock; the value is stored only if no `.clear` bumped `gen` since the miss -
   otherwise it is returned but NOT cached, so an in-flight compute can never
   resurrect an entry a concurrent invalidate meant to kill. The gen is read/
       ...
       (let [g0#
         ...
         (locking m#
           (if (== g0# (.get ^AtomicLong gen#))
             (or (.get ^LinkedHashMap m# k#) (do (.put ^LinkedHashMap m# k# v#) v#))
             v#))))))

(defn clear-cache!
  "Clear `m` and bump `gen` under `m`'s monitor, so any compute in flight since
   its miss is refused at put time instead of resurrecting a now-stale entry
   (see `with-cache`). Bumping under the SAME monitor the `with-cache` put phase
   reads under is what makes the guard race-free."
  [^LinkedHashMap m ^AtomicLong gen]
  (locking m (.clear m) (.incrementAndGet gen)))
...
(defn invalidate-cache!
  ...
  []
  (clear-cache! fmt-cache fmt-cache-gen))
...
(defn- live-throttled-cached*
  "Like `cached*`, but debounces the LIVE progress body. When
   `live-body-throttle-ms` is positive and the content key `k` changed STRICTLY
   after — but within that many ms of — the last full re-projection, reuse the
   previous body instead of re-walking the growing iteration. The SAME content
   key (a pure spinner tick — no new content) or a 0 throttle falls straight
   through to `cached*`, so a quiescent stream still recomputes immediately once
   the window lapses. A non-advancing clock (`now-ms <= at`, e.g. a frozen test
                ...
                (let [in-answer? (> i (long answer-start))
                  ...
                  (cond
                    ...
                    (let [raw (subs line 1)
                      ...
                      (case (:kind meta)
                        ...
                        (let [diff-kind (:diff-kind meta)
                          ...
                          (p/set-colors! g diff-fg diff-bg)
                    ...
                    (let [nested-code? (and in-answer? (:list-nested-code? meta))
                      ...
                      (paint-ansi-line! g
                                        (+ (long x) (long code-block-h-pad) nlx)
                                        y
                                        (subs line (min (count line) (+ 1 nlx)))
                                        cfg
                                        cbg))
...
(defn message-detail-expansions-key
  ...
  (cond ;; Fast path: no disclosure state at all -> the key is ALWAYS [] regardless
        ;; of message. This is the streaming-common case (nothing toggled), and it
        ;; is called ~N times per layout tick, so short-circuiting here avoids
...
(defn- activity-diffs [row] (filterv #(= "diff" (activity-evidence-kind %)) (:evidence row)))

(defn- activity-step-delta
  "What a step's patches added and removed, summed over every diff it left."
  [row]
  (reduce (fn [total evidence]
            (-> total
                (update :additions + (long (or (:additions evidence) 0)))
                (update :deletions + (long (or (:deletions evidence) 0)))))
...
(defn- activity-file-rows
  ...
  [resources diffs]
  (let [by-path
        (into {} (map (juxt #(str (:text %)) identity)) diffs)

        named
        (into #{} (map #(str (:id %))) resources)]

    (into (mapv (fn [resource]
                  (let [id (str (:id resource))]
                    {:id id :diff (get by-path id)}))
                resources)
          (comp (remove #(contains? named (str (:text %))))
                (map (fn [diff]
                       {:id (str (:text diff)) :diff diff})))
          diffs)))

(defn- activity-path-cells
  "A PATH THAT KEEPS ITS NAME.

   `ellipsize-cols` eats the END of a string, which in a monorepo is the only part
   that differs: eight rows of `src/com/blockether/vis/internal/…` are eight identical
   rows. The DIRECTORY is what gives way, so the file being looked for is always
   whole, and the caller paints the two halves in two inks because the filename is the
  ...
  (let [shown
        ...
        directory
        (if cut (subs shown 0 (inc (long cut))) "")

        file-name
        (if cut (subs shown (inc (long cut))) shown)]

    (cond (<= (p/display-width shown) max-w) {:text shown :dir directory :name file-name}
          (>= (p/display-width file-name) max-w) (let [clipped (ellipsize-cols file-name max-w)]
                                                   {:text clipped :dir "" :name clipped})
          :else (let [room
                      (max 0 (- max-w (p/display-width file-name) 1))

                      kept
                      (str "…" (subs directory (max 0 (- (count directory) room))))]
  ...
  (let [rows
        ...
        (fn [row-id resources diffs ^long col]
          (let [lead
                ...
                entries
                (activity-file-rows resources diffs)

                hidden
                (max 0 (- (count entries) activity-files-shown))
                ...
                (mapcat
                  (fn [{:keys [id diff]}]
                    (let [file-key
                          ...
                          cells
                          (activity-path-cells id (max 1 (- width col 2)))

                          head
                          {:line (str activity-marker
                                      (ellipsize-cols (str lead mark " " (:text cells)) width))
                           :meta (merge meta-base
                                        (cond-> {:kind :activity-file
                                                 :item-id row-id
                                                 :mark mark
                                                 :mark-col col
                                                 :path-col (+ col 2)
                                          ...
                                          diff
                                          (assoc :node-id
                                            (str node-id ":" file-key) :collapsed?
                                            (not open?))))}]
