...
(defn clamp-index
  ...
  (let [total (long (max 0 (or total 0)))]
    (if (zero? total)
      0
      (max 0 (min (dec total) (long (or idx 0)))))))
...
(defn move-index
  ...
  (let [total (long (max 0 (or total 0)))]
    (if (zero? total)
      0
      (mod (+ (long (or idx 0)) (long (or delta 0))) total))))
...
(defn- command-args
  [cmd]
  (or (:args cmd) (:cmd/args cmd) []))

(defn- arg-name
  [arg]
  (str (or (:name arg) (:cmd.arg/name arg))))
...
(defn- arg-token
  ...
  (let [n (arg-name arg)
        ...
        token (case kind
                :flag (if (= :boolean type)
                        (flag-token arg)
                        (str (flag-token arg) " <" n ">"))
                :positional (str "<" n ">")
                (str "<" n ">"))]
    ...
    (cond
      ...
      (loop [qi 0 ci 0 gaps 0 last-ci -1]
        (cond
          ...
          (recur (inc qi) (inc ci)
            (+ gaps (if (neg? last-ci) ci (- ci last-ci 1))) ci)
          :else
          (recur qi (inc ci) gaps last-ci))))))
...
(defn selected-suggestion
  [suggestions]
  (or (some #(when (:slash/selected? %) %) suggestions)
    (first suggestions)))

(defn completion-text
  [suggestion]
  (let [args (str/trim (or (:slash/args suggestion) ""))]
    (str "/" (:slash/name suggestion)
      (if (str/blank? args)
        " "
        (str " " args)))))
