...
(deftest geometry-positions-thumb-by-fraction
  (let [g #(scrollbar/geometry 100 20 %)]
    (testing "scroll=0 → thumb at top"
      (is (= 0 (:thumb-top-rel (g 0)))))
    (testing "scroll=80 (max) → thumb at bottom of track"
      (is (= 19 (:thumb-top-rel (g 80)))))
    (testing "nil scroll auto-pins to bottom"
      (is (= 19 (:thumb-top-rel (g nil)))))
    (testing "scroll=40 → thumb in middle of free track"
      (is (= 9 (:thumb-top-rel (g 40)))))
    (testing "thumb stays one cell tall (Terminal.app ladder bug)"
      (is (= 1 (:thumb-h (g 0)))))
    (testing "max-scroll is total - inner"
      (is (= 80 (:max-scroll (g 0)))))))
...
(defn- mouse [type x y]
  (MouseAction. type 1 (TerminalPosition. (int x) (int y))))

(defn- mouse-with-count [type x y n]
...
(deftest on-track-and-on-thumb-honour-bar-column
  (let [bar  {:col 50 :top 10 :track-h 8}
        geom (scrollbar/geometry 100 8 nil)]
...
(deftest scroll-from-mouse-y-maps-track-to-scroll
  (testing "Track top maps to scroll 0"
    (is (= 0 (scrollbar/scroll-from-mouse-y 10 10 8 100 20))))
