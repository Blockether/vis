...
(defdescribe
  ...
  (let [ctx
    ...
    (it "git-command-parts recovers subcommand + failure from the summary note"
        (let [ok
              (git-command-parts (git-form "status --short" nil))

              ex
              (git-command-parts (git-form "push origin main (exit 1)" nil))

              to
              (git-command-parts (git-form "commit -m foo (timed out)" nil))]
    ...
    (it
      "git-command-parts lifts a commit subject from its message blockquote"
      (let
        [c
         (git-command-parts
           (git-form
    ...
    (it "a narrated head still groups; its narration renders ABOVE the band"
        ...
        (let [pairs
              [[0 {:forms [(git-form "add -A" nil)] :thinking "let me commit"}] (gi 1 "commit -m x")
               (gi 2 "push")]

              out
              (entry-text (render-iteration-entries pairs iter-fn false true ctx))

              band-ix
              (first (keep-indexed (fn [i l]
                                     (when (str/includes? l "3 commands") i))
                                   out))

              normal-ix
              (first (keep-indexed (fn [i l]
                                     (when (str/includes? l "NORMAL#0") i))
                                   out))]
    ...
    (it "an INTERIOR narrated call breaks the run (never floats above the band)"
        (let [pairs [(gi 0 "add") [1 {:forms [(git-form "commit" nil)] :thinking "hmm"}]
                     (gi 2 "push")]]
    ...
    (it "collapsed band shows a chip per command; expanded shows each `$` row"
        (let [forms
              [(git-form "add -A" nil) (git-form "push" "```\nmain -> main\n```")]

              node-id
              (@#'render/detail-node-id
               {:session-turn-id "t" :iteration-number 5 :section :iteration :kind :git-group})

              collapsed
              (entry-text (git-group-entries forms (assoc ctx :iteration-number 5)))

              expanded
              (entry-text (git-group-entries forms
                                             (assoc ctx
                                               :iteration-number 5
          ...
          (expect (every? #(str/starts-with? % "  ") (filter #(str/includes? % "$ ") expanded)))))
    (it "the band opens with a leading breathe row (padding 1 up, like an op-card)"
        (let [entries
              (git-group-entries [(git-form "commit -m x" nil) (git-form "push" nil)]
    ...
    (it
      "a GIT band drops a preceding iteration-pad gap so the seam is one blank"
      (let [ip-ch
            (first (str @#'render/iteration-pad-marker))

            rm-ch
            (first (str @#'render/result-marker))
            ...
            pairs
            [(py 0) (gi 1 "commit -m x") (gi 2 "push")]

            out
            (render-iteration-entries pairs pad-fn false true ctx)

            band-ix
            (first (keep-indexed (fn [i e]
                                   (when (str/includes? (str (:line e)) "commands") i))
                                 out))

            before
            (str (:line (nth out (dec band-ix))))]
...
(defdescribe
  ...
  (let [ctx
    ...
    (it "groupable-iteration-forms tags git AND rg, rejects other tools + impure steps"
        ...
        (expect (= "rg"
                   (first (groupable-iteration-forms {:forms [(rg-form "`x` · 1 file" nil)]}))))
        (expect (nil? (groupable-iteration-forms {:forms [{:vis/tool-name "ls"
                                                           :result-summary "x"}]})))
        ;; Narration no longer disqualifies grouping — it renders above the band.
        (expect (some? (groupable-iteration-forms {:assistant-prose "hi"
                                                   :forms [(rg-form "`x` · 1 file" nil)]}))))
    (it "rg-command-parts drops the query prefix for the collapsed chip, keeps the full summary"
        (let [content
              (rg-command-parts (rg-form "`a` OR `b` · 6 hits in 1 file"
                                         "\n`c.clj`\n\n```\n 1: x\n```"))

              files
              (rg-command-parts (rg-form "`q` · 4 files" "\n```\n  a\n```"))]
    ...
    (it "a parallel GATHER of rg calls in ONE iteration still collapses into a band"
        ...
        (let [pairs
              [[0 {:forms [(rg-form "`a` · 4 files" nil) (rg-form "`b` · 2 hits in 1 file" nil)]}]]]
          (expect (= 1 (rg-band-count pairs)))
          (expect (= 0 (normal-count pairs)))))
    ...
    (it "adjacent git-run and rg-run stay TWO separate bands, never merged"
        (let [pairs [(gf 0 "add -A") (gf 1 "commit -m x") (ri 2 "`a` · 1 file" nil)
                     (ri 3 "`b` · 2 files" nil)]]
    ...
    (it "collapsed band shows count chips; expanded shows each search's full query + matches"
        (let [forms
              [(rg-form "`paste` OR `Pasted` · 4 files" "\n```\n  a.clj\n```")
               (rg-form "`render-find` · 6 hits in 1 file" "\n`core.clj`\n\n```\n 12: foo\n```")]

              node-id
              (@#'render/detail-node-id
               {:session-turn-id "t" :iteration-number 5 :section :iteration :kind :rg-group})

              collapsed
              (entry-text (rg-group-entries forms (assoc ctx :iteration-number 5)))

              expanded
              (entry-text (rg-group-entries forms
                                            (assoc ctx
                                              :iteration-number 5
                                              :detail-expansions {["s1" node-id] true})))]
    ...
    (it "the rg band opens with a leading breathe row, like an op-card"
        (let [texts (entry-text (rg-group-entries [(rg-form "`a` · 1 file" nil)
                                                   (rg-form "`b` · 2 files" nil)]
                                                  (assoc ctx :iteration-number 9)))]
          (expect (str/blank? (first texts)))
...
(defn- del-form
  [summary]
  {:vis/tool-name "delete"
   :success? true
   :code ""
   :result-summary summary
   :result-render nil
   :tool-color-role :tool-color/delete
   :result {}})

(defdescribe
  delete-band-grouping-test
  ...
  (let [ctx
    ...
    (it "groupable-iteration-forms tags a lone delete iteration"
        (expect (= "delete"
                   (first (groupable-iteration-forms {:forms [(del-form "deleted `a.clj`")]}))))
        (expect (nil? (groupable-iteration-forms {:forms [{:vis/tool-name "ls"
                                                           :result-summary "x"}]}))))
    (it "delete-command-parts shows the basename chip, keeps the full summary"
        (let [p
              (delete-command-parts (del-form "deleted `src/com/blockether/vis/ports.clj`"))

              n
              (delete-command-parts (del-form "nothing to delete at `gone.clj`"))]
    ...
    (it "collapsed band shows a basename chip per delete; expanded shows each full path"
        (let [forms
              [(del-form "deleted `a/ports.clj`") (del-form "deleted `a/ports_test.clj`")]

              node-id
              (@#'render/detail-node-id
               {:session-turn-id "t" :iteration-number 5 :section :iteration :kind :delete-group})

              collapsed
              (entry-text (delete-group-entries forms (assoc ctx :iteration-number 5)))

              expanded
              (entry-text (delete-group-entries forms
                                                (assoc ctx
                                                  :iteration-number 5
                                                  :detail-expansions {["s1" node-id] true})))]
    ...
    (it "the delete band opens with a leading breathe row, like an op-card"
        (let [texts (entry-text (delete-group-entries [(del-form "deleted `a.clj`")
                                                       (del-form "deleted `b.clj`")]
                                                      (assoc ctx :iteration-number 9)))]
          (expect (str/blank? (first texts)))
...
(defdescribe
  ...
  (let [ctx
    ...
    (it "groupable-iteration-forms tags a lone find_files iteration"
        (expect (= "find_files"
                   (first (groupable-iteration-forms {:forms [(find-form "2 matches for \"x\""
                                                                         nil)]}))))
        (expect (nil? (groupable-iteration-forms {:forms [{:vis/tool-name "ls"
                                                           :result-summary "x"}]}))))
    (it "find-command-parts rides the quoted query on the chip, keeps the full summary"
        (let [q
              (find-command-parts (find-form
                                    "20 matches for \"tui render tool\" · terms: render, tool"
                                    "\n```\n  a.clj\n```"))

              u
              (find-command-parts (find-form "3 matches" nil))]
    ...
    (it "collapsed shows the two queries; expanded restores each full summary + paths"
        (let [forms
              [(find-form "20 matches for \"tui render tool\" · terms: render, tool, tui"
                          "\n```\n  a/render.clj\n```")
              ...
              node-id
              (@#'render/detail-node-id
               {:session-turn-id "t" :iteration-number 5 :section :iteration :kind :find-group})

              collapsed
              (entry-text (find-group-entries forms (assoc ctx :iteration-number 5)))

              expanded
              (entry-text (find-group-entries forms
                                              (assoc ctx
                                                :iteration-number 5
                                                :detail-expansions {["s1" node-id] true})))]
    ...
    (it "the find band opens with a leading breathe row, like an op-card"
        (let [texts (entry-text (find-group-entries [(find-form "1 match for \"a\"" nil)
                                                     (find-form "2 matches for \"b\"" nil)]
                                                    (assoc ctx :iteration-number 9)))]
          (expect (str/blank? (first texts)))
          (expect (str/includes? (second texts) "2 searches"))))))
...
(defdescribe
  occurrences-band-grouping-test
  ...
  (let [ctx
    ...
    (it "groupable-iteration-forms tags a lone occurrences iteration"
        (expect (= "occurrences"
                   (first (groupable-iteration-forms
                            {:forms [(occ-form "4 · 1 def in 3 files of `foo`" nil)]}))))
        (expect (nil? (groupable-iteration-forms {:forms [{:vis/tool-name "ls"
                                                           :result-summary "x"}]}))))
    (it "occurrences-command-parts rides the traced symbol on the chip, keeps the full summary"
        (let [q
              (occurrences-command-parts (occ-form "4 · 1 def in 3 files of `ensure-repl-for-dir!`"
                                                   "\n```\n  used: 239\n```"))

              u
              (occurrences-command-parts (occ-form "3 · 0 defs in 1 file" nil))]
    ...
    (it "two consecutive occurrences iterations collapse into ONE band"
        (let [pairs [(oc 0 "4 · 1 def in 3 files of `a`" nil)
                     (oc 1 "6 · 1 def in 3 files of `b`" nil)]]
    ...
    (it "a non-occurrences step between two lookups breaks the run"
        (let [pairs [(oc 0 "4 · 1 def in 3 files of `a`" nil) (py 1)
                     (oc 2 "6 · 1 def in 3 files of `b`" nil)]]
          (expect (= 0 (occ-band-count pairs)))
          (expect (= 3 (normal-count pairs)))))
    (it "collapsed shows the two traced symbols; expanded restores each full summary + uses"
        (let [forms
              [(occ-form "4 · 1 def in 3 files of `ensure-repl-for-dir!`" "\n```\n  used: 239\n```")
              ...
              (@#'render/detail-node-id
               {:session-turn-id "t"
                :iteration-number 5
                :section :iteration
                :kind :occurrences-group})

              collapsed
              (entry-text (occurrences-group-entries forms (assoc ctx :iteration-number 5)))

              expanded
              (entry-text (occurrences-group-entries forms
                                                     (assoc ctx
                                                       :iteration-number 5
                                                       :detail-expansions {["s1" node-id] true})))]
    ...
    (it "the occurrences band opens with a leading breathe row, like an op-card"
        (let [texts (entry-text (occurrences-group-entries
                                  [(occ-form "4 · 1 def in 3 files of `a`" nil)
                                   (occ-form "6 · 1 def in 3 files of `b`" nil)]
                                  (assoc ctx :iteration-number 9)))]
          (expect (str/blank? (first texts)))
          (expect (str/includes? (second texts) "2 lookups"))))))

(defdescribe
  edit-band-grouping-test
  ...
  (let [ctx
    ...
    (it "groupable-iteration-forms tags a lone struct_patch/patch/write iteration"
        (expect (= "struct_patch"
                   (first (groupable-iteration-forms {:forms [(ef "struct_patch" base)]}))))
    ...
    (it "the band label is honest about which edit tool ran"
        (let [sp
              (entry-text (edit-group-entries "struct_patch"
                                              ...
                                              (assoc ctx :iteration-number 5)))

              wr
              (entry-text (edit-group-entries "write"
                                              [(ef "write" base) (ef "write" base)]
                                              (assoc ctx :iteration-number 6)))]
    ...
    (it "collapsed shows each basename; expanded restores each diff"
        (let [forms
              ...
              node-id
              (@#'render/detail-node-id
               {:session-turn-id "t" :iteration-number 7 :section :iteration :kind :edit-group})

              collapsed
              (entry-text (edit-group-entries "struct_patch" forms (assoc ctx :iteration-number 7)))
...
(defdescribe
  ...
  (let [ctx
        ...
        cat-form
        (fn [path a b]
          (let [rows (for [ln (range a (inc b))]
                       (str (format "%5s" ln) "  code-" ln))]
            {:vis/tool-name "cat"
             :success? true
             :code ""
