...
(defdescribe live-vs-restore-order-test
  (it "LIVE re-slots out-of-order arrivals back into loop order (not arrival order)"
    (let [{:keys [on-chunk get-timeline]} (progress/make-progress-tracker)
          chunk (fn [pos code silent?]
                  {:phase :form-result :iteration 1 :position pos
                   :code code :result (str "r" pos) :silent? silent?})]
  ...
  (it "RESTORE keeps the DB forms-vector order"
    ...
    (let [it       {:id "t1" :code "" :produced-answer? false
                    :forms [{:src "(f0)" :result "r0"}
                            {:src "(f1-silent)" :result "r1" :vis/silent true}
                            {:src "(f2)" :result "r2"}]}
          restored (it->iteration-entry {:produced-answer? false :last-iteration-id nil} it)]
  ...
  (it "LIVE order == RESTORE order == loop order (no divergence)"
    (let [{:keys [on-chunk get-timeline]} (progress/make-progress-tracker)
      ...
      (on-chunk (chunk 0 "(f0)" false))
      (on-chunk (chunk 1 "(f1-silent)" true))
      (let [live     (codes (first (get-timeline)))
            it       {:id "t1" :code "" :produced-answer? false
                      :forms [{:src "(f0)" :result "r0"}
                              {:src "(f1-silent)" :result "r1" :vis/silent true}
                              {:src "(f2)" :result "r2"}]}
            restored (codes (it->iteration-entry {:produced-answer? false :last-iteration-id nil} it))]
