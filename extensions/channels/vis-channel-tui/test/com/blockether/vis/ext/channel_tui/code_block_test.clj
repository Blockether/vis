...
(defn- content-rows
  ...
  [lines]
  (->> lines
    (map (fn [line] (apply str (map :text (:runs line)))))
    (remove #(= "" %))
    vec))
...
(defdescribe tui-code-block-wrap-test
  (it "soft-folds a wide one-line value to the budget when :wrap? is set"
    ...
    (let [wide  (apply str (repeat 100 "ab"))
          width 40
          lines (code-block->lines [:code {:lang "clojure" :wrap? true} wide] width {})
          rows  (content-rows lines)]
  ...
  (it "keeps a wide value verbatim (one overflowing row) without :wrap?"
    (let [wide  (apply str (repeat 100 "ab"))
          width 40
          lines (code-block->lines [:code {:lang "clojure"} wide] width {})
          rows  (content-rows lines)]
      (expect (= 1 (count rows)))
  ...
  (it "preserves leading indentation when folding"
    (let [src   "    {\"message\": \"Fix live tab title update in the TUI\", \"all\": true}"
          width 24
          lines (code-block->lines [:code {:lang "clojure" :wrap? true} src] width {})
          rows  (content-rows lines)]
      (expect (every? #(<= (p/display-width %) width) rows))
