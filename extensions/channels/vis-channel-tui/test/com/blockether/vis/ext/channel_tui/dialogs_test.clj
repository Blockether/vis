  ...
  (it "file picker can hand the selected path to the shared external opener"
    (let [calls (atom [])
      ...
      (with-redefs [opener/open! (fn [path]
        ...
        (expect (= {:status :ok :target "deps.edn"}
                  @(open-picker-item! {:path "deps.edn"})))
        (expect (= ["deps.edn"] @calls)))))

  (it "file picker opener preserves active workspace root across worker thread"
    (let [seen-root (promise)
      ...
      (with-redefs [opener/open! (fn [path]
        ...
        (binding [workspace/*workspace-root* "/tmp/vis-dialog-ws"]
          @(open-picker-item! {:path "deps.edn"}))
        (expect (= "/tmp/vis-dialog-ws" (deref seen-root 1000 ::timeout)))))))
...
(defdescribe navigator-row-model-test
...
(defdescribe file-picker-table-test
  (it "file picker renders a full boxed table with headers and side borders"
    (let [table-widths (var-get #'dlg/file-picker-table-widths)
          ...
          top-line     (border-line widths :top)
          header-line  (row-line widths headers)]
      (expect (= 72 (count top-line)))
      (expect (= \┌ (first top-line)))
      (expect (= \┐ (last top-line)))
      (expect (= \│ (first header-line)))
      (expect (= \│ (last header-line)))
      (expect (re-find #"Status.*File.*Size.*Modified" header-line))))

  (it "status is rendered as a word, not a bracket badge"
    (let [table-widths (var-get #'dlg/file-picker-table-widths)
          boxed-row    (requiring-resolve 'com.blockether.vis.ext.channel-tui.table/boxed-row-line)
          row-line     (fn [widths cells] (boxed-row widths cells (repeat :left)))
  ...
  (it "scrollbar geometry sanity (file picker drives `scrollbar/geometry` per frame)"
