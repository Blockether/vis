...
(defdescribe tui-channel-registration-test
  ...
  (it "loads the screen ns when --session-id resolves through the gateway facade"
    (let [resolve-calls (atom [])
      ...
      (with-redefs [clojure.core/requiring-resolve
                    ...
                    vis/shutdown!  (fn [] nil)
                    vis/gateway-soul           (fn [id] (swap! soul-calls conj id) {:id id :channel "tui"})
