...
(defn- toggle-region [row col session-id node-id collapsed?]
  {:bounds {:row row :col col :width 8}
   :kind :toggle-details :session-id session-id :node-id node-id :collapsed? collapsed?})

(defdescribe assign-labels-test
  (it "labels only :toggle-details regions, home row first, in paint order"
    (let [pairs (cr/assign-labels
                  [(region 1 0 5 "https://a")
                   (toggle-region 3 4 "s" "n1" true)
                   (toggle-region 6 4 "s" "n2" false)])]
      (expect (= [["a" "n1"] ["s" "n2"]]
                (mapv (fn [[l r]] [l (:node-id r)]) pairs)))))
  ...
  (it "caps at the alphabet length; extras stay unlabeled"
    (let [many  (mapv #(toggle-region % 0 "s" (str "n" %) true)
                  (range (+ 3 (count cr/label-alphabet))))
          pairs (cr/assign-labels many)]
