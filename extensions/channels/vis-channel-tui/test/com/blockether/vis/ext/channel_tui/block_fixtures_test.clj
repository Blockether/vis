...
(defn- single-form-entry
  ...
  (cond-> {:position 0
           :code     (:code form)
           :forms    [(cond-> form
                        error (assoc :success? false :error error))]}
...
(def ^:private git-fence-stdout
  "STATUS  3 files\nADD  .\nCOMMIT  1 file\nPUSH  main")

(defn- git-fence-entry []
  (single-form-entry
    (ok-form 0 "t6/i1/f1" git-fence-code :mutation git-fence-stdout)))
...
(defn- def-bind-entry []
  (single-form-entry
    (ok-form 0 "t3/i1/f1" "(def x (cat \"x\"))" :observation "CAT  x")))
...
(defn- plain-value-entry []
  {:position 0
   :forms [{:scope "t1/i1/f1" :code "(+ 1 2)" :src "(+ 1 2)"
            :result-kind :value
            :stdout nil :duration-ms 1 :success? true :error nil
            :render-segments nil}]})
  ...
  (let [code "(/ 1 0)"
    ...
    {:position 0
     :forms [{:scope "t1/i1/f1" :code code :src code
              :result-kind :error
              :stdout nil :error err :duration-ms 1 :success? false
              :render-segments nil}]}))
...
(defn- timeout-entry []
  (single-form-entry
    (ok-form 0 "t5/i1/f1" "(slow-tool)" :observation "SLOW  running")
    :status :timeout))
