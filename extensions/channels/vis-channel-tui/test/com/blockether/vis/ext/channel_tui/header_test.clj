...
(defdescribe draw-header-tab-entries-test
  (it "shows clickable arrows when workspaces overflow the 60 percent center slot"
    ...
    (let [writes (atom [])
      ...
      (let [left-arrow  (some #(when (and (= :workspace-entry (:kind %)) (= :prev (:index %))) %) (cr/current))
        ...
        ;; right arrow is unchanged: `+` shrinks the window's width by the same 4
        ;; it pushed the left edge, so `left+width` (the right arrow's anchor) is
        ;; invariant.
  ...
  (it "pads workspace labels with breathing room inside each cell"
    ;; The 62-col centre slot reserves 4 cols at the left for the ` + ` new-session
    ;; button (3) + a 1-col gap, leaving 58 for the tabs. With 3 workspaces and 2
    ;; dividers that's 56 shared → cells of 19/19/18. The first cell is 19 wide.
