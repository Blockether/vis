...
(defdescribe generic-limit-label-test
...
(defdescribe format-limit-usage-test
  ...
  (it "renders the full triple `used/limit used (remaining left)` when available"
    (expect (= "3/5 used (2 left)"
              (lfmt/format-limit-usage {:used 3 :limit 5 :remaining 2}))))

  (it "renders `used/limit used` when remaining is missing"
    (expect (= "3/5 used"
              (lfmt/format-limit-usage {:used 3 :limit 5}))))

  (it "renders `remaining/limit left` when used is missing"
    (expect (= "2/5 left"
              (lfmt/format-limit-usage {:remaining 2 :limit 5}))))

  (it "handles `:unlimited?` explicitly"
    (expect (= "unlimited"
              (lfmt/format-limit-usage {:unlimited? true}))))

  (it "returns nil when no usage signal is present"
    (expect (nil? (lfmt/format-limit-usage {})))))

(defdescribe generic-limit-has-signal?-test
  (it "true on :unlimited?"
    (expect (lfmt/generic-limit-has-signal? {:unlimited? true})))
...
(defdescribe dynamic-summary-test
  ...
  (it "caps to `max-rows` (default 2)"
    (let [limits {:dynamic {:limits [{:id :a :label "a" :remaining 10}
                                     {:id :b :label "b" :remaining 20}
                                     {:id :c :label "c" :remaining 30}]}}
          out    (lfmt/dynamic-summary limits)]
      (expect (= 1 (count (re-seq #" · " out))))))

  (it "prefers rows with signal; falls back to all when none have signal"
    (let [limits {:dynamic {:limits [{:id :a :label "a" :remaining 0}
                                     {:id :b :label "b" :remaining 0}]}}
          out    (lfmt/dynamic-summary limits)]
      (expect (some? out))))
