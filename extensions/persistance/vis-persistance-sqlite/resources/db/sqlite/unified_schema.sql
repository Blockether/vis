-- =============================================================================
-- vis — UNIFIED SQLite schema (equivalent to migrations V1..V10 applied)
--
-- This is a CONSOLIDATED snapshot: every ALTER/rename/table-rebuild from the
-- incremental migrations is folded inline into clean CREATE statements. It is
-- NOT wired into Flyway (the filename does not match the `V<n>__` pattern), so
-- it is inert until you deliberately promote it. When you collapse the history,
-- replace the migration/ files with a single `V1__schema.sql` carrying this
-- content and reset the flyway baseline.
--
-- Folded from:
--   V1  base schema
