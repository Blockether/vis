...
(def ^:private AGGREGATE_FILE_PREFIX
  (str common/bench-root "/results/aggregate-"))
...
(defn- list-result-files
  []
  (let [dir (io/file (str common/bench-root "/results"))]
    (if (.exists dir)
      (filter (fn [f]
                (let [n (.getName f)]
                  (and (.isFile f)
...
(defn- print-aggregated-scores []
  (let [files (list-result-files)
    ...
    (if (empty? aggregates)
      (println (str "No benchmark result EDNL files found in " common/bench-root "/results"))
      (let [sorted-aggs (sort-by (fn [r] [(get bench-order (:bench r) 999)
        ...
        (println (str "\nAggregated benchmark scores (from " common/bench-root "/results/*.ednl)\n"))
...
(defn print-summary
  ...
  (println "=======================")
  (println (format "Mode:       %s" (if (= mode :vis) "vis! (RLM)" (str mode))))
...
(defn- print-list []
  ...
  (doseq [{:keys [name description tests]} BENCHMARKS]
    ...
    (println))
  (println "Usage: clojure -M:bench -- --bench <name> [--agent vis|pi] [--provider blockether|zai-coding|zai] [--model MODEL] [--limit N] [--offset N]")
  (println "       clojure -M:bench -- --bench 4clojure --agent vis --provider zai-coding --model glm-5-turbo")
